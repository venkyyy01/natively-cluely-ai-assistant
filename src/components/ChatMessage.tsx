...
export const UserMessage: React.FC<ChatMessageProps> = ({
	content,
	timestamp,
	isNew = false,
}) => (
				...
				>
					<ReactMarkdown
						...
						components={{
							p: ({ node, ...props }: any) => (
								<p
									className="mb-4 last:mb-0 whitespace-pre-wrap text-[15px] font-normal leading-[1.8] text-slate-900 dark:text-slate-50 antialiased"
									{...props}
								/>
							),
							...
							em: ({ node, ...props }: any) => (
								<em
									className="italic font-medium text-slate-800 dark:text-slate-200"
									{...props}
								/>
							),
							ul: ({ node, ...props }: any) => (
								<ul
									className="list-disc ml-5 mb-4 space-y-2 text-slate-900 dark:text-slate-50"
									{...props}
								/>
							),
							ol: ({ node, ...props }: any) => (
								<ol
									className="list-decimal ml-5 mb-4 space-y-2 text-slate-900 dark:text-slate-50"
									{...props}
								/>
							),
							li: ({ node, ...props }: any) => (
