		...
		<div className="space-y-6 animated fadeIn">
			<div className="space-y-3.5">
					...
					<div className="flex flex-col gap-1">
						<div className="flex items-center gap-2">
							...
							<h3 className="text-lg font-bold text-text-primary">
								{isUndetectable ? "Privacy mode" : "Visible mode"}
							</h3>
						</div>
				...
				<div>
					...
					<p className="text-xs text-text-secondary mb-2">
						Customize how Natively works for you
					</p>
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-4">
								...
								<div>
									<h3 className="text-sm font-bold text-text-primary">
										Open Natively when you log in
									</h3>
									<p className="text-xs text-text-secondary mt-0.5">
										Natively will open automatically when you log in to your
										computer
									</p>
								</div>
							</div>
							<div
								onClick={async () => {
									...
									try {
										const result =
											await window.electronAPI?.setOpenAtLogin(newState);
										if (!result?.success) {
											throw new Error(
												result?.error || "Unable to update login preference",
											);
										}
									} catch (error: any) {
										setOpenOnLogin(!newState);
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-4">
								...
								<div>
									...
									<p className="text-xs text-text-secondary mt-0.5">
										Neural Engine acceleration for faster embeddings and context
										assembly (Apple Silicon)
									</p>
								</div>
							</div>
							<div
								onClick={async () => {
									...
									try {
										const result =
											await window.electronAPI?.setAccelerationMode(newState);
										if (result && !result.success) {
											throw new Error(
												result.error?.message ||
													"Unable to update acceleration mode",
											);
										}
									} catch (error: any) {
										setAccelerationModeEnabled(!newState);
										showGeneralSettingsError(
											error?.message || "Unable to update acceleration mode",
										);
									}
								}}
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-4">
								...
								<div>
									...
									<p className="text-xs text-text-secondary mt-0.5">
										Phase-aware interview assistance with structured responses
									</p>
								</div>
							</div>
							<div
								onClick={async () => {
									...
									try {
										...
										if (result && !result.success) {
											throw new Error(
												result.error?.message ||
													"Unable to update conscious mode",
											);
										}
									} catch (error: any) {
										setConsciousModeEnabled(!newState);
										showGeneralSettingsError(
											error?.message || "Unable to update conscious mode",
										);
									}
								}}
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-4">
								...
								<div>
									...
									<p className="text-xs text-text-secondary mt-0.5">
										Customize how Natively looks on your device
									</p>
								</div>
							</div>
						...
						<div className="flex items-start justify-between gap-4">
							<div className="flex items-start gap-4">
								...
								<div>
									...
									<p className="text-xs text-text-secondary mt-0.5">
										Natively v{packageJson.version}
									</p>
								</div>
							</div>
						</div>
							...
							<div className="flex items-start justify-between gap-4">
								<div className="flex items-start gap-3">
									...
									<div>
										...
										<p className="text-xs text-text-secondary mt-0.5">
											Let mouse clicks pass through the meeting overlay to the
											app underneath while keeping the overlay visible.
										</p>
									</div>
								</div>
								...
								<button
									...
									onClick={() =>
										setOverlayClickthroughEnabled(!overlayClickthroughEnabled)
									}
								...
								>
									<div
										className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${overlayClickthroughEnabled ? "translate-x-5" : "translate-x-0"}`}
									/>
								</button>
							</div>
						</div>
			...
			<div className="bg-bg-item-surface rounded-xl p-5 border border-border-subtle">
				<div className="flex flex-col gap-1 mb-3">
					...
					<p className="text-xs text-text-secondary">
						Disguise Natively as another application to prevent detection during
						screen sharing.
				...
				>
					{isUndetectable && (
						<p className="col-span-2 text-xs text-yellow-500/80 -mt-1 mb-1">
							{"\u26A0\uFE0F"} Disable Undetectable mode first to change
							disguise.
						</p>
					)}
					{DISGUISE_OPTIONS.map((option) => (
						...
						>
							<div
								className={`w-8 h-8 rounded-md flex items-center justify-center shrink-0 ${disguiseMode === option.id ? "bg-white/20 text-white" : "bg-bg-item-surface text-text-secondary"}`}
							>
								{option.icon}
							</div>
