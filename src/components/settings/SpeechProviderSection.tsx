...
export const SpeechProviderSection: React.FC<SpeechProviderSectionProps> = (
	props,
) => {
		...
		<div>
			...
			<div className="space-y-4">
				<div className="bg-bg-card rounded-xl border border-border-subtle p-4 space-y-3">
					<label htmlFor="stt-provider-select" className="text-xs font-medium text-text-secondary block">
						Speech Provider
					</label>
				...
				{sttProvider === "groq" && (
					<div className="bg-bg-card rounded-xl border border-border-subtle p-4">
						<span className="text-xs font-medium text-text-secondary mb-2.5 block">
							Whisper Model
						</span>
						<div className="grid grid-cols-2 gap-2">
							{[
								...
								>
									...
									<span
										className={`text-[11px] transition-colors ${groqSttModel === m.id ? "text-white/70" : "text-text-tertiary"}`}
									>
										{m.desc}
									</span>
								</button>
				...
				{sttProvider !== "google" && (
					<div className="bg-bg-card rounded-xl border border-border-subtle p-4 space-y-3">
						...
						{sttProvider === "azure" && (
							<div>
								<label htmlFor="azure-region-input" className="text-xs font-medium text-text-secondary mb-1.5 block">
									Azure Region
								</label>
								<div className="flex gap-2">
									<input
										...
										className="flex-1 bg-bg-input border border-border-subtle rounded-lg px-3 py-2 text-sm text-text-primary"
									/>
									<button
										type="button"
										onClick={() =>
											electronAPI.setAzureRegion(sttAzureRegion.trim())
										}
										className="px-4 py-2 bg-bg-input hover:bg-bg-elevated border border-border-subtle rounded-lg text-xs font-medium text-text-primary transition-colors"
									>
										Save
									</button>
								</div>
							...
							<button
								...
								onClick={() => {
									const urls: Record<string, string> = {
										...
										azure:
											"https://portal.azure.com/#create/Microsoft.CognitiveServicesSpeech",
										ibmwatson:
											"https://cloud.ibm.com/catalog/services/speech-to-text",
									};
									if (urls[sttProvider])
										electronAPI.openExternal(urls[sttProvider]);
								}}
