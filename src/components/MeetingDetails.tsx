...
const MeetingDetails: React.FC<MeetingDetailsProps> = ({
	...
	const updateMeetingSummary = getOptionalElectronMethod(
	...
	const handleCopy = async () => {
		...
		if (
			activeTab === "summary" &&
			(meeting.detailedSummary || meeting.summary)
		) {
			textToCopy = `
...
${meeting.detailedSummary?.keyPoints?.map((item) => `- ${item}`).join("\n") || "None"}
			...
			textToCopy = meeting.transcript
				.map(
					(t) =>
						`[${formatTime(t.timestamp)}] ${t.speaker === "user" ? "Me" : "Them"}: ${t.text}`,
				)
				.join("\n");
											...
											<div className="flex items-center justify-between mb-4">
												<EditableTextBlock
													...
													onSave={(val) => {
														...
														void updateMeetingSummary?.(meeting.id, {
															actionItemsTitle: val,
														});
													}}
													tagName="h2"
											...
											<div className="flex items-start gap-4">
												<div className="mt-1 w-6 h-6 rounded-full bg-bg-input flex items-center justify-center border border-border-subtle shrink-0">
													<img
														src={NativelyLogo}
														alt="AI"
														className="w-4 h-4 opacity-50 grayscale object-contain"
													/>
												</div>
												<div>
