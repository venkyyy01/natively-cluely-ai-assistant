...
const GlobalChatOverlay: React.FC<GlobalChatOverlayProps> = ({
	...
	const latestReadableMessage =
		messages.find((msg) => msg.role === "assistant") || null;
	...
	const submitQuestion = useCallback(
		async (question: string) => {
			if (
				!question.trim() ||
				chatStateRef.current === "waiting_for_llm" ||
				chatStateRef.current === "streaming_response"
			)
				return;
			...
			try {
				...
				if (result?.fallback) {
					...
					await window.electronAPI?.streamGeminiChat(
						...
						undefined,
						undefined,
						{ skipSystemPrompt: false, requestId: geminiRequestId },
					);
				}
			} catch (error) {
				if (!isCurrentRequest()) return;
					...
					>
						<div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle shrink-0">
							<div className="flex items-center gap-2 text-text-tertiary">
								<img
									src={nativelyIcon}
									className="w-3.5 h-3.5 force-black-icon brightness-0 dark:brightness-100 dark:opacity-50 dark:grayscale"
									alt="logo"
								/>
						...
						<div
							...
							className="flex-1 overflow-y-auto px-8 py-6 pb-32 custom-scrollbar flex flex-col"
						>
