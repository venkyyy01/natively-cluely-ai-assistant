...
const SettingsPopup = () => {
	...
	const onConsciousModeChanged = getOptionalElectronMethod(
		"onConsciousModeChanged",
	);
	const updateContentDimensions = getOptionalElectronMethod(
	...
	useEffect(() => {
		...
		if (getFastResponseConfig) {
			getFastResponseConfig()
				...
				.catch((error) => {
					console.warn(
						"[SettingsPopup] Failed to load Fast Response config:",
						error,
					);
				});
		}
	...
	return (
		<div className="w-fit h-fit bg-transparent flex flex-col">
			<div
				ref={contentRef}
				className="w-[200px] bg-[#1E1E1E]/80 backdrop-blur-md border border-white/10 rounded-[16px] overflow-hidden shadow-2xl shadow-black/40 px-2 pt-2 pb-2 flex flex-col animate-scale-in origin-top-left justify-between"
			>
				{/* Undetectability */}
				<div className="flex items-center justify-between px-3 py-2 hover:bg-white/5 rounded-lg transition-colors duration-200 group cursor-default">
					<div className="flex items-center gap-3">
						...
						>
							{isUndetectable ? "Privacy" : "Visible"}
						</span>
					</div>
					<button
						onClick={() => {
							...
							void setUndetectable(newState).catch((error) => {
								console.error(
									"[SettingsPopup] Failed to toggle undetectable mode:",
									error,
								);
								setIsUndetectable(!newState);
							});
				...
				<div
					className={`flex items-center justify-between px-3 py-2 rounded-lg transition-colors duration-200 group ${hasStoredKey[fastResponseConfig.provider] === false ? "opacity-50 grayscale cursor-not-allowed" : "hover:bg-white/5 cursor-default"}`}
					title={
						hasStoredKey[fastResponseConfig.provider] === false
							? `Requires ${fastResponseConfig.provider === "cerebras" ? "Cerebras" : "Groq"} API Key to be configured in Settings`
							: ""
					}
				>
					<div className="flex items-center gap-3">
						...
						/>
						<span
							className={`text-[12px] font-medium transition-colors ${fastResponseConfig.enabled ? "text-white" : "text-slate-400 group-hover:text-slate-200"}`}
						>
							{SESSION_MENU_TOGGLE_ORDER[0]}
						</span>
					</div>
					<button
						onClick={async () => {
					...
					>
						<div
							className={`w-[15px] h-[15px] rounded-full bg-black shadow-sm transition-transform duration-300 ease-spring ${fastResponseConfig.enabled ? "translate-x-[12px]" : "translate-x-0"}`}
						/>
					</button>
				</div>
				...
				<div className="flex items-center justify-between px-3 py-2 hover:bg-white/5 rounded-lg transition-colors duration-200 group cursor-default">
					<div className="flex items-center gap-3">
						...
						/>
						<span
							className={`text-[12px] font-medium transition-colors ${consciousModeEnabled ? "text-white" : "text-slate-400 group-hover:text-slate-200"}`}
						>
							{SESSION_MENU_TOGGLE_ORDER[2]}
						</span>
					</div>
					<button
						onClick={async () => {
							...
							try {
								...
								console.error(
									"[SettingsPopup] Failed to toggle Conscious Mode:",
									error,
								);
								setConsciousModeEnabled(!nextState);
							}
					...
					>
						<div
							className={`w-[15px] h-[15px] rounded-full bg-black shadow-sm transition-transform duration-300 ease-spring ${consciousModeEnabled ? "translate-x-[12px]" : "translate-x-0"}`}
						/>
					</button>
				</div>
				...
				{hasProfile && (
					<div
						className={`flex items-center justify-between px-3 py-2 rounded-lg transition-colors duration-200 group hover:bg-white/5 cursor-default`}
					>
						<div className="flex items-center gap-3">
							<User
							...
							/>
							<span
								className={`text-[12px] font-medium transition-colors ${profileMode ? "text-white" : "text-slate-400 group-hover:text-slate-200"}`}
							>
								Profile Mode
							</span>
						</div>
						...
						>
							<div
								className={`w-[15px] h-[15px] rounded-full bg-black shadow-sm transition-transform duration-300 ease-spring ${profileMode ? "translate-x-[12px]" : "translate-x-0"}`}
							/>
						</button>
					</div>
				)}
				...
				<div className="flex items-center justify-between px-3 py-2 hover:bg-white/5 rounded-lg transition-colors duration-200 group interaction-base interaction-press">
					...
					<div className="flex gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
						...
						{(shortcuts.toggleVisibility || ["⌘", "⌥", "⇧", "V"]).map(
							(key, index) => (
								<div
									key={index}
									className="px-1.5 py-0.5 rounded border border-white/10 bg-white/5 text-[10px] text-slate-500 font-medium min-w-[20px] text-center"
								>
									{key}
								</div>
							),
				...
				<div className="flex items-center justify-between px-3 py-2 hover:bg-white/5 rounded-lg transition-colors duration-200 group interaction-base interaction-press">
					...
					<div className="flex gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
						...
						{(shortcuts.takeScreenshot || ["⌘", "⌥", "⇧", "S"]).map(
							(key, index) => (
								<div
									key={index}
									className="px-1.5 py-0.5 rounded border border-white/10 bg-white/5 text-[10px] text-slate-500 font-medium min-w-[20px] text-center"
