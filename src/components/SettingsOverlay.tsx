	...
	return (
		<div ref={containerRef} className="relative z-20 font-sans">
			...
			>
				...
				<div
					className={`w-8 h-8 rounded-full flex items-center justify-center text-text-tertiary transition-transform duration-300 group-hover:bg-bg-surface ${isOpen ? "rotate-180 bg-bg-surface text-text-primary" : ""}`}
				>
									...
									>
										...
										<div className="flex-1 min-w-0 text-left">
											<div className="flex items-center justify-between mb-0.5">
												<div className="flex items-center gap-2">
													<span
														className={`text-[13px] font-medium transition-colors ${isSelected ? "text-white" : "text-text-primary"}`}
													>
														{option.label}
													</span>
													{option.badge && (
											...
											<span
												className={`text-[11px] block truncate transition-colors ${isSelected ? "text-white/70" : "text-text-tertiary"}`}
											>
												{option.desc}
											</span>
										</div>
										{/* Hover Indicator */}
										{!isSelected && (
											<div className="absolute inset-0 rounded-[10px] ring-1 ring-inset ring-white/0 group-hover:ring-white/5 pointer-events-none" />
										)}
									</button>
								);
							})}
...
const SettingsOverlay: React.FC<SettingsOverlayProps> = ({
	...
	const [overlayClickthroughEnabled, setOverlayClickthroughEnabled] = useState(
		() => localStorage.getItem("natively_overlay_clickthrough") === "true",
	);
	...
	const [hasStoredGoogleSearchKey, setHasStoredGoogleSearchKey] =
		useState(false);
	const [hasStoredGoogleSearchCseId, setHasStoredGoogleSearchCseId] =
		useState(false);
	...
	useEffect(() => {
		if (window.electronAPI?.onAccelerationModeChanged) {
			const unsubscribe = window.electronAPI.onAccelerationModeChanged(
				(newState: boolean) => {
	...
	useEffect(() => {
		if (window.electronAPI?.onConsciousModeChanged) {
			const unsubscribe = window.electronAPI.onConsciousModeChanged(
				(newState: boolean) => {
	...
	const showGeneralSettingsError = React.useCallback((message: string) => {
		...
		window.setTimeout(() => {
			setGeneralSettingsError((current) =>
	...
	useEffect(() => {
		localStorage.setItem(
			"natively_overlay_clickthrough",
			String(overlayClickthroughEnabled),
		);
		window.electronAPI
			?.setOverlayClickthrough?.(overlayClickthroughEnabled)
			.catch(() => {
	...
	useEffect(() => {
		const unsubscribe = window.electronAPI?.onOverlayClickthroughChanged?.(
			(enabled) => {
	...
	const sttSaveInFlightRef = React.useRef<null | {
		requestId: number;
		provider: string;
	}>(null);
	const sttTestInFlightRef = React.useRef<null | {
		requestId: number;
		provider: string;
	}>(null);
	...
	const hasConfiguredSttProvider = React.useCallback(
	...
	const isCurrentSttProviderRequest = React.useCallback(
		(requestId: number, provider: string) => {
			return (
				isCurrentSttRequest(requestId) && sttProviderRef.current === provider
			);
		},
		[isCurrentSttRequest],
	);
	...
	const handleRemoveGoogleSearchKey = async (type: "apikey" | "cseid") => {
		if (
			!confirm(
				`Are you sure you want to remove the Google Search ${type === "apikey" ? "API Key" : "CSE ID"}?`,
			)
		)
			return;
									...
									<div className="space-y-6 animated fadeIn">
										...
										<div className="mb-5">
											...
											<p className="text-xs text-text-secondary mb-2">
												This engine constructs an intelligent representation of
												your career history.
											</p>
										</div>
												...
												<div className="p-5 pb-4">
													<div className="flex items-center justify-between">
														...
														<div className="flex items-center gap-3">
															...
															<div
																className={`flex items-center gap-2 bg-bg-input px-3 py-1.5 rounded-full border border-border-subtle`}
															>
																...
																>
																	<div
																		className={`absolute top-1 left-1 w-3 h-3 rounded-full bg-white transition-transform ${profileStatus.profileMode ? "translate-x-4" : "translate-x-0"}`}
																	/>
																</div>
															</div>
														</div>
														...
														<div className="min-w-0">
															...
															{profileUploading ? (
																...
																<p className="text-xs text-text-secondary truncate pr-4">
																	Provide a resume file to seed the intelligence
																	engine.
																</p>
															)}
														</div>
														...
														<div className="min-w-0">
															...
															{jdUploading ? (
																...
																<p className="text-xs text-text-secondary">
																	Upload a JD to enable persona tuning and
																	company research.
																</p>
															)}
														</div>
												...
												<div className="p-5">
													...
													<div className="space-y-3">
														<div>
															<div className="flex justify-between items-center mb-1.5">
																...
																{hasStoredGoogleSearchKey && (
																	<button
																		onClick={() =>
																			handleRemoveGoogleSearchKey("apikey")
																		}
															...
															<input
																...
																onChange={(e) =>
																	setGoogleSearchApiKey(e.target.value)
																}
																placeholder={
																	hasStoredGoogleSearchKey
																		? "••••••••••••"
														...
														<div>
															<div className="flex justify-between items-center mb-1.5">
																...
																{hasStoredGoogleSearchCseId && (
																	<button
																		onClick={() =>
																			handleRemoveGoogleSearchKey("cseid")
																		}
																		className="text-[10px] flex items-center gap-1 text-red-400 hover:text-red-300 transition-colors bg-red-500/10 hover:bg-red-500/20 px-1.5 py-0.5 rounded"
																		title="Remove CSE ID"
																	>
															...
															<input
																...
																onChange={(e) =>
																	setGoogleSearchCseId(e.target.value)
																}
																placeholder={
																	hasStoredGoogleSearchCseId
																		? "••••••••••••"
														...
														<button
															onClick={async () => {
																if (
																	!googleSearchApiKey.trim() &&
																	!googleSearchCseId.trim()
																)
																	return;
																...
																try {
																	...
																	console.error(
																		"Failed to save Google Search keys:",
																		e,
																	);
																} finally {
													...
													<div className="mt-3 flex items-start gap-2 px-3 py-2.5 bg-bg-input/50 rounded-lg">
														...
														<p className="text-[10px] text-text-tertiary leading-relaxed">
															If not provided, LLM general knowledge is used for
															company research, which may be outdated. Get your
															API key from the{" "}
															<span
													...
													<div className="bg-bg-item-surface rounded-xl border border-border-subtle p-5">
														...
														{companyDossier && (
															<div className="space-y-4 border-t border-border-subtle pt-4 mt-2">
																...
																{companyDossier.salary_estimates?.length >
																	0 && (
																	<div>
																		<div className="text-[10px] font-bold text-text-primary uppercase tracking-wide mb-1">
																			Salary Estimates
																...
																{companyDossier.sources?.length > 0 && (
																	<div className="text-[10px] text-text-tertiary mt-2">
																		Sources:{" "}
																		{
																			companyDossier.sources.filter(Boolean)
																				.length
																		}{" "}
																		references
																	</div>
									...
									<div className="space-y-5 animated fadeIn select-text pb-4">
										<div className="flex items-start justify-between">
											<div>
												...
												<p className="text-xs text-text-secondary">
													Natively works with these easy to remember commands.
												</p>
											</div>
											<button
														...
														<div className="flex items-center gap-3">
															...
															<div className="flex flex-col">
																...
																<span className="text-[11px] text-text-tertiary">
																	{globalShortcutAlternates.toggleVisibility}
																</span>
															</div>
														</div>
														<KeyRecorder
													...
													<div className="flex items-center justify-between py-1.5 group">
														<div className="flex items-center gap-3">
															...
															<div className="flex flex-col">
																...
																<span className="text-[11px] text-text-tertiary">
																	{globalShortcutAlternates.takeScreenshot}
																</span>
															</div>
														</div>
														<KeyRecorder
													...
													<div className="flex items-center justify-between py-1.5 group">
														<div className="flex items-center gap-3">
															...
															<div className="flex flex-col">
																...
																<span className="text-[11px] text-text-tertiary">
																	{globalShortcutAlternates.selectiveScreenshot}
																</span>
															</div>
														</div>
														<KeyRecorder
													...
													<div className="flex items-center justify-between py-1.5 group">
														<div className="flex items-center gap-3">
															...
															<div className="flex flex-col">
																...
																<span className="text-[11px] text-text-tertiary">
																	{globalShortcutAlternates.toggleClickthrough}
																</span>
															</div>
														</div>
