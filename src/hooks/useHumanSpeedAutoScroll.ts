...
function estimateDurationMs(content: string): number {
	...
	return clamp(
		(words / HUMAN_WORDS_PER_MINUTE) * 60_000,
...
export function useHumanSpeedAutoScroll({
	...
	useEffect(() => {
		...
		if (
			!container ||
			!latestMessage ||
			!eligibleRoles.includes(latestMessage.role)
		) {
			return;
		}
		...
		const userIsHoldingManualScroll =
			isPointerInteractingRef.current && isHoldingManualScrollRef.current;
