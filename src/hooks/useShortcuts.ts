...
export const useShortcuts = () => {
	...
	const mapBackendToFrontend = useCallback((backendKeybinds: any[]) => {
		setShortcuts((prev) => {
			...
			backendKeybinds.forEach((kb) => {
				...
				else if (kb.id === "window:move-down")
					newShortcuts.moveWindowDown = keys;
				else if (kb.id === "window:move-left")
					newShortcuts.moveWindowLeft = keys;
				else if (kb.id === "window:move-right")
					newShortcuts.moveWindowRight = keys;
				// General
				else if (kb.id === "general:toggle-visibility")
					newShortcuts.toggleVisibility = keys;
				else if (kb.id === "general:process-screenshots")
					newShortcuts.processScreenshots = keys;
				else if (kb.id === "general:reset-cancel")
					newShortcuts.resetCancel = keys;
				else if (kb.id === "general:take-screenshot")
					newShortcuts.takeScreenshot = keys;
				else if (kb.id === "general:selective-screenshot")
					newShortcuts.selectiveScreenshot = keys;
			});
	...
	const updateShortcut = useCallback(
		async (actionId: keyof ShortcutConfig, keys: string[]) => {
			...
			// General
			else if (actionId === "toggleVisibility")
				backendId = "general:toggle-visibility";
			else if (actionId === "processScreenshots")
				backendId = "general:process-screenshots";
			else if (actionId === "resetCancel") backendId = "general:reset-cancel";
			else if (actionId === "takeScreenshot")
				backendId = "general:take-screenshot";
			else if (actionId === "selectiveScreenshot")
				backendId = "general:selective-screenshot";
