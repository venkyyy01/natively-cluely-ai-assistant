...
const RendererStartupFallback: React.FC<{ errorMessage: string }> = ({
	errorMessage: _errorMessage,
}) => <div className="h-full min-h-0 w-full bg-black" aria-hidden="true" />;

const getStoredAudioDeviceId = (
	storageKey: string,
	fallback = "default",
): string => {
...
const MeetingAudioBanner: React.FC<MeetingAudioBannerProps> = ({
	message,
	title,
	variant,
	onDismiss,
	...
	const motionProps =
		variant === "overlay"
			? {
					...
					containerClassName:
						"rounded-2xl border border-[#ff3333]/35 bg-[#1A1A1A]/95 px-4 py-3 shadow-2xl backdrop-blur-md",
				}
			: {
					...
					containerClassName:
						"pointer-events-auto rounded-2xl border border-[#ff3333]/35 bg-[#1A1A1A]/96 px-5 py-4 shadow-2xl backdrop-blur-md",
				};
...
export const PrivacyShieldWindowContent: React.FC<
  PrivacyShieldWindowContentProps
> = ({ variant, onEndMeeting }) => {
  ...
  if (isOverlay) {
    return (
      <ErrorBoundary context="PrivacyShield">
        <div
          className="flex h-full min-h-0 w-full items-center justify-center bg-black"
          onClick={
            onEndMeeting
              ? () => {
                void onEndMeeting();
          ...
          role={onEndMeeting ? "button" : undefined}
          tabIndex={onEndMeeting ? 0 : undefined}
        />
      </ErrorBoundary>
    );
  }
  ...
  return (
    <ErrorBoundary context="PrivacyShield">
      <div className="flex h-full min-h-0 w-full flex-col items-center justify-center bg-[#0D0D0D]">
        <div className="bg-[#1A1A1A] border border-[#ff3333]/30 shadow-2xl rounded-2xl p-5 max-w-[340px] flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff3333" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
              <line x1="12" y1="9" x2="12" y2="13" />
              <line x1="12" y1="17" x2="12.01" y2="17" />
            </svg>
            <span className="text-sm font-semibold text-white">Privacy Shield Active</span>
          </div>
          <p className="text-xs leading-relaxed text-[#B8B8B8]">
            Sensitive content is hidden while a capture risk is detected. The app will restore automatically when it is safe to do so.
          </p>
        </div>
      </div>
    </ErrorBoundary>
	...
	<ErrorBoundary context="Overlay">
		<div className="w-full relative bg-transparent">
			<AppProviders>
				...
				<div
					style={{ opacity: overlayOpacity, transition: "opacity 75ms ease" }}
				>
							...
							<div className="flex items-start gap-3">
								...
								<div>
									...
									<p className="text-[#A0A0A0] text-xs mt-1 leading-relaxed">
										{incompatibleWarning.count} meetings used your previous AI
										provider ({incompatibleWarning.oldProvider}) and won't
										appear in search results under{" "}
...
const useWindowAnalytics = ({
	kind,
	isDefaultLauncherWindow,
}: AppWindowContext) => {
	useEffect(() => {
		const analyticsPlan = getWindowAnalyticsPlan({
			kind,
			isDefaultLauncherWindow,
		});
...
const App: React.FC = () => {
	const windowContext = useMemo(
		() => resolveWindowContext(window.location.search),
		[],
	);
	...
	const [meetingAudioError, setMeetingAudioError] = useState<string | null>(
		null,
	);
	const [privacyShieldState, setPrivacyShieldState] =
		useState<PrivacyShieldState>(() => ({
	...
	useEffect(() => {
		...
		const onOllamaPullProgress = getOptionalElectronMethod(
			"onOllamaPullProgress",
		);
		const onOllamaPullComplete = getOptionalElectronMethod(
		...
		const onIncompatibleProviderWarning = getOptionalElectronMethod(
		...
		const onMeetingAudioError = getOptionalElectronMethod(
			"onMeetingAudioError",
		);
		...
		const getPrivacyShieldState = getOptionalElectronMethod(
	...
	useEffect(() => {
		...
		const onOverlayOpacityChanged = getOptionalElectronMethod(
	...
	const handleStartMeeting = async () => {
		if (!electronAPI) {
			setMeetingAudioError(
				electronBridgeError?.message ?? "Electron API bridge is unavailable",
			);
			return;
		}
		...
		try {
			...
			setMeetingAudioError(
				err instanceof Error ? err.message : "Audio pipeline failed to start.",
			);
		}
	};
	...
	if (privacyShieldState.active) {
		return (
			<PrivacyShieldWindowContent
				variant={windowKind === "overlay" ? "overlay" : "launcher"}
				onEndMeeting={windowKind === "overlay" ? handleEndMeeting : undefined}
			/>
		);
	}
