export type AppWindowKind = 'settings' | 'launcher' | 'overlay' | 'model-selector'
...
export const getWindowAnalyticsPlan = ({ kind }: AppWindowContext): WindowAnalyticsPlan => ({
  trackAppLifecycle: kind === 'launcher',
  trackAssistantLifecycle: kind === 'overlay',
})

export const shouldListenForOverlayOpacity = ({ kind }: AppWindowContext) => kind === 'overlay'

export const getProfileToasterThresholdMs = (environment: 'development' | 'production') =>
