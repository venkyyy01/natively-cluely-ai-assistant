...
const logFile = path_1.default.join(
...
class AppState {
	...
	constructor() {
		...
		(0, optimizations_1.syncOptimizationFlagsFromSettings)(
		...
		this.settingsWindowHelper =
			new SettingsWindowHelper_1.SettingsWindowHelper();
		this.modelSelectorWindowHelper =
			new ModelSelectorWindowHelper_1.ModelSelectorWindowHelper();
		...
		const stealthManager = new StealthManager_1.StealthManager({
		...
		this.intelligenceManager = new IntelligenceManager_1.IntelligenceManager(
	...
	async ensureMeetingAudioAccess() {
		...
		const screenStatus =
			electron_1.systemPreferences.getMediaAccessStatus("screen");
	...
	async validateMeetingAudioSetup(metadata) {
		...
		if (
			...
			!inputDevices.some((device) => device.id === inputDeviceId)
		) {
	...
	async initializeAccelerationManager() {
		try {
			const { AccelerationManager } = await Promise.resolve().then(() =>
				__importStar(require("./services/AccelerationManager")),
			);
	...
	setupAutoUpdater() {
		...
		electron_updater_1.autoUpdater.on("update-available", async (info) => {
			...
			const releaseManager =
				ReleaseNotesManager_1.ReleaseNotesManager.getInstance();
	...
	async quitAndInstallUpdate() {
		...
		if (process.platform === "darwin") {
			try {
				...
				const updateFile =
					electron_updater_1.autoUpdater.downloadedUpdateHelper?.file;
		...
		try {
			...
			if (!this.systemAudioCapture) {
				...
				this.systemAudioCapture.on("error", (err) => {
					...
					this.broadcast(
						"meeting-audio-error",
						err.message || "System audio capture failed",
					);
				});
			}
			...
			return new Promise((resolve, reject) => {
				setTimeout(async () => {
					...
					try {
						...
						this.broadcast(
							"meeting-audio-error",
							err.message || "Audio pipeline failed to start",
						);
	...
	async endMeeting() {
		...
		try {
			...
			console.error(
				"[Main] Failed to stop system audio during endMeeting:",
				error,
			);
		}
		...
		try {
			...
			console.error(
				"[Main] Failed to stop interviewer STT during endMeeting:",
				error,
			);
		}
		// 4. Stop Microphone
		try {
			...
			console.error(
				"[Main] Failed to stop microphone capture during endMeeting:",
				error,
			);
		}
	...
	cleanupForQuit() {
		...
		this.intelligenceManager.stopMeeting().catch((err) => {
			console.error(
				"[Main] Failed to stop intelligence manager during quit:",
				err,
			);
		});
	}
	async processCompletedMeetingForRAG() {
		...
		try {
			...
			const meetings =
				DatabaseManager_1.DatabaseManager.getInstance().getRecentMeetings(1);
			if (meetings.length === 0) return;
			const meeting =
				DatabaseManager_1.DatabaseManager.getInstance().getMeetingDetails(
					meetings[0].id,
				);
	...
	showTray() {
		...
		try {
			if (require("fs").existsSync(templatePath)) {
				...
				const devTemplatePath = path_1.default.join(
					electron_1.app.getAppPath(),
	...
	setUndetectable(state) {
		...
		SettingsManager_1.SettingsManager.getInstance().set(
			"isUndetectable",
			state,
		);
	...
	setConsciousModeEnabled(enabled) {
		...
		const persisted = SettingsManager_1.SettingsManager.getInstance().set(
	...
	setAccelerationModeEnabled(enabled) {
		...
		(0, optimizations_1.syncOptimizationFlagsFromSettings)(
			settings.getAccelerationModeEnabled(),
		);
