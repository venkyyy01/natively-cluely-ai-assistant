...
electron_1.contextBridge.exposeInMainWorld("electronAPI", {
	updateContentDimensions: (dimensions) =>
		electron_1.ipcRenderer.invoke("update-content-dimensions", dimensions),
	setOverlayBounds: (bounds) =>
		electron_1.ipcRenderer.invoke("set-overlay-bounds", bounds),
	getRecognitionLanguages: () =>
		(0, types_1.invokeAndUnwrap)("get-recognition-languages"),
	takeScreenshot: () => (0, types_1.invokeAndUnwrap)("take-screenshot"),
	takeSelectiveScreenshot: () =>
		(0, types_1.invokeAndUnwrap)("take-selective-screenshot"),
	getScreenshots: () => (0, types_1.invokeAndUnwrap)("get-screenshots"),
	deleteScreenshot: (path) =>
		electron_1.ipcRenderer.invoke("delete-screenshot", path),
	// Event listeners
	...
	onSolutionStart: (callback) => {
		...
		electron_1.ipcRenderer.on(
			exports.PROCESSING_EVENTS.INITIAL_START,
			subscription,
		);
		return () => {
			electron_1.ipcRenderer.removeListener(
	...
	onDebugStart: (callback) => {
		const subscription = () => callback();
		electron_1.ipcRenderer.on(
			exports.PROCESSING_EVENTS.DEBUG_START,
			subscription,
		);
		return () => {
			electron_1.ipcRenderer.removeListener(
	...
	onDebugError: (callback) => {
		...
		electron_1.ipcRenderer.on(
			exports.PROCESSING_EVENTS.DEBUG_ERROR,
			subscription,
		);
		return () => {
			electron_1.ipcRenderer.removeListener(
	...
	onSolutionError: (callback) => {
		const subscription = (_, error) => callback(error);
		electron_1.ipcRenderer.on(
			exports.PROCESSING_EVENTS.INITIAL_SOLUTION_ERROR,
			subscription,
		);
		return () => {
			electron_1.ipcRenderer.removeListener(
	...
	onProcessingNoScreenshots: (callback) => {
		const subscription = () => callback();
		electron_1.ipcRenderer.on(
			exports.PROCESSING_EVENTS.NO_SCREENSHOTS,
			subscription,
		);
		return () => {
			electron_1.ipcRenderer.removeListener(
	...
	onProblemExtracted: (callback) => {
		const subscription = (_, data) => callback(data);
		electron_1.ipcRenderer.on(
			exports.PROCESSING_EVENTS.PROBLEM_EXTRACTED,
			subscription,
		);
		return () => {
			electron_1.ipcRenderer.removeListener(
	...
	onSolutionSuccess: (callback) => {
		const subscription = (_, data) => callback(data);
		electron_1.ipcRenderer.on(
			exports.PROCESSING_EVENTS.SOLUTION_SUCCESS,
			subscription,
		);
		return () => {
			electron_1.ipcRenderer.removeListener(
	...
	onUnauthorized: (callback) => {
		const subscription = () => callback();
		electron_1.ipcRenderer.on(
			exports.PROCESSING_EVENTS.UNAUTHORIZED,
			subscription,
		);
		return () => {
			electron_1.ipcRenderer.removeListener(
	...
	analyzeImageFile: (path) =>
		(0, types_1.invokeAndUnwrap)("analyze-image-file", path),
	...
	toggleAdvancedSettings: () =>
		electron_1.ipcRenderer.invoke("toggle-advanced-settings"),
	openExternal: (url) => electron_1.ipcRenderer.invoke("open-external", url),
	setUndetectable: (state) =>
		(0, types_1.invokeStatus)("set-undetectable", state),
	getUndetectable: async () =>
		(await (0, types_1.invokeAndUnwrap)("get-undetectable")).enabled,
	setConsciousMode: (enabled) =>
		electron_1.ipcRenderer.invoke("set-conscious-mode", enabled),
	...
	onConsciousModeChanged: (callback) => {
		...
		return () => {
			electron_1.ipcRenderer.removeListener(
				"conscious-mode-changed",
				subscription,
			);
		};
	},
	setAccelerationMode: (enabled) =>
		electron_1.ipcRenderer.invoke("set-acceleration-mode", enabled),
	getAccelerationMode: () =>
		electron_1.ipcRenderer.invoke("get-acceleration-mode"),
	onAccelerationModeChanged: (callback) => {
		const subscription = (_, enabled) => callback(enabled);
		electron_1.ipcRenderer.on("acceleration-mode-changed", subscription);
		return () => {
			electron_1.ipcRenderer.removeListener(
				"acceleration-mode-changed",
				subscription,
			);
		};
	},
	setOpenAtLogin: (open) =>
		(0, types_1.invokeStatus)("set-open-at-login", open),
	getOpenAtLogin: async () =>
		(await (0, types_1.invokeAndUnwrap)("get-open-at-login")).enabled,
	closeSettingsWindow: () => (0, types_1.invokeVoid)("close-settings-window"),
	setDisguise: (mode) => (0, types_1.invokeStatus)("set-disguise", mode),
	getDisguise: async () =>
		(await (0, types_1.invokeAndUnwrap)("get-disguise")).mode,
	...
	getCurrentLlmConfig: () =>
		(0, types_1.invokeAndUnwrap)("get-current-llm-config"),
	getAvailableOllamaModels: () =>
		(0, types_1.invokeAndUnwrap)("get-available-ollama-models"),
	switchToOllama: (model, url) =>
		electron_1.ipcRenderer.invoke("switch-to-ollama", model, url),
	switchToGemini: (apiKey, modelId) =>
		electron_1.ipcRenderer.invoke("switch-to-gemini", apiKey, modelId),
	testLlmConnection: (provider, apiKey) =>
		electron_1.ipcRenderer.invoke("test-llm-connection", provider, apiKey),
	...
	setGeminiApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-gemini-api-key", apiKey),
	setGroqApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-groq-api-key", apiKey),
	setCerebrasApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-cerebras-api-key", apiKey),
	setOpenaiApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-openai-api-key", apiKey),
	setClaudeApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-claude-api-key", apiKey),
	getStoredCredentials: () =>
		(0, types_1.invokeAndUnwrap)("get-stored-credentials"),
	// STT Provider Management
	setSttProvider: (provider) =>
		electron_1.ipcRenderer.invoke("set-stt-provider", provider),
	getSttProvider: () => (0, types_1.invokeAndUnwrap)("get-stt-provider"),
	setGroqSttApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-groq-stt-api-key", apiKey),
	setOpenAiSttApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-openai-stt-api-key", apiKey),
	setDeepgramApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-deepgram-api-key", apiKey),
	setElevenLabsApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-elevenlabs-api-key", apiKey),
	setAzureApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-azure-api-key", apiKey),
	setAzureRegion: (region) =>
		electron_1.ipcRenderer.invoke("set-azure-region", region),
	setIbmWatsonApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-ibmwatson-api-key", apiKey),
	setGroqSttModel: (model) =>
		electron_1.ipcRenderer.invoke("set-groq-stt-model", model),
	setSonioxApiKey: (apiKey) =>
		electron_1.ipcRenderer.invoke("set-soniox-api-key", apiKey),
	...
	generateSuggestion: (context, lastQuestion) =>
		(0, types_1.invokeAndUnwrap)("generate-suggestion", context, lastQuestion),
	getNativeAudioStatus: () =>
		(0, types_1.invokeAndUnwrap)("native-audio-status"),
	...
	setRecognitionLanguage: (key) =>
		electron_1.ipcRenderer.invoke("set-recognition-language", key),
	getAiResponseLanguages: () =>
		(0, types_1.invokeAndUnwrap)("get-ai-response-languages"),
	setAiResponseLanguage: (language) =>
		(0, types_1.invokeStatus)("set-ai-response-language", language),
	getSttLanguage: async () =>
		(await (0, types_1.invokeAndUnwrap)("get-stt-language")).language,
	getAiResponseLanguage: async () =>
		(await (0, types_1.invokeAndUnwrap)("get-ai-response-language")).language,
	// Intelligence Mode IPC
	generateAssist: () => electron_1.ipcRenderer.invoke("generate-assist"),
	generateWhatToSay: (question, imagePaths) =>
		electron_1.ipcRenderer.invoke("generate-what-to-say", question, imagePaths),
	generateFollowUp: (intent, userRequest) =>
		electron_1.ipcRenderer.invoke("generate-follow-up", intent, userRequest),
	generateFollowUpQuestions: () =>
		electron_1.ipcRenderer.invoke("generate-follow-up-questions"),
	generateRecap: () => electron_1.ipcRenderer.invoke("generate-recap"),
	submitManualQuestion: (question) =>
		electron_1.ipcRenderer.invoke("submit-manual-question", question),
	getIntelligenceContext: () =>
		electron_1.ipcRenderer.invoke("get-intelligence-context"),
	resetIntelligence: () => electron_1.ipcRenderer.invoke("reset-intelligence"),
	// Meeting Lifecycle
	startMeeting: (metadata) =>
		electron_1.ipcRenderer.invoke("start-meeting", metadata),
	...
	getMeetingDetails: (id) =>
		electron_1.ipcRenderer.invoke("get-meeting-details", id),
	updateMeetingTitle: (id, title) =>
		electron_1.ipcRenderer.invoke("update-meeting-title", { id, title }),
	updateMeetingSummary: (id, updates) =>
		electron_1.ipcRenderer.invoke("update-meeting-summary", { id, updates }),
	...
	// Window Mode
	setWindowMode: (mode) =>
		electron_1.ipcRenderer.invoke("set-window-mode", mode),
	setOverlayClickthrough: (enabled) =>
		(0, types_1.invokeVoid)("set-overlay-clickthrough", enabled),
	...
	getMeetingLifecycleState: () =>
		electron_1.ipcRenderer.invoke("get-meeting-lifecycle-state"),
	// Streaming Chat
	streamGeminiChat: (message, imagePaths, context, options) =>
		electron_1.ipcRenderer.invoke(
			"gemini-chat-stream",
			message,
			imagePaths,
	...
	cancelChat: (requestId) =>
		electron_1.ipcRenderer.invoke("gemini-chat-cancel", requestId),
	...
	setDefaultModel: (modelId) =>
		electron_1.ipcRenderer.invoke("set-default-model", modelId),
	toggleModelSelector: (coords) =>
		electron_1.ipcRenderer.invoke("toggle-model-selector", coords),
	forceRestartOllama: () => (0, types_1.invokeVoid)("force-restart-ollama"),
	// Settings Window
	toggleSettingsWindow: (coords) =>
		(0, types_1.invokeVoid)("toggle-settings-window", coords),
	// Fast Response Mode
	getFastResponseConfig: () =>
		(0, types_1.invokeAndUnwrap)("get-fast-response-config"),
	setFastResponseConfig: (config) =>
		electron_1.ipcRenderer.invoke("set-fast-response-config", config),
	// Demo
	seedDemo: () => electron_1.ipcRenderer.invoke("seed-demo"),
	// Custom Providers
	saveCustomProvider: (provider) =>
		electron_1.ipcRenderer.invoke("save-custom-provider", provider),
	getCustomProviders: () =>
		(0, types_1.invokeAndUnwrap)("get-custom-providers"),
	deleteCustomProvider: (id) =>
		electron_1.ipcRenderer.invoke("delete-custom-provider", id),
	// Follow-up Email
	generateFollowupEmail: (input) =>
		electron_1.ipcRenderer.invoke("generate-followup-email", input),
	extractEmailsFromTranscript: (transcript) =>
		electron_1.ipcRenderer.invoke("extract-emails-from-transcript", transcript),
	getCalendarAttendees: (eventId) =>
		electron_1.ipcRenderer.invoke("get-calendar-attendees", eventId),
	openMailto: (params) => electron_1.ipcRenderer.invoke("open-mailto", params),
	// Audio Test
	startAudioTest: (deviceId) =>
		electron_1.ipcRenderer.invoke("start-audio-test", deviceId),
	...
	calendarDisconnect: () =>
		electron_1.ipcRenderer.invoke("calendar-disconnect"),
	...
	ragQueryMeeting: async (meetingId, query) => {
		try {
			return await (0, types_1.invokeAndUnwrap)("rag:query-meeting", {
				meetingId,
				query,
			});
		} catch (error) {
	...
	ragCancelQuery: (options) =>
		(0, types_1.invokeStatus)("rag:cancel-query", options),
	ragIsMeetingProcessed: (meetingId) =>
		(0, types_1.invokeAndUnwrap)("rag:is-meeting-processed", meetingId),
	...
	reindexIncompatibleMeetings: () =>
		(0, types_1.invokeVoid)("rag:reindex-incompatible-meetings"),
	...
	setKeybind: (id, accelerator) =>
		electron_1.ipcRenderer.invoke("keybinds:set", id, accelerator),
	...
	markDonationToastShown: () =>
		electron_1.ipcRenderer.invoke("mark-donation-toast-shown"),
	setDonationComplete: () =>
		electron_1.ipcRenderer.invoke("set-donation-complete"),
	// Profile Engine API
	profileUploadResume: (filePath) =>
		(0, types_1.invokeStatus)("profile:upload-resume", filePath),
	profileGetStatus: () => (0, types_1.invokeAndUnwrap)("profile:get-status"),
	profileSetMode: (enabled) =>
		(0, types_1.invokeStatus)("profile:set-mode", enabled),
	...
	// JD & Research API
	profileUploadJD: (filePath) =>
		(0, types_1.invokeStatus)("profile:upload-jd", filePath),
	profileDeleteJD: () => (0, types_1.invokeStatus)("profile:delete-jd"),
	profileResearchCompany: async (companyName) => {
		try {
			return await (0, types_1.invokeAndUnwrap)(
	...
	// Google Search API
	setGoogleSearchApiKey: (apiKey) =>
		(0, types_1.invokeStatus)("set-google-search-api-key", apiKey),
	setGoogleSearchCseId: (cseId) =>
		(0, types_1.invokeStatus)("set-google-search-cse-id", cseId),
	// Dynamic Model Discovery
	fetchProviderModels: (provider, apiKey) =>
		electron_1.ipcRenderer.invoke("fetch-provider-models", provider, apiKey),
	...
	// License Management
	licenseActivate: (key) =>
		electron_1.ipcRenderer.invoke("license:activate", key),
	licenseCheckPremium: () =>
		electron_1.ipcRenderer.invoke("license:check-premium"),
	licenseDeactivate: () => electron_1.ipcRenderer.invoke("license:deactivate"),
	licenseGetHardwareId: () =>
		electron_1.ipcRenderer.invoke("license:get-hardware-id"),
	// Overlay Opacity (Stealth Mode)
	setOverlayOpacity: (opacity) =>
		(0, types_1.invokeVoid)("set-overlay-opacity", opacity),
	...
	logErrorToMain: (payload) =>
		electron_1.ipcRenderer.invoke("renderer:log-error", payload),
});
