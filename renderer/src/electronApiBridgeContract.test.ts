...
test('electron API helper provides a restart hint when a preload method is missing', () => {
  ...
  expect(source).toContain('Restart the app or Electron dev process to reload the preload bridge');
});
...
test('settings toggles use the guarded Electron bridge helper for conscious mode and undetectable mode', () => {
  ...
  expect(generalSettingsSource).toContain("requireElectronMethod('setUndetectable')");
  expect(generalSettingsSource).toContain("requireElectronMethod('setConsciousMode')");
  expect(settingsPopupSource).toContain("requireElectronMethod('setUndetectable')");
  expect(settingsPopupSource).toContain("requireElectronMethod('setConsciousMode')");
