...
describe('shared App bootstrap ownership', () => {
  ...
  test('keeps helper windows out of launcher analytics', () => {
    expect(getWindowAnalyticsPlan(resolveWindowContext('?window=settings'))).toEqual({
      trackAppLifecycle: false,
      trackAssistantLifecycle: false,
    });

    expect(getWindowAnalyticsPlan(resolveWindowContext('?window=model-selector'))).toEqual({
      trackAppLifecycle: false,
      trackAssistantLifecycle: false,
    });
  });
  ...
  test('routes overlay-specific lifecycle ownership through the overlay context only', () => {
    ...
    expect(shouldListenForOverlayOpacity(resolveWindowContext('?window=settings'))).toBe(false);
  });
