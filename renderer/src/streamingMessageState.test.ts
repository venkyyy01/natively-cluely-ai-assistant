...
describe("streaming message state helpers", () => {
	...
	test("updateMessageById returns same array when id is missing", () => {
		...
		const next = updateMessageById(prev, "missing", (message) => ({
			...message,
			text: "x",
		}));
	...
	test("clearing a message id removes every alias that points to the completed stream", () => {
		const activeIds = setActiveStreamingIds(
			{},
			["what_to_answer", "what_to_say"],
			"assistant-1",
		);
		const withUnrelated = setActiveStreamingIds(
			activeIds,
			["recap"],
			"assistant-2",
		);
		...
		expect(
			clearActiveStreamingIdsByMessageId(withUnrelated, "assistant-1"),
		).toEqual({
			recap: "assistant-2",
		});
	});
