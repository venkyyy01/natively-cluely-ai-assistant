...
export class SessionPersistence {
	...
	constructor(options: SessionPersistenceOptions = {}) {
		this.sessionsDir =
			...
			process.env[SESSIONS_DIR_ENV] ??
			join(homedir(), ".natively", "sessions");
	...
	async load(sessionId: string): Promise<PersistedSession | null> {
		...
		try {
			const content = await fs.readFile(
				join(this.sessionsDir, entry.filepath),
				"utf-8",
			);
	...
	private async loadIndex(): Promise<SessionIndex> {
		...
		try {
			...
			index =
				parsed && Array.isArray(parsed.sessions) ? parsed : { sessions: [] };
		} catch {
			index = { sessions: [] };
		}
	...
	private async scanOrphanSessions(
		index: SessionIndex,
