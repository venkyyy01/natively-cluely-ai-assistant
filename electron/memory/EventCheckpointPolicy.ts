...
interface EventCheckpointPolicyOptions {
  ...
  cooldownMs?: number;
  now?: () => number;
  triggerCheckpoint: (checkpointId: string, trigger: CheckpointTrigger, detail?: string) => Promise<void> | void;
...
export class EventCheckpointPolicy {
  ...
  private readonly triggerCheckpoint: (checkpointId: string, trigger: CheckpointTrigger, detail?: string) => Promise<void> | void;
  private readonly checkpointIdFactory: (trigger: CheckpointTrigger, detail?: string) => string;
  ...
  constructor(options: EventCheckpointPolicyOptions) {
    ...
    this.checkpointIdFactory = options.checkpointIdFactory ?? ((trigger, detail) => `${trigger}:${detail ?? 'auto'}:${this.now()}`);
  ...
  private async requestCheckpoint(trigger: CheckpointTrigger, detail?: string): Promise<boolean> {
    const now = this.now();
