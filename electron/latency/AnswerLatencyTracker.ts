...
import type { ProviderCapabilityClass } from "./providerCapability";
...
export type TrackedProviderCapabilityClass =
	| ProviderCapabilityClass
	| "non_streaming_custom";
...
export class AnswerLatencyTracker {
	...
	markDegradedToRoute(
		requestId: string,
		route: AnswerRoute,
		metadata: Partial<LatencyMetadata> = {},
	): void {
		const snapshot = this.getMutableSnapshot(requestId);
		if (!snapshot) return;
	...
	markProfileEnrichmentState(
		requestId: string,
		state: ProfileEnrichmentState,
		profileFallbackReason?: string,
	): void {
		const snapshot = this.getMutableSnapshot(requestId);
		if (!snapshot) return;
	...
	annotate(requestId: string, metadata: LatencyMetadata): void {
		...
		if (normalizedMetadata.firstVisibleAnswer !== undefined) {
			this.writeMark(
				snapshot,
				"firstVisibleAnswer",
				normalizedMetadata.firstVisibleAnswer,
			);
			delete normalizedMetadata.firstVisibleAnswer;
		}
	...
	markStaleStop(
		requestId: string,
		reason: StaleStopReason = "transcript_revision_changed",
