...
export class SessionTracker {
	...
	private threadDirector: ThreadDirector | null =
		isNativelyThreadDirectorEnabled()
	...
	noteUtteranceRevision(utteranceId: string | undefined): void {
		...
		this.utteranceRevisions.set(
			utteranceId,
			(this.utteranceRevisions.get(utteranceId) ?? 0) + 1,
		);
	}
	...
	private updateConsciousConversationState(transcript: string): void {
		...
		if (this.threadDirector) {
			this.threadDirector.handleObservedInterviewerTranscript(
				transcript,
				detect,
				setPhase,
			);
	...
	recordConsciousResponse(
		...
		if (this.threadDirector) {
			this.threadDirector.recordConsciousResponse(
				question,
				response,
				threadAction,
			);
	...
	getFullSessionContext(): string {
		return buildFullSessionContext(
			this.fullTranscript,
	...
	getHotState(
		now: number = Date.now(),
	): import("./memory/SessionPersistence").PersistedSessionMemoryEntry[] {
