...
function resolveClassifierLane():
	| Pick<RuntimeBudgetScheduler, "submit">
	| undefined {
...
export class SessionTracker {
	...
	private consciousThreadStore: ConsciousThreadStore =
		new ConsciousThreadStore();
	private observedQuestionStore: ObservedQuestionStore =
		new ObservedQuestionStore();
	private questionReactionClassifier: QuestionReactionClassifier =
		new QuestionReactionClassifier();
	private answerHypothesisStore: AnswerHypothesisStore =
		new AnswerHypothesisStore();
	private responsePreferenceStore: ConsciousResponsePreferenceStore =
		new ConsciousResponsePreferenceStore();
	...
	private compactSnapshotCache = new Map<
		string,
	...
	public setMeetingMetadata(metadata: any): void {
		...
		const inferredMeetingId =
			metadata?.meetingId || metadata?.calendarEventId || metadata?.title;
	...
	addTranscript(
		...
		if (!isInternalPrompt) {
			...
			if (this.fullTranscript.length > MAX_TRANSCRIPT_ENTRIES) {
				this.fullTranscript.splice(
					0,
					this.fullTranscript.length - MAX_TRANSCRIPT_ENTRIES,
				);
			}
	...
	addAssistantMessage(text: string): void {
		...
		if (this.assistantResponseHistory.length > MAX_ASSISTANT_HISTORY) {
			this.assistantResponseHistory = this.assistantResponseHistory.slice(
	...
	handleTranscript(
		...
		if (segment.speaker === "interviewer") {
			if (Math.random() < 0.05 || segment.final) {
				console.log(
					`[SessionTracker] RX Interviewer Segment: Final=${segment.final} Text="${segment.text.substring(0, 50)}..."`,
				);
			}
		...
		if (segment.final && segment.speaker === "interviewer") {
			...
			if (this.consciousModeEnabled) {
				const reaction = this.questionReactionClassifier.classify({
					...
					latestResponse:
						this.consciousThreadStore.getLatestConsciousResponse(),
		...
		if (
			...
			const changed = this.responsePreferenceStore.noteUserTranscript(
				segment.text,
				segment.timestamp,
			);
			if (changed) {
		...
		if (
			segment.final &&
			this.consciousModeEnabled &&
			(segment.speaker === "interviewer" || segment.speaker === "user")
		) {
	...
	getContext(lastSeconds: number = 120): ContextItem[] {
		...
		return this.getContextItems().filter(
			(item: ContextItem) => item.timestamp >= cutoff,
		);
	}
		...
		if (provider?.isInitialized()) {
			try {
				...
				const embedding = accelerationManager
					? await accelerationManager.runInLane("semantic", () =>
							provider.embed(text),
						)
					: await provider.embed(text);
				this.semanticEmbeddingCache.set(normalized, {
					embedding,
					createdAt: Date.now(),
				});
				return embedding;
				...
				console.warn(
					"[SessionTracker] Semantic embedding fallback to pseudo embedding:",
					error,
				);
			}
		}
	...
	private computeBM25Scores(query: string, documents: string[]): number[] {
		...
		const docTerms = documents.map((document) =>
			this.tokenizeForMemory(document),
		);
		const avgDocLength = Math.max(
			1,
			docTerms.reduce((sum, terms) => sum + terms.length, 0) /
				Math.max(1, docTerms.length),
		);
		const k1 = 1.5;
		const b = 0.75;
		...
		return docTerms.map((terms) => {
			...
			for (const term of queryTerms) {
				...
				score +=
					(idf * (tf * (k1 + 1))) /
					(tf + k1 * (1 - b + b * (terms.length / avgDocLength)));
			}
	...
	private computePhaseAlignmentScore(
		candidatePhase: InterviewPhase | undefined,
		...
		if (
			related.has(`${candidatePhase}:${currentPhase}`) ||
			related.has(`${currentPhase}:${candidatePhase}`)
		) {
			return 0.5;
		}
	...
	private semanticRedundancyScore(
		...
		for (let i = 0; i < selectedTexts.length; i++) {
			const semantic = selectedEmbeddings[i]?.length
				? this.cosineSimilarity(embedding, selectedEmbeddings[i])
				: 0;
			const lexical = this.lexicalOverlapScore(
				this.tokenizeForMemory(text),
				selectedTexts[i],
			);
	...
	private computeFacetQueryBoost(text: string, queryTokens: string[]): number {
		...
		if (
			...
			boost += 0.12;
			if (
				/(table|index|indexes|append-only|schema|secondary index|entity)/i.test(
					lower,
				)
			) {
				boost += 0.1;
		...
		if (
			lower.startsWith("api_contracts:") &&
		...
		if (
			lower.startsWith("failure_modes:") &&
	...
	private ensureFacetCoverage(
		...
		if (
			...
			desiredMatchers.push(
				(text) =>
					/^data_model:/i.test(text) &&
					/(table|index|append-only|schema|secondary index)/i.test(text),
			);
		}
		...
		for (const matcher of desiredMatchers) {
			...
			const candidate = rankedCandidates.find(
				(entry) =>
					matcher(entry.item.text) &&
					!current.some(
						(selectedEntry) => selectedEntry.item.text === entry.item.text,
					),
			);
			if (!candidate) {
				continue;
	...
	private async rankConsciousContextItems(
		...
		const preRanked = merged
			.map((candidate, index) => {
				const ageMinutes = Math.max(
					0,
					(now - candidate.item.timestamp) / 60_000,
				);
				...
				const phase = this.computePhaseAlignmentScore(
					candidate.item.phase,
					currentPhase,
				);
				const bm25 = bm25Scores[index] / maxBm25;
				const facetBoost = this.computeFacetQueryBoost(
					candidate.item.text,
					queryTokens,
				);
				return {
		...
		const shortlist = preRanked.slice(
			0,
			Math.max(16, Math.min(32, preRanked.length)),
		);
		const semanticEmbeddings = await Promise.all(
			shortlist.map(async (candidate) => ({
				...
				embedding:
					...
					candidate.item.embedding.length === queryEmbedding.length
						? candidate.item.embedding
						: await this.getSemanticEmbedding(candidate.item.text),
			})),
		);
		const embeddingByKey = new Map(
			semanticEmbeddings.map((entry) => [entry.key, entry.embedding]),
		);
		...
		while (scored.length > 0) {
			...
			for (let index = 0; index < scored.length; index++) {
				...
				const candidateTokens = this.tokenCounter.count(
					candidate.item.text,
					"openai",
				);
				...
				const mmrScore =
					lambda * candidate.finalScore - (1 - lambda) * redundancy;
	...
	isLikelyGeneralIntent(lastInterviewerTurn: string | null): boolean {
		return this.observedQuestionStore.isLikelyGeneralIntent(
			lastInterviewerTurn,
		);
	}
	...
	private getAdaptiveFallbackContext(tokenBudget: number): ContextItem[] {
		...
		if (merged.length === 0) {
			return this.getContext(120).slice(
				-Math.max(4, Math.floor(tokenBudget / 20)),
			);
		}
		return merged;
	}
	...
	async getAdaptiveContext(
		...
		const candidates: ContextEntry[] = this.getContextItems().map(
			(item: ContextItem) => ({
				role: item.role,
				text: item.text,
		...
		try {
			...
			console.warn(
				"[SessionTracker] Adaptive window timeout, falling back:",
				error,
			);
			return this.getAdaptiveFallbackContext(tokenBudget);
		}
	...
	async getConsciousRelevantContext(
		query: string,
		tokenBudget: number = 900,
	): Promise<ContextItem[]> {
		...
		const adaptive = await this.getAdaptiveContext(
			trimmedQuery,
			queryEmbedding,
			Math.max(320, Math.floor(tokenBudget * 0.7)),
		);
		const recentTurns = this.getContext(600).slice(-14);
		const designStateEntries = this.designStateStore
			.getRetrievalEntries(trimmedQuery, 2)
			.map((entry) => ({
				item: {
		...
		const constraintEntries = this.extractedConstraints
			.slice(-8)
			.map((constraint) => ({
				item: {
					role: "interviewer" as const,
		...
		const summaryEntries = this.transcriptEpochSummaries
			.slice(-3)
			.map((summary, index) => ({
				item: {
					role: "interviewer" as const,
					...
					timestamp:
						Date.now() -
						(this.transcriptEpochSummaries.length - index) * 60_000,
					phase: this.phaseDetector.getCurrentPhase(),
		...
		const ranked = await this.rankConsciousContextItems(
			...
			[
				...
				...recentTurns.map((item) => ({
					item,
					boost: item.role === "interviewer" ? 0.06 : 0.03,
				})),
				...designStateEntries,
				...pinnedEntries,
				...constraintEntries,
	...
	recordConsciousResponse(
		...
		this.consciousThreadStore.recordConsciousResponse(
			question,
			response,
			threadAction,
		);
		this.answerHypothesisStore.recordStructuredSuggestion(
			question,
			response,
			threadAction,
		);
		...
		const activeThread = this.consciousThreadStore
			.getThreadManager()
			.getActiveThread();
	...
	getConsciousResponsePreferenceSummary(
		...
		return this.responsePreferenceStore.getPlannerPreferenceSummary(
			questionMode,
		);
	}
	...
	getConsciousLongMemoryContext(query: string): string {
		...
		const summaryLines = this.selectConsciousMemoryLines(
			this.transcriptEpochSummaries.map((summary, index) => ({
				text: `[Earlier summary ${index + 1}] ${summary}`,
				timestamp:
					Date.now() - (this.transcriptEpochSummaries.length - index) * 60_000,
				boost: 0.12,
			})),
			queryTokens,
			queryEmbedding,
		...
		if (activeThread) {
			...
			if (activeThread.resumeKeywords.length > 0) {
				lines.push(
					`ACTIVE_THREAD_KEYWORDS: ${activeThread.resumeKeywords.slice(0, 12).join(", ")}`,
				);
			}
			if (activeThread.keyDecisions.length > 0) {
				lines.push(
					`ACTIVE_THREAD_DECISIONS: ${activeThread.keyDecisions.slice(0, 6).join(" | ")}`,
				);
			}
			if (activeThread.constraints.length > 0) {
				lines.push(
					`ACTIVE_THREAD_CONSTRAINTS: ${activeThread.constraints.slice(0, 6).join(" | ")}`,
				);
			}
		}
	...
	getFormattedContext(lastSeconds: number = 120): string {
		const baseCached = this.getCachedAssembledContext(
			`formatted:${lastSeconds}`,
		);
		if (baseCached) {
			return baseCached;
		}
	...
	getCompactTranscriptSnapshot(
		maxTurns: number = 12,
		snapshotType: "standard" | "fast" = "standard",
	): string {
	...
	getFullSessionContext(): string {
		return this.buildFullSessionContext(
			this.fullTranscript,
			this.transcriptEpochSummaries,
		);
	}

	private buildFullSessionContext(
		transcript: TranscriptSegment[],
	...
	getHotState(now: number = Date.now()): PersistedSessionMemoryEntry[] {
		...
		const transcriptEntries = this.fullTranscript
			...
			.map((segment, index) =>
				this.toTranscriptMemoryEntry(segment, `hot-transcript-${index}`),
			);
		const usageEntries = this.fullUsage
			.filter((entry) => entry.timestamp >= cutoff)
			.map((entry, index) =>
				this.toUsageMemoryEntry(entry, `hot-usage-${index}`),
			);
	...
	getWarmState(now: number = Date.now()): PersistedSessionMemoryEntry[] {
		...
		if (activeThread) {
			entries.push(
				this.toMemoryEntry(`warm-thread-${activeThread.id}`, {
					kind: "active-thread",
					timestamp: now,
		...
		this.pinnedItems.forEach((item, index) => {
			entries.push(
				this.toMemoryEntry(`warm-pin-${index}-${item.id}`, {
					kind: "pinned-item",
					text: item.text,
					timestamp: item.pinnedAt,
					label: item.label,
		...
		this.extractedConstraints.forEach((constraint, index) => {
			entries.push(
				this.toMemoryEntry(`warm-constraint-${index}`, {
					kind: "constraint",
					text: constraint.raw,
					timestamp: now,
		...
		this.transcriptEpochSummaries.forEach((summary, index) => {
			entries.push(
				this.toMemoryEntry(`warm-epoch-${index}`, {
					kind: "epoch-summary",
					text: summary,
					timestamp: now,
				}),
	...
	getColdState(now: number = Date.now()): PersistedSessionMemoryEntry[] {
		...
		const transcriptEntries = this.fullTranscript
			.filter((segment) => segment.timestamp < cutoff)
			.map((segment, index) =>
				this.toTranscriptMemoryEntry(segment, `cold-transcript-${index}`),
			);
		const usageEntries = this.fullUsage
			.filter((entry) => entry.timestamp < cutoff)
			.map((entry, index) =>
				this.toUsageMemoryEntry(entry, `cold-usage-${index}`),
			);
	...
	pinItem(text: string, label?: string, skipPersist: boolean = false): void {
		...
		this.designStateStore.notePinnedItem(
			trimmed,
			label,
			Date.now(),
	...
	hasConstraint(normalized: string): boolean {
		...
		return this.extractedConstraints.some(
			(constraint) => constraint.normalized.toLowerCase() === key,
		);
	}
	...
	getAdaptiveWindowStats(): {
		...
		const avgMs =
			this.adaptiveWindowStats.calls > 0
	...
	async restoreFromMeetingId(
		meetingId: string,
		requestId: number = this.restoreRequestId,
	): Promise<boolean> {
		...
		try {
			...
			if (this.consciousModeEnabled) {
				this.answerHypothesisStore.restorePersistenceSnapshot(
					session.consciousState?.hypothesisState,
				);
				this.responsePreferenceStore.restorePersistenceSnapshot(
					session.consciousState?.preferenceState,
				);
				this.designStateStore.restorePersistenceSnapshot(
				...
				if (session.activeThread) {
					this.consciousThreadStore.restoreActiveThread({
						...
						phase:
							(session.activeThread.phase as InterviewPhase) ||
							"requirements_gathering",
						turnCount: session.activeThread.turnCount,
					});
				}
				this.consciousThreadStore.restorePersistenceSnapshot(
	...
	ensureMeetingContext(meetingId?: string): void {
		...
		this.pendingRestorePromise = this.restoreFromMeetingId(
			normalizedMeetingId,
			requestId,
		)
			.then(() => {
	...
	createSnapshot(now: number = Date.now()): MeetingSnapshot {
		...
		return {
			...
			meetingMetadata: this.currentMeetingMetadata
	...
	flushInterimTranscript(): void {
		if (this.lastInterimInterviewer) {
			console.log(
				"[SessionTracker] Force-saving pending interim transcript:",
				this.lastInterimInterviewer.text,
			);
	...
	setCurrentPhase(
		phase: InterviewPhase,
		trigger: "interviewer_transcript" | "manual" = "manual",
	): void {
		const currentPhase = this.phaseDetector.getCurrentPhase();
	...
	detectPhaseFromTranscript(transcript: string): InterviewPhase {
		...
		const result = this.phaseDetector.detectPhase(
			...
			this.getContextItems()
				.slice(-5)
				.map((item: ContextItem) => item.text),
		);
	...
	private toTranscriptMemoryEntry(
		segment: TranscriptSegment,
		id: string,
	...
	private restorePersistedMemoryState(
		...
		const assistantSegments = this.fullTranscript.filter(
			(segment) => segment.speaker === "assistant",
		);
	...
	private async compactTranscriptIfNeeded(): Promise<void> {
		...
		const promise = (async () => {
			try {
				...
				console.error(
					"[SessionTracker] Error during transcript compaction:",
					error,
				);
				...
				try {
					...
					console.warn(
						"[SessionTracker] Using fallback compaction due to error",
					);
				} catch (fallbackError) {
					console.error(
						"[SessionTracker] Fallback compaction also failed:",
						fallbackError,
					);
				}
			} finally {
