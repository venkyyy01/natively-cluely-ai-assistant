...
class MockLLMHelper {
	...
	async *streamChat(
		...
		if (
			utterance.includes("Hi there") ||
			utterance.includes("Hello") ||
			utterance.includes("thanks")
		) {
			...
			utterance.includes("clarify") ||
			utterance.includes("explain") ||
			utterance.includes("What do you mean")
		) {
			yield JSON.stringify({ kind: "clarification", confidence: 0.85 });
		} else if (
			utterance.includes("shorter") ||
			utterance.includes("expand") ||
			utterance.includes("rephrase")
		) {
			...
			utterance.includes("got it") ||
			utterance.includes("understood") ||
			utterance.includes("Got it")
		) {
			yield JSON.stringify({ kind: "acknowledgement", confidence: 0.9 });
		} else if (
			utterance.includes("by the way") ||
			utterance.includes("By the way") ||
			utterance.includes("btw")
		) {
...
describe("ConsciousModeRouter", () => {
	...
	describe("LLM-based classification (feature flag enabled)", () => {
		...
		it("classifies clarification via LLM", async () => {
			const plan = await router.plan("Can you clarify that?", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
		...
		it("classifies refinement via LLM", async () => {
			const plan = await router.plan("Make it shorter", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
		...
		it("classifies acknowledgement via LLM", async () => {
			const plan = await router.plan("Got it!", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
		...
		it("classifies off topic aside via LLM", async () => {
			const plan = await router.plan("By the way, what time is it?", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
		...
		it("classifies technical questions via LLM", async () => {
			const plan = await router.plan("Explain React hooks", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
			assert.strictEqual(plan.kind, "technical");
		...
		it("classifies behavioral questions via LLM with STAR senior format", async () => {
			const plan = await router.plan("Tell me about a time you led a project", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
			...
			assert.match(
				plan.strategyHint || "",
				/Tradeoff: why this approach over alternatives/,
			);
			...
			assert.ok(
				mockLLMHelper.prompts.some((prompt) =>
					prompt.includes("- behavioral: STAR format interview questions"),
				),
			);
		});

		it("classifies pushback via LLM with reframing template", async () => {
			const plan = await router.plan("But what about edge cases?", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
			...
			assert.match(
				plan.strategyHint || "",
				/That would be a concern if X\. In our case, we did Y because Z\./,
			);
			assert.doesNotMatch(plan.strategyHint || "", /defend your reasoning/i);
			assert.ok(
				mockLLMHelper.prompts.some((prompt) =>
					prompt.includes("- pushback: interviewer challenge"),
				),
			);
		});
	...
	describe("refinement usefulness", () => {
		...
		it("accepts shorten when long", () => {
			const result = router.isRefinementUseful({
				intent: "shorten",
				previousAnswer:
					"This is a very long answer that goes on and on and on and on and on and on and on and on and on.",
			});
	...
	describe("refinement prompt building", () => {
		it("builds prompt for shorten", () => {
			...
			assert.strictEqual(
				prompt.includes("This is a long answer with many details."),
				true,
			);
		});
	...
	describe("technical classification (fallback)", () => {
		...
		it("detects coding patterns and adds code-first hint", async () => {
			const plan = await router.plan("Design a system for file storage", {
				enabled: true,
			});
			assert.strictEqual(plan.kind, "technical");
			assert.match(plan.strategyHint || "", /Direct answer \(1-2 lines\)/);
			assert.match(
				plan.strategyHint || "",
				/Code-first approach: show implementation, then explain/,
			);
			assert.match(
				plan.strategyHint || "",
		...
		it("detects deep dive patterns and adds structured hint", async () => {
			const plan = await router.plan("Explain how database indexing works", {
				enabled: true,
			});
			assert.strictEqual(plan.kind, "technical");
			assert.match(plan.strategyHint || "", /Direct answer \(1-2 lines\)/);
			assert.match(
				plan.strategyHint || "",
				/Structured approach: constraints → design → tradeoffs → alternatives/,
			);
			assert.match(
				plan.strategyHint || "",
		...
		it("boosts verification when inside thread", async () => {
			const plan = await router.plan("What about scalability?", {
				enabled: true,
				isInsideThread: true,
			});
			assert.strictEqual(plan.kind, "technical");
		...
		it("adds live coding hint when live coding", async () => {
			const plan = await router.plan("How do I handle errors?", {
				enabled: true,
				isLiveCoding: true,
			});
			assert.strictEqual(plan.kind, "technical");
	...
	describe("verification plans", () => {
		...
		it("moderate verification skips judge", async () => {
			const plan = await router.plan("Can you make it shorter?", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
			assert.strictEqual(plan.verificationLevel, "moderate");
		...
		it("relaxed verification runs only provenance", async () => {
			const plan = await router.plan("What do you mean?", {
				enabled: true,
				llmHelper: mockLLMHelper,
			});
			assert.strictEqual(plan.verificationLevel, "relaxed");
		...
		it("degraded mode skips judge", async () => {
			const plan = await router.plan("Technical question", {
				enabled: true,
				isDegraded: true,
			});
			assert.strictEqual(plan.verification.runJudge, false);
			assert.strictEqual(
				...
				"strict_verification_all_checkers_degraded_mode",
			);
		});
		...
		it("degraded mode skips provenance when flag is false", async () => {
			const plan = await router.plan("Technical question", {
				enabled: true,
				isDegraded: true,
				useDegradedProvenanceCheck: false,
			});
			assert.strictEqual(plan.verification.runJudge, false);
			assert.strictEqual(plan.verification.runProvenance, false);
		});
