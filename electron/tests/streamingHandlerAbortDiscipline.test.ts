...
import {
	ConsciousStreamingHandler,
	type StreamEvent,
} from "../conscious/ConsciousStreamingHandler";
...
test("NAT-008: start() while prior stream in-flight aborts it and emits cancelled for the prior streamId", async () => {
	...
	assert.ok(
		cancelledForFirst,
		"a cancelled event for the FIRST streamId must have been emitted",
	);
	assert.equal(
		handler.isAborted(),
		false,
		"second start() installs a fresh, non-aborted controller",
	);
});
...
test("NAT-008: streamReasoning from a superseded stream stops emitting chunks", async () => {
	...
	const chunksAfterSupersede = reasoningChunkStreamIds.filter(
		(id, idx) =>
			id === firstStreamId &&
			idx > 0 &&
			reasoningChunkStreamIds
				.slice(0, idx)
				.some((prior) => prior === firstStreamId) &&
...
test("NAT-008: cancelled event ordering — handler sees cancelled for prior id BEFORE any event for new id", async () => {
	...
	assert.ok(
		firstEventForSecondId >= 0,
		"at least one event for secondId was emitted",
	);
...
test("NAT-008: emit() stamps streamId automatically when caller does not supply one", async () => {
	...
	for (const sid of seenStreamIds) {
		assert.equal(
			sid,
			id,
			`every emitted event in this stream must carry streamId=${id}, got ${sid}`,
		);
	}
});
