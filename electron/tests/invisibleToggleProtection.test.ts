...
test("AppState strict invisible enable hides, protects, verifies, and does not commit on failed verification", async () => {
	...
	try {
		...
		const setUndetectableAsync = prototype.setUndetectableAsync as (
			this: any,
			state: boolean,
		) => Promise<void>;
		...
		const verifyUndetectableEnableProtection =
			prototype.verifyUndetectableEnableProtection as (this: any) => void;
...
test("AppState invisible enable commits to local-visible protected controls", async () => {
	...
	try {
		...
		assert.ok(
			calls.includes("intent:visible_safe_controls:undetectable_enabled"),
		);
...
test("AppState visible_safe_controls intent shows the protected local UI instead of hiding it", async () => {
	...
	try {
		const { AppState } = await import("../main");
		const requestVisibilityIntent = (AppState.prototype as any)
			.requestVisibilityIntent as (
			this: any,
			intent: string,
			source: string,
		) => void;
...
test("AppState startup stealth recovery clears containment for visible safe controls", async () => {
	...
	try {
		const { AppState } = await import("../main");
		const bindRuntimeCoordinatorEvents = (AppState.prototype as any)
			.bindRuntimeCoordinatorEvents as (this: any) => void;
		...
		await subscriptions.get("stealth:state-changed")?.({
			from: "ARMING",
			to: "FULL_STEALTH",
		});
...
test("AppState serializes opposite invisible toggle targets without interleaving", async () => {
	...
	try {
		const { AppState } = await import("../main");
		const setUndetectableAsync = (AppState.prototype as any)
			.setUndetectableAsync as (this: any, state: boolean) => Promise<void>;
