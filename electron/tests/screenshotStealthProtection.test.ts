...
test("ScreenshotHelper: pauseWatchdog pauses and resumes correctly", () => {
	...
	assert.equal(
		stealthManager.isEnabled(),
		true,
		"StealthManager should be enabled",
	);
	...
	assert.equal(
		stealthManager.isEnabled(),
		true,
		"StealthManager should still be enabled",
	);
});
...
test("ScreenshotHelper: pauseWatchdog with multiple calls maintains count", () => {
	...
	assert.equal(
		stealthManager.isEnabled(),
		true,
		"Should still be enabled after partial resume",
	);
	...
	assert.equal(
		stealthManager.isEnabled(),
		true,
		"Should still be enabled after full resume",
	);
});
...
test("ScreenshotHelper: disabled stealth manager screenshot still works", async () => {
	...
	assert.equal(
		stealthManager.isEnabled(),
		false,
		"StealthManager should be disabled",
	);
	...
	assert.equal(
		stealthManager.isEnabled(),
		false,
		"StealthManager should still be disabled",
	);
});
...
test("ScreenshotHelper: isEnabled returns correct state", () => {
	...
	assert.equal(
		enabledManager.isEnabled(),
		true,
		"Enabled manager should return true",
	);
	assert.equal(
		disabledManager.isEnabled(),
		false,
		"Disabled manager should return false",
	);
});
...
test("ScreenshotHelper: verifyManagedWindows temporarily pauses for 10 seconds", () => {
	...
	assert.ok(
		...
		"verifyManagedWindows should return quickly",
	);
});
...
test("ScreenshotHelper: edge case - very rapid pause resume in tight loop", () => {
	...
	assert.equal(
		stealthManager.isEnabled(),
		true,
		"Should maintain enabled state after extreme cycling",
	);
});
