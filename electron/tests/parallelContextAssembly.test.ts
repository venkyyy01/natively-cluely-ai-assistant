...
describe('ParallelContextAssembler', () => {
  ...
  it('includes assistant turns by default for continuity-sensitive follow-ups', async () => {
    const input: ContextAssemblyInput = {
      ...
      transcript: [
        { speaker: 'interviewer', text: 'How would you scale the cache?', timestamp: Date.now() - 30000 },
        { speaker: 'assistant', text: 'I would pick consistent hashing to reduce key movement during resharding.', timestamp: Date.now() - 20000 },
        { speaker: 'interviewer', text: 'Why did we pick consistent hashing?', timestamp: Date.now() - 10000 },
      ],
    ...
    assert.ok(result.relevantContext.some(ctx => ctx.role === 'assistant' && ctx.text.includes('consistent hashing')));
  ...
  it('can exclude assistant turns when a caller explicitly requests transcript-only retrieval', async () => {
    const transcriptOnly = new ParallelContextAssembler({ workerThreadCount: 2, includeAssistantTurns: false });
    const input: ContextAssemblyInput = {
      query: 'consistent hashing',
      transcript: [
        { speaker: 'assistant', text: 'consistent hashing reduced resharding movement', timestamp: Date.now() - 20000 },
        { speaker: 'interviewer', text: 'Tell me about cache scale', timestamp: Date.now() - 10000 },
      ],
      previousContext: { recentTopics: ['cache'], activeThread: null },
    };
    ...
    assert.equal(result.relevantContext.some(ctx => ctx.role === 'assistant'), false);
    assert.equal(result.bm25Results.some(ctx => ctx.role === 'assistant'), false);
  });
});
