	...
	try {
		...
		const fakeStealthManager = {
			applyToWindow(
				_win: unknown,
				enable: boolean,
				options: { role?: string; hideFromSwitcher?: boolean },
			) {
				calls.push({
					enable,
					role: options.role,
...
test("WindowHelper centers overlay using the overlay height", async () => {
	...
	try {
		...
		const helper = new WindowHelper(
			{} as never,
			{ applyToWindow() {}, reapplyAfterShow() {} } as never,
		);
		let appliedBounds: {
			x: number;
			y: number;
		...
		assert.deepEqual(appliedBounds, {
			x: 420,
			y: 300,
			width: 600,
			height: 300,
...
test("WindowHelper blocks startup launcher reveal in strict mode when verification fails", async () => {
	...
	try {
		...
		assert.equal((helper as any).pendingDirectLauncherReveal, false);
		assert.deepEqual(
			privacyFaults.map((fault) => fault.key),
...
test("WindowHelper forwards stealth runtime heartbeat events to the registered listener", async () => {
	...
	try {
		...
		const listener = (helper as any).stealthHeartbeatListener as
			| (() => void)
			| null;
		listener?.();
		listener?.();
