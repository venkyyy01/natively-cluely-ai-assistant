...
import { AdaptivePauseModel, type PauseFeatures } from '../pause/AdaptivePauseModel';
...
describe('AdaptivePauseModel', () => {
  it('should initialize with default weights on cold start', () => {
    ...
    assert.ok(weights.silenceDuration > 0, 'Should have positive silence duration weight');
    assert.ok(weights.transcriptCompleteness > 0, 'Should have positive transcript completeness weight');
    assert.ok(weights.semanticCompleteness > 0, 'Should have positive semantic completeness weight');
    assert.ok(weights.conversationRhythm > 0, 'Should have positive conversation rhythm weight');
    assert.ok(weights.audioEnergyDecay > 0, 'Should have positive audio energy decay weight');
  });
  ...
  it('should normalize weights after update', () => {
    ...
    assert.ok(Math.abs(weightSum - 1.0) < 0.01, `Weights should sum to 1.0, got ${weightSum}`);
  });
  ...
  it('should reset to default weights', () => {
    ...
    assert.strictEqual(model.getSampleCount(), 0, 'Sample count should be 0 after reset');
    assert.strictEqual(afterReset.silenceDuration, 0.25, 'Weights should reset to defaults');
    assert.strictEqual(afterReset.transcriptCompleteness, 0.30, 'Weights should reset to defaults');
    assert.strictEqual(afterReset.semanticCompleteness, 0.20, 'Weights should reset to defaults');
    assert.strictEqual(afterReset.conversationRhythm, 0.15, 'Weights should reset to defaults');
    assert.strictEqual(afterReset.audioEnergyDecay, 0.10, 'Weights should reset to defaults');
  });
  ...
  it('should handle different profiles independently', () => {
    ...
    assert.strictEqual(model1.getSampleCount(), 1, 'Model 1 should have 1 sample');
    assert.strictEqual(model2.getSampleCount(), 0, 'Model 2 should have 0 samples');
  });
