...
test("DeepgramStreamingSTT ignores stale socket close events after an in-place restart", async () => {
	...
	try {
		const { DeepgramStreamingSTT } = await import(
...
test("DeepgramStreamingSTT only emits final transcript events when Deepgram itself marks is_final (NAT-009)", async () => {
	...
	let stt:
		| InstanceType<
				typeof import("../audio/DeepgramStreamingSTT").DeepgramStreamingSTT
		  >
		| undefined;
	...
	try {
		...
		const telemetry: Array<{
			kind: string;
			hadPendingInterim: boolean;
			pendingInterimLength: number;
		}> = [];
		...
		socket.emit(
			...
			Buffer.from(
				JSON.stringify({
					type: "Results",
					channel: {
						alternatives: [{ transcript: "hello there", confidence: 0.75 }],
					},
					is_final: false,
				}),
			),
		...
		socket.emit(
			"message",
			Buffer.from(JSON.stringify({ type: "UtteranceEnd" })),
		);
		socket.emit(
			"message",
			Buffer.from(
				JSON.stringify({
					type: "Results",
					channel: {
						alternatives: [{ transcript: "general kenobi", confidence: 0.91 }],
					},
					is_final: true,
				}),
			),
		...
		assert.deepEqual(
			transcripts.map(({ text, isFinal, confidence }) => ({
				text,
				isFinal,
				confidence,
			})),
