...
test("MacosStealthEnhancer applies level and sharing protection through native bindings", async () => {
	...
	const enhancer = new MacosStealthEnhancer({
		...
		logger: silentLogger,
		nativeModule: {
			setMacosWindowLevel: (windowNumber, level) =>
				nativeCalls.push(`level:${windowNumber}:${level}`),
			applyMacosWindowStealth: (windowNumber) =>
				nativeCalls.push(`apply:${windowNumber}`),
		},
...
test("MacosStealthEnhancer removes sharing protection through native bindings", async () => {
	...
	const enhancer = new MacosStealthEnhancer({
		...
		logger: silentLogger,
		nativeModule: {
			removeMacosWindowStealth: (windowNumber) =>
				nativeCalls.push(`remove:${windowNumber}`),
		},
		commandRunner: async (command, args) => {
			calls.push({ command, args });
			return "";
