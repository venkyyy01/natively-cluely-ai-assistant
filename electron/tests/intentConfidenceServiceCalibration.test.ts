...
test('NAT-056: strong vs uncertain uses calibration map', () => {
  ...
  assert.equal(svc.isStrongConsciousIntent({ intent: 'general', confidence: 0.99, answerShape: '' }), false);

  assert.equal(
    svc.isStrongConsciousIntent({ intent: 'deep_dive', confidence: 0.83, answerShape: '' }),
    false,
  );
  assert.equal(
    svc.isStrongConsciousIntent({ intent: 'deep_dive', confidence: 0.85, answerShape: '' }),
    true,
  );

  assert.equal(svc.isUncertainConsciousIntent(null), true);
  assert.equal(svc.isUncertainConsciousIntent({ intent: 'general', confidence: 0.99, answerShape: '' }), true);
  assert.equal(
    svc.isUncertainConsciousIntent({ intent: 'clarification', confidence: 0.71, answerShape: '' }),
    true,
  );
  assert.equal(
    svc.isUncertainConsciousIntent({ intent: 'clarification', confidence: 0.72, answerShape: '' }),
    false,
  );
});
