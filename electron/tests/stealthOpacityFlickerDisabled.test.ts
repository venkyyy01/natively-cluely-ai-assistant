...
function buildManager(opts: { enableOpacityFlicker?: boolean }) {
  ...
  const intervalScheduler = (cb: () => Promise<void> | void, intervalMs: number) => {
  ...
  const manager = new StealthManager(
    { enabled: true },
    {
      ...
      featureFlags: opts.enableOpacityFlicker !== undefined
...
test('NAT-010: opacity flicker is NOT scheduled even when opted in if not macOS 15.4+', () => {
  ...
  const flickerIntervals = intervals.filter((entry) => entry.intervalMs === 500);
  assert.equal(flickerIntervals.length, 0, 'pre-15.4 macOS still gets no flicker interval');
});
