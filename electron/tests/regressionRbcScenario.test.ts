...
test("RBC Scenario: legacy trigger path response rate (baseline)", () => {
	...
	console.log(
		`  Legacy path: ${triggeredCount}/${RBC_MEETING_QUESTIONS.length} triggered = ${(rate * 100).toFixed(0)}%`,
	);
...
test("RBC Scenario: utterance buffer trigger response rate (fix)", () => {
	...
	console.log(
		`  Buffer path:  ${fired.length} utterances flushed, ${triggeredCount} triggered = ${(rate * 100).toFixed(0)}%`,
	);
...
test("RBC Scenario: admin phrases filtered with conscious mode ON", () => {
	...
	console.log(
		`  All ${adminPhrases.length} admin phrases correctly rejected with conscious mode ON`,
	);
});
...
test("RBC Scenario: substantive questions still trigger with fix", () => {
	...
	console.log(
		`  Substantive questions triggered: ${triggered}/${substantive.length}`,
	);
	assert.equal(
		triggered,
		substantive.length,
		"All substantive questions should trigger",
	);
});
