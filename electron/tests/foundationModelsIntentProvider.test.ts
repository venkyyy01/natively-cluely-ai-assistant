...
test("resolveFoundationModelsIntentHelperPath prefers packaged helper path when present", () => {
	const resolved = resolveFoundationModelsIntentHelperPath({
		...
		pathExists: (candidate) =>
			candidate ===
			"/Applications/Natively.app/Contents/Resources/bin/macos/foundation-intent-helper",
	});
...
test("resolveFoundationModelsIntentHelperPath falls back to dev helper binary candidates", () => {
	const resolved = resolveFoundationModelsIntentHelperPath({
		...
		pathExists: (candidate) =>
			candidate ===
			"/workspace/applesilicon/macos-foundation-intent-helper/.build/debug/foundation-intent-helper",
	});
...
test("FoundationModelsIntentProvider maps helper error envelope to typed error codes", async () => {
	...
	await assert.rejects(
		...
		(error: any) =>
			error?.code === "refusal" &&
			/model refused/i.test(String(error?.message)),
	);
});
...
test("FoundationModelsIntentProvider maps helper model_not_ready envelope to typed error code", async () => {
	...
	await assert.rejects(
		...
		(error: any) =>
			...
			/downloading/i.test(String(error?.message)),
	);
});
...
test("FoundationModelsIntentProvider maps helper unsupported_locale envelope to typed error code", async () => {
	...
	await assert.rejects(
		...
		(error: any) =>
			...
			/locale/i.test(String(error?.message)),
	);
});
...
test("FoundationModelsIntentProvider rejects invalid helper JSON envelopes", async () => {
	...
	await assert.rejects(
		...
		(error: any) =>
			...
			/invalid json/i.test(String(error?.message)),
	);
});
