...
class FakeLLMHelper {
	constructor(
		...
		private readonly knowledgeStatus?: {
			activeMode?: string;
			hasResume?: boolean;
			hasActiveJD?: boolean;
		},
	...
	async *streamChat(message: string): AsyncGenerator<string> {
		if (message.includes("STRUCTURED_REASONING_RESPONSE")) {
			yield this.structuredResponseOverride ??
				JSON.stringify({
					...
					codeTransition:
						"Then I would connect that to an implementation sketch.",
				});
			return;
		}
		...
		if (this.streamError) {
			if (this.preStreamDelayMs > 0) {
				await new Promise((resolve) =>
					setTimeout(resolve, this.preStreamDelayMs),
				);
			}
...
test("AnswerLatencyTracker covers all capability classes and uses firstVisibleAnswer mark for non-streaming visibility", () => {
	...
	assert.equal(
		streamingSnapshot?.marks.firstVisibleAnswer,
		streamingSnapshot?.marks.firstToken,
	);
	...
	assert.equal(
		nonStreamingSnapshot?.marks.firstVisibleAnswer !== undefined,
		true,
	);
	assert.equal(
		nonStreamingSnapshot?.firstVisibleAnswer,
		nonStreamingSnapshot?.marks.firstVisibleAnswer,
	);
...
test("AnswerLatencyTracker records answer.firstVisible from first visibility, not completion time", async () => {
	...
	try {
		...
		const firstVisibleMetric = (await instrumentation.readAll()).find(
			(event) => event.metric === "answer.firstVisible",
		);
...
test("IntelligenceEngine records conscious route provider start metadata", async () => {
	...
	const consciousEngine = new TestIntelligenceEngine(
		new FakeLLMHelper("non_streaming") as any,
		consciousSession,
	);
...
test("IntelligenceEngine marks first visible streaming update on first token and records enriched lifecycle outcomes", async () => {
	...
	assert.equal(
		successSnapshot?.marks.firstToken,
		successSnapshot?.marks.firstVisibleAnswer,
	);
...
test("IntelligenceEngine leaves non-profile failures outside profile lifecycle instrumentation", async () => {
	...
	const engine = new TestIntelligenceEngine(
		new FakeLLMHelper(
			...
			undefined,
			[],
			new Error("generic fast-path failure"),
		) as any,
		session,
	);
...
test("IntelligenceEngine initializes profile lifecycle when conscious fallback reclassifies to enriched route", async () => {
	...
	addInterviewerTurn(
		session,
		"Walk me through your background and how would you design a rate limiter?",
		Date.now(),
	);
...
test("IntelligenceEngine does not mark fallbackOccurred for superseded fallback output", async () => {
	...
	const staleSnapshot = tracker.completedSnapshots.find(
		(snapshot) =>
			snapshot?.route === "fast_standard_answer" &&
			snapshot?.marks.firstToken === undefined,
	);
...
test("IntelligenceEngine emits suggested answer metadata for duplicate cooldown deferrals", async () => {
	...
	const session = new SessionTracker();
	const engine = new TestIntelligenceEngine(
		new SlowStreamingLLMHelper() as any,
		session,
	);
	...
	engine.on(
		...
		(
			...
			_question: string,
			_confidence: number,
			metadata?: { cooldownSuppressedMs?: number; cooldownReason?: string },
		) => {
