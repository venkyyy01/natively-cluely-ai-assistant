...
describe('MeetingCheckpointer Reliability Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockDb = {
      ...
      setRejectImpl: (impl: () => Promise<void>): void => { rejectImpl = impl; },
      resetCallCount: (): void => { callCount = 0; },
    };
  ...
  describe('CRITICAL: Meeting Data Loss Prevention', () => {
    it('should retry checkpoint saves with exponential backoff on DB errors', async () => {
      ...
      const failedEvents = emittedEvents.filter(e => e.event === 'checkpoint-failed');
      assert.equal(failedEvents.length, 0);
    });

    it('should emit checkpoint-failed event after exhausting retries', { timeout: 15000 }, async () => {
      const mockSnapshot = createMockSnapshot({
        transcript: [{ text: 'test', speaker: 'User', timestamp: Date.now(), final: true }],
      });
      mockSessionTracker.createSnapshot = () => mockSnapshot;
      ...
      const savedEvents = emittedEvents.filter(e => e.event === 'checkpoint-saved');
      assert.ok(savedEvents.length > 0, 'Should emit checkpoint-saved event with fallback');
      assert.ok(savedEvents[0].args[0].usedFallback === true, 'Should use fallback');
    });

    it('should fallback to temp file when DB is completely unavailable', { timeout: 15000 }, async () => {
      const mockSnapshot = createMockSnapshot({
        transcript: [{ text: 'Critical meeting data', speaker: 'User', timestamp: Date.now(), final: true }],
        meetingMetadata: { title: 'Test Meeting' },
      });
      mockSessionTracker.createSnapshot = () => mockSnapshot;
      mockDb.setRejectImpl(() => Promise.reject(new Error('DB connection failed')));
    ...
    it('should handle concurrent checkpoint calls gracefully', async () => {
      ...
      mockSessionTracker.createSnapshot = () => mockSnapshot;
      mockDb.setRejectImpl(() => new Promise(resolve => setTimeout(resolve, 100)));
