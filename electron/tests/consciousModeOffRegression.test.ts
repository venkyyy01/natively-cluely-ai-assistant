...
class FakeLLMHelper {
	...
	async *streamChat(
		...
		if (
			message.includes("STRUCTURED_REASONING_RESPONSE") ||
			message.includes("ACTIVE_REASONING_THREAD")
		) {
...
test("Conscious Mode off keeps the existing answer path unchanged for technical questions", async () => {
	...
	session.addAssistantMessage(
		"I have spent the last five years on distributed systems.",
	);
	addInterviewerTurn(
		session,
	...
	assert.doesNotMatch(
		llmHelper.calls[0].context ?? "",
		/Can you summarize your background first\?/,
	);
...
test("Conscious Mode off leaves follow-up refinement behavior unchanged", async () => {
	...
	session.addAssistantMessage(
		"Start with a simple token bucket backed by Redis and add burst handling.",
	);
