...
describe('ConsciousRefinementOrchestrator', () => {
  ...
  it('builds a shorten prompt that preserves the previous answer', () => {
    const prompt = orchestrator.buildPrompt({
      intent: 'shorten',
      previousAnswer: 'I built a caching layer on top of the database to reduce p99 latency from 500ms to 50ms.',
  ...
  it('flattens a structured response into spoken text', () => {
    const structured = parseConsciousModeResponse(JSON.stringify({
      schemaVersion: 'conscious_mode_v1',
      mode: 'reasoning_first',
      openingReasoning: 'So basically I would shard by user_id.',
      implementationPlan: ['Add a hash ring on top of Postgres.', 'Migrate writes through a dual-write phase.'],
  ...
  it('isRefinementUseful rejects shortening already short answers', () => {
    assert.strictEqual(
      orchestrator.isRefinementUseful({ intent: 'shorten', previousAnswer: 'Short answer.' }),
      false,
    );
    assert.strictEqual(
  ...
  it('isRefinementUseful is false on empty previous answer', () => {
    assert.strictEqual(
      orchestrator.isRefinementUseful({ intent: 'rephrase', previousAnswer: '' }),
      false,
    );
  });
});
