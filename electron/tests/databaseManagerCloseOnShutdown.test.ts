...
try {
  ...
  DatabaseManager = (require('../db/DatabaseManager') as DatabaseManagerModule).DatabaseManager;
...
function attachManager(dbPath: string): DatabaseManagerInstance {
  ...
  const manager = Object.create(RealDatabaseManager.prototype) as DatabaseManagerInstance;
...
test('DatabaseManager.close() releases the handle and removes WAL/SHM sidecars', { skip: skipReason ?? false }, () => {
...
test('DatabaseManager.close() is idempotent — second call is a safe no-op', { skip: skipReason ?? false }, () => {
  const { dbPath, cleanup } = makeTmpDb();
  try {
    const manager = attachManager(dbPath);
    manager.close();
...
test('DatabaseManager.close() flushes WAL so re-opening the same path succeeds and preserves data', { skip: skipReason ?? false }, () => {
  const { dbPath, cleanup } = makeTmpDb();
  try {
    const manager = attachManager(dbPath);
    manager.close();
...
test('DatabaseManager.closeIfOpen() is a no-op when the singleton was never instantiated', { skip: skipReason ?? false }, () => {
  const RealDatabaseManager = DatabaseManager!;
  // Force the singleton slot to be empty regardless of test ordering.
  (RealDatabaseManager as unknown as { instance?: DatabaseManagerInstance }).instance = undefined;
  ...
  assert.equal(
    (RealDatabaseManager as unknown as { instance?: DatabaseManagerInstance }).instance,
    undefined,
