...
test("SessionTracker Conscious Integration - should create thread on conscious mode activation", () => {
	...
	const thread = tracker
		.getThreadManager()
		.createThread("Test topic", "high_level_design");
	assert.ok(thread);
...
test("SessionTracker Conscious Integration - should keep live conscious state from interviewer transcript", () => {
	...
	assert.equal(
		tracker.getThreadManager().getActiveThread()?.phase,
		"high_level_design",
	);
});
...
test("SessionTracker Conscious Integration - emits conscious phase and thread action events on supervisor bus", async () => {
	...
	tracker.recordConsciousResponse(
		"How would you design a rate limiter for an API?",
		{
			mode: "reasoning_first",
			openingReasoning:
				"I would start with a per-user token bucket backed by Redis.",
...
test("SessionTracker Conscious Integration - builds long memory context from thread, constraints, pins, and summaries", () => {
	...
	(tracker as any).transcriptEpochSummaries = [
		"Earlier discussion covered sharding, hotspots, and failover plans.",
	];

	const memoryBlock = tracker.getConsciousLongMemoryContext(
		"How do you handle hotspots and failover in the rate limiter?",
	);
...
test("SessionTracker Conscious Integration - retrieves design-state facts for long-horizon follow-ups", async () => {
	...
	tracker.recordConsciousResponse(
		"How would you design a billing ledger?",
		{
			mode: "reasoning_first",
	...
	const context = await tracker.getConsciousRelevantContext(
		"What failure modes and schema choices matter most in the ledger?",
		900,
	);
...
test("SessionTracker restoreFromMeetingId restores conscious reasoning and hypothesis state", async () => {
	...
	try {
		...
		assert.equal(
			tracker.getLatestConsciousResponse()?.openingReasoning,
			"I would start with cache aside.",
		);
		assert.equal(
			tracker.getActiveReasoningThread()?.rootQuestion,
			"How would you design a cache?",
		);
		...
		assert.ok(
			tracker
				.getConsciousLongMemoryContext("What tradeoffs matter in the cache?")
				.includes("ARCHITECTURE_DECISIONS:"),
		);
	} finally {
		persistence.findByMeeting = originalFindByMeeting;
	}
...
test("SessionTracker restoreFromMeetingId restores suspended conscious threads for resume continuity", async () => {
	...
	try {
		const restored = await tracker.restoreFromMeetingId(
			"meeting-suspended-threads",
		);
		assert.equal(restored, true);
		...
		assert.ok(
			suspended[0].keyDecisions.includes("Use capped exponential backoff"),
		);
	} finally {
		persistence.findByMeeting = originalFindByMeeting;
	}
...
test("SessionTracker persists cleared conscious state when clearing threads or disabling conscious mode", () => {
	...
	try {
		...
		assert.equal(
			lastSnapshot?.consciousState?.threadState?.latestConsciousResponse ??
				null,
			null,
		);
		assert.deepEqual(
			lastSnapshot?.consciousState?.preferenceState?.directives ?? [],
			[],
		);
	} finally {
...
test("SessionTracker restoreFromMeetingId rebuilds recent transcript and usage from persisted memory state", async () => {
	...
	try {
		...
		assert.equal(
			tracker
				.getContext(120)
				.some((item) => item.text === "Can you optimize the cache writes?"),
			true,
		);
	} finally {
		persistence.findByMeeting = originalFindByMeeting;
