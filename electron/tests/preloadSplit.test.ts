...
test('NAT-067: preload.ts is a thin barrel', () => {
  const source = fs.readFileSync(path.join(process.cwd(), 'electron', 'preload.ts'), 'utf8')
  assert.ok(source.length > 0, 'preload.ts should not be empty')
  assert.ok(source.includes("from './preload/api'"), 'should import from api module')
})
...
test('NAT-067: preload/api.ts exports ElectronAPI interface and PROCESSING_EVENTS', () => {
  ...
  assert.ok(source.includes('export interface ElectronAPI'), 'should export ElectronAPI')
  assert.ok(source.includes('export const PROCESSING_EVENTS'), 'should export PROCESSING_EVENTS')
})

test('NAT-067: preload/types.ts exports helpers', () => {
  const source = fs.readFileSync(path.join(PRELOAD_DIR, 'types.ts'), 'utf8')
  assert.ok(source.includes('export const isIpcResult'), 'should export isIpcResult')
  assert.ok(source.includes('export const invokeAndUnwrap'), 'should export invokeAndUnwrap')
})
