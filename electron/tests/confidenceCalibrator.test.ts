...
describe('IsotonicCalibrator', () => {
  ...
  it('should return raw score on cold start (no bins)', () => {
    ...
    assert.strictEqual(calibrated, rawScore, 'Cold start should return raw score');
  });
  ...
  it('should fit on synthetic monotonic data', () => {
    ...
    for (let i = 1; i < bins.length; i++) {
      assert.ok(bins[i].calibrated >= bins[i - 1].calibrated, 'Bins should be monotonic');
    }
  });
  ...
  it('should not fit with insufficient samples (< 10)', () => {
    ...
    calibrator.fit(samples);
    const bins = calibrator.getBins();
    assert.strictEqual(bins.length, 0, 'Should not fit with insufficient samples');
  });
  ...
  it('should calibrate scores using interpolation', () => {
    ...
    assert.ok(calibrated >= 0 && calibrated <= 1, 'Calibrated score should be in [0, 1]');
  });
  ...
  it('should add samples and fit from them', () => {
    ...
    assert.strictEqual(calibrator.getSampleCount(), 5, 'Should track sample count');
    
    // Not enough samples yet (need 10)
    calibrator.fitFromSamples();
    assert.strictEqual(calibrator.getBins().length, 0, 'Should not fit with < 10 samples');
  ...
  it('should persist and restore calibration data', () => {
    ...
    assert.strictEqual(loadedBins.length, originalBins.length, 'Should restore same number of bins');
    
    for (let i = 0; i < originalBins.length; i++) {
      assert.strictEqual(loadedBins[i].raw, originalBins[i].raw, 'Bin raw value should match');
      assert.strictEqual(loadedBins[i].calibrated, originalBins[i].calibrated, 'Bin calibrated value should match');
    }
  });

  it('should handle missing calibration file gracefully', () => {
    const calibrator = IsotonicCalibrator.load('non-existent-profile');
    assert.strictEqual(calibrator.getBins().length, 0, 'Should have empty bins for non-existent file');
    assert.strictEqual(calibrator.getSampleCount(), 0, 'Should have 0 samples for non-existent file');
  });
  ...
  it('should limit samples to maxSamples (500)', () => {
    ...
    assert.strictEqual(calibrator.getSampleCount(), 500, 'Should limit to 500 samples');
  });
  ...
  it('should clear bins and samples', () => {
    ...
    assert.ok(calibrator.getSampleCount() > 0, 'Should have samples before clear');

    calibrator.clear();

    assert.strictEqual(calibrator.getBins().length, 0, 'Should have no bins after clear');
    assert.strictEqual(calibrator.getSampleCount(), 0, 'Should have no samples after clear');
  });
  ...
  it('should pool adjacent violators correctly', () => {
    ...
    for (let i = 1; i < bins.length; i++) {
      assert.ok(bins[i].calibrated >= bins[i - 1].calibrated, `Bins should be monotonic at index ${i}`);
    }
  });
});
