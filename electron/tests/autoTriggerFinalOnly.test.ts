...
function buildIntelligenceManager(): {
  ...
  triggers: Array<{ context: string; lastQuestion: string; confidence: number }>;
} {
  const triggers: Array<{ context: string; lastQuestion: string; confidence: number }> = [];
...
test('NAT-006: non-interviewer speaker is rejected even when final', async () => {
  const { manager, triggers } = buildIntelligenceManager();
  const input = buildInput(manager, { speaker: 'me', final: true, confidence: 0.9 });
