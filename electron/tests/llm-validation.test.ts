...
test("should fail on too many sentences", () => {
	...
	assert.ok(
		result.violations.some((v: string) =>
			v.includes("Too many sentences: 3/2"),
		),
	);
});
...
test("should fail on long sentences", () => {
	const response =
		"This is a really long sentence that definitely exceeds the twenty-five word limit that we have established for maintaining conciseness and clarity in all of our communications.";
	const result = validateResponseQuality(response);
	assert.equal(result.isValid, false);
