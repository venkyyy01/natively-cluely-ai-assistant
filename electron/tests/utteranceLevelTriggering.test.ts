...
function buildManager(): {
  ...
  triggers: Array<{ context: string; lastQuestion: string; confidence: number; sourceUtteranceId?: string }>;
} {
  const triggers: Array<{ context: string; lastQuestion: string; confidence: number; sourceUtteranceId?: string }> = [];
  return {
    triggers,
    manager: {
...
test('fallback trigger uses punctuation, question prefixes, and buffer flush instead of raw word count', () => {
  assert.equal(getTranscriptSuggestionDecision('five plain words without question', false, null).shouldTrigger, false);
  assert.equal(getTranscriptSuggestionDecision('Can you repeat that for me', false, null).shouldTrigger, true);
  assert.equal(getTranscriptSuggestionDecision('Walk me through the cache design', false, null).shouldTrigger, true);
  assert.equal(getTranscriptSuggestionDecision('plain buffered statement', false, null, true).shouldTrigger, false);
});
...
test('useUtteranceLevelTriggering buffers fragments and fires one merged trigger on flush', async () => {
  ...
  const utteranceBuffer = new InterviewerUtteranceBuffer({ silenceMs: 5_000, maxBufferMs: 10_000 });

  const first = await maybeHandleSuggestionTriggerFromTranscript(buildInput(manager, {
    text: 'Walk me through',
    utteranceBuffer,
    triggerAuditLog: audit,
  }));

  const second = await maybeHandleSuggestionTriggerFromTranscript(buildInput(manager, {
  ...
  assert.deepEqual(audit.getEntries().map((entry) => entry.reasonCode), ['declined_no_punctuation', 'fired', 'completed']);
...
test('utterance-level triggering handles multi-question interviewer turns', async () => {
  ...
  const triggered = await maybeHandleSuggestionTriggerFromTranscript(buildInput(manager, {
    text: 'Tell me about a project you led. Also, what did you learn?',
    utteranceBuffer,
  }));

  assert.equal(triggered, true);
  assert.deepEqual(triggers.map((trigger) => trigger.lastQuestion), [
    'Tell me about a project you led.',
    'Also, what did you learn?',
  ]);
  assert.deepEqual(triggers.map((trigger) => trigger.sourceUtteranceId), ['utterance-1', 'utterance-1:2']);
  utteranceBuffer.dispose();
});
...
test('user mic transcript triggers are rejected by default and accepted behind flag', async () => {
  ...
  assert.equal(await maybeHandleSuggestionTriggerFromTranscript(buildInput(rejected.manager, {
    speaker: 'user',
  ...
  assert.equal(await maybeHandleSuggestionTriggerFromTranscript(buildInput(accepted.manager, {
    speaker: 'user',
    text: 'How would you design this?',
  })), true);
