...
describe('HumanLikeConversationEngine', () => {
  ...
  it('shouldBypassStructuredSchema is true only for non-technical turns', () => {
    ...
    assert.strictEqual(engine.shouldBypassStructuredSchema('Make it shorter'), true);
    assert.strictEqual(
      engine.shouldBypassStructuredSchema('Explain consistent hashing in detail'),
      false,
    );
  });
  ...
  it('preferFreeForm is true for non-technical turns', () => {
    ...
    assert.strictEqual(engine.classify('What do you mean?').preferFreeForm, true);
    assert.strictEqual(
