...
function createHandlerRegistry() {
  ...
  return {
    ...
    safeHandleValidated: <T extends unknown[]>(channel: string, parser: (args: unknown[]) => T, listener: any) => {
      handlers.set(channel, (event: unknown, ...args: unknown[]) => listener(event, ...parser(args)));
    },
  };
}
...
test('Conscious Mode IPC persists backend truth and returns spec-shaped success contracts', async () => {
  const userDataPath = fs.mkdtempSync(path.join(os.tmpdir(), 'conscious-mode-ipc-'));
  const { SettingsManager, registerSettingsHandlers, restoreElectron } = await loadSettingsModules(userDataPath);
  ...
  assert.deepEqual(await getConsciousMode?.({}), { success: true, data: { enabled: false } } satisfies ConsciousModeSuccess);
  assert.deepEqual(await setConsciousMode?.({}, true), { success: true, data: { enabled: true } } satisfies ConsciousModeSuccess);
  assert.deepEqual(await getConsciousMode?.({}), { success: true, data: { enabled: true } } satisfies ConsciousModeSuccess);
  ...
  (SettingsManager as any).instance = undefined;
  assert.equal(SettingsManager.getInstance().get('consciousModeEnabled' as any), true);

  restoreElectron();
});

test('Conscious Mode IPC returns structured failure contracts when persistence does not complete', async () => {
  const userDataPath = fs.mkdtempSync(path.join(os.tmpdir(), 'conscious-mode-ipc-errors-'));
  const { SettingsManager, registerSettingsHandlers, restoreElectron } = await loadSettingsModules(userDataPath);
  (SettingsManager as any).instance = undefined;
...
test('Conscious Mode IPC returns structured failure contracts when backend setter throws', async () => {
  const userDataPath = fs.mkdtempSync(path.join(os.tmpdir(), 'conscious-mode-ipc-throws-'));
  const { SettingsManager, registerSettingsHandlers, restoreElectron } = await loadSettingsModules(userDataPath);
  (SettingsManager as any).instance = undefined;
...
test('SettingsManager narrows persisted Conscious Mode and disguise settings on load', async () => {
  const userDataPath = fs.mkdtempSync(path.join(os.tmpdir(), 'conscious-mode-validate-'));
  const settingsPath = path.join(userDataPath, 'settings.json');
