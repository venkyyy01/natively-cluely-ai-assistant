...
describe('StartupProtectionGate', () => {
  it('allows default observe-only startup reveal while recording the would-block failure', async () => {
    ...
    const decision = await gate.evaluateReveal({ source: 'test.observe', windowRole: 'primary' });
  ...
  it('blocks strict startup reveal when protection verification fails', async () => {
    ...
    const decision = await gate.evaluateReveal({ source: 'test.strict', windowRole: 'primary' });
  ...
  it('allows strict startup reveal after positive verification', async () => {
    ...
    const decision = await gate.evaluateReveal({ source: 'test.pass', windowRole: 'primary' });
  ...
  it('fails closed on strict startup verification timeout', async () => {
    ...
    const decision = await gate.evaluateReveal({ source: 'test.timeout', windowRole: 'primary' });
  ...
  it('fails closed on strict startup verification errors', async () => {
    ...
    const decision = await gate.evaluateReveal({ source: 'test.error', windowRole: 'primary' });
