  ...
  (Module as any)._load = function patchedLoad(request: string, parent: unknown, isMain: boolean): unknown {
    if (request === './nativeModule') {
      return {
        ...
        assertNativeAudioAvailable: (): NativeAudioModule | null => assertedModule,
...
test('MicrophoneCapture reports a missing native constructor when the native module fails to load', async () => {
  ...
  try {
    ...
    assert.throws(() => new MicrophoneCapture(), /Rust class implementation not found/);
  } finally {
    restore();
  }
});
...
test('SystemAudioCapture reports a missing native constructor when validation cannot provide one', async () => {
  ...
  try {
    ...
    assert.throws(() => new SystemAudioCapture(), /Rust class implementation not found/);
  } finally {
    restore();
  }
});
