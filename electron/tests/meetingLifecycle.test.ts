...
describe("Meeting Lifecycle Race Conditions", () => {
	...
	it("should handle start→end race conditions", async () => {
		const startPromise = (async () => {
			...
			if (
				startSequence !== mockState.meetingStartSequence ||
				mockState.meetingLifecycleState !== "starting"
			) {
				return;
			}
	...
	it("should save meeting data on forced quit", async () => {
		...
		assert.strictEqual(
			mockPersistence.waitForPendingSaves.mock.calls.length,
			1,
		);
		assert.strictEqual(
			mockPersistence.waitForPendingSaves.mock.calls[0].arguments[0],
			10000,
		);
	...
	it("should prevent duplicate meeting starts", async () => {
		...
		await Promise.all([startMeeting(1), startMeeting(2)]);
