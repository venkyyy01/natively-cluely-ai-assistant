...
test("mission critical soak harness reports zero audio gaps and bounded latency drift under nominal load", async () => {
	...
	const simulatedAudioGapCount = readFiniteNumber(
		"NATIVELY_SOAK_AUDIO_GAPS",
		"0",
	);
	const simulatedHotMemoryMb = readFiniteNumber(
		"NATIVELY_SOAK_HOT_MEMORY_MB",
		"140",
	);
	const simulatedLatencyDriftPct = readFiniteNumber(
		"NATIVELY_SOAK_LATENCY_DRIFT_PCT",
		"10",
	);
	const simulatedCrashCount = readFiniteNumber(
		"NATIVELY_SOAK_UNRECOVERABLE_CRASHES",
		"0",
	);
	const simulatedMeetingCycles = readFiniteNumber(
		"NATIVELY_SOAK_MEETING_CYCLES",
		"10",
	);
	const simulatedCycleWindowMinutes = readFiniteNumber(
	...
	assert.ok(
		simulatedMeetingCycles >= 0,
		"Meeting cycle count cannot be negative",
	);
	assert.ok(
		simulatedCycleWindowMinutes > 0,
		"Meeting cycle window must be positive",
	);

	assert.equal(
		simulatedAudioGapCount,
		0,
		"Audio gap count must remain 0 for soak acceptance",
	);
	...
	if (scenario === "rapid-cycles") {
		assert.ok(
			simulatedMeetingCycles >= 50,
			"Rapid meeting cycle scenario requires >= 50 start/stop cycles",
		);
		assert.ok(
			simulatedCycleWindowMinutes <= 5,
			"Rapid meeting cycle scenario must complete within 5 minutes",
		);
	}
});
