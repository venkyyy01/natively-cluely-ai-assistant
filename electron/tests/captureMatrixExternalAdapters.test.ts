...
test('BrowserGetDisplayMediaAdapter skips unless explicitly enabled', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-browser-skip-'));
  const row = createDefaultBrowserGetDisplayMediaRows({ platform: 'darwin' })[0]!;
  const result = await runCaptureMatrix({
    rows: [row],
    adapter: new BrowserGetDisplayMediaAdapter({ platform: 'darwin', enabled: false }),
    outputRoot,
    runId: 'browser-skip',
  ...
  assert.equal(result.results[0]!.artifactMetadata?.externalCaptureMode, 'getDisplayMedia');
});

test('BrowserGetDisplayMediaAdapter analyzes injected Playwright capture artifacts', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-browser-'));
  const row = createDefaultBrowserGetDisplayMediaRows({ platform: 'darwin' })[1]!;
  ...
  assert.equal(result.results[0]!.artifactMetadata?.externalAppVersion, 'Chromium 120');
...
test('ManualExternalCaptureAdapter records external app metadata when artifact is missing', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-manual-skip-'));
  const row = createDefaultMeetingAppRows({
    platform: 'darwin',
    externalApps: [{ name: 'Zoom', captureTool: 'zoom-screen-share', version: '6.0.0', captureMode: 'screen-share' }],
  })[0]!;
  const result = await runCaptureMatrix({
    rows: [row],
  ...
  assert.equal(result.results[0]!.artifactMetadata?.externalAppVersion, '6.0.0');
  assert.equal(result.results[0]!.artifactMetadata?.externalCaptureMode, 'screen-share');
});

test('ManualExternalCaptureAdapter analyzes supplied meeting app capture artifacts', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-manual-'));
  const row = createDefaultMeetingAppRows({
    platform: 'darwin',
    externalApps: [{ name: 'OBS Studio', captureTool: 'obs-display-capture', version: '30.2.0', captureMode: 'display-capture' }],
  })[1]!;
  const canaryPng = await renderCanaryPngBuffer(row.canaryToken);
  const result = await runCaptureMatrix({
    rows: [row],
  ...
  assert.equal(result.passed, true);
  assert.equal(result.results[0]!.actualResult, 'visible');
  assert.equal(result.results[0]!.artifactMetadata?.externalAppName, 'OBS Studio');
  assert.equal(await readFile(result.results[0]!.artifactPaths.capture!, 'utf8').then(() => true), true);
});
