...
import { SessionPersistence, type PersistedSession, type SessionEvent } from '../memory/SessionPersistence';
...
test('NAT-059: replayEvents returns multiple events in order', async () => {
  ...
  assert.deepEqual(events.map((e) => e.eventId), ['evt-0', 'evt-1', 'evt-2']);
