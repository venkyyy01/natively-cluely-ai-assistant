...
test("NAT-034: generic user-agent does not contain Electron", () => {
	...
	assert.ok(
		!expectedUa.includes("Electron"),
		"user-agent must not contain Electron substring",
	);
	assert.ok(
		expectedUa.includes("Chrome"),
		"user-agent should contain Chrome for provider compatibility",
	);
});
