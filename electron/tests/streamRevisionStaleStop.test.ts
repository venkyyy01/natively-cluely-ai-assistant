...
test("NAT-007 / audit A-7: stream stops mid-flight when the source utterance revision advances", async () => {
	...
	const answer = await engine.runWhatShouldISay(
		undefined,
		0.9,
		undefined,
		0,
	...
	assert.ok(
		(session.getUtteranceRevision("utterance-1") ?? 0) >
			(initialUtteranceRev ?? 0),
		"sanity: utterance revision for utterance-1 actually advanced during the run",
	);

	const snapshot =
		tracker.completedSnapshots[tracker.completedSnapshots.length - 1];
	assert.equal(
	...
	const helper = new ScheduledLLMHelper([
		{
			...
			onYield: () => {
				addUserTurn(
					session,
					"I would start with requirements.",
					Date.now() + 100,
					"utterance-2",
				);
			},
...
test("NAT-007: AnswerLatencyTracker.markStaleStop finalizes snapshot exactly once", () => {
	...
	const stale = tracker.markStaleStop(
		requestId,
		"source_utterance_revision_changed",
	);
