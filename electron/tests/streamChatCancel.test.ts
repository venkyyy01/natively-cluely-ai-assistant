...
test("NAT-036: gemini-chat-stream rejects non-UUID requestId at schema level", () => {
	assert.throws(() => {
		parseIpcInput(
			ipcSchemas.geminiChatArgs,
			["hello", undefined, undefined, { requestId: "not-a-uuid" }],
			"gemini-chat-stream",
		);
	}, /requestId/);
});
...
test("NAT-036: streamGeminiChat passes abortSignal to LLMHelper.streamChat", async () => {
	...
	const controller = new AbortController();
	const gen = mockLlmHelper.streamChat(
		"hello",
		undefined,
		undefined,
		undefined,
...
test("NAT-036: pre-aborted signal causes streamChat to yield nothing", async () => {
	...
	const tokens: string[] = [];
	for await (const token of mockLlmHelper.streamChat(
		"hello",
		undefined,
		undefined,
		undefined,
	...
	assert.equal(
		tokens.length,
		0,
		"pre-aborted signal should produce zero tokens",
	);
});
