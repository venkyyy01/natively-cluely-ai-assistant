...
describe('SetFitReactionClassifier', () => {
  ...
  it('should return null for low confidence (below threshold)', async () => {
    ...
    if (result === null) {
      ...
      assert.ok(result.confidence < 0.8, 'If not null, confidence should be below threshold');
    }
  });
  ...
  it('should handle model loading failure gracefully', async () => {
    ...
    assert.ok(classifier.isModelLoaded() === false || classifier.isModelLoaded() === true, 'Model load state should be defined');
  });

  it('should classify clear tradeoff language', async () => {
    const classifier = new SetFitReactionClassifier();
    const result = await classifier.classify('What are the tradeoffs of this approach?');
  ...
  it('should classify challenge', async () => {
    const classifier = new SetFitReactionClassifier();
    const result = await classifier.classify('Why did you choose this over that?');
  ...
  it('should classify example request', async () => {
    const classifier = new SetFitReactionClassifier();
    const result = await classifier.classify('Can you give me a concrete example?');
  ...
  it('should classify deep dive', async () => {
    const classifier = new SetFitReactionClassifier();
    const result = await classifier.classify('What happens if the system fails?');
  ...
  it('should classify topic shift', async () => {
    const classifier = new SetFitReactionClassifier();
    const result = await classifier.classify("Let's switch gears and talk about something else.");
