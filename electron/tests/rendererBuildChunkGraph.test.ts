...
const getJavaScriptChunkImports = (filePath: string): string[] => {
  ...
  const matches = source.matchAll(/import(?:[^'";]*from\s*)?["'](\.\/[^"']+\.js)["']/g);
...
test('production renderer build does not emit circular entry chunk imports', () => {
  ...
  assert.ok(referencedChunks.length > 0, 'expected index.html to reference built JavaScript chunks');
  ...
  for (const relativeChunkPath of referencedChunks) {
    ...
    const normalizedImports = getJavaScriptChunkImports(absoluteChunkPath).map((importPath) =>
      path.posix.normalize(path.posix.join(path.posix.dirname(normalizedChunkPath), importPath.replace(/^\.\//, ''))),
    );
  ...
  assert.deepEqual(circularPairs, [], `expected build entry chunks to be acyclic, got ${circularPairs.join(', ')}`);
});
