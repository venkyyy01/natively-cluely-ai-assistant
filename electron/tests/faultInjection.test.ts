...
test("fault injection: stealth heartbeat miss emits fail-closed fault and sheds critical lanes", async () => {
	...
	const inference = new InferenceSupervisor({
		...
		delegate: {
			...
			stop: async () => {
				calls.push("inference:stop");
			},
			onStealthFault: async (reason) => {
				calls.push(`inference:stealth-fault:${reason}`);
			},
	...
	const audio = new AudioSupervisor({
		...
		delegates: {
			...
			onStealthFault: async (reason) => {
				calls.push(`audio:stealth-fault:${reason}`);
			},
		},
	});
	...
	const stt = new SttSupervisor({
		...
		delegates: {
			startSpeaker: async () => {
				calls.push("stt:start");
			},
			stopSpeaker: async () => {
				calls.push("stt:stop");
			},
			onStealthFault: async (reason) => {
				calls.push(`stt:stealth-fault:${reason}`);
			},
		},
	});
	...
	const stealth = new StealthSupervisor(
		...
		bus,
		{
			nativeBridge: {
				arm: async () => ({
					connected: true,
					sessionId: "fault-injection-heartbeat",
					surfaceId: "surface-fault-injection-heartbeat",
				}),
				heartbeat: async () => ({ connected: true, healthy: true }),
				fault: async () => {},
				setHelperFaultHandler(
					handler: ((reason: string) => void | Promise<void>) | undefined,
				) {
...
test("NAT-084: speculative invalidation between preview and finalize yields no suggested_answer", async () => {
	...
	assert.equal(
		result,
		null,
		"finalize after invalidation must return null, never a suggested_answer",
	);
