...
test('retry normalization treats provider status objects consistently', async () => {
  ...
  assert.equal(helper.isRetryableError({ status: 429, message: 'rate limited' }), true);
  assert.equal(helper.isRetryableError({ response: { status: 503 }, message: 'unavailable' }), true);
  assert.equal(helper.isRetryableError({ code: 'ECONNRESET', message: 'socket hang up' }), true);
  assert.equal(helper.isRetryableError({ status: 400, message: 'bad request' }), false);
  helper.scrubKeys();
});
