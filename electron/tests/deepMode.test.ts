...
test("classifyDeepModeQuestion qualifies real technical questions", () => {
	assert.strictEqual(
		classifyDeepModeQuestion("how would you design a rate limiter?"),
		true,
	);
	...
	assert.strictEqual(
		classifyDeepModeQuestion("tell me about your architecture decisions"),
		true,
	);
	assert.strictEqual(
		classifyDeepModeQuestion("what tradeoffs did you consider"),
		true,
	);
});
