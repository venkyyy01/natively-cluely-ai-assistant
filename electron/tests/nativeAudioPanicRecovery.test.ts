...
class PanickingSystemAudioMonitor {
  ...
  start(callback: AudioCallback): void {
    ...
    setImmediate(() => {
      const err = new Error(`audio_thread_panic: ${PanickingSystemAudioMonitor.panicMessage}`);
...
class PanickingMicrophoneMonitor {
  ...
  start(callback: AudioCallback): void {
    this.callback = callback;
    setImmediate(() => {
      const err = new Error(`audio_thread_panic: ${PanickingMicrophoneMonitor.panicMessage}`);
      this.callback?.(err, undefined);
    });
  }
...
test('SystemAudioCapture surfaces a Rust DSP panic as an AUDIO_THREAD_PANIC error event', async () => {
  ...
  try {
    ...
    const err = errors[0];
    assert.match(err.message, /audio_thread_panic/, 'error message should include audio_thread_panic prefix');
    assert.match(err.message, /speaker stream poisoned/, 'error message should preserve native panic text');
...
test('MicrophoneCapture surfaces a Rust DSP panic as an AUDIO_THREAD_PANIC error event', async () => {
  ...
  try {
    ...
    const err = errors[0];
    assert.match(err.message, /audio_thread_panic/, 'error message should include audio_thread_panic prefix');
    assert.match(err.message, /mic ringbuf overrun/, 'error message should preserve native panic text');
    assert.equal((err as Error & { code?: string }).code, 'AUDIO_THREAD_PANIC');
  } finally {
    restore();
  }
...
test('Panic Error never reaches the data-event handler as a chunk', async () => {
  ...
  try {
    ...
    capture.on('data', (chunk: Buffer) => {
      assert.ok(Buffer.isBuffer(chunk), 'data handler must only see Buffer chunks');
    });
    capture.on('error', () => {});
