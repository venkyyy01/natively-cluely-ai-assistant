...
import {
	NativeStealthBridge,
	type NativeStealthBridgeClient,
...
test("StealthSupervisor fails closed when required native helper arm is skipped by the arm guard", async () => {
	...
	await assert.rejects(
		...
		/native stealth helper did not arm/,
	);
	...
	const supervisor = new StealthSupervisor(
		...
		bus,
		{
			nativeBridge: {
				arm: async () => ({
					connected: true,
					sessionId: "session-proactive-fault",
					surfaceId: "surface-proactive-fault",
				}),
...
test("StealthSupervisor stays in FULL_STEALTH after a native helper sleep/wake cycle when the bridge restart succeeds", async () => {
	...
	const healthyEnvelope = (
		...
		data: {
			...
			state: (presenting
				? "presenting"
				: "attached") as MacosLayer3SessionState,
...
test("StealthSupervisor fails closed once the native helper disconnects after exhausting restart budget", async () => {
	...
	assert.deepEqual(faultReasons, [
		"native-bridge-restart-exhausted: heartbeat",
	]);
	assert.equal(createCalls, 3);
});
