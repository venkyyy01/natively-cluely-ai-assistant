...
test('NAT-058: ConsciousCache enforces maxMemoryMB under large entries', () => {
  ...
  assert.ok(stats.memoryBytes <= maxBytes + 256 * 1024, 'tracked bytes stay bounded by maxMemoryMB with headroom for estimate error');
  assert.ok(stats.size < 3, 'at least one LRU eviction for memory should drop an entry');
});
...
test('NAT-058: ConsciousCache semantic search respects bind partition', () => {
  ...
  assert.equal(cache.get('hello world', { bind: bindA, embedding: [1, 0, 0] }), 'from-A');
