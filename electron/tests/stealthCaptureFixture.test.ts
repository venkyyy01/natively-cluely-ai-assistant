...
test('NAT-082: mock mode passes when all windows have content protection', async () => {
  ...
  assert.ok(results.every((r) => r.passed), 'all protected windows should pass');
  assert.ok(results.every((r) => r.blank), 'all should report blank');
});
...
test('NAT-082: mock mode fails when content protection is removed', async () => {
  ...
  assert.equal(results.length, 5);
  assert.ok(results.every((r) => !r.passed), 'all unprotected windows should fail');
  assert.ok(results.every((r) => !r.blank), 'none should report blank');
});
...
test('NAT-082: all default windows have expected NSWindow level', () => {
  ...
  for (const w of windows) {
    assert.equal(w.expectedNsWindowLevel, 19, `${w.name} should have NSWindow level 19`);
  }
});
...
test('NAT-082: live mode returns placeholder failure', async () => {
  ...
  assert.equal(results.length, 5);
  assert.ok(results.every((r) => !r.passed), 'live mode should return not-yet-implemented');
  assert.ok(results.every((r) => r.reason?.includes('live mode not yet implemented')));
});
