...
test('NAT-038: three triggers within 200 ms produce exactly one downstream runWhatShouldISay call', async () => {
  ...
  (engine as any).runWhatShouldISay = async function (...args: Parameters<typeof originalRun>) {
  ...
  assert.ok(downstreamCallCount <= 3, 'no more than 3 triggers (legacy behavior bound)');
});
