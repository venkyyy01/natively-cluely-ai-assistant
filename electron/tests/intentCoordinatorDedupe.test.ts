...
import { IntentClassificationCoordinator } from "../llm/providers/IntentClassificationCoordinator";
import type {
...
function makeInput(
	...
	return {
		...
		preparedTranscript:
			"[INTERVIEWER] Tell me about a difficult conflict you handled.",
...
test("NAT-039: identical concurrent classify calls share a single primary invocation", async () => {
	...
	assert.equal(
		primaryCalls,
		1,
		"primary should only be invoked once across concurrent identical calls",
	);
...
test("NAT-039: classify failures are evicted so the next caller can retry", async () => {
	...
	assert.equal(
		primaryCalls,
		2,
		"failed entry must not poison subsequent retries",
	);
});
