...
test('CaptureMatrixLocalAdapters detects rendered canary marker pixels', async () => {
  const positive = await renderCanaryPngBuffer('NATIVELY_CAPTURE_CANARY_CONTROL');
  ...
  const negative = Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="320" height="180"><rect width="100%" height="100%" fill="#000"/></svg>');
...
test('CaptureMatrixLocalAdapters writes canary HTML and PNG fixture files', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-canary-'));
...
test('CaptureMatrixHarness persists capture artifacts and records explicit skips', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-artifact-'));
  ...
  class ArtifactAdapter implements CaptureMatrixAdapter {
    ...
    async collectArtifact(row: CaptureMatrixRow, _session: CaptureAdapterSession): Promise<CaptureMatrixArtifact> {
    ...
    async analyze(row: CaptureMatrixRow, artifact: CaptureMatrixArtifact) {
      return {
        actualResult: artifact.canaryVisible ? 'visible' as const : 'hidden' as const,
        passed: row.expectedResult === (artifact.canaryVisible ? 'visible' : 'hidden'),
      };
    }
  ...
  assert.equal(await readFile(result.results[1]!.artifactPaths.capture!, 'utf8'), 'capture');
...
test('MacosScreencaptureAdapter can prove visible control from injected capture output', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-screencapture-'));
  const controlRow = createDefaultMacosScreencaptureRows({ platform: 'darwin' })[1]!;
...
test('MacosScreencaptureAdapter skips when canary surface is not armed', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-screencapture-disarmed-'));
  const row = createDefaultMacosScreencaptureRows({ platform: 'darwin' })[0]!;
  const result = await runCaptureMatrix({
    rows: [row],
    adapter: new MacosScreencaptureAdapter({ platform: 'darwin', commandPath: '/fake/screencapture' }),
    outputRoot,
...
test('MacosScreencaptureAdapter skips when platform is not darwin', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-screencapture-skip-'));
...
test('MacosCgWindowEnumerationAdapter detects canary in native window titles', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-cgwindow-'));
...
test('MacosScreenCaptureKitAdapter is explicit opt-in and otherwise skips', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-sck-'));
  const row = createDefaultMacosScreenCaptureKitRows({ platform: 'darwin' })[0]!;
  const result = await runCaptureMatrix({
    rows: [row],
    adapter: new MacosScreenCaptureKitAdapter({ platform: 'darwin', enabled: false }),
    outputRoot,
    runId: 'sck-skip',
    generatedAt: '1970-01-01T00:00:00.000Z',
  });
...
test('WindowsCaptureAdapterStub records explicit skip reason', async () => {
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), 'capture-matrix-windows-'));
