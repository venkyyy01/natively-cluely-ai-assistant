...
function response(
	...
	return {
		...
		implementationPlan: [
			...
			"Use Kafka for the async fan-out",
		],
		tradeoffs: [],
		edgeCases: [],
		scaleConsiderations: [],
...
function behavioralResponse(
	...
	return response({
		behavioralAnswer: {
			...
			action:
				"I analyzed metrics, identified a slow database query, added proper indexing, and implemented query caching.",
			result:
				"API latency dropped from 200ms to 120ms, improving user experience and reducing timeouts.",
			whyThisAnswerWorks: [
...
describe("StarScorer", () => {
	it("should score high-depth action > 0.7", () => {
		...
		const highDepthResponse = behavioralResponse({
			action:
				"I analyzed production metrics, identified a slow N+1 query pattern in the user service, implemented Redis caching with a 5-minute TTL, and added database connection pooling to handle peak load.",
			result:
				"API latency dropped from 200ms to 120ms, reducing timeout errors by 60% and improving user satisfaction scores.",
		});

		const score = scorer.score(highDepthResponse);
		assert.ok(
			score.overall > 0.7,
			`High-depth action should score > 0.7, got ${score.overall}`,
		);
	});
	...
	it("should score low-depth action < 0.5", () => {
		...
		assert.ok(
			score.overall < 0.5,
			`Low-depth action should score < 0.5, got ${score.overall}`,
		);
	});
	...
	it("should accept concise but high-quality answers", () => {
		...
		const conciseResponse = behavioralResponse({
			action:
				"I added Redis caching and database indexing to reduce API latency.",
			result: "Latency dropped from 200ms to 120ms, improving user experience.",
		});

		const score = scorer.score(conciseResponse);
		assert.ok(
			scorer.isAcceptable(score),
			`Concise high-quality answer should be acceptable, got score ${score.overall}`,
		);
	});
	...
	it("should return 0 for missing behavioral field", () => {
		...
		assert.strictEqual(
			score.overall,
			0,
			"Missing behavioral field should score 0",
		);
		assert.ok(
			!scorer.isAcceptable(score),
			"Missing behavioral field should not be acceptable",
		);
	});
	...
	it("should extract action word count correctly", () => {
		...
		assert.strictEqual(
			score.details.actionWordCount,
			7,
			"Action word count should be 7",
		);
	});
	...
	it("should extract result word count correctly", () => {
		...
		const score = scorer.score(testResponse);
		assert.strictEqual(
			score.details.resultWordCount,
			6,
			"Result word count should be 6",
		);
	});
	...
	it("should compute action-to-situation ratio correctly", () => {
		...
		const testResponse = behavioralResponse({
			situation: "The system was slow.",
			action:
				"I analyzed metrics, identified the bottleneck, implemented caching, and optimized the database queries.",
		});

		const score = scorer.score(testResponse);
		assert.ok(
			score.details.actionToSituationRatio > 1,
			"Action should be longer than situation",
		);
	});
	...
	it("should compute action-to-task ratio correctly", () => {
		...
		const score = scorer.score(testResponse);
		assert.ok(
			score.details.actionToTaskRatio > 1,
			"Action should be longer than task",
		);
	});
	...
	it("should return scores in [0, 1] range", () => {
		...
		assert.ok(
			...
			`Score should be in [0, 1], got ${score.overall}`,
		);
	});
	...
	it("should accept high-quality answers with threshold", () => {
		...
		const highQualityResponse = behavioralResponse({
			action:
				"I analyzed production metrics, identified a slow N+1 query pattern, implemented Redis caching with proper invalidation, and added database connection pooling.",
			result:
				"API latency dropped from 200ms to 120ms, reducing timeout errors by 60% and improving user satisfaction.",
		});

		const score = scorer.score(highQualityResponse);
		assert.ok(
			scorer.isAcceptable(score),
			`High-quality answer should be acceptable with threshold 0.55, got ${score.overall}`,
		);
	});
	...
	it("should reject low-quality answers with threshold", () => {
		...
		assert.ok(
			!scorer.isAcceptable(score),
			`Low-quality answer should not be acceptable with threshold 0.55, got ${score.overall}`,
		);
	});
});
