...
test("window asset paths prefer unpacked preloads in packaged builds", () => {
	...
	const unpackedPreload =
		"/Applications/Natively.app/Contents/Resources/app.asar.unpacked/dist-electron/electron/preload.js";
	const unpackedShellPreload =
		"/Applications/Natively.app/Contents/Resources/app.asar.unpacked/dist-electron/electron/stealth/shellPreload.js";
	const rendererEntry =
		"/Applications/Natively.app/Contents/Resources/app.asar/dist/index.html";
	const existing = new Set<string>([
		unpackedPreload,
		unpackedShellPreload,
		rendererEntry,
	]);

	const context = {
		electronDir:
			"/Applications/Natively.app/Contents/Resources/app.asar/dist-electron/electron",
		isPackaged: true,
		nodeEnv: "production",
		appPath,
		resourcesPath,
	...
	assert.equal(
		resolveRendererStartUrl(context),
		"file:///Applications/Natively.app/Contents/Resources/app.asar/dist/index.html",
	);
});
...
test("window asset paths encode file URLs safely for paths with spaces", () => {
	...
	assert.equal(
		resolveRendererStartUrl(context),
		"file:///tmp/Natively%20Preview/dist/index.html",
	);
});
...
test("window asset paths tolerate missing process.resourcesPath outside Electron-managed contexts", () => {
	...
	assert.equal(
		resolveRendererStartUrl(context),
		"file:///tmp/build/dist/index.html",
	);
	assert.equal(
		resolveRendererPreloadPath(context),
		"/tmp/build/dist-electron/electron/preload.js",
	);
});
