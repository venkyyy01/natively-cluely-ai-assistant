...
describe('ProtectionStateMachine', () => {
  it('tracks the normal hidden protection flow without violations', () => {
    ...
    machine.record('protection-apply-started', { source: 'test', windowRole: 'primary', visible: false });
    machine.record('protection-apply-finished', { source: 'test', windowRole: 'primary', visible: false });
    const snapshot = machine.record('verification-passed', { source: 'test', windowRole: 'primary' });
  ...
  it('moves to visible-protected only after verification', () => {
    ...
    machine.record('protection-apply-started', { source: 'test', visible: false });
    machine.record('protection-apply-finished', { source: 'test', visible: false });
    machine.record('verification-passed', { source: 'test' });
    const visible = machine.record('shown', { source: 'test.show', visible: true });

    assert.equal(visible.state, 'visible-protected');
    assert.equal(visible.violations.length, 0);

    const hidden = machine.record('hidden', { source: 'test.hide', visible: false });
  ...
  it('enters fault-contained and can begin recovery', () => {
    ...
    const faulted = machine.record('fault', { source: 'test.fault', reason: 'verification failed' });
    assert.equal(faulted.state, 'fault-contained');

    const recovering = machine.record('recovery-requested', { source: 'test.recovery' });
