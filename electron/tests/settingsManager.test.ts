...
test("SettingsManager preserves configured capture tool patterns as strings", async () => {
	const userDataPath = fs.mkdtempSync(
		path.join(os.tmpdir(), "capture-patterns-validate-"),
	);
	...
	const { SettingsManager, restoreElectron } =
		await loadSettingsManager(userDataPath);
	try {
		const settings = SettingsManager.getInstance();
		assert.deepEqual(settings.get("captureToolPatterns" as any), [
			"obs",
			"screen studio",
		]);
	} finally {
...
test("SettingsManager refuses to initialize before Electron is ready", async () => {
	const userDataPath = fs.mkdtempSync(
		path.join(os.tmpdir(), "settings-not-ready-"),
	);
	const { SettingsManager, restoreElectron } = await loadSettingsManager(
		userDataPath,
		false,
	);
	...
	try {
		assert.throws(
			...
			/Cannot initialize before app\.whenReady/,
		);
	} finally {
		restoreElectron();
	}
...
test("SettingsManager sanitizes mixed persisted settings values", async () => {
	const userDataPath = fs.mkdtempSync(
		path.join(os.tmpdir(), "settings-sanitize-"),
	);
	const settingsPath = path.join(userDataPath, "settings.json");
	fs.writeFileSync(
		settingsPath,
	...
	try {
		...
		assert.deepEqual(settings.get("captureToolPatterns"), [
			"obs",
			" screen studio ",
		]);
...
test("SettingsManager falls back to empty settings for non-object JSON payloads", async () => {
	const userDataPath = fs.mkdtempSync(
		path.join(os.tmpdir(), "settings-invalid-json-shape-"),
	);
	const settingsPath = path.join(userDataPath, "settings.json");
...
test("SettingsManager rolls back failed saves without mutating in-memory settings", async () => {
	const userDataPath = fs.mkdtempSync(
		path.join(os.tmpdir(), "settings-save-failure-"),
	);
	const { SettingsManager, restoreElectron } =
		await loadSettingsManager(userDataPath);
