...
test("DesignStateStore captures requirements, architecture, and open questions from live turns", () => {
	...
	store.noteInterviewerTurn({
		transcript:
			"We have a $300k budget, 6 engineers, and a 99.9% availability target. How would you design the API gateway?",
		timestamp: now,
	...
	const block = store.buildContextBlock(
		"How do you scale the API gateway under burst traffic?",
	);
...
test("DesignStateStore captures structured reasoning facets and restores from persistence", () => {
	...
	const block = restored.buildContextBlock(
		"What failure modes and tradeoffs matter most in the ledger?",
	);
