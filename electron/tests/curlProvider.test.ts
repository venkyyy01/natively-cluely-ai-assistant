...
test('chatWithCurl treats null responsePath extraction as a failure instead of returning "null"', async () => {
	...
	LLMHelperCtor.__testAxios = async () => ({
		data: { choices: [{ message: { content: null as string | null } }] },
	});
...
test("chatWithCurl falls back to common-format extraction when responsePath misses", async () => {
	...
	const originalAxios = LLMHelperCtor.__testAxios;
	LLMHelperCtor.__testAxios = async () => ({
		data: { choices: [{ message: { content: "fallback content" } }] },
	});
	...
	helper.setModel("curl-provider", [
		{
			...
			name: "cURL",
			curlCommand:
				'curl https://example.com -H \'Content-Type: application/json\' -d \'{"prompt":"{{TEXT}}","user":"{{USER_MESSAGE}}","context":"{{CONTEXT}}","image":"{{IMAGE_BASE64}}","images":{{IMAGE_BASE64S}},"count":"{{IMAGE_COUNT}}"}\'',
			responsePath: "choices[0].message.content",
		},
	]);
	...
	helper.setModel("curl-provider", [
		{
			...
			name: "cURL",
			curlCommand:
				'curl https://example.com -H \'Content-Type: application/json\' -d \'{"image":"{{IMAGE_BASE64}}","count":"{{IMAGE_COUNT}}"}\'',
			responsePath: "choices[0].message.content",
		},
	]);
...
test("chatWithCurl supports OpenAI-compatible {{OPENAI_MESSAGES}} payloads with image + text", async () => {
	...
	helper.setModel("curl-provider", [
		{
			...
			name: "cURL",
			curlCommand:
				'curl https://example.com -H \'Content-Type: application/json\' -d \'{"model":"gpt-4o-mini","messages":{{OPENAI_MESSAGES}}}\'',
			responsePath: "choices[0].message.content",
		},
	]);
	...
	LLMHelperCtor.__testAxios = async (config: any) => {
		seenRequest = config;
		return {
			data: { choices: [{ message: { content: "openai-compatible-ok" } }] },
		};
	};
	...
	try {
		...
		assert.deepEqual(
			Array.isArray(seenRequest.data.messages[1]?.content),
			true,
		);
		assert.equal(
			seenRequest.data.messages[1].content.some(
