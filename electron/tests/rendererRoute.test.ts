  ...
  ;(Module as any)._load = function patchedLoad(request: string, parent: unknown, isMain: boolean) {
    if (request === 'electron') {
      return {
        app: {
          isPackaged,
          getAppPath: () => '/Applications/Natively.app/Contents/Resources/app.asar',
        },
      }
    }
...
test('loadRendererRoute uses loadFile with query for packaged windows', async () => {
  ...
  try {
    const { getRendererRouteUrl, loadRendererRoute } = await import('../rendererRoute')
    const calls: Array<{ type: 'file' | 'url'; value: string; options?: Record<string, unknown> }> = []
    ...
    assert.deepEqual(calls, [
      {
        type: 'file',
        value: '/Applications/Natively.app/Contents/Resources/app.asar/dist/index.html',
...
test('loadRendererRoute uses dev server URLs during local development', async () => {
  ...
  try {
    ...
    assert.equal(getRendererRouteUrl('overlay'), 'http://localhost:5180?window=overlay')
    assert.deepEqual(calls, [{ type: 'url', value: 'http://localhost:5180?window=overlay' }])
  } finally {
