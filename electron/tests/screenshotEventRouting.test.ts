	...
	helper.setModel("curl-provider", [
		{
			...
			name: "cURL",
			curlCommand:
				'curl https://example.com -d "{"messages":{{OPENAI_MESSAGES}}}"',
...
test("chatWithGemini falls back to tesseract text only when cURL template has no image support", async () => {
	...
	helper.extractImageTextWithTesseract = async () =>
		"Extracted fallback text from screenshot";
...
test("streamChat falls back to tesseract text only when cURL template has no image support", async () => {
	...
	helper.extractImageTextWithTesseract = async () =>
		"Stream fallback text from screenshot";
...
test("streamChat keeps forced screenshot OCR fallback on the buffered cURL path", async () => {
	...
	helper.extractImageTextWithTesseract = async () =>
		"Forced OCR fallback text from screenshot";
...
test("analyzeImageFiles uses the screenshot prompt and OCR fallback for text-only cURL providers", async () => {
	...
	helper.extractImageTextWithTesseract = async () =>
		"Analyze image OCR fallback text";
	...
	try {
		...
		assert.match(
			captured?.systemPrompt || "",
			/coding \/ technical interview problem/,
		);
	...
	helper.setModel("curl-provider", [
		{
			...
			name: "cURL",
			curlCommand:
				'curl https://example.com -d "{"image":"{{IMAGE_BASE64}}","prompt":"{{TEXT}}"}"',
			responsePath: "choices[0].message.content",
		},
	]);
...
test("streamChat preserves structured system prompt overrides for screenshot reasoning requests", async () => {
	...
	helper.extractImageTextWithTesseract = async () =>
		"Structured screenshot fallback text";
	...
	try {
		const structuredPromptOverride =
...
test("chatWithGemini retries cURL screenshot requests with OCR text when image input is rejected", async () => {
	...
	helper.extractImageTextWithTesseract = async () =>
		"OCR retry text from screenshot";
...
test("streamChat retries selected OpenAI screenshot requests with OCR text when image input is rejected", async () => {
	...
	helper.streamWithOpenai = async function* (
		userMessage: string,
		systemPrompt?: string,
	) {
