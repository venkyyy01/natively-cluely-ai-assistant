...
test("summarizeIntentEvalOutcomes computes accuracy, provider split, fallback rate, and confusion matrix", () => {
	...
	const lowMidBucket = summary.confidenceBuckets.find(
		(bucket) => bucket.label === "0.40-0.59",
	);
	const highBucket = summary.confidenceBuckets.find(
		(bucket) => bucket.label === "0.80-1.00",
	);
