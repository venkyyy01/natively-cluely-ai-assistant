...
import { WorkerPool, WORKER_POOL_QUEUE_FULL_ERROR } from '../runtime/WorkerPool';
...
test('NAT-016: WorkerPool accepts up to maxQueueDepth and rejects overflow with worker_pool_queue_full', async () => {
  ...
  function submit() {
    return pool.submit({ lane: 'background' }, () => new Promise<void>((res) => {
      releases.push(res);
    })).then(
      () => settled.push('ok'),
      () => settled.push('fail'),
  ...
  await pool.submit({ lane: 'background' }, () => Promise.resolve()).catch((err) => {
    rejectedReason = err;
  });
...
test('NAT-016: 2x burst rejects exactly the overflow half synchronously', async () => {
  ...
  const releases: Array<() => void> = [];
  const outcomes: Array<{ idx: number; status: 'ok' | 'fail'; reason?: string }> = [];
  ...
  for (let i = 0; i < 6; i += 1) {
    const idx = i;
    promises.push(
      pool.submit({ lane: 'background' }, () => new Promise<void>((res) => {
        releases.push(res);
      })).then(
        () => outcomes.push({ idx, status: 'ok' }),
        (err) => outcomes.push({ idx, status: 'fail', reason: (err as Error).message }),
      ),
    );
  }
  ...
  assert.equal(failures.length, 3, 'exactly 3 of the 6 submissions must be rejected');
