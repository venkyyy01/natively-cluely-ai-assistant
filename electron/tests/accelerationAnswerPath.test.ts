...
test("Acceleration Mode skips intent and temporal prompt augmentation for what-to-say answers", async () => {
	...
	session.addAssistantMessage(
		"I have spent most of my career building backend systems.",
	);
	addTurn(
		session,
		"interviewer",
		"What tradeoffs mattered on that project?",
		Date.now() - 4000,
	);
	session.addAssistantMessage(
		"Latency and operational simplicity mattered most.",
	);
	addTurn(
		session,
		"interviewer",
	...
	assert.match(
		llmHelper.calls[0].context ?? "",
		/I would start with a token bucket in Redis\./,
	);
	assert.doesNotMatch(
		llmHelper.calls[0].context ?? "",
		/Tell me about your background\./,
