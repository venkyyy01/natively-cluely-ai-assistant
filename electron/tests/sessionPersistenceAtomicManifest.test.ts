...
import { SessionPersistence, type PersistedSession } from '../memory/SessionPersistence';
...
test('NAT-060: indexed sessions are not duplicated during orphan scan', async () => {
  ...
  assert.equal(entriesForSession.length, 1, 'session should appear exactly once in index');
