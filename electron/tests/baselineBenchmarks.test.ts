...
test('baseline benchmark probes capture a meeting activation, answer, stealth toggle, and stop cycle', async () => {
  ...
  try {
    ...
    instrumentation.recordDuration('meeting.activation', activationStartedAt, { runtime: 'coordinator' });
    ...
    instrumentation.recordDuration('meeting.deactivation', deactivationStartedAt, { runtime: 'coordinator' });

    await instrumentation.flush();

    const events = await instrumentation.readAll();
    const metrics = new Map(events.map((event) => [event.metric, event.durationMs]));
