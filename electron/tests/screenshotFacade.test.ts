...
test("ScreenshotFacade delegates screenshot and queue operations", async () => {
	...
	assert.deepEqual(await facade.deleteScreenshot("/tmp/user-data/one.png"), {
		success: true,
	});
	assert.equal(await facade.takeScreenshot(), "/tmp/user-data/screenshot.png");
	assert.equal(
		await facade.takeSelectiveScreenshot(),
		"/tmp/user-data/selective.png",
	);
	assert.equal(
		await facade.getImagePreview("/tmp/user-data/one.png"),
	...
	assert.deepEqual(facade.getExtraScreenshotQueue(), [
		"/tmp/user-data/two.png",
	]);
	facade.clearQueues();
