...
function installIpcHandlersTestHarness(options?: {
	...
	const llmHelper = {
		generateSuggestion: async (context: string, lastQuestion: string) =>
			`${context} -> ${lastQuestion}`,
		analyzeImageFiles: async (paths: string[]) => [
			{ summary: `analyzed:${paths[0]}` },
		],
	...
	const electronMock = {
		app: {
			getPath: (name: string) =>
				name === "userData" ? "/tmp/user-data" : "/tmp",
			quit: () => {
		...
		dialog: {
			showOpenDialog: async () =>
				options?.serviceAccountResult ?? { canceled: true, filePaths: [] },
		},
		desktopCapturer: {},
		systemPreferences: {},
		BrowserWindow: {
			getAllWindows: () => [
				{
					...
					webContents: {
						send: (channel: string, payload: unknown) =>
							browserEvents.push({ channel, payload }),
					},
				},
			],
		},
	...
	(Module as any)._load = function patchedLoad(
		...
		if (request === "./db/DatabaseManager") {
			return {
				DatabaseManager: {
					getInstance: () => ({ deleteMeeting: async () => true }),
				},
			};
		}
		...
		if (
			request === "./services/CredentialsManager" ||
			request === "../services/CredentialsManager"
		) {
		...
		if (
			request === "./services/OllamaManager" ||
			request === "../services/OllamaManager"
		) {
			return {
				OllamaManager: { getInstance: () => ({ init: async () => {} }) },
			};
		}
	...
	const appState = {
		...
		getWindowHelper: () => ({
			...
			getLauncherWindow: (): null => null,
			getLauncherContentWindow: (): null => null,
			setOverlayDimensions: (): void => {},
		}),
...
async function initializeHandlers(
	harness: ReturnType<typeof installIpcHandlersTestHarness>,
) {
	...
	delete require.cache[
		require.resolve("../ipc/registerLlmCredentialsIpcHandlers")
	];
	delete require.cache[
		require.resolve("../ipc/registerProviderSttAndTestIpcHandlers")
	];
...
async function loadPreloadModule(
	invokeImpl: (channel: string, ...args: unknown[]) => Promise<unknown>,
) {
	const originalLoad = (Module as any)._load;
	let exposedApi: any;
...
function createHandlerRegistry() {
	...
	return {
		...
		safeHandleValidated: <T extends unknown[]>(
			...
			listener: (event: unknown, ...args: T) => Promise<unknown> | unknown,
		) => {
...
async function withPatchedModules<T>(
...
test("root IPC handlers validate inputs and return normalized contracts", async () => {
	...
	assert.throws(
		() => harness.handlers.get("generate-suggestion")?.({}, "   ", "question"),
		/Invalid IPC payload/,
	);
	assert.throws(
		() => harness.handlers.get("set-overlay-opacity")?.({}, Number.NaN),
		/Invalid IPC payload/,
	);
	...
	assert.deepEqual(
		await harness.handlers.get("generate-suggestion")?.(
			{},
			"context",
			"question",
		),
	...
	assert.deepEqual(
		await harness.handlers.get("set-overlay-opacity")?.({}, 1.4),
		{
			success: true,
			data: { opacity: 1 },
		},
	);
	assert.deepEqual(harness.browserEvents, [
		{ channel: "overlay-opacity-changed", payload: 1 },
	]);
...
test("root IPC handlers normalize cancellation and failures into success/data/error shapes", async () => {
	const harness = installIpcHandlersTestHarness({
		restartOllamaError: "restart failed",
	});
	await initializeHandlers(harness);
...
test("root screenshot handlers prefer ScreenshotFacade when available", async () => {
	...
	assert.deepEqual(
		await harness.handlers.get("delete-screenshot")?.(
			{},
			"/tmp/user-data/facade.png",
		),
		{
			success: true,
		},
	...
	assert.deepEqual(
		await harness.handlers.get("take-selective-screenshot")?.({}),
		{
			success: true,
			data: {
	...
	assert.deepEqual(await harness.handlers.get("reset-queues")?.({}), {
		success: true,
	});
...
test("root STT handlers prefer SttSupervisor when supervisor runtime is enabled", async () => {
	...
	assert.deepEqual(
		await harness.handlers.get("set-stt-provider")?.({}, "deepgram"),
		{ success: true },
	);
	assert.deepEqual(
		await harness.handlers.get("set-groq-stt-model")?.({}, "whisper-large-v3"),
		{ success: true },
	);
	assert.deepEqual(
		await harness.handlers.get("set-azure-region")?.({}, "westus"),
		{ success: true },
	);
	assert.deepEqual(await harness.handlers.get("select-service-account")?.({}), {
		success: true,
...
test("root inference sync handlers prefer InferenceSupervisor when supervisor runtime is enabled", async () => {
	...
	assert.deepEqual(
		await harness.handlers.get("set-gemini-api-key")?.({}, "key-123"),
		{ success: true },
	);
	assert.deepEqual(
		await harness.handlers.get("switch-to-curl-provider")?.({}, "curl-1"),
		{ success: true },
	);
	assert.equal(
		await harness.handlers.get("gemini-chat")?.({}, "hello", [], undefined, {
...
test("root theme handlers prefer SettingsFacade when available", async () => {
	...
	assert.deepEqual(
		await harness.handlers.get("theme:set-mode")?.({}, "light"),
		{ success: true },
	);
	assert.deepEqual(calls, [
		"getThemeMode",
...
test("root IPC handlers validate high-risk LLM and STT payloads before dispatch", async () => {
	...
	await assert.rejects(
		async () =>
			harness.handlers.get("switch-to-ollama")?.({}, { model: "llama3" }),
		/Invalid IPC payload/,
	);
	await assert.rejects(
		async () =>
			harness.handlers.get("switch-to-gemini")?.({}, { apiKey: "key-123" }),
		/Invalid IPC payload/,
	);
	await assert.rejects(
		async () =>
			harness.handlers.get("test-llm-connection")?.(
				{},
				"not-a-provider",
				"key-123",
			),
	...
	await assert.rejects(
		async () =>
			harness.handlers.get("set-gemini-api-key")?.({}, { apiKey: "key-123" }),
		/Invalid IPC payload/,
	);
	await assert.rejects(
		async () =>
			harness.handlers.get("set-openai-api-key")?.({}, "x".repeat(5000)),
		/Invalid IPC payload/,
	);
	await assert.rejects(
		async () =>
			harness.handlers.get("set-openai-stt-api-key")?.(
				{},
				{ apiKey: "key-123" },
			),
		/Invalid IPC payload/,
	...
	await assert.rejects(
		async () =>
			harness.handlers.get("set-deepgram-api-key")?.({}, { apiKey: "key-123" }),
		/Invalid IPC payload/,
	);
	await assert.rejects(
		async () =>
			harness.handlers.get("set-elevenlabs-api-key")?.(
				{},
				{ apiKey: "key-123" },
			),
		/Invalid IPC payload/,
	...
	await assert.rejects(
		async () =>
			harness.handlers.get("set-soniox-api-key")?.({}, { apiKey: "key-123" }),
		/Invalid IPC payload/,
	);
...
test("root model selector handlers prefer WindowFacade when available", async () => {
	...
	assert.equal(
		await harness.handlers.get("show-model-selector")?.({}, { x: 10, y: 20 }),
		undefined,
	);
	...
	assert.equal(
		await harness.handlers.get("toggle-model-selector")?.({}, { x: 30, y: 40 }),
		undefined,
	);
...
test("fast response and stored credential IPC contracts include Cerebras-aware state", async () => {
	...
	assert.deepEqual(
		await harness.handlers.get("get-fast-response-config")?.({}),
		{
			success: true,
	...
	assert.deepEqual(
		await harness.handlers.get("set-fast-response-config")?.(
			{},
...
test("preload unwraps normalized root IPC contracts into typed renderer helpers", async () => {
	...
	const { exposedApi, restore } = await loadPreloadModule(
		async (channel: string, ...args: unknown[]) => {
			...
			if (channel === "take-screenshot") {
				return {
					success: true,
					data: { path: "/tmp/user-data/screenshot.png", preview: "preview" },
				};
			}
	...
	assert.deepEqual(await exposedApi.takeScreenshot(), {
		path: "/tmp/user-data/screenshot.png",
		preview: "preview",
	});
...
test("preload exposes fast response config helpers and Cerebras key setter", async () => {
	...
	const { exposedApi, restore } = await loadPreloadModule(
		async (channel: string, ...args: unknown[]) => {
			...
			if (
				channel === "set-fast-response-config" ||
				channel === "set-cerebras-api-key"
			) {
				return { success: true };
			}
	...
	assert.deepEqual(await exposedApi.getFastResponseConfig(), {
		enabled: true,
		provider: "cerebras",
		model: "gpt-oss-120b",
	});
	assert.deepEqual(
		await exposedApi.setFastResponseConfig({
			enabled: false,
			provider: "groq",
			model: "llama-3.3-70b-versatile",
		}),
	...
	assert.deepEqual(await exposedApi.setCerebrasApiKey("csk_test"), {
		success: true,
	});
...
test("settings handlers validate inputs and normalize success contracts", async () => {
	await withPatchedModules(
		...
		async () => {
			...
			const appState = {
				processingHelper: {
					getLLMHelper: () => ({ setAiResponseLanguage: () => {} }),
				},
			...
			assert.throws(
				() => registry.handlers.get("set-disguise")?.({}, "spaceship"),
				/Invalid IPC payload/,
			);
			...
			assert.deepEqual(
				await registry.handlers.get("set-undetectable")?.({}, false),
				{
					success: true,
					data: { enabled: false },
				},
			...
			assert.deepEqual(
				await registry.handlers.get("set-open-at-login")?.({}, true),
				{
					success: true,
					data: { enabled: true },
				},
...
test("settings window handlers prefer WindowFacade when available", async () => {
	await withPatchedModules(
		...
		async () => {
			...
			assert.deepEqual(
				await registry.handlers.get("toggle-settings-window")?.(
					{},
					{ x: 10, y: 20 },
				),
				{
			...
			assert.deepEqual(
				await registry.handlers.get("close-settings-window")?.({}),
				{
					success: true,
					data: null,
				},
...
test("settings state handlers prefer SettingsFacade when available", async () => {
	await withPatchedModules(
		...
		async () => {
			...
			assert.deepEqual(
				await registry.handlers.get("set-conscious-mode")?.({}, true),
				{
					success: true,
					data: { enabled: true },
				},
			...
			assert.deepEqual(
				await registry.handlers.get("get-conscious-mode")?.({}),
				{
					success: true,
					data: { enabled: true },
				},
			...
			assert.deepEqual(
				await registry.handlers.get("set-acceleration-mode")?.({}, false),
				{
					success: true,
					data: { enabled: false },
				},
			...
			assert.deepEqual(
				await registry.handlers.get("get-acceleration-mode")?.({}),
				{
					success: true,
					data: { enabled: false },
				},
			...
			assert.deepEqual(
				await registry.handlers.get("set-disguise")?.({}, "terminal"),
				{
					success: true,
					data: { mode: "terminal" },
				},
...
test("meeting handlers route lifecycle through AppState meeting APIs", async () => {
	...
	delete require.cache[modulePath];
	const { registerMeetingHandlers } = await import(
		"../ipc/registerMeetingHandlers"
	);
	...
	const metadata = {
		audio: { inputDeviceId: "mic-1", outputDeviceId: "speaker-1" },
	};
	assert.deepEqual(
		await registry.handlers.get("start-meeting")?.({}, metadata),
		{
			success: true,
		},
	);
...
test("meeting handlers prefer audio, stt, and inference supervisors for auxiliary flows", async () => {
	...
	await withPatchedModules(
		...
		async () => {
			...
			assert.deepEqual(
				await registry.handlers.get("start-audio-test")?.({}, "mic-1"),
				{ success: true },
			);
			assert.deepEqual(await registry.handlers.get("stop-audio-test")?.({}), {
				success: true,
			});
			assert.deepEqual(
				await registry.handlers.get("set-recognition-language")?.({}, "en-US"),
				{ success: true },
			);
			assert.deepEqual(await registry.handlers.get("seed-demo")?.({}), {
				success: true,
			});
			await assert.rejects(
				async () => registry.handlers.get("start-audio-test")?.({}, "   "),
				/Invalid IPC payload/,
			);
			await assert.rejects(
				async () => registry.handlers.get("get-meeting-details")?.({}, ""),
				/Invalid IPC payload/,
			);
			await assert.rejects(
				async () =>
					registry.handlers.get("open-external")?.({}, "file:///tmp/unsafe"),
				/Invalid IPC payload/,
			);
...
test("settings handlers route stealth toggles through AppState", async () => {
	await withPatchedModules(
		...
		async () => {
			...
			assert.deepEqual(
				await registry.handlers.get("set-undetectable")?.({}, true),
				{
					success: true,
					data: { enabled: true },
				},
...
test("settings handlers prefer InferenceSupervisor LLM helper when supervisor runtime is enabled", async () => {
	await withPatchedModules(
		...
		async () => {
			...
			assert.deepEqual(
				await registry.handlers.get("set-ai-response-language")?.({}, "fr"),
				{
					success: true,
					data: { language: "fr" },
				},
...
test("email handlers prefer InferenceSupervisor LLM helper when supervisor runtime is enabled", async () => {
	await withPatchedModules(
		...
		async () => {
			...
			delete require.cache[modulePath];
			const { registerEmailHandlers } = await import(
				"../ipc/registerEmailHandlers"
			);
			const registry = createHandlerRegistry();
			const calls: string[] = [];
			const appState = {
			...
			assert.equal(
				await registry.handlers.get("generate-followup-email")?.(
					{},
					{
...
test("intelligence handlers prefer InferenceSupervisor when supervisor runtime is enabled", async () => {
	...
	delete require.cache[modulePath];
	const { registerIntelligenceHandlers } = await import(
		"../ipc/registerIntelligenceHandlers"
	);
			...
			getSupervisor: (name: string) => {
				...
				return {
					...
					runWhatShouldISay: async (
						...
						calls.push(
							`what:${question ?? "none"}:${confidence}:${imagePaths?.length ?? 0}`,
						);
						return "answer";
					},
	...
	assert.deepEqual(await registry.handlers.get("generate-assist")?.({}), {
		insight: "insight",
	});
	assert.deepEqual(
		await registry.handlers.get("generate-what-to-say")?.({}, "question", [
			"img-1",
		]),
		{
			answer: "answer",
	...
	assert.deepEqual(
		await registry.handlers.get("generate-follow-up")?.(
			{},
			"tradeoff",
			"more detail",
		),
	...
	assert.deepEqual(await registry.handlers.get("generate-recap")?.({}), {
		summary: "summary",
	});
	assert.deepEqual(
		await registry.handlers.get("generate-follow-up-questions")?.({}),
		{ questions: ["q1"] },
	);
	assert.deepEqual(
		await registry.handlers.get("submit-manual-question")?.(
			{},
			"manual question",
		),
		{
	...
	assert.deepEqual(
		await registry.handlers.get("get-intelligence-context")?.({}),
		{
...
test("intelligence handlers return canceled status when what-to-say yields no answer", async () => {
	...
	assert.deepEqual(
		await registry.handlers.get("generate-what-to-say")?.({}, undefined, []),
		{
			answer: null,
			question: "inferred from context",
			status: "canceled",
			error:
				"Request canceled before completion. Retry with the current settings.",
		},
	);
});
...
test("intelligence handlers return error status when what-to-say throws", async () => {
	...
	assert.deepEqual(
		await registry.handlers.get("generate-what-to-say")?.({}, "why now?", [
			"img-1",
		]),
		{
			answer: null,
...
test("screenshot capture output flows into a completed screenshot-backed what-to-say request", async () => {
	...
	const screenshotResult = (await harness.handlers.get("take-screenshot")?.(
		{},
	)) as {
		success: true;
	...
	registerIntelligenceHandlers({
		appState: harness.appState as any,
		...registry,
	} as any);
...
test("window handlers preserve window control and resize contracts", async () => {
	...
	delete require.cache[modulePath];
	const { registerWindowHandlers } = await import(
		"../ipc/registerWindowHandlers"
	);

	registerWindowHandlers({ appState: appState as any, ...registry } as any);

	assert.deepEqual(
		await registry.handlers.get("set-window-mode")?.({}, "overlay"),
		{
			success: true,
		},
	);
	assert.deepEqual(
		await registry.handlers.get("set-overlay-clickthrough")?.({}, true),
		{
			success: true,
			data: { enabled: true },
		},
	...
	assert.deepEqual(
		await registry.handlers.get("set-overlay-clickthrough")?.({}, false),
		{
			success: true,
			data: { enabled: false },
		},
	...
	assert.deepEqual(
		await registry.handlers.get("center-and-show-window")?.({}),
		{
			success: true,
			data: null,
		},
	...
	await registry.handlers.get("update-content-dimensions")?.(
		{ sender: { id: 101 } },
	...
	assert.throws(
		() => registry.handlers.get("set-overlay-clickthrough")?.({}, "yes"),
		/Invalid IPC payload/,
	);
});
...
test("window handlers prefer WindowFacade when available", async () => {
	...
	assert.deepEqual(
		await registry.handlers.get("set-window-mode")?.({}, "launcher"),
		{ success: true },
	);
	assert.deepEqual(
		await registry.handlers.get("set-overlay-clickthrough")?.({}, true),
	...
	assert.deepEqual(await registry.handlers.get("move-window-left")?.({}), {
		success: true,
		data: null,
	});
	assert.deepEqual(await registry.handlers.get("move-window-right")?.({}), {
	...
	assert.deepEqual(await registry.handlers.get("move-window-up")?.({}), {
		success: true,
		data: null,
	});
	assert.deepEqual(await registry.handlers.get("move-window-down")?.({}), {
		success: true,
		data: null,
	});
	assert.deepEqual(
...
test("profile handlers validate inputs and normalize errors", async () => {
	await withPatchedModules(
		...
		async () => {
			...
			assert.throws(
				() => registry.handlers.get("profile:upload-resume")?.({}, "   "),
				/Invalid IPC payload/,
			);
			assert.deepEqual(
				await registry.handlers.get("profile:upload-resume")?.(
					{},
					"/tmp/resume.pdf",
				),
				{
			...
			assert.deepEqual(
				await registry.handlers.get("profile:select-file")?.({}),
				{
					success: true,
					data: { cancelled: true },
				},
...
test("rag handlers validate inputs and preload unwraps normalized rag and profile contracts", async () => {
	...
	assert.throws(
		() =>
			registry.handlers.get("rag:query-meeting")?.(
				{ sender: { send: () => {} } },
	...
	assert.deepEqual(
		await registry.handlers.get("rag:query-meeting")?.(
			{ sender: { send: () => {} } },
	...
	const { exposedApi, restore } = await loadPreloadModule(
		async (channel: string, ...args: unknown[]) => {
			...
			if (channel === "profile:get-status") {
				return {
					success: true,
					data: { hasProfile: true, profileMode: false, name: "Ada" },
				};
			}
			...
			if (channel === "profile:upload-resume") {
				return {
					success: false,
					error: { code: "PROFILE_UPLOAD_FAILED", message: "bad file" },
				};
			}
			...
			if (channel === "rag:query-live") {
				return {
					success: false,
					error: { code: "RAG_QUERY_FAILED", message: "stream failed" },
				};
			}
	...
	assert.deepEqual(await exposedApi.profileGetStatus(), {
		hasProfile: true,
		profileMode: false,
		name: "Ada",
	});
	assert.deepEqual(await exposedApi.profileUploadResume("/tmp/resume.pdf"), {
		success: false,
		error: "bad file",
	});
	assert.deepEqual(await exposedApi.ragQueryLive("hello"), {
		success: false,
		error: "stream failed",
	});
	assert.deepEqual(calls, [
	...
	assert.deepEqual(senderEvents, [
		{
			...
			payload: { meetingId: "meeting-1", chunk: "chunk-1" },
		},
		{ channel: "rag:stream-complete", payload: { meetingId: "meeting-1" } },
	]);
	assert.deepEqual(calls, [
		"processed:meeting-1",
		"queryMeeting:meeting-1:hello",
		"queueStatus",
	]);
});
...
test("profile handlers prefer InferenceSupervisor knowledge orchestrator when supervisor runtime is enabled", async () => {
	await withPatchedModules(
		...
		async () => {
			...
			assert.deepEqual(
				await registry.handlers.get("profile:get-status")?.({}),
				{
					success: true,
					data: {
						hasProfile: true,
			...
			assert.deepEqual(
				await registry.handlers.get("profile:set-mode")?.({}, false),
				{
					success: true,
					data: { success: true },
				},
			...
			assert.deepEqual(
				await registry.handlers.get("profile:get-profile")?.({}),
				{
					success: true,
					data: {
						activeJD: {
			...
			assert.deepEqual(
				await registry.handlers.get("profile:upload-jd")?.({}, "/tmp/jd.pdf"),
				{
					success: true,
			...
			assert.deepEqual(
				await registry.handlers.get("profile:research-company")?.({}, "Acme"),
				{
					success: true,
					data: {
						success: true,
						dossier: { companyName: "Acme", summary: "researched" },
					},
				},
			);
			assert.deepEqual(
				await registry.handlers.get("profile:generate-negotiation")?.({}),
				{
					success: true,
					data: {
						success: true,
