...
test('system prompt cache reuses identical mapped prompts', async () => {
  ...
  const first = await helper.withSystemPromptCache('openai', 'gpt-5.4-chat', 'base-prompt', async () => {
    builds += 1;
    return 'mapped-prompt';
  });
  const second = await helper.withSystemPromptCache('openai', 'gpt-5.4-chat', 'base-prompt', async () => {
    builds += 1;
    return 'mapped-prompt';
  });
  const third = await helper.withSystemPromptCache('claude', 'claude-sonnet-4-6', 'base-prompt', async () => {
    builds += 1;
    return 'mapped-prompt';
  });
...
test('final payload cache reuses identical assembly work and returns safe clones', async () => {
  ...
  const first = await helper.withFinalPayloadCache('openai', 'gpt-5.4-chat', 'sys-hash', 'payload-hash', async () => {
    builds += 1;
  ...
  const second = await helper.withFinalPayloadCache('openai', 'gpt-5.4-chat', 'sys-hash', 'payload-hash', async () => {
    builds += 1;
    return { messages: [{ role: 'user', content: 'hello' }] };
  });
...
test('response cache dedupes identical in-flight requests and reuses short ttl hits', async () => {
  ...
  const execute = () => helper.withResponseCache(
    'openai',
    'gpt-5.4-chat',
    'sys-hash',
    'payload-hash',
