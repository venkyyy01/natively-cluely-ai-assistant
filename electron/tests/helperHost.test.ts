...
test("NAT-063: HelperHost sanitizes env by dropping secrets", async () => {
	...
	const sanitized = (host as any).sanitizeEnv({
		PATH: "/usr/bin",
		API_SECRET: "shh",
		NORMAL_VAR: "ok",
	});
...
test("NAT-063: HelperHost handles JSON response parsing", async () => {
	...
	assert.ok(
		responses.length >= 1 || !host.isRunning(),
		"should have emitted response or exited",
	);
...
test("NAT-063: HelperHost rejects send when process not running", async () => {
	...
	await assert.rejects(host.send({ id: "r1", payload: "test" }), /not running/);
});
