...
test('InterviewerUtteranceBuffer merges final fragments and flushes on terminal punctuation', () => {
  ...
  assert.deepEqual(buffer.pushFragment('interviewer', 'Can you explain', true), []);
  const flushed = buffer.pushFragment('interviewer', 'the caching layer?', true);
...
test('InterviewerUtteranceBuffer flushes on silence and speaker changes', async () => {
  ...
  buffer.pushFragment('interviewer', 'Tell me about a time you handled conflict', true);
  await wait(30);
...
test('InterviewerUtteranceBuffer splits multi-question turns at sentence boundaries', () => {
  ...
  buffer.pushFragment('interviewer', 'Tell me about a project you led. Also, what did you learn?', true);
