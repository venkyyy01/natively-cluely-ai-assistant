...
test('NAT-054: assemble awaits embedding, bm25, and phase in one Promise.all wave', async () => {
  ...
  try {
    ...
    const origRun = (asm as unknown as { runInWorker: (t: string, p: unknown) => Promise<unknown> }).runInWorker.bind(asm);
    (asm as unknown as { runInWorker: (t: string, p: unknown) => Promise<unknown> }).runInWorker = async (type: string, payload: unknown) => {
      if (type === 'bm25') {
        bm25Start = Date.now();
        await new Promise((r) => setTimeout(r, 35));
        bm25End = Date.now();
    ...
    await asm.assemble({
      ...
      transcript: [
        { speaker: 'interviewer', text: 'hello world design architecture', timestamp: 1 },
      ],
      previousContext: { recentTopics: [], activeThread: null },
    });

    assert.ok(embedStart > 0 && bm25Start > 0, 'both embedding and bm25 paths should have started');
    const windowsOverlap = Math.max(embedStart, bm25Start) < Math.min(embedEnd, bm25End);
    assert.ok(windowsOverlap, 'expected bm25 and embedding time windows to overlap (parallel)');
  } finally {
