...
import { VisibilityController, type VisibilityCapableWindow } from '../stealth/VisibilityController';
...
describe('VisibilityController', () => {
  it('routes show through request/show event pair without changing behavior', () => {
    ...
    assert.deepEqual(events.map((event) => event.type), ['show-requested', 'shown']);
    assert.deepEqual(events.map((event) => event.visible), [false, true]);
  });
  ...
  it('uses showInactive when available', () => {
    ...
    controller.requestShowInactive(win, { source: 'test', windowRole: 'auxiliary' });
