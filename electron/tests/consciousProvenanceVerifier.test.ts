...
test("ConsciousProvenanceVerifier rejects unsupported technology claims", async () => {
	...
	const verdict = await verifier.verify({
		response: response({
			openingReasoning: "I would use Cassandra for the core path.",
			implementationPlan: ["Use Cassandra for the write path"],
		}),
...
test("ConsciousProvenanceVerifier rejects unsupported metric claims", async () => {
	...
	const verdict = await verifier.verify({
		response: response({
			openingReasoning:
				"I would keep the current architecture and measure carefully.",
			implementationPlan: [],
...
test("ConsciousProvenanceVerifier accepts grounded technology and metric content", async () => {
	...
	const verdict = await verifier.verify({
		response: response({
			scaleConsiderations: [
				"I would watch the existing 70ms p99 baseline first",
			],
		}),
		semanticContextBlock:
			"<conscious_semantic_memory>\n[PROJECT] Tenant Analytics Platform: Technologies: Redis, Kafka. Reduced p99 latency to 70ms.\n</conscious_semantic_memory>",
...
test("ConsciousProvenanceVerifier rejects unsupported dynamic technology claims", async () => {
	...
	const verdict = await verifier.verify({
		response: response({
			openingReasoning:
				"I would use Pinecone for the vector index and SQS for fan-out.",
			implementationPlan: [
				"Store embeddings in Pinecone",
				"Push async work through SQS",
			],
		}),
		semanticContextBlock:
			"<conscious_semantic_memory>\n[PROJECT] Tenant Analytics Platform: Technologies: Redis, Kafka\n</conscious_semantic_memory>",
...
test("NAT-004 / audit A-9: question text is NOT treated as grounding for technology claims", async () => {
	...
	const verdict = await verifier.verify({
		response: response(),
		semanticContextBlock:
			"<conscious_semantic_memory>Grounding enabled.</conscious_semantic_memory>",
...
test("NAT-004 / audit A-4: empty grounding + no verifiable claim still passes", async () => {
	...
	const verdict = await verifier.verify({
		response: response({
			openingReasoning:
				"I would scope the requirements first and confirm priorities with the team.",
			implementationPlan: [
				"Confirm acceptance criteria",
				"Sketch a minimal happy-path flow",
			],
			tradeoffs: [],
			edgeCases: [],
			scaleConsiderations: [],
