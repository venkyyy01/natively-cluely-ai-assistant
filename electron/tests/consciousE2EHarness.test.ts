...
test("NAT-083: default E2E scenarios cover all required families", () => {
	...
	assert.ok(
		families.has("acceleration_overlay"),
		"must have acceleration_overlay family",
	);
	assert.ok(
		families.has("circuit_breaker"),
		"must have circuit_breaker family",
	);
	assert.ok(
		families.has("topical_compatibility"),
		"must have topical_compatibility family",
	);
...
test("NAT-083: acceleration-overlay-prefetch-boost overrides with strong prefetch", async () => {
	const { results } = await runConsciousE2EHarness({});
	const result = results.find(
		(r) => r.scenario.id === "acceleration-overlay-prefetch-boost",
	);
	assert.ok(result, "scenario should exist");
	assert.equal(
