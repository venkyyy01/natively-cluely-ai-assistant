...
function makeSnapshot(transcript: MeetingSnapshot['transcript']): MeetingSnapshot {
  return {
    transcript,
    usage: [],
...
test('NAT-061: checkpoint emits event instead of broadcasting to all windows', async () => {
  ...
  const checkpointer = new MeetingCheckpointer(
    ...
    () => ({
      createSnapshot() {
        return makeSnapshot([{ speaker: 'interviewer', text: 'hello', timestamp: Date.now(), final: true }]);
      },
    }) as never,
  );
...
test('NAT-061: idle stretch skips checkpoint when no new transcript since last checkpoint', async () => {
  ...
  const checkpointer = new MeetingCheckpointer(
    ...
    () => ({
      createSnapshot() {
        return makeSnapshot([{ speaker: 'interviewer', text: 'hello', timestamp: transcriptTimestamp, final: true }]);
      },
    }) as never,
  );
  ...
  await checkpointer.checkpointNow();
  assert.equal(writes.length, 1, 'second checkpoint should be skipped due to idle');

  // Now add a newer transcript
  transcriptTimestamp = Date.now() + 1;
  await checkpointer.checkpointNow();
  assert.equal(writes.length, 2, 'checkpoint after new transcript should write again');
...
test('NAT-061: stop resets idle tracking state', async () => {
  ...
  assert.equal(writes.length, 2, 'checkpoint after stop/start should write because idle state was reset');
...
test('NAT-061: empty transcript still short-circuits before idle check', async () => {
  ...
  assert.equal(writes.length, 0, 'empty transcript should not trigger any write');
});
