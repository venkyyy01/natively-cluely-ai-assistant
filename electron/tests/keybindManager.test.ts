...
test("default keybinds include overlay clickthrough toggle shortcut", async () => {
	...
	try {
		...
		const shortcut = DEFAULT_KEYBINDS.find(
			(item) => item.id === "general:toggle-clickthrough",
		);
		assert.ok(shortcut);
		...
		assert.equal(
			DEFAULT_KEYBINDS.find((item) => item.id === "general:toggle-visibility")
				?.accelerator,
			"Command+Alt+Shift+V",
		);
		assert.deepEqual(
			DEFAULT_KEYBINDS.find((item) => item.id === "general:toggle-visibility")
				?.alternateAccelerators,
			["F13"],
		);
		assert.equal(
			DEFAULT_KEYBINDS.find((item) => item.id === "general:take-screenshot")
				?.accelerator,
			"Command+Alt+Shift+S",
		);
		assert.deepEqual(
		...
		assert.equal(
			DEFAULT_KEYBINDS.find(
				(item) => item.id === "general:selective-screenshot",
			)?.accelerator,
			"Command+Alt+Shift+A",
		);
		assert.deepEqual(
		...
		assert.equal(
			DEFAULT_KEYBINDS.find(
				(item) => item.id === "general:restore-full-stealth",
			)?.accelerator,
			"Shift+Esc",
		);
		assert.equal(
		...
		assert.equal(
			DEFAULT_KEYBINDS.find((item) => item.id === "chat:scrollUp")?.accelerator,
			"Command+Up",
		);
		assert.equal(
			DEFAULT_KEYBINDS.find((item) => item.id === "chat:scrollUp")?.isGlobal,
			true,
		);
		assert.equal(
			DEFAULT_KEYBINDS.find((item) => item.id === "chat:scrollDown")
				?.accelerator,
			"Command+Down",
		);
		assert.equal(
			DEFAULT_KEYBINDS.find((item) => item.id === "chat:scrollDown")?.isGlobal,
			true,
		);
		assert.equal(
			DEFAULT_KEYBINDS.find((item) => item.id === "window:move-up")
				?.accelerator,
			"Command+Alt+Up",
		);
		assert.equal(
			DEFAULT_KEYBINDS.find((item) => item.id === "window:move-down")
				?.accelerator,
			"Command+Alt+Down",
		);

		const globalIds = DEFAULT_KEYBINDS.filter((item) => item.isGlobal)
			.map((item) => item.id)
			.sort();
