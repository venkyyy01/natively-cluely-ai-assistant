...
test('ConsciousRetrievalOrchestrator builds live RAG block from high-signal recent context', () => {
  ...
  assert.ok(block.includes('[USER] I would isolate hot tenants to dedicated partitions and rebalance asynchronously.'));
  assert.ok(block.includes('[INTERVIEWER] What failure modes should we watch first?'));
  assert.ok(block.includes('</conscious_live_rag>'));
});
...
test('ConsciousRetrievalOrchestrator buildPack injects sanitized live RAG and skips fallback-like assistant items', () => {
  ...
  assert.ok(pack.combinedContext.includes('[INTERVIEWER] How do you handle (failover) events?'));
  assert.ok(pack.combinedContext.includes('[USER] I would use multi-region replication and a controlled leader failover runbook.'));
...
test('ConsciousRetrievalOrchestrator buildPack skips guardrail refusal assistant items', () => {
  ...
  const pack = orchestrator.buildPack({ question: 'How would you design cache invalidation?' });
