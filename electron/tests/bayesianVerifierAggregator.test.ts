...
describe('BayesianVerifierAggregator', () => {
  it('should accept when all verifiers pass', () => {
    ...
    assert.ok(aggregation.posterior >= 0.85, `Posterior should be >= 0.85, got ${aggregation.posterior}`);
  });
  ...
  it('should reject when all verifiers fail', () => {
    ...
    assert.ok(aggregation.posterior <= 0.55, `Posterior should be <= 0.55, got ${aggregation.posterior}`);
  });
  ...
  it('should reroute when single verifier fails on otherwise-good response', () => {
    ...
    assert.ok(aggregation.posterior > 0.55 && aggregation.posterior < 0.85, `Posterior should be in uncertain range, got ${aggregation.posterior}`);
  });
  ...
  it('should handle missing verifiers gracefully', () => {
    ...
    const aggregation = aggregator.aggregate(results);
    assert.ok(aggregation.decision !== undefined, 'Should make a decision even with missing verifier');
  });
  ...
  it('should handle zero-weight verifiers', () => {
    ...
    const aggregation = aggregator.aggregate(results);
    assert.ok(aggregation.decision !== undefined, 'Should handle zero-weight verifiers');
  });
  ...
  it('should clamp posterior to [0, 1]', () => {
    ...
    assert.ok(passAggregation.posterior <= 1, `Posterior should be <= 1, got ${passAggregation.posterior}`);
    ...
    assert.ok(failAggregation.posterior >= 0, `Posterior should be >= 0, got ${failAggregation.posterior}`);
  });
  ...
  it('should normalize weights correctly', () => {
    ...
    const aggregation = aggregator.aggregate(results);
    assert.ok(aggregation.decision !== undefined, 'Should handle uneven weights');
  });
});
