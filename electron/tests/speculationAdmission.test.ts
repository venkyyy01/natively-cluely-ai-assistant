...
test('PredictivePrefetcher admits speculation only when the runtime budget scheduler allows it', async () => {
  setOptimizationFlags({ accelerationEnabled: true, usePrefetching: true, maxPrefetchPredictions: 3 });
  ...
  assert.equal(await prefetcher.getContext('What are the main components?'), null);
});
