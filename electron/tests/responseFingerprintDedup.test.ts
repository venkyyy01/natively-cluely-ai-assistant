...
test("NAT-048: identical consecutive answers result in a single emission", () => {
	...
	const answer =
		"I would partition the queue by tenant, with a per-tenant rate cap and a shared overflow shelf.";
	...
	const firstFinalCount = harness.emissions.filter(
		(e) => e.type === "final",
	).length;
	...
	assert.equal(
		finalEmissions.length,
		1,
		"duplicate must not produce a second final emission",
	);
	assert.equal(
	...
	assert.equal(
		harness.sessionMessages.length,
		1,
		"session must not double-record the same answer",
	);
	assert.equal(
		harness.usagePushes.length,
		1,
		"usage must not double-count the duplicate",
	);
	assert.equal(
		harness.tracker.getSnapshot(requestId2)?.terminalStatus,
		"suppressed",
	);
	assert.equal(
		harness.tracker.getSnapshot(requestId2)?.suppressionReason,
		"duplicate_answer",
	);
...
test("NAT-048: distinct answers are both emitted", () => {
	...
	const first =
		"I would use a leader-follower replication topology with quorum writes.";
	const second =
		"For the cache layer, an LRU with a 90-second TTL prevents stampedes.";
...
test("NAT-048: clearing the fingerprinter (e.g. on session switch) re-allows previously-seen answers", () => {
	...
	const answer =
		"I would shard the database by user_id with consistent hashing.";
...
test("NAT-048: the same answer can be emitted again for a different question", () => {
	...
	const answer =
		"I would start with a token bucket and then add per-tenant quotas.";
	...
	assert.equal(
		finalEmissions.length,
		2,
		"question changes must reset duplicate suppression",
	);
});
