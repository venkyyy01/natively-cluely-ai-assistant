...
test("NAT-020: 3 consecutive failures trip the breaker and emit bus:listener-circuit-open", async () => {
	...
	assert.equal(
		calls,
		LISTENER_FAILURE_THRESHOLD,
		`expected ${LISTENER_FAILURE_THRESHOLD} failing-listener invocations, got ${calls}`,
	);
	...
	assert.equal(
		...
		1,
		"expected exactly one bus:listener-circuit-open",
	);
	const breaker = breakerEvents[0];
	if (breaker.type !== "bus:listener-circuit-open")
