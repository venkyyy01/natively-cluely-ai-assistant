...
test('behavioral prompt rules cover hidden behavioral questions and action-heavy grounded stories', () => {
  for (const prompt of [FAST_STANDARD_ANSWER_PROMPT, UNIVERSAL_WHAT_TO_ANSWER_PROMPT]) {
    assert.match(prompt, /How do you manage people|management style|difficult decisions|influence without authority/i);
    assert.match(prompt, /approach.*example|short approach statement/i);
    assert.match(prompt, /Action section should be the deepest|make Action section the most detailed/i);
    ...
    assert.match(prompt, /learned|would change next time|would be improved next time/i);
...
test('behavioral prompt rules explicitly override generic short-answer and no-header constraints', () => {
  for (const prompt of [FAST_STANDARD_ANSWER_PROMPT, UNIVERSAL_WHAT_TO_ANSWER_PROMPT]) {
    assert.match(prompt, /behavioral.*may use headers|behavioral.*headers are required|except for behavioral/i);
    assert.match(prompt, /behavioral.*can exceed.*1-3 sentences|behavioral.*not limited to 1-3 sentences|behavioral.*not limited by the generic short-answer rules/i);
    assert.match(prompt, /behavioral.*can exceed.*100 words|100 words.*unless behavioral|word limit.*except for behavioral/i);
  }
});
