...
class StubClassifierLane implements Pick<RuntimeBudgetScheduler, "submit"> {
	...
	async submit<T>(
		runtimeLane: "realtime" | "local-inference" | "semantic" | "background",
		task: () => Promise<T> | T,
	): Promise<T> {
...
test("ConsciousAccelerationOrchestrator routes pause-action classification through semantic lane", async () => {
	...
	orchestrator.onUserSpeaking(
		"How would this architecture handle regional failover?",
	);
	orchestrator.updateTranscriptSegments(
		[
			{
				speaker: "interviewer",
				text: "How would this architecture handle regional failover?",
				timestamp: Date.now(),
			},
		],
		1,
	);
	orchestrator.onSilenceStart(
		"How would this architecture handle regional failover?",
	);
...
test("ConsciousAccelerationOrchestrator preclassifies the latest transcript intent on silence and caches it by revision", async () => {
	...
	assert.equal(
		orchestrator.getPrefetchedIntent(question, 7)?.intent,
		"behavioral",
	);
});
...
test("ConsciousAccelerationOrchestrator invalidates prefetched intents when transcript revision advances", async () => {
	...
	await new Promise((resolve) => setTimeout(resolve, 40));
	assert.equal(
		orchestrator.getPrefetchedIntent(question, 3)?.intent,
		"behavioral",
	);
...
test("ConsciousAccelerationOrchestrator deduplicates concurrent intent prefetches for the same query and revision", async () => {
	...
	assert.equal(classifierCalls, 1);
	assert.equal(
		orchestrator.getPrefetchedIntent(question, 21)?.intent,
		"behavioral",
	);
});
...
test("ConsciousAccelerationOrchestrator suppresses stale speculative previews and finalized answers after revision changes", async () => {
	...
	orchestrator.setEnabled(true);
	orchestrator.setSpeculativeExecutor(
		async function* (_query, _revision, abortSignal) {
	...
	const previewPromise = orchestrator.getSpeculativeAnswerPreview(
		question,
		12,
		80,
	);
	...
	orchestrator.updateTranscriptSegments(
		[
			{
				speaker: "interviewer",
				text: `${question} with dead-letter handling`,
				timestamp: Date.now(),
			},
		],
		13,
