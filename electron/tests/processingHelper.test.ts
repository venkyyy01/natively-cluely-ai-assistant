...
test('loadStoredCredentials restores saved keys and fast response config at startup', async () => {
  ...
  class FakeLLMHelper {
    ...
    setApiKey(value: string) { llmCalls.push({ method: 'setApiKey', value }); }
    setGroqApiKey(value: string) { llmCalls.push({ method: 'setGroqApiKey', value }); }
    setCerebrasApiKey(value: string) { llmCalls.push({ method: 'setCerebrasApiKey', value }); }
    setOpenaiApiKey(value: string) { llmCalls.push({ method: 'setOpenaiApiKey', value }); }
    setClaudeApiKey(value: string) { llmCalls.push({ method: 'setClaudeApiKey', value }); }
    setFastResponseConfig(value: unknown) { llmCalls.push({ method: 'setFastResponseConfig', value }); }
    setModel(value: string) { llmCalls.push({ method: 'setModel', value }); }
    setSttLanguage(value: string) { llmCalls.push({ method: 'setSttLanguage', value }); }
    setAiResponseLanguage(value: string) { llmCalls.push({ method: 'setAiResponseLanguage', value }); }
    async initModelVersionManager() { llmCalls.push({ method: 'initModelVersionManager', value: true }); }
  }
  ...
  const credentialsManager = {
    ...
    getFastResponseConfig: () => ({ enabled: true, provider: 'cerebras', model: 'gpt-oss-120b' }),
  ...
  try {
    ...
    const appState = {
      getIntelligenceManager: () => ({ initializeLLMs: () => { initializeCount += 1; } }),
      getRAGManager: (): any => ({
        initializeEmbeddings: () => { ragCalls.push('initializeEmbeddings'); },
        retryPendingEmbeddings: async () => { ragCalls.push('retryPendingEmbeddings'); },
        ensureDemoMeetingProcessed: async () => { ragCalls.push('ensureDemoMeetingProcessed'); },
        cleanupStaleQueueItems: () => { ragCalls.push('cleanupStaleQueueItems'); },
      }),
    };
    ...
    assert.deepEqual(
      ...
      [
        ...
        { method: 'setFastResponseConfig', value: { enabled: true, provider: 'cerebras', model: 'gpt-oss-120b' } },
...
test('loadStoredCredentials skips optional startup branches when credentials and services are absent', async () => {
  ...
  const credentialsManager = {
    ...
    getOpenaiApiKey: () => '',
    getClaudeApiKey: () => '',
    getFastResponseConfig: () => ({ enabled: false, provider: 'groq', model: '' }),
    getDefaultModel: () => '',
    getCustomProviders: (): any[] => [],
    getCurlProviders: (): any[] => [],
    getSttLanguage: () => '',
  ...
  try {
    ...
    assert.equal(initializeCount, 1);
    assert.deepEqual(llmCalls.filter(call => call.method !== 'initModelVersionManager'), [
      { method: 'setFastResponseConfig', value: { enabled: false, provider: 'groq', model: '' } },
    ]);
  } finally {
    (Module as any)._load = originalLoad;
  }
