...
test("SessionTracker adaptive context preserves original speaker roles", async () => {
	...
	tracker.addAssistantMessage(
		"I would prioritize durability and idempotency before throughput tuning.",
	);

	const context = await tracker.getAdaptiveContext(
		"queue durability",
		[1, 0, 0],
		500,
	);
	...
	assert.ok(
		context.some((item) => item.role === "user") ||
			context.some((item) => item.role === "assistant"),
	);
	assert.equal(
		context.every((item) => item.role === "interviewer"),
		false,
	);
});
...
test("ParallelContextAssembler preserves user role and timestamps for relevant context", async () => {
	...
	const result = await assembler.assemble({
		...
		transcript: [
			{
				speaker: "interviewer",
				text: "How would you design durable queues?",
				timestamp: Date.now() - 3000,
			},
			{
				speaker: "user",
				text: "I would use append-only log with retry semantics.",
				timestamp: tsUser,
			},
		],
	...
	const userEntry = result.relevantContext.find((item) =>
		item.text.includes("append-only log"),
	);
	assert.ok(userEntry);
