...
test('buildMailtoLink encodes recipient and preserves %20 spacing', () => {
  const link = buildMailtoLink('a@example.com,b@example.com', 'Hello world', 'Line one\nLine two');
...
test('buildGmailComposeUrl includes expected query parameters', () => {
  const url = new URL(buildGmailComposeUrl('to@example.com', 'Status update', 'Body text'));
...
test('generateEmailSubject handles interview and default meeting types', () => {
  assert.equal(generateEmailSubject(' "Platform Review" ', 'interview'), 'Following up on our conversation - Platform Review');
  assert.equal(generateEmailSubject('*Weekly Sync*'), 'Following up - Weekly Sync');
});
