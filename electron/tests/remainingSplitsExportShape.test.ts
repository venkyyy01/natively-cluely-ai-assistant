...
test('NAT-067: StealthManager.ts still exports StealthManager class', () => {
  const source = fs.readFileSync(path.join(STEALTH_DIR, 'StealthManager.ts'), 'utf8')
  assert.ok(source.includes('export class StealthManager'), 'should export StealthManager class')
})

test('NAT-067: SessionTracker split modules exist', () => {
  const modules = ['sessionTypes.ts', 'sessionContext.ts', 'sessionPersistence.ts']
  for (const mod of modules) {
...
test('NAT-067: SessionTracker.ts still exports SessionTracker class', () => {
  const source = fs.readFileSync(path.join(process.cwd(), 'electron', 'SessionTracker.ts'), 'utf8')
  assert.ok(source.includes('export class SessionTracker'), 'should export SessionTracker class')
})
