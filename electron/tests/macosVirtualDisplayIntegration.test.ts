...
import {
	type MacosVirtualDisplayClient,
	MacosVirtualDisplayCoordinator,
} from "../stealth/MacosVirtualDisplayClient";
import {
...
test("resolveMacosVirtualDisplayHelperPath checks packaged XPC helper candidates before extraResources helper", () => {
	...
	const resolved = resolveMacosVirtualDisplayHelperPath({
		...
		pathExists: (candidate) => {
			...
			return (
				candidate ===
				"/Applications/Natively.app/Contents/XPCServices/macos-full-stealth-helper.xpc/Contents/MacOS/macos-full-stealth-helper"
			);
		},
	});
...
test("createMacosVirtualDisplayCoordinator returns a coordinator with the given helper path", () => {
	const coordinator = createMacosVirtualDisplayCoordinator(
...
test("resolveMacosVirtualDisplayHelperPath uses resourcesPath when provided", () => {
	const resolved = resolveMacosVirtualDisplayHelperPath({
		...
		resourcesPath: "/app/resources",
		pathExists: (candidate) =>
			candidate === "/app/resources/bin/macos/system-services-helper",
	});
...
test("MacosVirtualDisplayCoordinator dispose releases active sessions and disposes client", async () => {
	...
	await coordinator.ensureIsolationForWindow({
		sessionId: "s1",
		windowId: "w1",
		width: 1280,
		height: 720,
