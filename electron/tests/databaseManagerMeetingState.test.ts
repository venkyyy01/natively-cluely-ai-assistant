        ...
        all(limit: number) {
          ...
          return [
            ...
            },
            {
              ...
              duration_ms: 61_000,
              summary_json: JSON.stringify({ legacySummary: 'Meeting processing failed' }),
              calendar_event_id: null as string | null,
              source: 'manual',
              is_processed: -1,
            },
...
test('DatabaseManager.markMeetingProcessingFailed normalizes placeholder titles', () => {
  ...
  const fakeDb = {
    prepare(sql: string) {
      if (sql.startsWith('SELECT title, summary_json FROM meetings WHERE id = ?')) {
        return {
          get(id: string) {
            assert.equal(id, 'meeting-1');
            return {
              title: 'Processing...',
              summary_json: JSON.stringify({ legacySummary: 'Generating summary...' }),
            };
          },
        };
      }

      if (sql.startsWith('UPDATE meetings SET title = ?, is_processed = -1, summary_json = ? WHERE id = ?')) {
        return {
