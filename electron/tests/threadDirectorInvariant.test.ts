...
test('NAT-055: ThreadDirector keeps design and reasoning active thread ids aligned', () => {
  ...
  store.getThreadManager().createThread('Design a distributed cache', 'high_level_design');
...
test('NAT-055: continue updates reasoning threadId if design thread is still active', () => {
  ...
  store.getThreadManager().createThread('Rate limiting topic', 'high_level_design');
  store.recordConsciousResponse('How would you rate limit?', minimalResponse, 'start');
  store.recordConsciousResponse('What about burst traffic?', minimalResponse, 'continue');
