...
describe('SemanticThreadMatcher', () => {
  ...
  it('should detect semantic compatibility between paraphrased questions', async () => {
    ...
    const isCompatible = await matcher.isCompatible('What about at scale?', thread);
    assert.strictEqual(isCompatible, true, 'Paraphrased questions should be compatible');
  });
  ...
  it('should reject semantically distinct questions', async () => {
    ...
    const isCompatible = await matcher.isCompatible('What cache are you using?', thread);
    assert.strictEqual(isCompatible, false, 'Semantically distinct questions should not be compatible');
  });
  ...
  it('should fall back to false for very short questions (< 4 words)', async () => {
    ...
    assert.strictEqual(isCompatible, false, 'Very short questions should fall back to false');
  });
  ...
  it('should detect referential follow-up cues', async () => {
    ...
    const isCompatible = await matcher.isCompatible('How does that work?', thread);
    assert.strictEqual(isCompatible, true, 'Referential follow-up cues should be detected');
  });
  ...
  it('should use cached thread embedding when available', async () => {
    ...
    const isCompatible = await matcher.isCompatible('What about scalability?', thread);
  ...
  it('should handle model loading failure gracefully', async () => {
    ...
    const isCompatible = await matcher.isCompatible('How does it work?', thread);
    assert.strictEqual(typeof isCompatible, 'boolean');
  });
