...
import {
	AccelerationManager,
	setActiveAccelerationManager,
} from "../services/AccelerationManager";
...
test("speculative fast answer is reused by the live what-to-say path", async () => {
	...
	const metadataSeen: Array<{
		verifier?: { deterministic: string; provenance: string };
	}> = [];
	engine.on(
		"suggested_answer",
		(
			...
			_question: string,
			_confidence: number,
			metadata?: { verifier?: { deterministic: string; provenance: string } },
		) => {
	...
	consciousAcceleration.noteTranscriptText(
		"interviewer",
	...
	assert.equal(
		llmHelper.calls.some((call) => call.message === "What is polymorphism?"),
		true,
	);
	assert.deepEqual(metadataSeen, [
		{ verifier: { deterministic: "skipped", provenance: "skipped" } },
	]);
...
test("speculative answers are invalidated when transcript revision changes", async () => {
	...
	assert.equal(
		llmHelper.calls.some((call) => call.message === "Explain encapsulation."),
		true,
	);
...
test("speculative acceleration stays disabled when conscious mode is off", async () => {
	...
	await new Promise((resolve) => setTimeout(resolve, 120));
	assert.equal(
		await consciousAcceleration.getSpeculativeAnswer(
			"What is polymorphism?",
			session.getTranscriptRevision(),
			0,
		),
...
test("speculative answers stream early tokens before the final answer is committed", async () => {
	...
	assert.deepEqual(tokenEvents, [
		"speculative ",
		"answer for: What is polymorphism?",
	]);
	...
	const answer = await Promise.race([
		...
		new Promise<symbol>((_, reject) =>
			setTimeout(
				() =>
					reject(
						new Error("speculative stealth containment did not stop in time"),
					),
				200,
			),
		),
...
test("NAT-002: speculative selection refuses semantic hedging when the finalized question shifts", async () => {
	...
	orchestrator.setSpeculativeExecutor((query) =>
		(async function* () {
	...
	const answer = await orchestrator.getSpeculativeAnswer(
		"What consistency model does it provide for cross-region writes?",
		1,
		200,
	);
	assert.equal(answer, null);
