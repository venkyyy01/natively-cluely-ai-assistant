...
export function toMemoryEntry(id: string, value: PersistedSessionMemoryEntryValue): PersistedSessionMemoryEntry {
  return {
    id,
...
export function toTranscriptMemoryEntry(segment: TranscriptSegment, id: string): PersistedSessionMemoryEntry {
...
export function toUsageMemoryEntry(entry: UsageInteraction, id: string): PersistedSessionMemoryEntry {
  return toMemoryEntry(id, {
    kind: 'usage',
...
export function getHotState(tracker: SessionTracker, now: number = Date.now()): PersistedSessionMemoryEntry[] {
  const t = tracker as any;
  ...
  const transcriptEntries = t.fullTranscript
    ...
    .map((segment: TranscriptSegment, index: number) => toTranscriptMemoryEntry(segment, `hot-transcript-${index}`));
  const usageEntries = t.fullUsage
    .filter((entry: UsageInteraction) => entry.timestamp >= cutoff)
    .map((entry: UsageInteraction, index: number) => toUsageMemoryEntry(entry, `hot-usage-${index}`));

  return applyMemoryCeiling([...transcriptEntries, ...usageEntries], HOT_MEMORY_CEILING_BYTES);
}

export function getWarmState(tracker: SessionTracker, now: number = Date.now()): PersistedSessionMemoryEntry[] {
  const t = tracker as any;
  const entries: PersistedSessionMemoryEntry[] = [];
  const activeThread = t.consciousThreadStore.getThreadManager().getActiveThread();

  if (activeThread) {
    entries.push(toMemoryEntry(`warm-thread-${activeThread.id}`, {
      kind: 'active-thread',
      timestamp: now,
  ...
  t.pinnedItems.forEach((item: any, index: number) => {
    entries.push(toMemoryEntry(`warm-pin-${index}-${item.id}`, {
      kind: 'pinned-item',
      text: item.text,
  ...
  t.extractedConstraints.forEach((constraint: ExtractedConstraint, index: number) => {
    entries.push(toMemoryEntry(`warm-constraint-${index}`, {
      kind: 'constraint',
      text: constraint.raw,
      timestamp: now,
  ...
  t.transcriptEpochSummaries.forEach((summary: string, index: number) => {
    entries.push(toMemoryEntry(`warm-epoch-${index}`, {
      kind: 'epoch-summary',
      text: summary,
      timestamp: now,
    }));
...
export function getColdState(tracker: SessionTracker, now: number = Date.now()): PersistedSessionMemoryEntry[] {
  const t = tracker as any;
  const cutoff = now - HOT_MEMORY_WINDOW_MS;
  const transcriptEntries = t.fullTranscript
    .filter((segment: TranscriptSegment) => segment.timestamp < cutoff)
    .map((segment: TranscriptSegment, index: number) => toTranscriptMemoryEntry(segment, `cold-transcript-${index}`));
  const usageEntries = t.fullUsage
    .filter((entry: UsageInteraction) => entry.timestamp < cutoff)
    .map((entry: UsageInteraction, index: number) => toUsageMemoryEntry(entry, `cold-usage-${index}`));
...
export function buildPersistedSession(tracker: SessionTracker, now: number = Date.now()): PersistedSession {
  const t = tracker as any;
  ...
  return {
    ...
    suspendedThreads: threadManager.getSuspendedThreads().map((thread: any) => ({
      id: thread.id,
      topic: thread.topic,
      goal: thread.goal,
...
export async function appendTranscriptEvent(tracker: SessionTracker, segment: TranscriptSegment): Promise<void> {
  ...
  if (t.eventCount % t.EVENT_SNAPSHOT_INTERVAL === 0) {
    const snapshot = buildPersistedSession(tracker);
    await t.persistence.snapshotEvents(t.sessionId, snapshot).catch((err: any) => {
...
export async function restoreFromMeetingId(tracker: SessionTracker, meetingId: string, requestId: number = (tracker as any).restoreRequestId): Promise<boolean> {
  ...
  try {
    ...
    t.extractedConstraints = (session.constraints || []) as ExtractedConstraint[];
    ...
    if (t.consciousModeEnabled) {
      t.answerHypothesisStore.restorePersistenceSnapshot(session.consciousState?.hypothesisState);
      t.responsePreferenceStore.restorePersistenceSnapshot(session.consciousState?.preferenceState);
      t.designStateStore.restorePersistenceSnapshot(session.consciousState?.designState);
      if (session.activeThread) {
        t.consciousThreadStore.restoreActiveThread({
          ...
          phase: (session.activeThread.phase as any) || 'requirements_gathering',
...
export function ensureMeetingContext(tracker: SessionTracker, meetingId?: string): void {
  const t = tracker as any;
  if (t.disposed) {
    return;
  }
  ...
  t.pendingRestorePromise = restoreFromMeetingId(tracker, normalizedMeetingId, requestId)
    .then(() => {
...
export async function flushPersistenceNow(tracker: SessionTracker): Promise<void> {
  persistState(tracker);
  const t = tracker as any;
  await t.persistence.flushScheduledSave();
}

export function restorePersistedMemoryState(tracker: SessionTracker, memoryState?: PersistedSessionMemoryState): void {
  const t = tracker as any;
  if (!memoryState) {
    return;
  }
  ...
  const assistantSegments = t.fullTranscript.filter((segment: TranscriptSegment) => segment.speaker === 'assistant');
