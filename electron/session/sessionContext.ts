...
export function computePhaseAlignmentScore(
...
export function computeFacetQueryBoost(
	text: string,
	queryTokens: string[],
): number {
	...
	return docTerms.map((terms) => {
		...
		for (const term of queryTerms) {
			...
			const df = docTerms.filter((doc) =>
				doc.some((candidate) => candidate === term || candidate.includes(term)),
			).length;
...
export function takeWithinTokenBudget(
	tracker: SessionTracker,
	values: string[],
	maxTokens: number,
): string[] {
...
export async function getSemanticEmbedding(
	...
	if (provider?.isInitialized()) {
		try {
			...
			t.semanticEmbeddingCache.set(normalized, {
				embedding,
				createdAt: Date.now(),
			});
			return embedding;
...
export function semanticRedundancyScore(
	...
	for (let i = 0; i < selectedTexts.length; i++) {
		const semantic = selectedEmbeddings[i]?.length
			? cosineSimilarity(embedding, selectedEmbeddings[i])
			: 0;
...
export async function rankConsciousContextItems(
	...
	const preRanked = merged
		.map((candidate, index) => {
			...
			const phase = computePhaseAlignmentScore(
				candidate.item.phase,
				currentPhase,
			);
			const bm25 = bm25Scores[index] / maxBm25;
			const facetBoost = computeFacetQueryBoost(
				candidate.item.text,
				queryTokens,
			);
			return {
	...
	while (scored.length > 0) {
		...
		for (let index = 0; index < scored.length; index++) {
			...
			const candidateTokens = t.tokenCounter.count(
				candidate.item.text,
				"openai",
			);
...
export function inferItemPhase(
	tracker: SessionTracker,
...
export function getClampedQuery(
	tracker: SessionTracker,
	query: string,
): string {
	const t = tracker as any;
...
export function getCacheEntryKey(
	tracker: SessionTracker,
	query: string,
): string {
	const t = tracker as any;
...
export function getCachedAssembledContext(
	tracker: SessionTracker,
	query: string,
): string | null {
	const t = tracker as any;
...
export function setCachedAssembledContext(
	tracker: SessionTracker,
	query: string,
	assembled: string,
): void {
...
export function getAdaptiveFallbackContext(
	tracker: SessionTracker,
	tokenBudget: number,
): ContextItem[] {
	const t = tracker as any;
	...
	const pinnedAsContext: ContextItem[] = t.pinnedItems.map(
		(item: PinnedItem) => ({
			role: "interviewer",
...
export async function getAdaptiveContext(
	...
	const candidates: ContextEntry[] = t.contextItemsBuffer
		.toArray()
...
export async function getConsciousRelevantContext(
	tracker: SessionTracker,
	query: string,
	...
	const adaptive = await getAdaptiveContext(
		tracker,
		trimmedQuery,
		queryEmbedding,
	...
	const designStateEntries = t.designStateStore
	...
	const constraintEntries = t.extractedConstraints
		.slice(-8)
		.map((constraint: any) => ({
			item: {
				role: "interviewer" as const,
	...
	const summaryEntries = t.transcriptEpochSummaries
		.slice(-3)
		.map((summary: string, index: number) => ({
			item: {
				...
				timestamp:
					Date.now() - (t.transcriptEpochSummaries.length - index) * 60_000,
				phase: t.phaseDetector.getCurrentPhase(),
	...
	const ranked = await rankConsciousContextItems(
		...
		[
			...
			...recentTurns.map((item: ContextItem) => ({
				item,
...
export function getConsciousLongMemoryContext(
	tracker: SessionTracker,
	query: string,
): string {
	const t = tracker as any;
...
export function getFormattedContext(
	tracker: SessionTracker,
	lastSeconds: number = 120,
): string {
	const baseCached = getCachedAssembledContext(
		tracker,
...
export function getCompactTranscriptSnapshot(
	tracker: SessionTracker,
	maxTurns: number = 12,
...
export function buildFullSessionContext(
