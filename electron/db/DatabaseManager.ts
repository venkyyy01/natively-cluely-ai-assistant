...
function normalizeMeetingTitle(
	...
	if (
		...
		PLACEHOLDER_MEETING_TITLES.has(trimmed)
	) {
...
export class DatabaseManager {
	...
	private constructor() {
		...
		this.migrationBackupPath = path.join(
			userDataPath,
			"natively.db.migration-backup",
		);
		this.init();
	}
	...
	private init() {
		try {
			...
			try {
				const integrity = this.db.pragma("integrity_check(1)", {
					simple: true,
				});
				if (integrity !== "ok") {
					console.warn(
						"[DatabaseManager] integrity_check reported:",
						integrity,
					);
				}
			} catch (integrityError) {
				console.error(
					"[DatabaseManager] Failed to run integrity_check:",
					integrityError,
				);
			}
	...
	private runMigrations() {
		...
		if (version < 11) {
			console.log(
				"[DatabaseManager] Applying migration v10 → v11: Clean orphaned embedding queue + backfill meeting embeddings",
			);
	...
	private migrateExistingEmbeddings(): void {
		...
		try {
			...
			if (chunkRows.length > 0) {
				console.log(
					`[DatabaseManager] Migrating ${chunkRows.length} chunk embeddings in batches of ${MIGRATION_BATCH_SIZE}`,
				);
				...
				for (let i = 0; i < chunkRows.length; i += MIGRATION_BATCH_SIZE) {
					...
					if (chunkRows.length > MIGRATION_BATCH_SIZE) {
						console.log(
							`[DatabaseManager] Migrated chunk batch ${Math.floor(i / MIGRATION_BATCH_SIZE) + 1}/${Math.ceil(chunkRows.length / MIGRATION_BATCH_SIZE)}`,
						);
					}
				}
				console.log(
					`[DatabaseManager] Successfully migrated ${chunkRows.length} chunk embeddings to vec_chunks`,
				);
			}
		} catch (e) {
		...
		try {
			...
			if (summaryRows.length > 0) {
				console.log(
					`[DatabaseManager] Migrating ${summaryRows.length} summary embeddings in batches of ${MIGRATION_BATCH_SIZE}`,
				);
				const insert = this.db.prepare(
				...
				for (let i = 0; i < summaryRows.length; i += MIGRATION_BATCH_SIZE) {
					...
					if (summaryRows.length > MIGRATION_BATCH_SIZE) {
						console.log(
							`[DatabaseManager] Migrated summary batch ${Math.floor(i / MIGRATION_BATCH_SIZE) + 1}/${Math.ceil(summaryRows.length / MIGRATION_BATCH_SIZE)}`,
						);
					}
				}
				console.log(
					`[DatabaseManager] Successfully migrated ${summaryRows.length} summary embeddings to vec_summaries`,
				);
			}
		} catch (e) {
			console.error(
	...
	public close(): void {
		...
		try {
			...
			try {
				...
				console.warn(
					"[DatabaseManager] wal_checkpoint(TRUNCATE) failed on close:",
					error,
				);
			}
			try {
				...
				if (
					(error as NodeJS.ErrnoException | undefined)?.code !== "SQLITE_BUSY"
				) {
					console.warn(
						"[DatabaseManager] journal_mode=DELETE failed on close:",
						error,
					);
				}
			}
			this.db.close();
			console.log(
				"[DatabaseManager] Closed SQLite connection (WAL truncated).",
			);
		} catch (error) {
	...
	public static closeIfOpen(): void {
		const existing = (
			DatabaseManager as unknown as { instance?: DatabaseManager }
		).instance;
	...
	public saveMeeting(
		...
		const insertInteraction = this.db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        `);
		const deleteTranscripts = this.db.prepare(
		...
		try {
			...
			setupTransaction();
			console.log(
				`[DatabaseManager] Setup completed for meeting ${meeting.id}`,
			);

			// Second phase: Insert transcript in batches
			if (meeting.transcript && meeting.transcript.length > 0) {
				console.log(
					`[DatabaseManager] Processing ${meeting.transcript.length} transcript segments in batches of ${BATCH_SIZE}`,
				);
				...
				for (let i = 0; i < meeting.transcript.length; i += BATCH_SIZE) {
					...
					if (meeting.transcript.length > BATCH_SIZE) {
						console.log(
							`[DatabaseManager] Processed transcript batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(meeting.transcript.length / BATCH_SIZE)}`,
						);
					}
				}
			}
			...
			if (meeting.usage && meeting.usage.length > 0) {
				console.log(
					`[DatabaseManager] Processing ${meeting.usage.length} interactions in batches of ${BATCH_SIZE}`,
				);
				...
				for (let i = 0; i < meeting.usage.length; i += BATCH_SIZE) {
					...
					if (meeting.usage.length > BATCH_SIZE) {
						console.log(
							`[DatabaseManager] Processed interaction batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(meeting.usage.length / BATCH_SIZE)}`,
						);
					}
				}
			}
			...
			console.log(
				`[DatabaseManager] Successfully saved meeting ${meeting.id} with batched operations`,
			);
		} catch (err) {
			console.error(
			...
			try {
				...
				this.db
					.prepare("DELETE FROM transcripts WHERE meeting_id = ?")
					.run(meeting.id);
				this.db
					.prepare("DELETE FROM ai_interactions WHERE meeting_id = ?")
					.run(meeting.id);
				console.log(
					`[DatabaseManager] Cleaned up partial data for meeting ${meeting.id}`,
				);
			} catch (cleanupErr) {
				console.error(
					`[DatabaseManager] Failed to cleanup meeting ${meeting.id}`,
					cleanupErr,
				);
			}
			throw err;
	...
	public createOrUpdateMeetingProcessingRecord(
		meeting: Meeting,
		startTimeMs: number,
		durationMs: number,
	): void {
	...
	public finalizeMeetingProcessing(
		meeting: Meeting,
		startTimeMs: number,
		durationMs: number,
	): void {
	...
	public markMeetingProcessingFailed(id: string, error: unknown): boolean {
		...
		try {
			const current = this.db
				.prepare("SELECT title, summary_json FROM meetings WHERE id = ?")
				.get(id) as { title?: string; summary_json?: string } | undefined;
			const existing = current?.summary_json
				? JSON.parse(current.summary_json)
				: {};
			const summaryJson = JSON.stringify({
				...existing,
			...
			console.error(
				`[DatabaseManager] Failed to mark meeting ${id} as failed:`,
				updateError,
			);
			return false;
		}
			...
			const deleteTransaction = this.db.transaction(() => {
				...
				for (const dim of this.getKnownVecDimensions()) {
					try {
						...
						console.warn(
							`[DatabaseManager] Failed to delete vec rows for dim=${dim}:`,
							vecError,
						);
					}
				}
...
natively.contact@gmail.com`;
		...
		const demoMeeting: Meeting = {
			...
			duration: "5:00",
			summary:
				"Complete guide to using Natively - your real-time AI meeting assistant.",
			detailedSummary: {
			...
			transcript: [
				...
				{
					speaker: "interviewer",
					text: "'Shorten' condenses the last response. 'Recap' summarizes the entire conversation so far.",
					timestamp: 22000,
				},
				{
					speaker: "user",
				...
				{
					speaker: "interviewer",
					text: "Yes! Press Cmd+Option+Shift+S for full screen or Cmd+Option+Shift+A to select an area. F14 and F15 also work as alternates. The AI will analyze it and help you.",
					timestamp: 50000,
				},
				{
					speaker: "user",
					text: "How do I hide Natively during screen share?",
					timestamp: 60000,
				},
				{
					speaker: "interviewer",
					text: "Press Cmd+Option+Shift+V to toggle visibility anytime. F13 also works as an alternate. You can also enable undetectable mode in settings.",
					timestamp: 65000,
				},
				{
					speaker: "user",
			...
			usage: [
				{
					type: "assist",
					timestamp: 15000,
					question: "What features does Natively have?",
					answer:
						"Natively offers 5 quick action buttons, screenshot analysis, real-time transcription, and comprehensive meeting notes.",
				},
				{
					type: "followup",
					timestamp: 40000,
					...
					answer:
						"Each button serves a specific purpose: suggest answers, shorten responses, recap conversations, generate follow-up questions, or get instant voice-to-answer responses.",
				},
			],
			isProcessed: true,
		};
	...
	private getKnownVecDimensions(): number[] {
		return Array.from(
			new Set([...DatabaseManager.KNOWN_DIMS, ...this.ensuredDims]),
		);
	}
}
