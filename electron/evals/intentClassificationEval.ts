...
export interface IntentEvalSummary {
  ...
  perIntent: Record<ConversationIntent, { total: number; correct: number; accuracy: number }>;
  confusionMatrix: Record<ConversationIntent, Record<ConversationIntent, number>>;
...
function createBaseSummary(): IntentEvalSummary {
  const perIntent = Object.fromEntries(
    INTENT_LABELS.map((intent) => [intent, { total: 0, correct: 0, accuracy: 0 }]),
  ) as Record<ConversationIntent, { total: number; correct: number; accuracy: number }>;

  const confusionMatrix = Object.fromEntries(
    INTENT_LABELS.map((expectedIntent) => [
      expectedIntent,
      Object.fromEntries(INTENT_LABELS.map((predictedIntent) => [predictedIntent, 0])) as Record<ConversationIntent, number>,
    ]),
...
export async function runIntentEval(
  ...
  classify: (input: IntentEvalClassifierInput) => Promise<IntentEvalClassifierResult>,
...
export function summarizeIntentEvalOutcomes(outcomes: IntentEvalOutcome[]): IntentEvalSummary {
  ...
  for (const outcome of outcomes) {
    ...
    summary.providerSplit[outcome.providerUsed] = (summary.providerSplit[outcome.providerUsed] ?? 0) + 1;
    ...
    summary.confusionMatrix[outcome.expectedIntent][outcome.predictedIntent] += 1;

    const bucket = summary.confidenceBuckets[getBucketIndex(outcome.confidence)];
    bucket.count += 1;
    if (isCorrect) {
      bucket.correct += 1;
    }
    ...
    if (outcome.fallbackReason) {
      ...
      summary.fallbackReasons[outcome.fallbackReason] = (summary.fallbackReasons[outcome.fallbackReason] ?? 0) + 1;
    }
  }
...
function formatProviderSplit(providerSplit: Record<string, number>): string {
  ...
  return keys.map((provider) => `${provider}:${providerSplit[provider]}`).join(', ');
}
...
function formatConfusionMatrix(summary: IntentEvalSummary): string[] {
  ...
  const rows = INTENT_LABELS.map((expectedIntent) => {
    const rowCounts = INTENT_LABELS.map((predictedIntent) => String(summary.confusionMatrix[expectedIntent][predictedIntent]));
...
export function formatIntentEvalSummary(summary: IntentEvalSummary): string {
  ...
  for (const intent of INTENT_LABELS) {
    const row = summary.perIntent[intent];
    lines.push(`- ${intent}: ${formatPercent(row.accuracy)} (${formatRatio(row.correct, row.total)})`);
  }
  lines.push('');
  lines.push('Confidence buckets:');
  for (const bucket of summary.confidenceBuckets) {
    lines.push(`- ${bucket.label}: ${formatPercent(bucket.accuracy)} (${formatRatio(bucket.correct, bucket.count)})`);
  }
  lines.push('');
...
export const DEFAULT_INTENT_EVAL_CASES: IntentEvalCase[] = [
  {
    ...
    lastInterviewerTurn: 'Tell me about a time you disagreed with your manager and how you resolved it.',
    preparedTranscript: '[INTERVIEWER]: Tell me about a time you disagreed with your manager and how you resolved it.',
  ...
  },
  {
    ...
    expectedIntent: 'behavioral',
    lastInterviewerTurn: 'Describe a situation where you had to influence people without formal authority.',
    preparedTranscript: '[INTERVIEWER]: Describe a situation where you had to influence people without formal authority.',
    assistantResponseCount: 0,
    tags: ['baseline', 'behavioral'],
  },
  {
    ...
    expectedIntent: 'behavioral',
    lastInterviewerTurn: 'Walk me through a failure you owned end to end and what you learned.',
    preparedTranscript: '[INTERVIEWER]: Walk me through a failure you owned end to end and what you learned.',
    assistantResponseCount: 0,
  ...
  },
  {
    ...
    expectedIntent: 'coding',
    lastInterviewerTurn: 'Debug this function: it should deduplicate IDs but still returns duplicates.',
    preparedTranscript: '[INTERVIEWER]: Debug this function: it should deduplicate IDs but still returns duplicates.',
    assistantResponseCount: 0,
    tags: ['baseline', 'coding'],
  },
  {
    ...
    preparedTranscript: '[INTERVIEWER]: Design and code an LRU cache in TypeScript.',
    assistantResponseCount: 0,
  ...
  },
  {
    ...
    lastInterviewerTurn: 'Why would you choose Kafka over RabbitMQ for this architecture?',
    preparedTranscript: '[INTERVIEWER]: Why would you choose Kafka over RabbitMQ for this architecture?',
    assistantResponseCount: 0,
  ...
  },
  {
    ...
    expectedIntent: 'deep_dive',
    lastInterviewerTurn: 'Explain the tradeoffs you made between consistency and availability.',
    preparedTranscript: '[INTERVIEWER]: Explain the tradeoffs you made between consistency and availability.',
    assistantResponseCount: 0,
    tags: ['baseline', 'deep_dive'],
  },
  {
  ...
  },
  {
    ...
    lastInterviewerTurn: 'When you say eventual consistency here, what exactly do you mean?',
    preparedTranscript: [
  ...
  },
  {
    ...
    expectedIntent: 'clarification',
    lastInterviewerTurn: 'Can you unpack what you meant by backpressure in your queue workers?',
    preparedTranscript: [
  ...
  },
  {
    ...
    expectedIntent: 'clarification',
    lastInterviewerTurn: 'Sorry, can you clarify the scope boundary you drew between services?',
    preparedTranscript: [
  ...
  },
  {
    ...
    expectedIntent: 'example_request',
    lastInterviewerTurn: 'What is one specific instance where this tradeoff hurt you?',
    preparedTranscript: [
  ...
  },
  {
    ...
    expectedIntent: 'coding',
    lastInterviewerTurn: 'Can you show an example API payload and handler code for this endpoint?',
    preparedTranscript: '[INTERVIEWER]: Can you show an example API payload and handler code for this endpoint?',
    assistantResponseCount: 0,
  ...
  },
  {
    ...
    lastInterviewerTurn: 'So you are saying writes stay synchronous while fan-out is async, right?',
    preparedTranscript: [
  ...
  },
  {
    ...
    expectedIntent: 'summary_probe',
    lastInterviewerTurn: 'Let me make sure I got this: you sharded by tenant before region?',
    preparedTranscript: [
  ...
  },
  {
    ...
    expectedIntent: 'summary_probe',
    lastInterviewerTurn: 'So to summarize, your first step is hot partition isolation?',
    preparedTranscript: [
  ...
  },
  {
    ...
    preparedTranscript: '[INTERVIEWER]: What interests you most about this role?',
    assistantResponseCount: 0,
  ...
  },
  {
    ...
    expectedIntent: 'general',
    lastInterviewerTurn: 'What kind of team environment helps you do your best work?',
    preparedTranscript: '[INTERVIEWER]: What kind of team environment helps you do your best work?',
    assistantResponseCount: 0,
    tags: ['general'],
  },
  {
    ...
    preparedTranscript: '[INTERVIEWER]: Do you have any questions for us about the position?',
    assistantResponseCount: 0,
    tags: ['general'],
  },
  {
    id: 'ambiguous-01-example-vs-behavioral',
    description: 'Example ask without past-life cue should stay example_request',
    expectedIntent: 'example_request',
    lastInterviewerTurn: 'Can you give a concrete example of retry jitter in practice?',
    preparedTranscript: '[INTERVIEWER]: Can you give a concrete example of retry jitter in practice?',
    assistantResponseCount: 0,
  ...
  },
  {
    ...
    description: 'Continuation prompt without summary framing should stay follow_up',
    expectedIntent: 'follow_up',
