...
export const ipcSchemas = {
	...
	followUpEmailInput: z
		.object({
			meeting_type: z.enum([
				...
				"discussion",
				"meeting",
			] as [FollowUpMeetingType, ...FollowUpMeetingType[]]),
			...
			tone: z
				.enum(["friendly", "neutral", "formal"] as [
					FollowUpTone,
					...FollowUpTone[],
				])
				.optional(),
		...
		.strict(),
	transcriptEntries: z
		.array(z.object({ text: boundedString(10000) }).strict())
		.max(2000),
	settingsWindowCoords: z
		.object({
	...
	providerPreferredModel: z.tuple([
		z.enum(["gemini", "groq", "openai", "claude", "cerebras"] as [
			ProviderKind,
			...ProviderKind[],
		]),
		boundedString(256),
	]),
	fastResponseConfig: z
		.object({
			enabled: z.boolean(),
			provider: z.enum(["groq", "cerebras"] as [
				FastResponseProvider,
				...FastResponseProvider[],
			]),
	...
	updateMeetingSummaryPayload: z
		...
		.strict(),
	updateMeetingTitlePayload: z
		.object({
			id: boundedString(128),
			title: boundedString(300),
		})
		.strict(),
	rendererLogPayload: z
		.object({
	...
	sttConnectionArgs: z.tuple([
		sttProviderEnum.exclude(["google"]),
		boundedString(4096),
		z.string().trim().max(128).optional(),
	]),
	llmConnectionArgs: z.tuple([
		llmProviderEnum,
		z.string().trim().max(4096).optional(),
	]),
	providerModelFetchArgs: z.tuple([
		llmProviderEnum,
		z.string().trim().max(4096),
	]),
	providerSwitchGeminiArgs: z.tuple([
		z.string().trim().max(4096).optional(),
		z.string().trim().max(256).optional(),
	]),
	ollamaSwitchArgs: z.tuple([
		z.string().trim().max(256).optional(),
