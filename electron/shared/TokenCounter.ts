...
function isCodeHeavy(text: string): boolean {
	...
	if (/[{}();=<>"]/.test(text)) return true;
	return /\b(function|class|const|let|var|import|export|return|async|await|interface|type)\b/i.test(
		text,
	);
}
...
function charsPerTokenForModel(modelHint: string): number {
	...
	if (
		hint.includes("llama") ||
		hint.includes("mixtral") ||
		hint.includes("gemma") ||
		hint.includes("groq") ||
		hint.includes("ollama")
	) {
		return 3.7;
	}
...
export class TokenCounter {
	...
	estimateCharacterBudget(
		tokenBudget: number,
		modelHint: string = this.defaultModelHint,
	): number {
	...
	private heuristicCount(text: string, modelHint: string): number {
		...
		const charsPerToken = charsPerTokenForModel(
			codeLike ? `${modelHint}:code` : modelHint,
		);
		const charBased = cjk
