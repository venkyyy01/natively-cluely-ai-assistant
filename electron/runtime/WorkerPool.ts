...
import {
	type AppleSiliconQoSClass,
	type AppleSiliconQoSHandle,
	appleSiliconQoS,
} from "./AppleSiliconQoS";
...
export class WorkerPool {
	...
	submit<T>(
		options: WorkerPoolSubmitOptions,
		task: () => Promise<T> | T,
	): Promise<T> {
			...
			Promise.resolve()
				.then(() => {
					try {
						...
						this.logger.warn(
							`[WorkerPool] QoS placement failed for lane "${task.lane}", continuing without QoS:`,
							error,
						);
					}
					return task.run();
				...
				.catch((error) => {
					this.logger.warn(
						`[WorkerPool] Task failed on lane "${task.lane}":`,
						error,
					);
					task.reject(error);
				})
