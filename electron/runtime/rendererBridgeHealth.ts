...
export type RendererBridgeHealthResult =
	| "ready"
	| "reloading"
	| "failed"
	| "destroyed"
	| "unprobeable";
export type RendererBridgeSettledResult = Exclude<
	RendererBridgeHealthResult,
	"reloading"
>;
...
async function probeRendererBridge(
	...
	if (window.isDestroyed()) {
		logger.warn(
			`[RendererBridge] ${label} bridge probe skipped: window destroyed`,
		);
		return "destroyed";
	}
	...
	if (typeof webContents.executeJavaScript !== "function") {
		logger.warn(
			`[RendererBridge] ${label} bridge probe skipped: executeJavaScript unavailable`,
		);
		return "unprobeable";
	}

	try {
		const hasBridge = await webContents.executeJavaScript(
			BRIDGE_PROBE_SCRIPT,
			true,
		);
		if (hasBridge === true) {
