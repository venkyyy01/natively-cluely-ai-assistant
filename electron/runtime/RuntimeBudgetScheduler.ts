...
export function compareScheduledLaneTasks(
	left: ScheduledLaneTaskSortKey,
	right: ScheduledLaneTaskSortKey,
): number {
...
export class RuntimeBudgetScheduler {
	...
	private readonly queues = new Map<
		RuntimeLane,
		Array<ScheduledLaneTask<unknown>>
	>();
	private readonly runningCounts = new Map<RuntimeLane, number>();
	private readonly lastPressure = new Map<
		RuntimeLane,
		BudgetPressureLevel | null
	>();
	private sequence = 0;
	...
	private pump(): void {
		for (;;) {
			...
			this.runningCounts.set(
				next.lane,
				(this.runningCounts.get(next.lane) ?? 0) + 1,
			);
				...
				try {
					const result = this.workerPool
						? await this.workerPool.submit(
								{ lane: next.lane, priority: next.priority },
								next.run,
							)
						: await next.run();
					next.resolve(result);
					...
					this.logger.warn(
						`[RuntimeBudgetScheduler] Lane "${next.lane}" task failed:`,
						error,
					);
					next.reject(error);
				} finally {
					this.runningCounts.set(
						next.lane,
						Math.max(0, (this.runningCounts.get(next.lane) ?? 1) - 1),
					);
					this.updatePressure(next.lane);
					this.pump();
				}
	...
	private updatePressure(lane: RuntimeLane): void {
		...
		if (nextPressure === "critical" && lane !== "background") {
			...
			this.shedBackgroundQueue(
				`critical pressure on ${criticalForegroundLane}`,
			);
		}
	...
	private computePressure(lane: RuntimeLane): BudgetPressureLevel | null {
		...
		const heapUsedMb = this.memoryUsageReader() / (1024 * 1024);
		const utilization =
			budget.memoryCeilingMb > 0 ? heapUsedMb / budget.memoryCeilingMb : 0;

		if (
			utilization >= 0.8 ||
			(queued > 0 && running >= budget.maxConcurrent && lane !== "background")
		) {
			return "critical";
		}
