...
export class RuntimeCoordinator {
	...
	private readonly warmStandbyManager?: WarmStandbyManager<
		unknown,
		unknown,
		unknown
	>;
	...
	constructor(
		...
		this.managedSupervisorNames = options.managedSupervisorNames ?? [
			"recovery",
			"audio",
			"stt",
			"inference",
	...
	getSupervisor<TSupervisor extends ISupervisor = ISupervisor>(
		name: SupervisorName,
	): TSupervisor {
	...
	async stopSupervisors(names: SupervisorName[]): Promise<void> {
		...
		for (const name of [...names].reverse()) {
			try {
				...
				this.logger.warn(
					`[RuntimeCoordinator] Failed stopping supervisor "${name}":`,
					error,
				);
				firstError ??= error;
			}
	...
	async activate(metadata?: unknown): Promise<void> {
		if (this.lifecycleState !== "idle") {
			throw new Error(
				`Cannot activate meeting while runtime is ${this.lifecycleState}`,
			);
		}
	...
	async deactivate(): Promise<void> {
		if (this.lifecycleState === "idle") {
			this.logger.warn(
				"[RuntimeCoordinator] Ignoring deactivate while runtime is idle",
			);
			return;
		}
		...
		if (this.lifecycleState === "stopping") {
			this.logger.warn(
				"[RuntimeCoordinator] Ignoring duplicate deactivate while runtime is stopping",
			);
			return;
		}
		...
		try {
			...
			this.logger.warn(
				"[RuntimeCoordinator] Error during supervisor shutdown, continuing to finalize deactivation:",
				error,
			);
			errors.push(error);
		}
		...
		try {
			...
			this.logger.warn(
				"[RuntimeCoordinator] Error while finalizing meeting deactivation:",
				error,
			);
			errors.push(error);
		} finally {
		...
		if (errors.length > 0) {
			throw buildCoordinatorError(
				"RuntimeCoordinator deactivation completed with errors",
				errors,
			);
		}
	}
	...
	private async rollbackCoordinatorActivation(): Promise<void> {
		try {
			...
			this.logger.warn(
				"[RuntimeCoordinator] Failed stopping supervisors during activation rollback:",
				error,
			);
		}
		...
		try {
			...
			this.logger.warn(
				"[RuntimeCoordinator] Failed unbinding warm standby during activation rollback:",
				error,
			);
		}
		...
		try {
			...
			this.logger.warn(
				"[RuntimeCoordinator] Failed finalizing meeting state during activation rollback:",
				error,
			);
		}
	}

	private async rollbackStartedSupervisors(
		names: SupervisorName[],
	): Promise<void> {
		for (const name of [...names].reverse()) {
			try {
				await this.getSupervisor(name).stop();
				...
				this.logger.warn(
					`[RuntimeCoordinator] Failed rolling back supervisor "${name}":`,
					error,
				);
			}
		}
