...
export class RouteDirector {
   ...
   */
  async runTurn<T>(input: RouteDirectorRunTurnInput, execute: () => Promise<T>): Promise<T | null> {
  ...
  async raceParallelCandidates<T>(
    ...
    try {
      ...
      while (pool.length > 0) {
        ...
        const loserPromises = indexed.filter((x) => x.i !== winIndex).map((x) => x.promise);
        await Promise.race([
          Promise.allSettled(loserPromises),
          new Promise<void>((resolve) => setTimeout(resolve, cancelLoserWithinMs)),
        ]);
