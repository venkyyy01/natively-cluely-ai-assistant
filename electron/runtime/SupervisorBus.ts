...
export class SupervisorBus {
  ...
  constructor(logger: BusLogger = console, now: () => number = () => Date.now()) {
    this.logger = logger;
    this.now = now;
  }
  ...
  async emit(event: SupervisorEvent): Promise<void> {
    await this.emitInternal(event, event.type !== 'bus:listener-error' && event.type !== 'bus:listener-circuit-open');
  }
  ...
  private async emitInternal(event: SupervisorEvent, emitFailureMetaEvent: boolean): Promise<void> {
    ...
    if (errors.length > 0 && emitFailureMetaEvent) {
      const sourceEventType = event.type as Exclude<SupervisorEventType, 'bus:listener-error' | 'bus:listener-circuit-open'>;
    ...
    for (const item of tripped) {
      ...
      const sourceEventType = (event.type === 'bus:listener-error' || event.type === 'bus:listener-circuit-open')
        ? 'any' as const
  ...
  private recordFailure(entry: ListenerEntry | AnyListenerEntry, ts: number): boolean {
