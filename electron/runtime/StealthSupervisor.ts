import type {
	NativeStealthArmRequest,
	NativeStealthBridge,
} from "../stealth/NativeStealthBridge";
...
export class StealthSupervisor implements ISupervisor {
	...
	async start(): Promise<void> {
		if (this.state !== "idle") {
			throw new Error(
				`Cannot start stealth supervisor while state is ${this.state}`,
			);
		}
	...
	private canRearmAfterFault(): boolean {
		...
		this.faultTimestamps = this.faultTimestamps.filter(
			(t) => now - t < StealthSupervisor.FAULT_WINDOW_MS,
		);

		if (this.faultTimestamps.length >= StealthSupervisor.MAX_RAPID_FAULTS) {
			this.options.logger?.error(
				"[StealthSupervisor] Too many rapid faults, refusing re-arm",
			);
			this.faultLoopDetected = true;
			void this.bus.emit({
				type: "stealth:fault-loop-detected",
				reason: "max-rapid-faults-exceeded",
			});
			return false;
		}
	...
	private async armStealth(): Promise<void> {
		...
		if (this.stealthState === "FAULT" && !this.canRearmAfterFault()) {
			this.options.logger?.warn(
				"[StealthSupervisor] Re-arm blocked due to fault cooldown or fault loop",
			);
			return;
		}

		await this.transitionTo(
			this.transitionStealthStateLogged(this.stealthState, "arm-requested"),
		);

		try {
			await this.armController.arm();
			await this.transitionTo(
				this.transitionStealthStateLogged(this.stealthState, "arm-succeeded"),
			);
	...
	private async disableStealth(): Promise<void> {
		...
		await this.transitionTo(
			transitionStealthState(this.stealthState, "disabled"),
		);
	}
	...
	private async failClosed(error: unknown): Promise<void> {
		...
		if (canFaultStealth(this.stealthState)) {
			await this.transitionTo(
				transitionStealthState(this.stealthState, "faulted"),
			);
		}
		this.pendingEnabled = false;
		this.lastRuntimeHeartbeatAt = null;
		...
		try {
			...
			this.options.logger?.warn(
				"[StealthSupervisor] Failed to stop heartbeat after fault:",
				heartbeatError,
			);
		}
	...
	private transitionStealthStateLogged(
		state: StealthState,
		...
		if (next === "FAULT" && event !== "faulted") {
			this.options.logger?.warn(
				`[StealthSupervisor] Illegal stealth transition: ${state} -> ${event}`,
			);
			void this.bus.emit({
				type: "stealth:illegal_transition",
				from: state,
				event,
			});
		}
	...
	private async armNativeStealth(): Promise<boolean> {
		...
		if (!armAllowed.allowed) {
			...
			this.options.logger?.warn(
				`[StealthSupervisor] Skipping native stealth helper arm: ${reason}`,
			);
	...
	private async isNativeArmAllowed(): Promise<{
		allowed: boolean;
		reason?: string;
	}> {
	...
	private async heartbeatNativeStealth(): Promise<{
	...
	private async verifyStealthWithNativeHealth(): Promise<boolean> {
		...
		return (
			nativeHealth.status === "healthy" ||
			nativeHealth.status === "not_applicable"
		);
	}
	...
	private verifyRuntimeHeartbeatFresh(): boolean {
		...
		return (
			this.now() - this.lastRuntimeHeartbeatAt <=
			this.runtimeHeartbeatStalenessMs
		);
	}
}
