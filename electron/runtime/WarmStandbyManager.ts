...
interface WarmStandbyManagerOptions<TAudioResource, TSttResource, TWorkerPoolResource> {
  bus?: SupervisorBus;
...
> {
  ...
  constructor(options: WarmStandbyManagerOptions<TAudioResource, TSttResource, TWorkerPoolResource> = {}) {
    ...
    this.bus.subscribe('budget:pressure', async (event) => {
      if (event.lane === 'background' && event.level === 'critical' && this.activeMeetingId === null) {
  ...
  async isWorkerPoolResourceHealthy(): Promise<boolean> {
    return this.checkResourceHealth(this.workerPoolHandler, this.workerPoolResource);
  }
  ...
  private async checkResourceHealth<TResource>(
    ...
    try {
      ...
      this.logger.warn('[WarmStandbyManager] Resource health check failed:', error);
      return false;
    }
  }
  ...
  private hasAnyWarmResources(): boolean {
    return this.audioResource !== null || this.sttResource !== null || this.workerPoolResource !== null;
  }
}
