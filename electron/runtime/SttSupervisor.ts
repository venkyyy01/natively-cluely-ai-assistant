...
export class SttSupervisor implements ISupervisor {
	...
	async handleTranscript(
		...
		const span = startSpan(
			correlationId,
			`stt.transcript.${speaker}.${final ? "final" : "interim"}`,
		);
		...
		if (span) {
			...
			setSpanAttribute(
				...
				span.spanId,
				"stt.confidence",
				traceContext?.confidence ?? 0,
			);
			setSpanAttribute(
				correlationId,
				span.spanId,
				"stt.provider",
				traceContext?.provider ?? "unknown",
			);
			setSpanAttribute(
				correlationId,
				span.spanId,
				"stt.sample_rate",
				traceContext?.sampleRate ?? 0,
			);
		}
		...
		traceLogger.logSttEvent(
			correlationId,
			span?.spanId,
			final ? "transcript.final" : "transcript.received",
			{
				speaker,
	...
	getProviderHealth(speaker: SttSpeaker): ProviderHealthSnapshot {
		return (
			this.delegates.getProviderHealth?.(speaker) ?? {
				state: "healthy",
				retryCount: 0,
				recentErrorCount: 0,
	...
	private async startSpeakerWithWarmStandbyFallback(
		speaker: SttSpeaker,
	): Promise<void> {
		...
		const canUseWarmStandby =
			...
			this.delegates.startSpeakerFromWarmStandby;
		...
		try {
			...
			await this.warmStandby?.invalidateSttResource();
			this.logger.warn(
				`[SttSupervisor] Warm STT activation failed for ${speaker}, falling back to cold start:`,
				error,
			);
			await this.delegates.startSpeaker(speaker);
		}
