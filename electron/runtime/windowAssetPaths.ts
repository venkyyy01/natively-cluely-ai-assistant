...
export function getWindowAssetCandidates(
	context: WindowAssetResolverContext,
): WindowAssetCandidates {
...
export function resolveRendererStartUrl(
	context: WindowAssetResolverContext,
): string {
	...
	const entryPath = resolveExistingPath(
		getWindowAssetCandidates(context).rendererEntry,
		fileExists,
	);
	return pathToFileURL(entryPath).toString();
}

export function resolveRendererPreloadPath(
	context: WindowAssetResolverContext,
): string {
	const fileExists = context.fileExists ?? safeExists;
	return resolveExistingPath(
		getWindowAssetCandidates(context).preload,
		fileExists,
	);
}

export function resolveStealthShellHtmlPath(
	context: WindowAssetResolverContext,
): string {
	const fileExists = context.fileExists ?? safeExists;
	return resolveExistingPath(
		getWindowAssetCandidates(context).shellHtml,
		fileExists,
	);
}

export function resolveStealthShellPreloadPath(
	context: WindowAssetResolverContext,
): string {
	const fileExists = context.fileExists ?? safeExists;
	return resolveExistingPath(
		getWindowAssetCandidates(context).shellPreload,
		fileExists,
	);
}
