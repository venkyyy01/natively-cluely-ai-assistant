...
export type CaptureMatrixActualResult =
	| CaptureMatrixExpectedResult
	| "skipped"
	| "failed";
...
export function analyzeCanaryVisibility(
	...
	const actualResult: CaptureMatrixExpectedResult = canaryVisible
		? "visible"
		: "hidden";
	...
	return {
		...
		reason: passed
			? undefined
			: `expected ${row.expectedResult} but canary was ${actualResult}`,
	};
}
...
export class MockCaptureAdapter implements CaptureMatrixAdapter {
	...
	async collectArtifact(
		...
		const canaryVisible =
			this.canaryVisibilityByRow.get(row.id) ??
			row.expectedResult === "visible";
		return {
			canaryVisible,
...
export function createDefaultMockCaptureMatrixRows(
	input: {
		platform?: NodeJS.Platform | "unknown";
		osVersion?: string;
		appVersion?: string;
...
export async function runCaptureMatrix(
	options: RunCaptureMatrixOptions,
): Promise<CaptureMatrixRunResult> {
