...
export class TCCMonitor extends EventEmitter {
	...
	start(): void {
		...
		this.logger.log(
			"[TCCMonitor] Started monitoring ScreenCapture permissions",
		);
	}
	...
	private async checkTCCDatabase(): Promise<void> {
		try {
			...
			const db = new Database(TCC_DB_PATH, {
				readonly: true,
				fileMustExist: true,
			});
			try {
				...
				for (const row of rows) {
					...
					if (!this.grantedApps.has(bundleId)) {
						this.logger.log(
							`[TCCMonitor] New ScreenCapture grant: ${bundleId}`,
						);
	...
	private async checkEnterpriseTools(): Promise<void> {
			...
			try {
				...
				if (match) {
					...
					if (!this.suspiciousToolsDetected.has(tool.name)) {
						this.logger.log(
							`[TCCMonitor] Enterprise tool detected: ${tool.name} (${tool.category})`,
						);
		...
		for (const toolName of this.suspiciousToolsDetected) {
			if (!newSuspicious.has(toolName)) {
				this.logger.log(
					`[TCCMonitor] Enterprise tool no longer running: ${toolName}`,
				);
