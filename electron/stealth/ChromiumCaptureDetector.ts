...
export class ChromiumCaptureDetector extends EventEmitter {
					...
					for (const line of lines) {
						...
						if (Number.isFinite(pid) && pid > 0) {
							...
							if (!this.detectedBrowsers.has(key)) {
								this.logger.log(
									`[ChromiumCaptureDetector] Detected ${browser.name} (PID: ${pid})`,
								);
	...
	private async checkCaptureActivity(): Promise<void> {
		...
		try {
			...
			if (hasActiveCapture && !this.captureActive) {
				...
				this.logger.log(
					"[ChromiumCaptureDetector] Browser-based screen capture likely active",
				);
			...
			this.logger.warn(
				"[ChromiumCaptureDetector] Capture activity check failed:",
				error,
			);
		}
	}
	...
	private async checkBrowserWindowCapture(): Promise<boolean> {
		try {
			...
			if (stdout && stdout.trim()) {
				...
				const ppidStdout = await this.execPromise("ps", [
					"-o",
					"ppid=",
					"-p",
					screenCaptureAgentPid,
				]);
				if (ppidStdout && ppidStdout.trim()) {
					const ppid = parseInt(ppidStdout.trim(), 10);
					if (Number.isFinite(ppid)) {
						const parentStdout = await this.execPromise("ps", [
							"-o",
							"comm=",
							"-p",
							String(ppid),
						...
						if (parentStdout) {
							...
							for (const browser of BROWSER_PATTERNS) {
								if (browser.pattern.test(parentName)) {
									this.logger.log(
										`[ChromiumCaptureDetector] ScreenCaptureAgent spawned by ${browser.name}`,
									);
									return true;
								}
							}
		...
		try {
			const stdout = await this.execPromise("ioreg", [
				"-r",
				"-c",
				"AppleDisplay",
				"-l",
			...
			if (stdout && stdout.toLowerCase().includes("screen")) {
				const hasBrowserCapture =
					await this.checkCGWindowListForBrowserCapture();
	...
	private async checkCGWindowListForBrowserCapture(): Promise<boolean> {
		try {
			const stdout = await this.execPromise("python3", [
				"-c",
				`
import Quartz
import sys
