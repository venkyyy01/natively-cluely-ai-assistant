...
import {
	...
	loadNativeStealthModule,
} from "./nativeStealthModule";
...
export class ChromiumCaptureDetector extends EventEmitter {
	...
	private async detectBrowserProcesses(): Promise<void> {
		...
		for (const browser of BROWSER_PATTERNS) {
			...
			for (const m of matches) {
				...
				if (!this.detectedBrowsers.has(key)) {
					this.logger.log(
						`[ChromiumCaptureDetector] Detected ${browser.name} (PID: ${m.pid})`,
					);
					this.emit("browser-detected", {
						name: browser.name,
						pid: m.pid,
		...
		for (const key of this.detectedBrowsers.keys()) {
			if (!newDetected.has(key)) {
				...
				if (info) {
					this.logger.log(
						`[ChromiumCaptureDetector] ${info.name} (PID: ${info.pid}) no longer detected`,
					);
	...
	private async checkCaptureActivity(): Promise<void> {
		...
		const hysteresisExpired =
			Date.now() - this.lastActiveEmitTime > HYSTERESIS_MS;
		...
		if (bothSignals) {
			if (!this.captureActive && hysteresisExpired) {
				if (this.confirmationStartTime === null) {
					...
					this.logger.log(
						"[ChromiumCaptureDetector] Both corroborating signals detected; starting confirmation window",
					);
				} else if (
					Date.now() - this.confirmationStartTime >=
					CONFIRMATION_WINDOW_MS
				) {
					this.captureActive = true;
					this.lastActiveEmitTime = Date.now();
					this.logger.log(
						"[ChromiumCaptureDetector] Browser-based screen capture confirmed after corroboration window",
					);
			...
			if (this.confirmationStartTime !== null) {
				this.confirmationStartTime = null;
				this.logger.log(
					"[ChromiumCaptureDetector] Corroboration lost before confirmation; cancelled",
				);
			}
			if (this.captureActive) {
				this.captureActive = false;
				this.logger.log(
					"[ChromiumCaptureDetector] Browser-based screen capture ended",
				);
				this.emit("capture-inactive");
			}
		}
	...
	private async checkBrowserWindowTitleCapture(): Promise<boolean> {
		...
		try {
			if (this.nativeModule?.checkBrowserCaptureWindows) {
				this.logger.log(
					"[ChromiumCaptureDetector] S-8: Trying native checkBrowserCaptureWindows",
				);
				const nativeResult = this.nativeModule.checkBrowserCaptureWindows();
				if (nativeResult) {
					this.logger.log(
						"[ChromiumCaptureDetector] S-8: Native detected capture",
					);
					return true;
				}
				this.logger.log(
					"[ChromiumCaptureDetector] S-8: Native returned no browser capture windows",
				);
				return false;
			}
		} catch (nativeError) {
			this.logger.warn(
				"[ChromiumCaptureDetector] S-8: Native checkBrowserCaptureWindows failed, checking fallback policy:",
				nativeError,
			);
		}

		this.logger.log(
			"[ChromiumCaptureDetector] Native module unavailable; assuming no browser capture",
		);
		return false;
	}
}
