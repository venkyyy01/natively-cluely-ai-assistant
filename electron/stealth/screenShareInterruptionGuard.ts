...
export async function detectExternalScreenShare(
  ...
  const processEnumerator = options.processEnumerator ?? defaultProcessEnumerator;
  ...
  if (captureAgentLines.length === 0) {
    return { active: false, reason: 'no-screen-capture-agent', processLines: [] };
  }
  ...
  return {
    active: true,
    reason: externalAppLines.length > 0
