...
export class BrowserGetDisplayMediaAdapter implements CaptureMatrixAdapter {
  ...
  constructor(options: BrowserGetDisplayMediaAdapterOptions = {}) {
    ...
    this.enabled = options.enabled ?? process.env.NATIVELY_CAPTURE_MATRIX_ENABLE_BROWSER_GDM === '1';
    this.testPagePath = options.testPagePath ?? path.resolve('stealth-projects/integration-harness/capture-matrix/browser/get-display-media.html');
  ...
  async prepare(row: CaptureMatrixRow): Promise<void> {
    assertValidRow(row);
    if (row.mode !== 'browser') {
      throw new Error(`browser getDisplayMedia adapter requires browser mode, got ${row.mode}`);
    }
  }
  ...
  async startCapture(row: CaptureMatrixRow): Promise<ExternalCaptureSession> {
    return {
      ...
      artifactDir: await mkdtemp(path.join(os.tmpdir(), `capture-matrix-browser-${row.id}-`)),
    };
  }
  ...
  async collectArtifact(row: CaptureMatrixRow, session: CaptureAdapterSession): Promise<CaptureMatrixArtifact> {
    ...
    const capture = await this.runner(row, externalSession.artifactDir, this.testPagePath);
    ...
    return {
      ...
      metadata: {
        ...
        externalAppVersion: capture.externalAppVersion ?? baseMetadata.externalAppVersion,
        externalCaptureMode: capture.externalCaptureMode ?? baseMetadata.externalCaptureMode,
      },
    };
  }
...
export class ManualExternalCaptureAdapter implements CaptureMatrixAdapter {
  ...
  private readonly detect: typeof detectCanaryMarkerInImage;
  private readonly versionResolver?: (row: CaptureMatrixRow) => string | undefined;
  ...
  async prepare(row: CaptureMatrixRow): Promise<void> {
    assertValidRow(row);
    if (row.mode !== 'manual') {
      throw new Error(`manual external adapter requires manual mode, got ${row.mode}`);
    }
  }
  ...
  async startCapture(row: CaptureMatrixRow): Promise<ExternalCaptureSession> {
    return {
      ...
      artifactDir: await mkdtemp(path.join(os.tmpdir(), `capture-matrix-manual-${row.id}-`)),
    };
  }
  ...
  private createMetadata(row: CaptureMatrixRow): Record<string, unknown> {
    return {
      ...
      externalAppVersion: this.versionResolver?.(row) ?? row.externalAppVersion ?? 'unknown',
      externalCaptureMode: row.externalCaptureMode ?? 'manual-screen-share',
    };
  }
}

export function createDefaultBrowserGetDisplayMediaRows(input: {
...
export function createDefaultMeetingAppRows(input: {
  platform?: NodeJS.Platform | 'unknown';
  osVersion?: string;
  appVersion?: string;
  strict?: boolean;
  externalApps?: Array<{ name: string; captureTool: string; version?: string; captureMode: string }>;
} = {}): CaptureMatrixRow[] {
  const apps = input.externalApps ?? [
    { name: 'Zoom', captureTool: 'zoom-screen-share', captureMode: 'screen-share' },
    { name: 'Google Meet', captureTool: 'google-meet-screen-share', captureMode: 'browser-screen-share' },
    { name: 'Microsoft Teams', captureTool: 'microsoft-teams-screen-share', captureMode: 'screen-share' },
    { name: 'OBS Studio', captureTool: 'obs-display-capture', captureMode: 'display-capture' },
  ];

  return apps.flatMap((app) => createExternalRows({
    ...input,
...
function asExternalCaptureSession(session: CaptureAdapterSession): ExternalCaptureSession {
