...
import type { ManagedWindowRecord, StealthCapableWindow } from "./stealthTypes";
...
export class OpacityFlickerController {
	...
	constructor(private deps: OpacityFlickerDeps) {}
		...
		this.handle = this.deps.intervalScheduler(() => this.apply(), 500);
			...
			if (typeof win.setOpacity === "function") {
				try {
					...
					this.deps.timeoutScheduler(() => {
						if (
							!this.deps.isWindowDestroyed(win) &&
