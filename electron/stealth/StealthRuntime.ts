...
export class StealthRuntime {
	...
	private boundInputHandler:
		| ((event: IpcMainEvent, payload: StealthInputEvent) => void)
		| null = null;
	...
	constructor(options: StealthRuntimeOptions) {
		...
		this.shellHtmlPath =
			options.shellHtmlPath ??
			resolveStealthShellHtmlPath({
				electronDir: path.resolve(__dirname, ".."),
			});
		if (
			!this.shellHtmlPath.endsWith(".html") ||
			this.shellHtmlPath.includes("..")
		) {
			...
			target: {
				send: (channel, payload) => {
					if (channel === "stealth-shell:frame" && !this.firstFrameReceived) {
						...
						if (
							this.showRequestedBeforeFirstFrame &&
							this.shellWindow &&
							!this.shellWindow.isDestroyed()
						) {
							this.showRequestedBeforeFirstFrame = false;
							this.requestWindowShow(
								this.shellWindow,
								"StealthRuntime.firstFrame",
							);
						...
						Promise.resolve(this.onFirstFrame?.()).catch((error) => {
							this.logger.warn(
								"[StealthRuntime] Failed to propagate first-frame event:",
								error,
							);
						});
					}
	...
	createPrimaryStealthSurface(
		...
		try {
			...
			this.recordProtectionEvent(
				"window-created",
				contentWindow,
				"StealthRuntime.createPrimaryStealthSurface.content",
			);
			this.recordProtectionEvent(
				"window-created",
				this.shellWindow,
				"StealthRuntime.createPrimaryStealthSurface.shell",
			);
		} catch (error) {
			contentWindow?.close();
		...
		this.detachContentBridgeMonitor = attachRendererBridgeMonitor(
			"Stealth content",
			this.contentWindow,
			{
		...
		this.frameBridge.attach(
			this.contentWindow.webContents as unknown as Parameters<
				FrameBridge["attach"]
			>[0],
		);
		...
		for (const win of [this.contentWindow, this.shellWindow]) {
			this.recordProtectionEvent(
				...
				win,
				"StealthRuntime.createPrimaryStealthSurface",
			);
			if (win && typeof (win as any).setContentProtection === "function") {
				try {
					...
					this.logger.warn(
						"[StealthRuntime] setContentProtection failed:",
						err,
					);
				}
			}
			if (win && typeof (win as any).setExcludeFromCapture === "function") {
				try {
					...
					this.logger.warn(
						"[StealthRuntime] setExcludeFromCapture failed:",
						err,
					);
				}
			}
		...
		this.shellWindow.webContents.on("did-fail-load", (_event, code, desc) => {
			this.logger.warn(
				`[StealthRuntime] Shell window did-fail-load: ${code} ${desc}`,
			);
		});
		...
		this.contentWindow.webContents.on(
			"did-fail-load",
			(_event, code, desc, url) => {
				this.logger.warn(
					`[StealthRuntime] Content window did-fail-load: ${code} ${desc} URL: ${url}`,
				);
			},
		);
		...
		this.contentWindow.webContents.on("crashed", (_event, killed) => {
			this.logger.warn(
				`[StealthRuntime] Content window crashed (killed=${killed})`,
			);
		...
		this.contentWindow.webContents.on(
			...
			(_event, details) => {
				this.logger.warn(
					`[StealthRuntime] Content render process gone: ${details.reason} exitCode=${details.exitCode}`,
				);
		...
		this.firstFrameTimeout = setTimeout(() => {
			if (!this.firstFrameReceived) {
				this.logger.warn(
					"[StealthRuntime] No frames received after 10s - content window may have failed to load",
				);
	...
	destroy(): void {
		...
		if (this.boundHeartbeatHandler) {
			this.ipcMain.removeListener(
				"stealth-shell:heartbeat",
				this.boundHeartbeatHandler,
			);
	...
	syncBounds(): void {
		if (
			...
			!this.contentWindow ||
			this.shellWindow.isDestroyed() ||
			this.contentWindow.isDestroyed()
		) {
			return;
		}
	...
	private bindShellEvents(): void {
		...
		this.boundReadyHandler = (event) => {
			if (
				!this.shellWindow ||
				event.sender.id !== this.shellWindow.webContents.id
			) {
				return;
			}
			this.syncBounds();
	...
	private requestInitialFrame(reason: string): void {
		...
		try {
			...
			this.logger.warn(
				`[StealthRuntime] Failed to request content repaint (${reason}):`,
				error,
			);
		}
	}
	...
	private handleContentCrash(reason: string): void {
		...
		try {
			this.requestWindowHide(
				this.shellWindow,
				"StealthRuntime.handleContentCrash",
			);
	...
	private attemptContentRecovery(): void {
		...
		try {
			...
			this.logger.error(
				"[StealthRuntime] Content window recovery failed:",
				error,
			);
	...
	private emitFault(reason: string): void {
		if (!this.onFault) {
			...
			this.logger.error(
				`[StealthRuntime] CRITICAL: Fault with no handler, hiding shell window. Reason: ${reason}`,
			);
			try {
				this.requestWindowHide(
					this.shellWindow,
					"StealthRuntime.emitFaultFallback",
				);
				this.setWindowOpacity(
					this.shellWindow,
					0,
		...
		Promise.resolve(this.onFault(reason)).catch((error) => {
			this.logger.warn(
				`[StealthRuntime] Failed to propagate runtime fault (${reason}):`,
				error,
			);
		});
	}
	...
	private emitHeartbeat(): void {
		...
		Promise.resolve(this.onHeartbeat()).catch((error) => {
			this.logger.warn(
				"[StealthRuntime] Failed to propagate runtime heartbeat:",
				error,
			);
		});
	}
