...
export class StealthRuntime {
	...
	constructor(options: StealthRuntimeOptions) {
		...
		this.shellHtmlPath =
			...
			path.join(app.getAppPath(), "electron", "renderer", "shell.html");
		if (
		...
		this.preloadPath =
			options.preloadPath ?? path.join(__dirname, "../preload.js");
		this.shellPreloadPath =
			options.shellPreloadPath ?? path.join(__dirname, "./shellPreload.js");
	...
	createPrimaryStealthSurface(
		...
		this.frameBridge.attach = (source) => {
			...
			const originalSend = this.frameBridge["target"].send.bind(
				this.frameBridge["target"],
			);
