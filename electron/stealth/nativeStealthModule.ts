...
export function loadNativeStealthModule(options?: {
	...
	if (options?.retryOnFailure && loadAttempts >= MAX_LOAD_ATTEMPTS) {
		console.warn(
			`[NativeStealthModule] Max retry attempts (${MAX_LOAD_ATTEMPTS}) reached, giving up. Privacy protection is operating in Layer 0 mode only (setContentProtection).`,
		);
		return cachedModule;
	}
	...
	cachedModule = null;
	console.warn(
		"[NativeStealthModule] All candidates failed. Privacy protection is DEGRADED - operating in Layer 0 mode only (setContentProtection). Native stealth APIs are unavailable.",
	);
	return cachedModule;
}
