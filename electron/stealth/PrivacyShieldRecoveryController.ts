...
function needsRecovery(snapshot: PrivacyShieldRecoverySnapshot): boolean {
	return (
		snapshot.isUndetectable &&
		snapshot.faultReason !== null &&
		snapshot.stealthState === "FAULT"
	);
}
...
export class PrivacyShieldRecoveryController {
	...
	update(): void {
		...
		const now = Date.now();
		this.hourlyRecoveryTimestamps = this.hourlyRecoveryTimestamps.filter(
			(t) => now - t < 3600000,
		);
		if (
			this.hourlyRecoveryTimestamps.length >=
			PrivacyShieldRecoveryController.MAX_RECOVERIES_PER_HOUR
		) {
			this.logger.warn(
				"[PrivacyShieldRecovery] Hourly recovery cap reached. Manual recovery required via shortcut.",
			);
			return;
		}
	...
	private async runRecoveryBody(
		source: "shortcut" | "timeout",
		preRecoveryWarnings: readonly string[],
	): Promise<void> {
		try {
