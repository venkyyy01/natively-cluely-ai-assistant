...
const CAPTURE_RISK_REASON =
	"Sensitive content hidden while capture risk is detected.";
const PROTECTION_FAULT_REASON =
	"Sensitive content hidden until privacy protection is restored.";
const PROTECTED_INTENT_REASON =
	"Sensitive content hidden while privacy mode is active.";

export function hasCaptureRiskWarnings(
	warnings: readonly string[] = [],
): boolean {
	return warnings.some((warning) => CAPTURE_RISK_WARNINGS.has(warning));
}

export function derivePrivacyShieldState(
	options: DerivePrivacyShieldStateOptions = {},
): PrivacyShieldState {
