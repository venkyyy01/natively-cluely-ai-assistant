...
function resolveProduction(env: NodeJS.ProcessEnv, explicit?: boolean): boolean {
...
export function decideStealthFallback(input: StealthFallbackPolicyInput): StealthFallbackDecision {
    ...
    case 'sck-audio': {
      if (input.activeScreenShare) {
        return {
          ...
          strict,
          production,
          reason: 'ScreenCaptureKit audio fallback is blocked while another screen share is active',
      ...
      return {
        ...
        strict,
        production,
        reason: 'ScreenCaptureKit audio fallback requires NATIVELY_ALLOW_SCK_AUDIO_FALLBACK=1',
