...
function nextStateForEvent(current: ProtectionState, event: ProtectionEvent): ProtectionState {
  switch (event.type) {
    case 'window-created':
...
function violationForEvent(current: ProtectionState, event: ProtectionEvent): ProtectionViolationType | null {
  if (event.type === 'show-requested' && current !== 'verified-hidden' && current !== 'visible-protected') {
    return 'show-before-verified';
  }

  if (event.type === 'shown' && current !== 'verified-hidden' && current !== 'visible-protected') {
...
export class ProtectionStateMachine {
  ...
  record(type: ProtectionEventType, context: ProtectionEventContext = {}): ProtectionSnapshot {
    ...
    if (violationType) {
      ...
      if (this.violations.length > this.maxViolations) {
        this.violations = this.violations.slice(this.violations.length - this.maxViolations);
      }
      this.logger?.warn?.(
