...
interface StartupProtectionGateOptions {
	verifyProtection: (
		context: StartupProtectionGateContext,
...
export class StartupProtectionGate {
	...
	constructor(options: StartupProtectionGateOptions) {
		...
		this.isStrictProtectionEnabled =
			options.isStrictProtectionEnabled ?? (() => false);
		this.recordProtectionEvent = options.recordProtectionEvent;
		this.onBlocked = options.onBlocked;
		this.timeoutMs =
			options.timeoutMs ??
			parseInt(process.env.NATIVELY_STARTUP_GATE_TIMEOUT_MS || "1500", 10);
	...
	async evaluateReveal(
		context: StartupProtectionGateContext,
	): Promise<StartupProtectionGateDecision> {
		...
		try {
			...
			error = caught;
			reason = isTimeoutError(caught)
				? "startup-verification-timeout"
				: "startup-verification-error";
			this.logger.error(
				"[StartupProtectionGate] startup protection verification failed:",
				caught,
			);
		}
