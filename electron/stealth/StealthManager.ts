...
export class StealthManager extends EventEmitter {
	...
	private readonly processEnumerator: (
		command: string,
		args: string[],
	) => Promise<string>;
	...
	applyToWindow(
		...
		record.hideFromSwitcher =
			options.hideFromSwitcher ?? this.defaultHideFromSwitcher(record.role);
	...
	private applyChromiumCountermeasures(): void {
		for (const record of this.managedWindows) {
			...
			// Window remains visible to user but protected from screen capture
			// setContentProtection and setExcludeFromCapture handle the protection
		}
	}
	...
	private getOrCreateRecord(
		...
		const record: ManagedWindowRecord = {
			...
			hideFromSwitcher:
				...
				this.defaultHideFromSwitcher(options.role ?? "primary"),
			allowVirtualDisplayIsolation:
				options.allowVirtualDisplayIsolation ?? false,
	...
	private safeGetMediaSourceId(win: StealthCapableWindow): string | null {
		...
		try {
			...
			this.logger.warn(
				"[StealthManager] Failed to read media source id from managed window:",
				error,
			);
			return null;
		}
	...
	private async detectCaptureProcesses(): Promise<RegExp[]> {
		if (this.platform === "win32") {
			const stdout = await this.processEnumerator("tasklist", [
				"/FO",
				"CSV",
				"/NH",
			]);
		...
		if (this.platform === "darwin") {
			...
			if (processSnapshot !== null) {
				return this.captureToolPatterns.filter((pattern) =>
					pattern.test(processSnapshot),
				);
			}

			const matches: RegExp[] = [];
			const browserPatterns = this.getBrowserCapturePatterns();
			const nonBrowserPatterns = this.captureToolPatterns.filter(
				(p) => !browserPatterns.includes(p),
			);

			for (const pattern of nonBrowserPatterns) {
				const stdout = await this.processEnumerator("pgrep", [
					"-lif",
					pattern.source,
				]);
			...
			const browserStdout = await this.processEnumerator("pgrep", [
				"-lif",
			...
			if (browserStdout) {
				...
				for (const line of browserLines) {
					const appPathMatch = line.match(
						/\/(Chrome|Chromium|Microsoft Edge|Brave Browser)\.app\//i,
					);
					if (appPathMatch) {
	...
	private async checkSCStreamActive(): Promise<boolean> {
		...
		try {
			...
			const stdout2 = await this.processEnumerator("pgrep", [
				"-lf",
				"controlcenter",
			]);
			if (stdout2 && stdout2.trim()) {
				...
				if (
					controlCenterOutput.includes("screen") ||
					controlCenterOutput.includes("capture")
				) {
					return true;
				}
			}
	...
	private async getWindowNumbersVisibleToCapture(): Promise<Set<number>> {
		...
		try {
			const stdout = await this.processEnumerator("python3", [
				"-c",
				`
import Quartz
import sys
...
`,
	...
	private ensureOpacityFlicker(): void {
		...
		this.logger.log(
			"[StealthManager] macOS 15.4+ opacity flicker enabled (500ms interval)",
		);
	}
		...
		for (const record of this.managedWindows) {
			...
			if (typeof win.setOpacity === "function") {
				try {
					...
					this.logger.warn(
						"[StealthManager] Opacity flicker failed, continuing with Layer 0+1:",
						error,
					);
				}
			}
	...
	private hideAndRestoreVisibleWindows(): void {
		...
		const attemptRestore = async (): Promise<void> => {
			...
			if (stillRunning.length > 0) {
				this.logger.log(
					"[StealthManager] Capture tools still running, delaying window restore",
				);
				...
				if (recheck.length > 0) {
					this.logger.warn(
						"[StealthManager] Capture tools persist, keeping windows hidden",
					);
