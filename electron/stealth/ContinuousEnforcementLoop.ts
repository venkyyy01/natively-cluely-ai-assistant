...
import type { DetectedThreat, MonitoringDetector } from "./MonitoringDetector";
...
export class ContinuousEnforcementLoop {
	...
	private async runWindowProtection(): Promise<void> {
		...
		try {
			...
			this.logger.warn(
				"[ContinuousEnforcementLoop] Window protection failed:",
				error,
			);
	...
	private async runProcessDetection(): Promise<void> {
		...
		try {
			...
			this.logger.warn(
				"[ContinuousEnforcementLoop] Process detection failed:",
				error,
			);
		} finally {
	...
	private async runDisguiseValidation(): Promise<void> {
		...
		try {
			...
			this.logger.warn(
				"[ContinuousEnforcementLoop] Disguise validation failed:",
				error,
			);
		} finally {
	...
	private async handleCriticalThreat(threat: DetectedThreat): Promise<void> {
		this.logger.error(
			`[ContinuousEnforcementLoop] CRITICAL threat detected: ${threat.name} (PID: ${threat.pid})`,
		);
	...
	private async handleWarningThreat(threat: DetectedThreat): Promise<void> {
		this.logger.warn(
			`[ContinuousEnforcementLoop] Warning threat detected: ${threat.name} (PID: ${threat.pid})`,
		);
	...
	private async validateDisguise(): Promise<void> {
		...
		if (process.platform === "darwin" && app.dock?.isVisible?.()) {
			this.logger.warn(
				"[ContinuousEnforcementLoop] Dock is visible in stealth mode",
			);
			app.dock.hide();
		}
	...
	private recordViolation(type: string): void {
		...
		this.violations = this.violations.filter(
			(v) => now - v.timestamp < ContinuousEnforcementLoop.VIOLATION_WINDOW_MS,
		);

		// Check if we exceed max violations
		if (this.violations.length >= ContinuousEnforcementLoop.MAX_VIOLATIONS) {
			this.logger.error(
				"[ContinuousEnforcementLoop] Maximum violations exceeded, triggering kill switch",
			);
