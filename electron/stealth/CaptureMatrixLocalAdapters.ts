...
export interface MacosScreenCaptureKitAdapterOptions {
	...
	capture?: (
		...
		artifactDir: string,
	) => Promise<{ capturePath: string; log?: string }>;
	detector?: typeof detectCanaryMarkerInImage;
}
...
export function createCanarySvg(canaryToken: string): string {
	...
	for (let row = 0; row < rows; row++) {
		for (let column = 0; column < columns; column++) {
			...
			squares.push(
				`<rect x="${24 + column * square}" y="${20 + row * square}" width="${square}" height="${square}" fill="${fill}"/>`,
			);
		}
	}
...
export async function renderCanaryPngBuffer(
	canaryToken: string,
): Promise<Buffer> {
	return sharp(Buffer.from(createCanarySvg(canaryToken), "utf8"))
		.png()
		.toBuffer();
}

export async function writeCanaryFixtureFiles(
	row: CaptureMatrixRow,
	outputDir: string,
): Promise<CanaryFixturePaths> {
	const pngPath = path.join(
		outputDir,
		`${sanitizePathSegment(row.id)}-canary.png`,
	);
	const htmlPath = path.join(
		outputDir,
		`${sanitizePathSegment(row.id)}-canary.html`,
	);
...
export async function detectCanaryMarkerInImage(
	...
	const minPixelsPerColor =
		options.minPixelsPerColor ?? DEFAULT_MIN_PIXELS_PER_COLOR;
	...
	const markerRatio =
		totalPixels > 0 ? (primaryPixels + secondaryPixels) / totalPixels : 0;
	const visible =
		primaryPixels >= minPixelsPerColor &&
...
export class MacosScreencaptureAdapter implements CaptureMatrixAdapter {
	...
	constructor(options: MacosScreencaptureAdapterOptions = {}) {
		...
		this.canaryArmed =
			options.canaryArmed ??
			process.env.NATIVELY_CAPTURE_MATRIX_CANARY_ARMED === "1";
	}
	...
	async prepare(row: CaptureMatrixRow): Promise<void> {
		...
		if (this.platform !== "darwin") {
			throw new CaptureMatrixSkipError(
				"macOS screencapture adapter only runs on darwin",
			);
		}
		if (row.mode !== "screenshot") {
			throw new CaptureMatrixSkipError(
				`macOS screencapture adapter requires screenshot mode, got ${row.mode}`,
			);
		}
	}
	...
	async startCapture(row: CaptureMatrixRow): Promise<LocalCaptureSession> {
		const artifactDir = await mkdtemp(
			path.join(os.tmpdir(), `capture-matrix-${row.id}-`),
		);
	...
	async collectArtifact(
		...
		const capturePath = path.join(
			localSession.artifactDir,
			`${sanitizePathSegment(row.id)}-screencapture.png`,
		);
		if (!this.canaryArmed) {
			throw new CaptureMatrixSkipError(
				[
		...
		try {
			await this.runExecFile(this.commandPath, ["-x", capturePath], {
				timeoutMs: 15000,
			});
		} catch (error) {
			throw new CaptureMatrixSkipError(
				`screencapture unavailable or not permitted: ${getErrorMessage(error)}`,
			);
		}
		...
		try {
			...
			throw new CaptureMatrixSkipError(
				`captured image could not be analyzed: ${getErrorMessage(error)}`,
			);
		}
...
export class MacosCgWindowEnumerationAdapter implements CaptureMatrixAdapter {
	...
	constructor(options: MacosCgWindowEnumerationAdapterOptions = {}) {
		this.platform = options.platform ?? process.platform;
		this.nativeModule =
			options.nativeModule ?? loadNativeStealthModule({ retryOnFailure: true });
		this.canaryArmed =
			options.canaryArmed ??
			process.env.NATIVELY_CAPTURE_MATRIX_CANARY_ARMED === "1";
	}
	...
	async prepare(row: CaptureMatrixRow): Promise<void> {
		...
		if (this.platform !== "darwin") {
			throw new CaptureMatrixSkipError(
				"CGWindow enumeration adapter only runs on darwin",
			);
		}
		if (row.mode !== "window-enumeration") {
			throw new CaptureMatrixSkipError(
				`CGWindow enumeration adapter requires window-enumeration mode, got ${row.mode}`,
			);
		}
		if (!this.nativeModule?.listVisibleWindows) {
			throw new CaptureMatrixSkipError(
				"native listVisibleWindows binding is unavailable",
			);
		}
		if (!this.canaryArmed) {
			throw new CaptureMatrixSkipError(
				"canary surface is not armed; expose the canary token in the protected/control window title and set NATIVELY_CAPTURE_MATRIX_CANARY_ARMED=1",
			);
		}
	}
	...
	async collectArtifact(
		...
		try {
			...
			throw new CaptureMatrixSkipError(
				`native CGWindow enumeration failed: ${getErrorMessage(error)}`,
			);
		}

		const matches = windows.filter((window) =>
			windowContainsCanary(window, row.canaryToken),
		);
		return {
...
export class MacosScreenCaptureKitAdapter implements CaptureMatrixAdapter {
	...
	constructor(options: MacosScreenCaptureKitAdapterOptions = {}) {
		this.platform = options.platform ?? process.platform;
		this.enabled =
			options.enabled ?? process.env.NATIVELY_CAPTURE_MATRIX_ENABLE_SCK === "1";
	...
	async prepare(row: CaptureMatrixRow): Promise<void> {
		...
		if (this.platform !== "darwin") {
			throw new CaptureMatrixSkipError(
				"ScreenCaptureKit adapter only runs on darwin",
			);
		}
		if (row.mode !== "video") {
			throw new CaptureMatrixSkipError(
				`ScreenCaptureKit adapter requires video mode, got ${row.mode}`,
			);
		}
		if (!this.enabled) {
			throw new CaptureMatrixSkipError(
				"ScreenCaptureKit adapter is explicit opt-in; set NATIVELY_CAPTURE_MATRIX_ENABLE_SCK=1 for release qualification",
			);
		}
		if (!this.capture) {
			throw new CaptureMatrixSkipError(
				"ScreenCaptureKit capture runner is not configured",
			);
		}
	}
	...
	async startCapture(row: CaptureMatrixRow): Promise<LocalCaptureSession> {
		return {
			...
			artifactDir: await mkdtemp(
				path.join(os.tmpdir(), `capture-matrix-sck-${row.id}-`),
			),
		};
	}
...
export class WindowsCaptureAdapterStub implements CaptureMatrixAdapter {
	...
	async prepare(row: CaptureMatrixRow): Promise<void> {
		...
		if (this.platform !== "win32") {
			throw new CaptureMatrixSkipError(
				"Windows capture adapter only runs on win32",
			);
		}
		throw new CaptureMatrixSkipError(
			"Windows Graphics Capture automation is not implemented in this harness yet",
		);
	}
...
export function createDefaultMacosScreencaptureRows(
	input: LocalCaptureAdapterOptions = {},
): CaptureMatrixRow[] {
...
export function createDefaultMacosCgWindowRows(
	input: LocalCaptureAdapterOptions = {},
): CaptureMatrixRow[] {
	return createProtectedAndControlRows({
		...input,
...
export function createDefaultMacosScreenCaptureKitRows(
	input: LocalCaptureAdapterOptions = {},
): CaptureMatrixRow[] {
	return createProtectedAndControlRows({
		...input,
...
export function createDefaultWindowsCaptureRows(
	input: LocalCaptureAdapterOptions = {},
): CaptureMatrixRow[] {
	return createProtectedAndControlRows({
		...input,
...
function createProtectedAndControlRows(
	input: LocalCaptureAdapterOptions & {
		captureTool: string;
...
function asLocalCaptureSession(
	session: CaptureAdapterSession,
): LocalCaptureSession {
...
function windowContainsCanary(
	window: WindowInfo,
	canaryToken: string,
): boolean {
	return (
		window.windowTitle.includes(canaryToken) ||
		window.ownerName.includes(canaryToken)
	);
}

function matchesColor(
	actual: CanaryColor,
	expected: CanaryColor,
	tolerance: number,
): boolean {
	return (
