...
export type TriggerDecisionOutcome = 'accepted' | 'declined' | 'stale' | 'completed';
...
function defaultAuditPath(): string {
  ...
  return path.join(os.tmpdir(), 'natively-trigger-audit', `trigger-audit-${date}.jsonl`);
}
...
export class TriggerAuditLog {
  ...
  constructor(options: TriggerAuditLogOptions = {}) {
    ...
    this.persistLine = options.persistLine ?? (async (line: string) => {
  ...
  record(entry: TriggerDecisionAuditEntry): void {
    ...
    if (this.persistEnabled) {
      ...
      void Promise.resolve(this.persistLine(line)).catch((error) => {
        console.warn('[TriggerAuditLog] Failed to persist trigger audit entry:', error);
      });
    }
  }
