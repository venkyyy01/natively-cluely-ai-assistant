...
function writeCrashCount(count: number): void {
  try {
    fs.writeFileSync(getCrashCountPath(), JSON.stringify({ count, last: Date.now() }));
  } catch {
    // Best effort
  }
}
...
export function writeOnnxFailureCount(count: number): void {
  try {
    fs.writeFileSync(getOnnxFailurePath(), JSON.stringify({ count, last: Date.now() }));
  } catch {
    // Best effort
  }
}
...
function killZombieProcesses(): boolean {
  ...
  let killed = false;
  try {
    const stdout = execSync('pgrep -f "Natively.app/Contents/MacOS/Natively" || true', {
      encoding: 'utf-8',
      timeout: 3000,
    });
    const pids = stdout
    ...
    for (const pid of pids) {
      try {
        ...
        console.log(`[StartupHealer] Sent SIGTERM to zombie Natively pid=${pid}`);
        killed = true;
      } catch {
...
export function recordStartupFailure(): void {
  ...
  console.warn(`[StartupHealer] Recorded startup failure. Crash count = ${count}`);
}
