...
export class PredictivePrefetcher {
	...
	updateTranscriptSegments(
		...
		const signature = trimmed
			.map(
				(segment) => `${segment.speaker}:${segment.timestamp}:${segment.text}`,
			)
			.join("\u0000");
	...
	async getSemanticEmbedding(text: string): Promise<number[]> {
		...
		if (provider?.isInitialized()) {
			try {
				...
				console.warn(
					"[PredictivePrefetcher] Real embedding provider failed, disabling semantic lookup for this query:",
					error,
				);
			}
		}
	...
	private async startPrefetching(): Promise<void> {
		...
		for (const prediction of predictions.slice(
			0,
			flags.maxPrefetchPredictions,
		)) {
			if (this.isUserSpeaking) break;
			if (
				this.budgetScheduler &&
				!this.budgetScheduler.shouldAdmitSpeculation(
	...
	private predictFollowUpDrafts(): PredictedFollowUpDraft[] {
		...
		const topicPredictions = Object.entries(TOPIC_FOLLOWUPS)
			...
			.flatMap(([, followUps]) =>
				followUps.map((followUp) => `How does ${followUp} relate here?`),
			);
	...
	private async assembleContext(query: string): Promise<{
		...
		try {
			...
			const relevantContext = bm25Results.slice(0, 5).map((r) => ({
				role:
					docs.find(
						(doc) =>
							doc.text.trim().toLowerCase() === r.text.trim().toLowerCase(),
			...
			console.warn(
				"[PredictivePrefetcher] BM25 search failed, returning empty context:",
				error,
			);
			return {
				relevantContext: [],
