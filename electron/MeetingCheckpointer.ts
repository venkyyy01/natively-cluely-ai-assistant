...
export class MeetingCheckpointer {
	...
	private async checkpoint(): Promise<void> {
		...
		console.log(
			`[MeetingCheckpointer] Writing checkpoint for meeting ${this.meetingId}...`,
		);

		try {
			this.dbManager.createOrUpdateMeetingProcessingRecord(
				meetingData,
				snapshot.startTime,
				snapshot.durationMs,
			);
			...
			console.error(
				"[MeetingCheckpointer] Failed to save checkpoint to database",
				e,
			);
		}
	}
