...
export class StreamTokenBatcher {
  ...
  constructor(opts: StreamTokenBatcherOptions) {
    this.intervalMs = Math.max(1, opts.intervalMs ?? STREAM_TOKEN_BATCHER_DEFAULTS.INTERVAL_MS);
    this.maxTokens = Math.max(1, opts.maxTokens ?? STREAM_TOKEN_BATCHER_DEFAULTS.MAX_TOKENS);
    this.send = opts.send;
