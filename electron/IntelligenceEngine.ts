...
export class IntelligenceEngine extends EventEmitter {
	...
	attachAccelerationManager(
		...
		accelerationManager
			...
			.setSpeculativeExecutor((query, transcriptRevision) =>
	...
	async runWhatShouldISay(
		...
		while (true) {
			...
			const cooldownRemaining = Math.max(
				0,
				this.triggerCooldown - (now - lastTriggerForKey),
			);
		...
		try {
			...
			const capability =
				typeof (this.llmHelper as any).getProviderCapabilityClass === "function"
			...
			const routePreparation =
				this.consciousPreparationCoordinator.prepareRoute({
					baseQuestion,
					knowledgeStatus,
					screenshotBackedLiveCodingTurn: this.isScreenshotBackedLiveCodingTurn(
						resolvedQuestion,
						imagePaths,
					),
				});
					...
					if (
						...
						this.latencyTracker.markDegradedToRoute(requestId, effectiveRoute, {
							profileEnrichmentState:
								fallbackReason === "profile_timeout" ? "timed_out" : "failed",
