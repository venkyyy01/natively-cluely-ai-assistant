...
export class AudioCaptureReconnector extends EventEmitter {
  ...
  private lastSuccessfulConfig: { deviceId?: string; sampleRate?: number; channels?: number } = {};
  ...
  public async reconnect(
    ...
    console.log(`[AudioReconnector] Starting reconnection for ${captureInstance.type} audio`);
    ...
    try {
      ...
      const strategies = this.getReconnectionStrategies(captureInstance, currentDeviceId);
      ...
      for (const strategy of strategies) {
        ...
        const delay = Math.min(
          this.config.baseDelayMs * Math.pow(2, this.attempts.length - 1),
          this.config.maxDelayMs
        );
        
        console.log(`[AudioReconnector] Strategy ${strategy} failed, waiting ${delay}ms before next attempt`);
      ...
      if (success) {
        ...
        console.log(`[AudioReconnector] Successfully reconnected after ${this.attempts.length} attempts in ${duration}ms`);
        this.resetAttempts();
      } else {
        this.emit('reconnection-failed', result);
        console.error(`[AudioReconnector] Failed to reconnect after ${this.attempts.length} attempts`);
      }
    }
    ...
    try {
      ...
      switch (strategy) {
        ...
        case ReconnectionStrategy.SWITCH_TO_DEFAULT:
          const defaultDevice = this.availableDevices.find(d => d.isDefault && d.kind === 'audioinput');
          if (defaultDevice) {
        ...
        case ReconnectionStrategy.SCAN_ALTERNATIVE_DEVICES:
          ...
          for (const device of alternativeDevices) {
            try {
              ...
              console.warn(`[AudioReconnector] Device ${device.label} failed:`, deviceError);
              continue;
            }
          }
  ...
  private async updateAvailableDevices(): Promise<void> {
    try {
      ...
      const devices: AudioDevice[] = [
        { id: 'default', label: 'Default - System Audio', kind: 'audioinput', isDefault: true },
      ...
      if (devices.length !== previousCount) {
        console.log(`[AudioReconnector] Device list updated: ${devices.length} devices available`);
