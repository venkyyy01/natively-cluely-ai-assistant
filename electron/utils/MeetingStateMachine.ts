...
export class MeetingStateMachine extends EventEmitter {
	...
	private readonly validTransitions: Map<MeetingState, Set<MeetingState>> =
		new Map();

	private initValidTransitions(): void {
		this.validTransitions.set(
			MeetingState.IDLE,
			new Set([MeetingState.INITIALIZING, MeetingState.ERROR]),
		);
		this.validTransitions.set(
			MeetingState.INITIALIZING,
			new Set([MeetingState.ACTIVE, MeetingState.ERROR, MeetingState.STOPPING]),
		);
		this.validTransitions.set(
			MeetingState.ACTIVE,
			new Set([
		...
		this.validTransitions.set(
			MeetingState.PAUSING,
			new Set([MeetingState.PAUSED, MeetingState.ERROR, MeetingState.STOPPING]),
		);
		this.validTransitions.set(
			MeetingState.PAUSED,
			new Set([
		...
		this.validTransitions.set(
			MeetingState.STOPPING,
			new Set([MeetingState.IDLE, MeetingState.ERROR]),
		);
		this.validTransitions.set(
			MeetingState.ERROR,
			new Set([MeetingState.STOPPING, MeetingState.IDLE]),
		);
	}
	...
	public isMeetingActive(): boolean {
		return (
			this.currentState === MeetingState.ACTIVE ||
	...
	public async startMeeting(reason?: string): Promise<boolean> {
		if (this.currentState !== MeetingState.IDLE) {
			const error = new Error(
				`Cannot start meeting from state: ${this.currentState}`,
			);
			console.error("[MeetingStateMachine]", error.message);
		...
		const initSuccess = await this.transitionTo(
			MeetingState.INITIALIZING,
			reason || "Meeting start requested",
		);
	...
	public async completeInitialization(reason?: string): Promise<boolean> {
		return await this.transitionTo(
			MeetingState.ACTIVE,
			reason || "Meeting initialization complete",
		);
	}
	...
	public async stopMeeting(reason?: string): Promise<boolean> {
		...
		const stopSuccess = await this.transitionTo(
			MeetingState.STOPPING,
			reason || "Meeting stop requested",
		);
	...
	public async pauseMeeting(reason?: string): Promise<boolean> {
		if (this.currentState !== MeetingState.ACTIVE) {
			const error = new Error(
				`Cannot pause meeting from state: ${this.currentState}`,
			);
			console.error("[MeetingStateMachine]", error.message);
			this.emit("state-error", error, this.currentState);
			return false;
		...
		const pauseSuccess = await this.transitionTo(
			MeetingState.PAUSING,
			reason || "Meeting pause requested",
		);
	...
	public async resumeMeeting(reason?: string): Promise<boolean> {
		if (this.currentState !== MeetingState.PAUSED) {
			const error = new Error(
				`Cannot resume meeting from state: ${this.currentState}`,
			);
			console.error("[MeetingStateMachine]", error.message);
			this.emit("state-error", error, this.currentState);
			return false;
		...
		const resumeSuccess = await this.transitionTo(
			MeetingState.RESUMING,
			reason || "Meeting resume requested",
		);
	...
	public async completeResume(reason?: string): Promise<boolean> {
		return await this.transitionTo(
			MeetingState.ACTIVE,
			reason || "Meeting resume complete",
		);
	}
	...
	public async recoverFromError(reason?: string): Promise<boolean> {
		if (this.currentState !== MeetingState.ERROR) {
			console.warn(
				`[MeetingStateMachine] Not in error state, cannot recover (current: ${this.currentState})`,
			);
			return false;
		}
	...
	public getDebugInfo(): object {
		return {
			...
			validTransitions: Array.from(
				this.validTransitions.get(this.currentState) || [],
			),
		};
	}
}
