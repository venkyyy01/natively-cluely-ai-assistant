...
} from "./EmbeddingProviderResolver";
...
export class EmbeddingPipeline {
	...
	private async _doInitialize(config: AppAPIConfig): Promise<void> {
		...
		this.fallbackProvider = new LocalEmbeddingProvider();
		console.log(
			`[EmbeddingPipeline] Local fallback provider prepared (${this.fallbackProvider.dimensions}d, lazy init)`,
		);
		...
		try {
			...
			if (lastProvider && lastProvider !== this.provider.name) {
				const count = this.vectorStore.getIncompatibleMeetingsCount(
					this.provider.name,
				);
				if (count > 0) {
					console.log(
						`[EmbeddingPipeline] Found ${count} incompatible meetings from ${lastProvider}.`,
					);
			...
			this.db
				.prepare(
					"INSERT OR REPLACE INTO app_state (key, value) VALUES ('last_embedding_provider', ?)",
				)
				.run(this.provider.name);
		} catch (err) {
			console.error(
				"[EmbeddingPipeline] Failed to initialize primary provider:",
				err,
			);
			...
			console.warn(
				"[EmbeddingPipeline] Falling back to local-only mode for all meetings.",
			);
			...
			try {
				...
				/* non-fatal — DB may not have app_state yet in edge cases */
			}
		}
	}
				...
				new Promise<void>((_, reject) =>
					setTimeout(
						() =>
							reject(
								new Error(
									`Embedding pipeline initialization timed out after ${timeoutMs}ms`,
								),
							),
						timeoutMs,
					),
	...
	async queueMeeting(meetingId: string): Promise<void> {
		...
		if (chunks.length === 0) {
			console.log(
				`[EmbeddingPipeline] No chunks to embed for meeting ${meetingId}`,
			);
			return;
		}
		...
		console.log(
			`[EmbeddingPipeline] Queued ${chunks.length} chunks + 1 summary for meeting ${meetingId}`,
		);
	...
	async processQueue(): Promise<void> {
		...
		if (stuckCount > 0) {
			console.warn(
				`[EmbeddingPipeline] Recovered ${stuckCount} stuck 'processing' items from prior crash.`,
			);
		}
		...
		try {
			while (true) {
				...
				if (!activeProvider) {
					...
					console.warn(
						"[EmbeddingPipeline] No provider available (not even local fallback). Stopping queue processing.",
					);
					break;
				}
				...
				try {
					...
					const newRetryCount =
						(pending.retry_count === -1 ? 0 : pending.retry_count) + 1;
					console.error(
					...
					if (
						...
						if (!useFallback) {
							const delay = RETRY_DELAY_BASE_MS * 2 ** pending.retry_count;
	...
	private async embedChunk(
		...
		// Get chunk text
		const row = this.db
			.prepare("SELECT cleaned_text, meeting_id FROM chunks WHERE id = ?")
			.get(chunkId) as any;
		if (!row) {
	 ...
	 */
	private async embedMeetingSummary(
		meetingId: string,
		provider?: IEmbeddingProvider,
	): Promise<void> {
		const p = provider ?? this.provider;
		...
		// Get summary text
		const row = this.db
			.prepare("SELECT summary_text FROM chunk_summaries WHERE meeting_id = ?")
			.get(meetingId) as any;

		if (!row) {
			console.log(
				`[EmbeddingPipeline] No summary for meeting ${meetingId}, skipping`,
			);
			return;
		}
		...
		console.log(
			`[EmbeddingPipeline] Embedded summary for meeting ${meetingId} via ${p.name}`,
		);
	}
	...
	getQueueStatus(): {
		...
		const effectivelyStalled = this.db
			.prepare(`
		...
		result.pending = Math.max(
			0,
			result.pending - (effectivelyStalled.count || 0),
		);
