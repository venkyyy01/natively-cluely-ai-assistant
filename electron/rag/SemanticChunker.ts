...
import { type CleanedSegment, estimateTokens } from "./TranscriptPreprocessor";
...
function startsTopicShift(text: string): boolean {
	return /^(anyway|now|next|switching gears|moving on|on another note|separately|however|but|so)\b/i.test(
		text.trim(),
	);
}
 ...
 */
function calculateOverlap(segments: CleanedSegment[]): {
	overlapSegments: CleanedSegment[];
...
function shouldCarryCrossSpeakerOverlap(
	previousChunk: CleanedSegment[],
	nextSegment: CleanedSegment,
): boolean {
	...
	for (const seg of segments) {
		...
		if (shouldSplit && currentChunk.length > 0) {
			...
			if (
				...
				const { overlapSegments, overlapTokens } =
					calculateOverlap(currentChunk);
