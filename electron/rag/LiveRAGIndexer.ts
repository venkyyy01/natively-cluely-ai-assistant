...
export class LiveRAGIndexer {
	...
	start(meetingId: string): void {
		if (this.isActive) {
			console.warn(
				`[LiveRAGIndexer] Restarting while active; dropping previous live session ${this.meetingId}`,
			);
			if (this.timer) {
	...
	feedSegments(segments: RawSegment[]): void {
		...
		const previousLastSegment =
			this.allSegments[this.allSegments.length - 1] ?? null;
		...
		if (this.allSegments.length > LIVE_SEGMENT_BACKLOG_LIMIT) {
			...
			this.indexedSegmentCount = Math.max(
				0,
				this.indexedSegmentCount - overflow,
			);
		}
	...
	private async tick(force: boolean = false): Promise<void> {
		...
		try {
			...
			try {
				...
				console.warn(
					"[LiveRAGIndexer] Worker chunk save failed; using main thread:",
					err,
				);
			...
			if (this.embeddingPipeline.isReady()) {
				...
				for (let i = 0; i < chunkIds.length; i += MAX_EMBED_BATCH) {
					...
					const batchTexts = indexedChunks
						.slice(i, i + MAX_EMBED_BATCH)
						.map((c) => c.text);
					try {
						const accelerationManager = getActiveAccelerationManager();
						const runBatch = () =>
							this.embeddingPipeline.embedDocumentsBatch(batchTexts);
						...
						console.warn(
							`[LiveRAGIndexer] Failed to embed batch at offset ${i}:`,
							err,
						);
					}
				}
	...
	async stop(): Promise<void> {
		...
		if (this.runId !== runId) {
			console.warn(
				"[LiveRAGIndexer] Stop detected a newer run; skipping reset of current session",
			);
			return;
		}
	...
	private async saveChunksViaWorker(chunks: Chunk[]): Promise<number[]> {
		...
		return new Promise((resolve, reject) => {
			...
			const onMessage = (
				msg: { ok: true; chunkIds: number[] } | { ok: false; error: string },
			) => {
				cleanup();
				if (msg.ok && "chunkIds" in msg) {
					...
					reject(
						new Error("error" in msg ? msg.error : "Worker chunk save failed"),
					);
				}
			};
	...
	private shouldFlushSoon(
		segments: RawSegment[],
		previousLastSegment: RawSegment | null,
	): boolean {
		...
		const hasInterviewerFinalTurn = segments.some((segment) => {
			...
			return (
				/interviewer|speaker|system/.test(speaker) &&
		...
		return Boolean(
			previousLastSegment &&
				firstNewSegment.timestamp - previousLastSegment.timestamp >=
					LIVE_PAUSE_GAP_MS,
		);
	}
}
