...
export class RAGManager {
	...
	constructor(config: RAGManagerConfig) {
		...
		this.liveIndexer = new LiveRAGIndexer(
			this.vectorStore,
	...
	async processMeeting(
		...
		console.log(
			`[RAGManager] Processing meeting ${meetingId} with ${transcript.length} segments`,
		);

		// 1. Preprocess transcript
		const cleaned = preprocessTranscript(transcript);
		console.log(
			`[RAGManager] Preprocessed to ${cleaned.length} cleaned segments`,
		);
		...
		if (this.embeddingPipeline.isReady()) {
			...
			console.log(
				`[RAGManager] Embeddings not ready, chunks saved without embeddings`,
			);
		}
	...
	async *queryMeeting(
		...
		if (!hasEmbeddings) {
			...
			if (isLiveMeeting && this.liveIndexer.hasIndexedChunks()) {
				console.log(
					`[RAGManager] Using JIT RAG for live meeting ${meetingId} (${this.liveIndexer.getIndexedChunkCount()} chunks)`,
				);
	...
	startLiveIndexing(meetingId: string): void {
		if (!this.embeddingPipeline.isReady()) {
			console.log(
				"[RAGManager] Embedding pipeline not ready, skipping live indexing",
			);
			return;
		}
		...
		try {
			...
			console.warn(
				"[RAGManager] Failed to create transient meeting row for live indexing",
				e,
			);
		}
	...
	deleteMeetingData(meetingId: string): void {
		this.db.transaction(() => {
			...
			try {
				...
				if (info.changes > 0) {
					console.log(
						`[RAGManager] Cleared ${info.changes} items from embedding_queue for meeting ${meetingId}`,
					);
				}
			} catch (e) {
				console.warn(
					`[RAGManager] Failed to clear embedding_queue for meeting ${meetingId}`,
					e,
				);
			}
	...
	async reprocessMeeting(meetingId: string): Promise<void> {
		...
		if (!meeting) {
			console.error(
				`[RAGManager] Meeting ${meetingId} not found for reprocessing`,
			);
			return;
		}
		...
		if (!meeting.transcript || meeting.transcript.length === 0) {
			console.log(
				`[RAGManager] Meeting ${meetingId} has no transcript, skipping`,
			);
			return;
		}
	...
	async ensureDemoMeetingProcessed(): Promise<void> {
		...
		console.log(
			"[RAGManager] Demo meeting found but not processed. Processing now...",
		);
	...
	public cleanupStaleQueueItems(): void {
		try {
			...
			if (info.changes > 0) {
				console.log(
					`[RAGManager] Cleaned up ${info.changes} stale queue items`,
				);
			}
		} catch (error) {
	...
	async reindexIncompatibleMeetings(): Promise<void> {
		...
		if (!providerName) {
			console.error(
				"[RAGManager] Cannot re-index: No active embedding provider available.",
			);
			return;
		}
		...
		console.log(
			`[RAGManager] Re-indexing ${count} incompatible meetings for ${providerName} pipeline...`,
		);
		const affectedMeetingIds =
			this.vectorStore.deleteEmbeddingsForMeetings(providerName);
		...
		console.log(
			`[RAGManager] Successfully requeued ${affectedMeetingIds.length} meetings for re-embedding.`,
		);
	}
}
