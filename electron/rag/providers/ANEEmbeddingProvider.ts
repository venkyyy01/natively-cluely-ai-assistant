...
import {
	type SafeOnnxSession,
	safeCreateOnnxSession,
} from "../../startup/onnxCrashGuard";
...
async function loadOnnxRuntime() {
	...
	try {
		...
		console.warn(
			"[ANEEmbeddingProvider] ONNX Runtime not available:",
			loadError.message,
		);
		return null;
	}
...
export class ANEEmbeddingProvider implements IEmbeddingProvider {
	...
	async initialize(): Promise<void> {
		...
		if (process.env.NATIVELY_DISABLE_ANE_EMBEDDINGS === "1") {
			console.warn(
				"[ANEEmbeddingProvider] Disabled by crash guard due to repeated ONNX failures in prior sessions",
			);
			return;
		}
		...
		if (!runtime) {
			console.warn(
				"[ANEEmbeddingProvider] ONNX Runtime failed to load, embeddings will fall back to existing provider",
			);
			return;
		}
		...
		try {
			...
			const executionProviders = isAppleSilicon() ? ["coreml", "cpu"] : ["cpu"];
			...
			if (!this.session) {
				console.warn(
					"[ANEEmbeddingProvider] safeCreateOnnxSession returned null, falling back to existing provider",
				);
			...
			console.log(
				`[ANEEmbeddingProvider] Initialized with: ${this.useANE ? "CoreML (ANE)" : "CPU"}`,
			);

			await this.warmup();
		} catch (error) {
			console.warn(
				"[ANEEmbeddingProvider] Failed to initialize, falling back to existing provider:",
				error,
			);
			this.initialized = false;
		}
	...
	async warmup(): Promise<void> {
		...
		try {
			...
			this.warmedUp = true;
			console.log(
				`[ANEEmbeddingProvider] Warmup complete in ${Date.now() - start}ms`,
			);
		} catch (error) {
	...
	private normalizeTokenization(tokens: any): {
		ids: number[];
		attentionMask: number[];
	} {
		const rawIds = tokens?.ids ?? tokens?.inputIds ?? tokens?.input_ids ?? [];
		const rawAttentionMask =
			tokens?.attentionMask ?? tokens?.attention_mask ?? [];

		const ids = Array.from(rawIds, Number);
		const attentionMask =
			rawAttentionMask.length > 0
