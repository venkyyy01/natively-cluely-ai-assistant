...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	getRecognitionLanguages: () =>
		invokeAndUnwrap<Record<string, any>>("get-recognition-languages"),
	takeScreenshot: () =>
		invokeAndUnwrap<{ path: string; preview: string }>("take-screenshot"),
	takeSelectiveScreenshot: () =>
		invokeAndUnwrap<{ path?: string; preview?: string; cancelled?: boolean }>(
			"take-selective-screenshot",
		),
	getScreenshots: () =>
		invokeAndUnwrap<Array<{ path: string; preview: string }>>(
			"get-screenshots",
		),
	...
	analyzeImageFile: (path: string) =>
		invokeAndUnwrap<unknown>("analyze-image-file", path),
	...
	getUndetectable: async () =>
		(await invokeAndUnwrap<{ enabled: boolean }>("get-undetectable")).enabled,
	setConsciousMode: (enabled: boolean) =>
		ipcRenderer.invoke("set-conscious-mode", enabled),
	...
	setAccelerationMode: (enabled: boolean) =>
		ipcRenderer.invoke("set-acceleration-mode", enabled),
	...
	getOpenAtLogin: async () =>
		(await invokeAndUnwrap<{ enabled: boolean }>("get-open-at-login")).enabled,
	closeSettingsWindow: () => invokeVoid("close-settings-window"),
	setDisguise: (mode: "terminal" | "settings" | "activity" | "none") =>
		invokeStatus("set-disguise", mode),
	getDisguise: async () =>
		(
			await invokeAndUnwrap<{
				mode: "none" | "terminal" | "settings" | "activity";
			}>("get-disguise")
		).mode,
	onDisguiseChanged: (
		callback: (mode: "terminal" | "settings" | "activity" | "none") => void,
	...
	getAvailableOllamaModels: () =>
		invokeAndUnwrap<string[]>("get-available-ollama-models"),
	...
	selectServiceAccount: async () => {
		try {
			const data = await invokeAndUnwrap<{
				path?: string;
				cancelled?: boolean;
	...
	setCerebrasApiKey: (apiKey: string) =>
		ipcRenderer.invoke("set-cerebras-api-key", apiKey),
	...
	generateSuggestion: (context: string, lastQuestion: string) =>
		invokeAndUnwrap<{ suggestion: string }>(
			"generate-suggestion",
			context,
			lastQuestion,
		),
	...
	getNativeAudioStatus: () =>
		invokeAndUnwrap<{ connected: boolean }>("native-audio-status"),
	...
	getAiResponseLanguages: () =>
		invokeAndUnwrap<Array<{ label: string; code: string }>>(
			"get-ai-response-languages",
		),
	setAiResponseLanguage: (language: string) =>
		invokeStatus("set-ai-response-language", language),
	getSttLanguage: async () =>
		(await invokeAndUnwrap<{ language: string }>("get-stt-language")).language,
	getAiResponseLanguage: async () =>
		(await invokeAndUnwrap<{ language: string }>("get-ai-response-language"))
			.language,
	...
	setOverlayClickthrough: (enabled: boolean) =>
		invokeVoid("set-overlay-clickthrough", enabled),
	...
	onMeetingLifecycleState: (callback: (state: string) => void) => {
		const subscription = (_: any, state: string) =>
			callback(state as "idle" | "starting" | "active" | "stopping");
	...
	getMeetingLifecycleState: () =>
		ipcRenderer.invoke("get-meeting-lifecycle-state"),
	...
	cancelChat: (requestId: string) =>
		ipcRenderer.invoke("gemini-chat-cancel", requestId),
	...
	// Model Management
	getDefaultModel: () =>
		invokeAndUnwrap<{ model: string }>("get-default-model"),
	...
	toggleSettingsWindow: (coords?: { x: number; y: number }) =>
		invokeVoid("toggle-settings-window", coords),

	// Fast Response Mode
	getFastResponseConfig: () =>
		invokeAndUnwrap<FastResponseConfig>("get-fast-response-config"),
	setFastResponseConfig: (config: FastResponseConfig) =>
		ipcRenderer.invoke("set-fast-response-config", config),

	// Demo
	seedDemo: () => ipcRenderer.invoke("seed-demo"),

	// Custom Providers
	saveCustomProvider: (provider: CustomProviderPayload) =>
		ipcRenderer.invoke("save-custom-provider", provider),
	getCustomProviders: () =>
		invokeAndUnwrap<CustomProviderPayload[]>("get-custom-providers"),
	deleteCustomProvider: (id: string) =>
		ipcRenderer.invoke("delete-custom-provider", id),

	// Follow-up Email
	generateFollowupEmail: (input: FollowUpEmailInput) =>
		ipcRenderer.invoke("generate-followup-email", input),
	extractEmailsFromTranscript: (transcript: TranscriptTextEntry[]) =>
	...
	getPrivacyShieldState: () =>
		invokeAndUnwrap<{ active: boolean; reason: string | null }>(
			"get-privacy-shield-state",
		),
	...
	onFastResponseConfigChanged: (
		...
		const subscription = (_: any, config: FastResponseConfig) =>
			callback(config);
	...
	ragQueryMeeting: async (meetingId: string, query: string) => {
		try {
			return await invokeAndUnwrap<{ success?: boolean; fallback?: boolean }>(
				"rag:query-meeting",
				{ meetingId, query },
			);
		} catch (error) {
	...
	ragCancelQuery: (options: {
		...
		global?: boolean;
		live?: boolean;
	}) => invokeStatus("rag:cancel-query", options),
	ragIsMeetingProcessed: (meetingId: string) =>
		invokeAndUnwrap<boolean>("rag:is-meeting-processed", meetingId),
	ragGetQueueStatus: () =>
		invokeAndUnwrap<{
			pending: number;
			processing: number;
	...
	reindexIncompatibleMeetings: () =>
		invokeVoid("rag:reindex-incompatible-meetings"),
	...
	profileUploadResume: (filePath: string) =>
		invokeStatus("profile:upload-resume", filePath),
	profileGetStatus: () =>
		invokeAndUnwrap<{
			hasProfile: boolean;
			profileMode: boolean;
	...
	profileSetMode: (enabled: boolean) =>
		invokeStatus("profile:set-mode", enabled),
	...
	profileUploadJD: (filePath: string) =>
		invokeStatus("profile:upload-jd", filePath),
	profileDeleteJD: () => invokeStatus("profile:delete-jd"),
	profileResearchCompany: async (companyName: string) => {
		try {
			return await invokeAndUnwrap<{ success: boolean; dossier?: any }>(
	...
	// Google Search API
	setGoogleSearchApiKey: (apiKey: string) =>
		invokeStatus("set-google-search-api-key", apiKey),
	setGoogleSearchCseId: (cseId: string) =>
		invokeStatus("set-google-search-cse-id", cseId),
	...
	setOverlayOpacity: (opacity: number) =>
		invokeVoid("set-overlay-opacity", opacity),
