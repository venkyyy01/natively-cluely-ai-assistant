...
export type IpcResult<T> = { success: true; data: T } | { success: false; error: IpcErrorContract }
...
export const invokeAndUnwrap = async <T>(channel: string, ...args: unknown[]): Promise<T> => {
...
export const invokeVoid = async (channel: string, ...args: unknown[]): Promise<void> => {
  await invokeAndUnwrap<unknown>(channel, ...args)
}

export const invokeStatus = async (channel: string, ...args: unknown[]): Promise<StatusResult> => {
  try {
