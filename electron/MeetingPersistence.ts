...
export class MeetingPersistence {
	...
	constructor(
		...
		llmHelper: LLMHelper,
		options: MeetingPersistenceOptions = {},
	) {
	...
	private notifyRenderer(channel: string, payload?: unknown): void {
		try {
			...
			console.warn(
				`[MeetingPersistence] Failed to notify renderer on ${channel}:`,
				error,
			);
		}
	}

	private async finalizeMeetingWithRetry(
		meetingData: Meeting,
		startTimeMs: number,
		durationMs: number,
	): Promise<number> {
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
			try {
				DatabaseManager.getInstance().finalizeMeetingProcessing(
					meetingData,
					startTimeMs,
					durationMs,
				);
	...
	private isMeaningfulTitle(title?: string | null): title is string {
		return (
			typeof title === "string" && !PLACEHOLDER_MEETING_TITLES.has(title.trim())
		);
	}
	...
	async waitForPendingSaves(timeoutMs: number = 10000): Promise<void> {
		...
		console.log(
			`[MeetingPersistence] Waiting for ${this.pendingSaves.size} pending saves...`,
		);

		const timeout = new Promise<void>((_, reject) =>
			setTimeout(
				() => reject(new Error("Timeout waiting for pending saves")),
				timeoutMs,
			),
		);

		try {
			await Promise.race([Promise.all(Array.from(this.pendingSaves)), timeout]);
			console.log("[MeetingPersistence] All pending saves completed");
		} catch (e) {
			console.warn(
				"[MeetingPersistence] Some saves may not have completed:",
				e,
			);
		}
	}
	...
	public async stopMeeting(meetingId?: string): Promise<SessionTracker> {
		...
		if (
			meetingId &&
			typeof (this.session as any).setActiveMeetingId === "function"
		) {
		...
		if (
			typeof (previousSession as unknown as { dispose?: () => Promise<void> })
				.dispose === "function"
		) {
			await (
				previousSession as unknown as { dispose: () => Promise<void> }
			).dispose();
		}
		...
		savePromise
			.catch((err) =>
				console.error(
					"[MeetingPersistence] Background processing failed:",
					err,
				),
			)
		...
		const placeholder: Meeting = {
			...
			title: this.isMeaningfulTitle(metadata?.title)
				? metadata.title
				: "Processing...",
		...
		try {
			DatabaseManager.getInstance().createOrUpdateMeetingProcessingRecord(
				placeholder,
				snapshot.startTime,
				snapshot.durationMs,
			);
	...
	private async processAndSaveMeeting(
		...
		let summaryData: { actionItems: string[]; keyPoints: string[] } = {
			actionItems: [],
			keyPoints: [],
		};

		const metadata: MeetingMetadataSnapshot | null =
			data.meetingMetadata || null;
    ...
    Return ONLY valid JSON (no markdown code blocks):
    {
      ...
      "actionItems": ["specific next steps, assigned tasks, or implied follow-ups. If absolutely none found, return empty array"]
				...
				if (generatedSummary) {
					const jsonMatch = generatedSummary.match(
						/```json\n([\s\S]*?)\n```/,
		...
		try {
			...
			await this.finalizeMeetingWithRetry(
				meetingData,
				data.startTime,
				data.durationMs,
			);
			...
			DatabaseManager.getInstance().markMeetingProcessingFailed(
				meetingId,
				error,
			);
			const retryCount =
				typeof (error as { finalizeRetryCount?: unknown })
					.finalizeRetryCount === "number"
					? Number(
							(error as { finalizeRetryCount?: number }).finalizeRetryCount,
						)
	...
	public async recoverUnprocessedMeetings(): Promise<void> {
		...
		console.log(
			`[MeetingPersistence] Found ${unprocessed.length} unprocessed meetings. recovering...`,
		);
		...
		for (const m of unprocessed) {
			try {
				...
				const snapshot: MeetingSnapshot = {
					...
					context: context,
					meetingMetadata: {
						title: this.isMeaningfulTitle(details.title)
							? details.title
							: undefined,
				...
				console.error(
					`[MeetingPersistence] Failed to recover meeting ${m.id}`,
					e,
				);
			}
		}
