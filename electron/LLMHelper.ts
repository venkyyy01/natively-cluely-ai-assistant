...
export class LLMHelper {
	...
	private getActiveFastResponseTarget(
		qualityTier: StreamQualityTier = "standard",
	...
	private async resolveOpenAiFallbackModel(
		failedModel: string,
	): Promise<string | null> {
	...
	private async runWithProviderFallbackBypass<T>(
		operation: () => Promise<T>,
	): Promise<T> {
...
ANSWER DIRECTLY:`;
	...
	private prepareUserContentForModel(
		provider: string,
		modelId: string,
		message: string,
		context?: string,
	...
	private async prepareScreenshotEventRouting(
	...
	public async chatWithGemini(
		...
		try {
			...
			const skipPromptForRequest = enforceSystemPrompt
				? false
				: skipSystemPrompt;
			...
			const buildMessage = (
				...
				const preparedUserContent = this.prepareUserContentForModel(
					provider,
					modelId,
					effectiveMessage,
					context,
			...
			const openaiUserContent = this.prepareUserContentForModel(
				"openai",
				activeOpenAiModel,
				effectiveMessage,
				context,
			);
			const claudeUserContent = this.prepareUserContentForModel(
				"claude",
				CLAUDE_MODEL,
				effectiveMessage,
				context,
			...
			const finalGeminiPrompt = await this.withSystemPromptCache(
				"gemini",
				this.currentModelId,
				geminiBasePrompt,
				() => buildSystemPrompt(geminiBasePrompt),
			);
			const finalGroqPrompt = await this.withSystemPromptCache(
				"groq",
				GROQ_MODEL,
				groqBasePrompt,
			...
			const fastResponseTarget = canUseFastResponse
				? this.getActiveFastResponseTarget()
				: null;
			if (fastResponseTarget) {
				...
				try {
					if (fastResponseTarget.provider === "cerebras") {
						return await this.generateWithCerebras(
							openaiUserContent,
							openaiSystemPrompt,
			...
			if (this.activeCurlProvider) {
				...
				return await this.runWithProviderFallbackBypass(() =>
					this.chatWithGemini(
						message,
						originalImagePaths,
						originalContext,
				...
				const response =
					isMultimodal && imagePaths?.length
						? await this.runWithScreenshotOcrFallback(
								...
								(fallbackMessage) => {
									const fallbackUserContent = this.prepareUserContentForModel(
										"custom",
										this.getCurrentModel(),
										fallbackMessage,
										context,
									);
									const fallbackCombinedMessage = this.joinPrompt(
										customSystemPrompt,
										fallbackUserContent,
										this.getInputTokenBudget("custom", this.getCurrentModel()),
									);
						...
						(fallbackMessage) => {
							...
							return this.generateWithGroq(
								this.joinPrompt(
									finalGroqPrompt,
									fallbackUserContent,
									this.getInputTokenBudget("groq", GROQ_MODEL),
								),
							);
						},
					);
			...
			if (isMultimodal) {
				...
				if (this.openaiClient) {
					providers.push({
						...
						execute: () =>
							this.runWithScreenshotOcrFallback(
								`OpenAI (${textOpenAI})`,
								imagePaths,
								effectiveMessage,
								() =>
						...
						execute: () =>
							this.runWithScreenshotOcrFallback(
								...
								(fallbackMessage) => {
									...
									return this.tryGenerateResponse(
										this.joinPrompt(
											finalGeminiPrompt,
											fallbackUserContent,
											this.getInputTokenBudget("gemini", textGeminiFlash),
										),
										undefined,
										textGeminiFlash,
									);
								...
								(fallbackMessage) => {
									...
									return this.tryGenerateResponse(
										this.joinPrompt(
											finalGeminiPrompt,
											fallbackUserContent,
											this.getInputTokenBudget("gemini_pro", textGeminiPro),
										),
										undefined,
										textGeminiPro,
									);
				...
				if (this.openaiClient) {
					providers.push({
						name: `OpenAI (${textOpenAI})`,
						execute: () =>
							this.generateWithOpenai(openaiUserContent, openaiSystemPrompt),
					});
				}
				if (this.claudeClient) {
					providers.push({
						name: `Claude (${textClaude})`,
						execute: () =>
							this.generateWithClaude(claudeUserContent, claudeSystemPrompt),
					});
				}
			}
	...
	private async generateWithCerebras(
		...
		const targetModel =
			modelOverride || this.getConfiguredFastModel("cerebras");
	...
	public async *streamChatWithGemini(
		...
		if (isMultimodal) {
			...
			if (this.openaiClient) {
				providers.push({
					name: `OpenAI (${textOpenAI})`,
					execute: () => this.streamWithOpenai(userContent, openaiSystemPrompt),
				});
			}
			if (this.claudeClient) {
				providers.push({
					name: `Claude (${textClaude})`,
					execute: () => this.streamWithClaude(userContent, claudeSystemPrompt),
				});
			}
			if (this.client) {
				providers.push({
	...
	public async *streamChat(
		...
		const preserveSystemPromptForStructuredOutput =
			structuredScreenshotRequest ||
			options?.qualityTier === "structured_reasoning";
		let screenshotRouting: ScreenshotEventRoutingResult | null = null;
		const forceTextFallback =
			this.shouldForceScreenshotTextFallback(imagePaths);
		...
		if (
			!hasScreenshotInput &&
			!options?.skipKnowledgeInterception &&
			this.knowledgeOrchestrator?.isKnowledgeMode()
		) {
			try {
				const knowledgeResult =
		...
		if (this.activeCurlProvider) {
			...
			try {
				...
				console.warn(
					`[LLMHelper] cURL provider (${this.activeCurlProvider.name}) returned no response while streaming. Falling back to standard routing.`,
				);
			} catch (error: any) {
				if (options?.abortSignal?.aborted) {
					throw error;
				...
				console.warn(
					`[LLMHelper] cURL provider (${this.activeCurlProvider.name}) streaming failed after ${CURL_PROVIDER_TIMEOUT_MS}ms timeout window. Falling back to standard routing:`,
					error.message,
				);
			}
		...
		try {
			...
			if (this.isModelNotFoundError(error)) {
				...
				if (fallbackModel && fallbackModel !== targetModel) {
					...
					yield* this.streamWithOpenaiUsingModel(
						userMessage,
						fallbackModel,
						systemPrompt,
						abortSignal,
		...
		try {
			...
			if (this.isModelNotFoundError(error)) {
				...
				if (fallbackModel && fallbackModel !== targetModel) {
					...
					yield* this.streamWithOpenaiMultimodalUsingModel(
						userMessage,
						imagePaths,
						fallbackModel,
						systemPrompt,
	...
	private async *streamWithGeminiParallelRace(
		...
		const streams = {
			flash: this.streamGeminiModelChunks(
				fullMessage,
				GEMINI_FLASH_MODEL,
				imagePaths,
			)[Symbol.asyncIterator](),
			pro: this.streamGeminiModelChunks(
				fullMessage,
				GEMINI_PRO_MODEL,
				imagePaths,
			)[Symbol.asyncIterator](),
	 ...
	 */
	private async *streamGeminiModelChunks(
		fullMessage: string,
		model: string,
		imagePaths?: string[],
	): AsyncGenerator<string, void, unknown> {
