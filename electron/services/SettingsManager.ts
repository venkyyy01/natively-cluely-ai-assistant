...
const ALLOWED_DISGUISE_MODES = new Set<AppSettings["disguiseMode"]>([
	"terminal",
	"settings",
	"activity",
	"none",
...
function sanitizeSettings(candidate: unknown): AppSettings {
	...
	if (typeof raw.enableCaptureDetectionWatchdog === "boolean") {
		sanitized.enableCaptureDetectionWatchdog =
			raw.enableCaptureDetectionWatchdog;
	}
	...
	if (Array.isArray(raw.captureToolPatterns)) {
		const patterns = raw.captureToolPatterns.filter(
	...
	if (
		typeof raw.disguiseMode === "string" &&
		ALLOWED_DISGUISE_MODES.has(raw.disguiseMode as AppSettings["disguiseMode"])
	) {
...
export class SettingsManager {
	...
	private constructor() {
		if (!app.isReady()) {
			throw new Error(
				"[SettingsManager] Cannot initialize before app.whenReady()",
			);
		}
	...
	public set<K extends keyof AppSettings>(
		key: K,
		value: AppSettings[K],
	): boolean {
		...
		try {
			if (fs.existsSync(this.settingsPath)) {
				...
				try {
					...
					console.error(
						"[SettingsManager] Failed to parse settings.json. Continuing with empty settings. Error:",
						parseError,
					);
					this.settings = {};
				}
