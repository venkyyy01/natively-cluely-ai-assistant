...
export class CredentialsManager {
	...
	public setFastResponseConfig(config: FastResponseConfig): void {
		...
		this.saveCredentials();
		console.log(
			`[CredentialsManager] Fast response config updated: ${this.credentials.fastResponseConfig.provider} (${this.credentials.fastResponseConfig.model || "auto"}) enabled=${this.credentials.fastResponseConfig.enabled}`,
		);
	}
			...
			if (value && typeof value === "object") {
				return Object.fromEntries(
					Object.entries(value as Record<string, unknown>).map(
						([key, nested]) => [key, scrubValue(nested)],
					),
				);
			}
			return value;
	...
	private saveCredentials(): void {
		try {
			if (!safeStorage.isEncryptionAvailable()) {
				throw new Error(
					"OS encryption is unavailable; refusing to persist credentials in plaintext",
				);
			}
		...
		try {
			...
			if (fs.existsSync(plaintextPath)) {
				try {
					fs.unlinkSync(plaintextPath);
					console.warn(
						"[CredentialsManager] Removed insecure plaintext credential file",
					);
				} catch (cleanupErr) {
					console.warn(
						"[CredentialsManager] Could not remove insecure plaintext credential file:",
						cleanupErr,
					);
				}
			}
