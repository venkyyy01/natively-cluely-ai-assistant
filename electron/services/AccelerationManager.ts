...
import type { StreamManager } from "../llm/StreamManager";
...
export function setActiveAccelerationManager(
	manager: AccelerationManager | null,
): void {
...
export class AccelerationManager {
	private static sharedLocalEmbeddingProvider: LocalEmbeddingProvider | null =
		null;
	...
	private readonly localEmbeddingAdapter: EmbeddingProvider = {
		embed: async (text: string): Promise<number[]> =>
			this.localEmbeddingProvider.embed(text),
	...
	private registerEmbeddingProvider(): void {
		if (
			isOptimizationActive("useANEEmbeddings") &&
			this.aneProvider.isInitialized()
		) {
			setEmbeddingProvider(this.aneProvider);
			console.log(
				"[AccelerationManager] ANE provider registered for real embeddings",
			);
			return;
		}
		...
		if (this.externalEmbeddingProvider) {
			...
			console.log(
				"[AccelerationManager] External embedding provider registered for semantic lookup",
			);
			return;
		}
	...
	async runInLane<T>(
		lane: "realtime" | "local-inference" | "semantic" | "background",
		task: () => Promise<T> | T,
	): Promise<T> {
	...
	shouldAdmitSpeculation(
		probability: number,
		valueOfPrefetch: number,
		costOfCompute: number,
	): boolean {
		return this.runtimeBudgetScheduler.shouldAdmitSpeculation(
			probability,
			valueOfPrefetch,
			costOfCompute,
		);
