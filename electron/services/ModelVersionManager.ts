...
export class ModelVersionManager {
	...
	public async onModelError(failedModelId: string): Promise<void> {
		...
		if (now - this.lastEventTriggeredDiscovery < EVENT_DISCOVERY_COOLDOWN_MS) {
			console.log(
				`[ModelVersionManager] Event-driven discovery skipped (cooldown active)`,
			);
			return;
		}

		console.log(
			`[ModelVersionManager] 🔥 Model error on "${failedModelId}" — triggering discovery`,
		);
		...
		try {
			...
			console.warn(
				`[ModelVersionManager] Event-driven discovery failed: ${err.message}`,
			);
		}
	}
	...
	public rollback(family: ModelFamily | TextModelFamily): boolean {
		...
		const rolledBack = !!(
			familyState.previousTier1 || familyState.previousLatest
		);

		if (familyState.previousTier1) {
			console.log(
				`[ModelVersionManager] ↩️ Rolling back ${family} Tier1: ${familyState.tier1} → ${familyState.previousTier1}`,
			);
		...
		if (familyState.previousLatest) {
			console.log(
				`[ModelVersionManager] ↩️ Rolling back ${family} Latest: ${familyState.latest} → ${familyState.previousLatest}`,
			);
	...
	public async initialize(): Promise<void> {
		const timeSinceLastDiscovery =
			Date.now() - this.state.lastDiscoveryTimestamp;

		if (
			timeSinceLastDiscovery >= DISCOVERY_INTERVAL_MS ||
			this.state.lastDiscoveryTimestamp === 0
		) {
			console.log(
				"[ModelVersionManager] Running initial model discovery (non-blocking)...",
			);
			try {
				await this.runDiscoveryAndUpgrade();
			} catch (err: any) {
				...
				console.warn(
					`[ModelVersionManager] Initial discovery failed (using baselines): ${err.message}`,
				);
			}
		} else {
			const daysUntilNext = Math.round(
				(DISCOVERY_INTERVAL_MS - timeSinceLastDiscovery) /
					(24 * 60 * 60 * 1000),
			);
			console.log(
				`[ModelVersionManager] Next scheduled discovery in ~${daysUntilNext} days`,
			);
		}
	...
	public async runDiscoveryAndUpgrade(): Promise<void> {
		...
		const discovered: Map<
			ModelFamily,
			{ modelId: string; version: ModelVersion }
		> = new Map();
		const textDiscovered: Map<
			TextModelFamily,
			{ modelId: string; version: ModelVersion }
		> = new Map();
		...
		if (this.openaiApiKey && this.shouldAttemptDiscovery("openai")) {
			discoveryPromises.push(
				this.discoverOpenAIModels(discovered, textDiscovered),
			);
		}
		if (this.geminiApiKey && this.shouldAttemptDiscovery("gemini")) {
			discoveryPromises.push(
				this.discoverGeminiModels(discovered, textDiscovered),
			);
		}
		if (this.claudeApiKey && this.shouldAttemptDiscovery("anthropic")) {
			discoveryPromises.push(
				this.discoverClaudeModels(discovered, textDiscovered),
			);
		}
		if (this.groqApiKey && this.shouldAttemptDiscovery("groq")) {
			discoveryPromises.push(
				this.discoverGroqModels(discovered, textDiscovered),
			);
		}
		...
		for (const [family, { modelId, version }] of textDiscovered) {
			const changed = this.applyUpgradeRulesForTextFamily(
				family,
				modelId,
				version,
			);
		...
		if (upgraded) {
			console.log(
				"[ModelVersionManager] ✅ Model tiers updated and persisted.",
			);
		} else {
			console.log(
				"[ModelVersionManager] ✅ Discovery complete. No tier changes needed.",
			);
		}
	}
	...
	private shouldAttemptDiscovery(provider: string): boolean {
		...
		const backoffFactor =
			...
			(failures - MAX_DISCOVERY_FAILURES_BEFORE_BACKOFF);
	...
	private recordDiscoveryFailure(provider: string): void {
		this.state.discoveryFailureCounts[provider] =
			(this.state.discoveryFailureCounts[provider] || 0) + 1;
	}
	...
	private async discoverOpenAIModels(
		...
		try {
			...
			if (!response.ok) {
				...
				console.warn(
					`[ModelVersionManager] OpenAI model listing failed: ${response.status}`,
				);
				return;
			}
			...
			this.recordDiscoveryFailure("openai");
			console.warn(
				`[ModelVersionManager] OpenAI discovery error: ${err.message}`,
			);
		}
	}
	...
	private async discoverGeminiModels(
		...
		try {
			...
			if (!response.ok) {
				...
				console.warn(
					`[ModelVersionManager] Gemini model listing failed: ${response.status}`,
				);
				return;
			}
			const json: any = await response.json();
			...
			this.recordDiscoveryFailure("gemini");
			console.warn(
				`[ModelVersionManager] Gemini discovery error: ${err.message}`,
			);
		}
	}
		...
		try {
			...
			while (hasMore) {
				...
				if (!response.ok) {
					...
					console.warn(
						`[ModelVersionManager] Anthropic model listing failed: ${response.status}`,
					);
					return;
				}
				...
				if (allModels.length > 500) {
					console.warn(
						"[ModelVersionManager] Anthropic discovery capped at 500 models",
					);
					break;
				}
			}
			...
			this.recordDiscoveryFailure("anthropic");
			console.warn(
				`[ModelVersionManager] Anthropic discovery error: ${err.message}`,
			);
		}
	}
	...
	private async discoverGroqModels(
		...
		try {
			...
			if (!response.ok) {
				...
				console.warn(
					`[ModelVersionManager] Groq model listing failed: ${response.status}`,
				);
				return;
			}
			const json: any = await response.json();
			...
			this.recordDiscoveryFailure("groq");
			console.warn(
				`[ModelVersionManager] Groq discovery error: ${err.message}`,
			);
		}
	}
	...
	private findLatestInFamily(
		...
		let best: { modelId: string; version: ModelVersion } | null =
			discovered.get(family) || null;
	...
	private applyUpgradeRules(
		...
		if (tier1Version) {
			...
			if (discoveredVersion.major > tier1Version.major) {
				...
				if (prevLatest && prevLatest !== prevTier1) {
					...
					if (
						prevLatestVersion &&
						compareVersions(prevLatestVersion, tier1Version) > 0
					) {
						console.log(
		...
		const changed =
			familyState.latest !== prevLatest || familyState.tier1 !== prevTier1;
	...
	private startBackgroundScheduler(): void {
		...
		this.discoveryTimer = setInterval(async () => {
			console.log(
				"[ModelVersionManager] ⏰ Scheduled model discovery triggered",
			);
			try {
				await this.runDiscoveryAndUpgrade();
			} catch (err: any) {
				console.error(
					"[ModelVersionManager] Scheduled discovery failed:",
					err.message,
				);
			}
		...
		if (
			this.discoveryTimer &&
			typeof this.discoveryTimer === "object" &&
			"unref" in this.discoveryTimer
		) {
			this.discoveryTimer.unref();
		}

		console.log(
			"[ModelVersionManager] 📅 Background scheduler started (every ~14 days)",
		);
	}
	...
	private loadState(): PersistedState {
		try {
			...
			console.warn(
				`[ModelVersionManager] Failed to load state: ${err.message}`,
			);
		}
	...
	public getSummary(): string {
		...
		lines.push(
			`  Last discovery: ${
				this.state.lastDiscoveryTimestamp
