import type { FamilyState, ModelVersion } from "./modelVersionTypes";
import {
	compareVersions,
	parseModelVersion,
	versionDistance,
...
export function applyTierUpgradeRules(
		...
		if (discoveredVersion.major > tier1Version.major) {
			if (prevLatest && prevLatest !== prevTier1) {
				...
				if (
					...
					const tag =
						majorLogTag === "text" ? "MAJOR text upgrade" : "MAJOR upgrade";
					console.log(
	...
	if (!changed) {
		...
		console.log(
			`[ModelVersionManager] ${logLabel}${kind}: Tier1=${familyState.tier1}, Tier2/3=${familyState.latest}`,
		);
	}
