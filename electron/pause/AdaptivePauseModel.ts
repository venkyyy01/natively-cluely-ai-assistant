...
export class AdaptivePauseModel {
  ...
  update(features: PauseFeatures, label: number): void {
    ...
    w.silenceDuration += lr * (error * features.silenceDuration - L2_REGULARIZATION * w.silenceDuration);
    w.transcriptCompleteness += lr * (error * features.transcriptCompleteness - L2_REGULARIZATION * w.transcriptCompleteness);
    w.semanticCompleteness += lr * (error * features.semanticCompleteness - L2_REGULARIZATION * w.semanticCompleteness);
    w.conversationRhythm += lr * (error * features.conversationRhythm - L2_REGULARIZATION * w.conversationRhythm);
    w.audioEnergyDecay += lr * (error * features.audioEnergyDecay - L2_REGULARIZATION * w.audioEnergyDecay);
    w.bias += lr * error;
