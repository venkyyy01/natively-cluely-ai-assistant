...
export class PauseThresholdTuner {
	...
	recordSuccessfulReuse(confidence: PauseConfidence): void {
		...
		this.profile.speculativeReuseRate = this.ema(
			this.profile.speculativeReuseRate,
			1,
			0.22,
		);
		this.profile.falsePositiveRate = this.ema(
			this.profile.falsePositiveRate,
			0,
			0.12,
		);
		this.profile.invalidationRate = this.ema(
			this.profile.invalidationRate,
			0,
			0.12,
		);
		this.profile.avgSuccessfulPauseMs = this.ema(
			this.profile.avgSuccessfulPauseMs,
			confidence.silenceMs,
			0.28,
		);
	...
	recordFalsePositiveResume(
		...
		const effectivePause = Math.max(
			confidence.silenceMs + 120,
			this.profile.avgSuccessfulPauseMs,
		);
		this.profile.avgSuccessfulPauseMs = this.ema(
			this.profile.avgSuccessfulPauseMs,
	...
	private recomputeThresholds(): void {
		const caution = clamp(
			this.profile.falsePositiveRate * 0.65 +
				this.profile.invalidationRate * 0.35 -
				this.profile.speculativeReuseRate * 0.55 +
				this.aggressiveBias,
			-0.35,
			0.45,
		);

		const learnedPauseMs = clamp(
			this.profile.avgSuccessfulPauseMs * 0.42,
			250,
			900,
		);
		const minSilenceMs = clamp(
			Math.round(
				learnedPauseMs * 0.55 +
					(this.baseConfig.minSilenceMs + caution * 260) * 0.45,
			),
			250,
			900,
		);
		...
		this.profile.softSpeculateThreshold = round3(
			clamp(this.baseConfig.softSpeculateThreshold + caution * 0.08, 0.3, 0.65),
		);
		this.profile.hardSpeculateThreshold = round3(
			clamp(this.baseConfig.hardSpeculateThreshold + caution * 0.12, 0.45, 0.8),
		);
		this.profile.commitThreshold = round3(
			clamp(this.baseConfig.commitThreshold + caution * 0.1, 0.6, 0.92),
		);
	}
}
