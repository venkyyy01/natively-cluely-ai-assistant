export type BufferedUtteranceFlushReason =
	| "punctuation"
	| "silence"
	| "max_buffer"
	| "speaker_change"
...
export class InterviewerUtteranceBuffer {
	...
	setOnUtterance(
		handler: ((utterance: BufferedUtterance) => void) | undefined,
	): void {
	...
	flush(reason: BufferedUtteranceFlushReason = "manual"): BufferedUtterance[] {
		...
		const utterances = parts.map(
			(part, index): BufferedUtterance => ({
				speaker: active.speaker,
				text: part,
				utteranceId:
					index === 0
						? active.utteranceId
						: `${active.utteranceId}:${index + 1}`,
