...
export function initializeIpcHandlers(appState: AppState): void {
	...
	const getInferenceLlmHelper = () => {
		try {
			const coordinator = (
				appState as { getCoordinator?: () => unknown }
			).getCoordinator?.() as
				| { getSupervisor?: (name: string) => unknown }
				| undefined;
			...
			if (llmHelper) {
				return llmHelper as ReturnType<
					typeof appState.processingHelper.getLLMHelper
				>;
			}
		} catch {
	...
	const getRuntimeCoordinator = (): RuntimeCoordinatorLike | null => {
		try {
			const coordinator = (
				appState as { getCoordinator?: () => unknown }
			).getCoordinator?.() as RuntimeCoordinatorLike | undefined;
	...
	const getSttSupervisor = (): SttSupervisorLike | null => {
		...
		return (
			(coordinator?.getSupervisor?.("stt") as SttSupervisorLike | undefined) ??
			null
		);
	};
	...
	const getInferenceSupervisor = (): InferenceSupervisorLike | null => {
		const coordinator = getRuntimeCoordinator();
		return (
			(coordinator?.getSupervisor?.("inference") as
	...
	const getAudioFacade = (): AudioFacadeLike | null => {
		if (
			"getAudioFacade" in appState &&
			typeof appState.getAudioFacade === "function"
		) {
	...
	const getScreenshotFacade = (): ScreenshotFacadeLike | null => {
		if (
			"getScreenshotFacade" in appState &&
			typeof appState.getScreenshotFacade === "function"
		) {
	...
	safeHandle("take-screenshot", async (event) => {
		try {
			...
			// Emit event for consistency with keybind workflow (only in real context, not tests)
			if (event?.sender) {
				event.sender.send("screenshot-taken", { path: screenshotPath, preview });
			}
	...
	safeHandle("take-selective-screenshot", async (event) => {
		try {
			...
			// Emit event for consistency with keybind workflow (only in real context, not tests)
			if (event?.sender) {
				event.sender.send("screenshot-attached", { path: screenshotPath, preview });
			}
			return ok({ path: screenshotPath, preview });
		} catch (error: any) {
	...
	safeHandle("get-current-llm-config", async () => {
		try {
			...
			return fail(
				...
				error,
				"Failed to read current LLM config",
			);
		}
	});
	...
	safeHandleValidated(
		"switch-to-ollama",
		(args) =>
			parseIpcInput(ipcSchemas.ollamaSwitchArgs, args, "switch-to-ollama"),
		async (_, model, url) => {
			try {
				const llmHelper = getInferenceLlmHelper();
	...
	safeHandle("force-restart-ollama", async () => {
		try {
			...
			return fail(
				"OLLAMA_FORCE_RESTART_FAILED",
				error,
				"Failed to force restart Ollama",
			);
		}
	});
	...
	safeHandleValidated(
		...
		(args) =>
			parseIpcInput(
				ipcSchemas.providerSwitchGeminiArgs,
				args,
				"switch-to-gemini",
			),
		async (_, apiKey, modelId) => {
			try {
				const llmHelper = getInferenceLlmHelper();
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-gemini-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
	...
	safeHandleValidated(
		"set-groq-api-key",
		(args) =>
			[parseIpcInput(ipcSchemas.apiKey, args[0], "set-groq-api-key")] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-cerebras-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-openai-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-claude-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.providerId, args[0], "delete-custom-provider"),
			] as const,
		async (_, id) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandle("get-curl-providers", async () => {
		try {
			...
			return fail(
				"CURL_PROVIDERS_READ_FAILED",
				error,
				"Failed to get curl providers",
			);
		}
	});
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.customProvider, args[0], "save-curl-provider"),
			] as const,
		async (_, provider) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.providerId, args[0], "delete-curl-provider"),
			] as const,
		async (_, id) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			parseIpcInput(
				ipcSchemas.providerModelFetchArgs,
				args,
				"fetch-provider-models",
			),
	...
	safeHandleValidated(
		...
		(args) =>
			parseIpcInput(
				ipcSchemas.providerPreferredModel,
				args,
				"set-provider-preferred-model",
			),
		async (_, provider, modelId) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
				...
				return {
					success: false,
					error: error?.message || "Failed to set preferred model",
				};
			}
		},
	);
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.sttProvider, args[0], "set-stt-provider"),
			] as const,
		async (_, provider) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandle("get-stt-provider", async () => {
		try {
			...
			return fail(
				"STT_PROVIDER_READ_FAILED",
				error,
				"Failed to get STT provider",
			);
		}
	});
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-groq-stt-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-openai-stt-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-deepgram-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.modelId, args[0], "set-groq-stt-model"),
			] as const,
		async (_, model) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-elevenlabs-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		"set-azure-api-key",
		(args) =>
			[parseIpcInput(ipcSchemas.apiKey, args[0], "set-azure-api-key")] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.azureRegion, args[0], "set-azure-region"),
			] as const,
		async (_, region) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-ibmwatson-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(ipcSchemas.apiKey, args[0], "set-soniox-api-key"),
			] as const,
		async (_, apiKey) => {
			try {
				const { CredentialsManager } = require("./services/CredentialsManager");
	...
	safeHandleValidated(
		"test-stt-connection",
		(args) =>
			parseIpcInput(ipcSchemas.sttConnectionArgs, args, "test-stt-connection"),
		async (_, provider, apiKey, region) => {
			console.log(
	...
	safeHandleValidated(
		"test-llm-connection",
		(args) =>
			parseIpcInput(ipcSchemas.llmConnectionArgs, args, "test-llm-connection"),
		async (_, provider, apiKey) => {
			console.log(
