...
function getInferenceSupervisor(
	appState: AppState,
): InferenceSupervisorLike | null {
	if (
...
export function registerIntelligenceHandlers({
	...
	safeHandle,
	safeHandleValidated,
}: RegisterIntelligenceHandlersDeps): void {
	...
	safeHandleValidated(
		...
		(args) => {
			const question =
				typeof args[0] === "undefined"
					? undefined
					: parseIpcInput(
							ipcSchemas.intelligenceQuestion,
							args[0],
							"generate-what-to-say",
						);
			const imagePaths =
				typeof args[1] === "undefined"
					? undefined
					: parseIpcInput(
							ipcSchemas.intelligenceImagePaths,
							args[1],
							"generate-what-to-say",
						);
		...
		async (_event, question?: string, imagePaths?: string[]) => {
			...
			const tryGenerate = async (attempt: number): Promise<string | null> => {
				const answer = await facade.runWhatShouldISay?.(
					question,
					0.8,
					imagePaths,
				);
				...
				if (attempt === 1) {
					console.warn(
						"[IPC] generate-what-to-say returned null on attempt 1, retrying after 400ms...",
					);
	...
	safeHandleValidated(
		"generate-follow-up",
		(args) => {
			const intent = parseIpcInput(
				ipcSchemas.intelligenceFollowUpIntent,
				args[0],
				"generate-follow-up",
			);
			const userRequest =
				typeof args[1] === "undefined"
					? undefined
					: parseIpcInput(
							ipcSchemas.intelligenceFollowUpUserRequest,
							args[1],
							"generate-follow-up",
						);
		...
		async (_event, intent: string, userRequest?: string) => {
			const refined = await getIntelligenceFacade(appState).runFollowUp?.(
				intent,
				userRequest,
			);
	...
	safeHandle("generate-follow-up-questions", async () => {
		const questions =
			await getIntelligenceFacade(appState).runFollowUpQuestions?.();
		return { questions };
	});
	...
	safeHandleValidated(
		...
		(args) =>
			[
				parseIpcInput(
					ipcSchemas.intelligenceManualQuestion,
					args[0],
					"submit-manual-question",
				),
			] as const,
		async (_event, question: string) => {
			const answer =
				await getIntelligenceFacade(appState).runManualAnswer?.(question);
