...
export function registerProfileHandlers({ appState, safeHandle, safeHandleValidated }: RegisterProfileHandlersDeps): void {
  safeHandleValidated('profile:upload-resume', (args) => [parseIpcInput(ipcSchemas.profileFilePath, args[0], 'profile:upload-resume')] as const, async (_event, filePath) => {
    try {
      ...
      return profileError('PROFILE_UPLOAD_FAILED', error?.message || 'Unable to upload resume');
    }
  });
  ...
  safeHandle('profile:get-status', async () => {
    try {
      ...
      return profileError('PROFILE_STATUS_READ_FAILED', error?.message || 'Unable to read profile status');
    }
  });
  ...
  safeHandleValidated('profile:set-mode', (args) => [parseIpcInput(ipcSchemas.booleanFlag, args[0], 'profile:set-mode')] as const, async (_event, enabled) => {
    try {
      ...
      return profileError('PROFILE_MODE_UPDATE_FAILED', error?.message || 'Unable to update profile mode');
    }
  });
  ...
  safeHandle('profile:delete', async () => {
    try {
      ...
      return profileError('PROFILE_DELETE_FAILED', error?.message || 'Unable to delete resume');
    }
  });
  ...
  safeHandle('profile:get-profile', async () => {
    try {
      ...
      return profileError('PROFILE_READ_FAILED', error?.message || 'Unable to read profile data');
    }
  });
  ...
  safeHandle('profile:select-file', async () => {
    try {
      ...
      if (result.canceled || result.filePaths.length === 0) return profileSuccess({ cancelled: true });
      return profileSuccess({ filePath: result.filePaths[0] });
    } catch (error: any) {
      return profileError('PROFILE_FILE_PICKER_FAILED', error?.message || 'Unable to select profile file');
    }
  });
  ...
  safeHandleValidated('profile:upload-jd', (args) => [parseIpcInput(ipcSchemas.profileFilePath, args[0], 'profile:upload-jd')] as const, async (_event, filePath) => {
    try {
      ...
      return profileError('PROFILE_JD_UPLOAD_FAILED', error?.message || 'Unable to upload job description');
    }
  });
  ...
  safeHandle('profile:delete-jd', async () => {
    try {
      ...
      return profileError('PROFILE_JD_DELETE_FAILED', error?.message || 'Unable to delete job description');
    }
  });
  ...
  safeHandleValidated('profile:research-company', (args) => [parseIpcInput(ipcSchemas.profileCompanyName, args[0], 'profile:research-company')] as const, async (_event, companyName) => {
    try {
      ...
      return profileError('PROFILE_COMPANY_RESEARCH_FAILED', error?.message || 'Unable to research company');
    }
  });
  ...
  safeHandle('profile:generate-negotiation', async () => {
    try {
      ...
      const status = orchestrator.getStatus();
      if (!status.hasResume) return profileError('PROFILE_MISSING_RESUME', 'No resume loaded');
      let dossier = null;
      ...
      return profileError('PROFILE_NEGOTIATION_FAILED', error?.message || 'Unable to prepare negotiation data');
    }
  });

  safeHandleValidated('set-google-search-api-key', (args) => [parseIpcInput(ipcSchemas.apiKey, args[0], 'set-google-search-api-key')] as const, async (_event, apiKey) => {
    try {
      const { CredentialsManager } = require('../services/CredentialsManager');
      CredentialsManager.getInstance().setGoogleSearchApiKey(apiKey);
      return profileSuccess({ success: true });
    } catch (error: any) {
      return profileError('PROFILE_SEARCH_CONFIG_FAILED', error?.message || 'Unable to save Google Search API key');
    }
  });

  safeHandleValidated('set-google-search-cse-id', (args) => [parseIpcInput(ipcSchemas.googleSearchCseId, args[0], 'set-google-search-cse-id')] as const, async (_event, cseId) => {
    try {
      const { CredentialsManager } = require('../services/CredentialsManager');
      CredentialsManager.getInstance().setGoogleSearchCseId(cseId);
      return profileSuccess({ success: true });
    } catch (error: any) {
      return profileError('PROFILE_SEARCH_CONFIG_FAILED', error?.message || 'Unable to save Google Search CSE ID');
    }
  });
}
