...
const PROVIDER_CONFIGS: Record<RestSttProvider, ProviderConfigFactory> = {
	...
	azure: (apiKey, region = "eastus", languageKey) => {
		const lang = languageKey
			? RECOGNITION_LANGUAGES[languageKey]?.bcp47
			: undefined;
...
export class RestSTT extends EventEmitter {
	...
	constructor(
		...
		console.log(
			`[RestSTT] Initialized for provider: ${provider}, model: ${this.config.model || "(default)"}`,
		);
	}
	...
	public notifySpeechEnded(): void {
		...
		console.log(
			`[RestSTT] Speech ended detected by native VAD — flushing buffer immediately`,
		);
		this.flushAndUpload();
	}
	...
	private async flushAndUpload(): Promise<void> {
		...
		if (this.isSilent(rawPcm)) {
			if (Math.random() < 0.1) {
				console.log(
					`[RestSTT] Skipping silent buffer (${rawPcm.length} bytes)`,
				);
			}
			return;
		}
		...
		try {
			...
			if (transcript && transcript.trim().length > 0) {
				console.log(
					`[RestSTT] Transcript: "${transcript.substring(0, 60)}..."`,
				);
