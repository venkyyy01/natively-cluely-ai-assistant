...
if (!NativeModule) {
	...
	console.error(
		"[MicrophoneCapture] ❌ Failed to load native module:",
...
export class MicrophoneCapture extends EventEmitter {
	...
	constructor(deviceId?: string | null) {
		...
		try {
			...
			console.log(
				"[MicrophoneCapture] ✅ Native audio module initialized successfully",
			);
		} catch (e) {
			...
			console.error(
				"[MicrophoneCapture] ❌ Failed to initialize native audio module:",
				e,
			);
	...
	public start(): void {
		...
		if (!this.isNativeAvailable) {
			console.warn(
				"[MicrophoneCapture] ⚠️ Cannot start: Native audio module not available",
			);
			this.emit(
				"error",
				new Error(
					"Native audio module not available - microphone capture disabled",
				),
			);
			return;
		}
		...
		if (!this.monitor) {
			...
			try {
				...
				console.error(
					"[MicrophoneCapture] Failed to re-initialize monitor:",
					e,
				);
				this.emit("error", e);
				throw e;
			...
			this.monitor.start(
				(first: Uint8Array | Error | null, second?: Uint8Array) => {
					...
					if (first instanceof Error) {
						...
						console.error(
							"[MicrophoneCapture] Native DSP thread panicked:",
							first,
						);
