...
export class SonioxStreamingSTT extends EventEmitter {
	...
	public setRecognitionLanguage(key: string): void {
		...
		if (config) {
			...
			console.log(
				`[SonioxStreaming] Language hint set to ${this.languageCode}`,
			);

			if (this.isActive) {
				console.log(
					"[SonioxStreaming] Language changed while active. Restarting...",
				);
				this.stop();
				this.start();
			}
	...
	public write(chunk: Buffer): void {
		...
		if (!this.ws || this.ws.readyState !== WebSocket.OPEN || !this.configSent) {
			...
			if (!this.isConnecting && this.shouldReconnect && !this.reconnectTimer) {
				console.log(
					"[SonioxStreaming] WS not ready. Lazy connecting on new audio...",
				);
				this.connect();
			}
			return;
	...
	private connect(): void {
		...
		this.ws.on("message", (data: WebSocket.Data) => {
			try {
				...
				if (msg.error_code) {
					console.error(
						`[SonioxStreaming] Server error: ${msg.error_code} - ${msg.error_message}`,
					);
					this.emit(
						"error",
						new Error(`Soniox: ${msg.error_code} - ${msg.error_message}`),
					);
					return;
				}
				...
				for (const token of tokens) {
					...
					if (token.text === "<fin>") {
						console.log(
							"[SonioxStreaming] Received <fin> manual finalization marker",
						);
						continue;
					}
					...
					if (token.text === "<end>") {
						console.log(
							"[SonioxStreaming] Received <end> endpoint detection marker",
						);
						continue;
					}
		...
		this.ws.on("close", (code: number, reason: Buffer) => {
			...
			this.clearKeepAlive();
			console.log(
				`[SonioxStreaming] Closed (code=${code}, reason=${reason.toString()})`,
			);
	...
	private scheduleReconnect(): void {
		...
		console.log(
			`[SonioxStreaming] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})...`,
		);
