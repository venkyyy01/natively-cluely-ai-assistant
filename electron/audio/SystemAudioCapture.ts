...
if (!NativeModule) {
	console.error(
		"[SystemAudioCapture] Failed to load native module:",
		getNativeAudioLoadError(),
	);
}
...
export class SystemAudioCapture extends EventEmitter {
	...
	constructor(deviceId?: string | null) {
		...
		try {
			...
			console.error(
				"[SystemAudioCapture] ❌ Failed to initialize native audio module:",
				e,
			);
			console.warn(
				"[SystemAudioCapture] 🔄 Falling back to software-only mode",
			);
	...
	private ensureMonitor(reason: "probe" | "start"): boolean {
		...
		if (!this.isNativeAvailable) {
			if (reason === "start") {
				console.error(
					"[SystemAudioCapture] ⚠️ Cannot start: Native audio module not available",
				);
				this.emit(
					"error",
					new Error(
						"Native audio module not available - system audio capture disabled",
					),
				);
			}
			return false;
		...
		if (!RustAudioCapture) {
			if (reason === "start") {
				const error =
					getNativeAudioLoadError()?.message ||
		...
		console.log(
			`[SystemAudioCapture] Creating native monitor (${reason === "probe" ? "sample-rate probe" : "lazy init"})...`,
		);
		try {
	...
	public getOutputSampleRate(): number {
		...
		if (
			this.monitor &&
			typeof this.monitor.getOutputSampleRate === "function"
		) {
	...
	public start(): void {
		...
		try {
			...
			if (typeof this.monitor.getSampleRate === "function") {
				...
				console.log(
					`[SystemAudioCapture] Detected sample rate: ${this.detectedSampleRate}`,
				);
			}

			this.monitor.start(
				(first: Uint8Array | Error | null, second?: Uint8Array) => {
					...
					if (first instanceof Error) {
						...
						console.error(
							"[SystemAudioCapture] Native DSP thread panicked:",
							first,
						);
