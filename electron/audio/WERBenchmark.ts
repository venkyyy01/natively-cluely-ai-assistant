...
export function computeWER(reference: string, hypothesis: string): BenchmarkResult {
  ...
  const rawWER = refWordCount === 0 ? (hypWords.length > 0 ? 1 : 0) : (substitutions + deletions + insertions) / refWordCount;
...
export function computeDiarizationAccuracy(reference: DiarizedTurn[], hypothesis: DiarizedTurn[]): DiarizationResult {
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    if (ref && hyp) {
      ...
      turns.push({ refSpeaker: ref.speaker, hypSpeaker: 'missing', text: ref.text, correct: false });
    } else if (hyp) {
      turns.push({ refSpeaker: 'missing', hypSpeaker: hyp.speaker, text: hyp.text, correct: false });
    }
  }
...
export function runSTTBenchmark(
  ...
  _runProvider: (audioPath: string) => Promise<{ text: string; turns?: DiarizedTurn[] }>,
): Promise<STTBenchmarkReport> {
  // Placeholder: real implementation would iterate corpus, call provider, compute metrics.
  throw new Error('runSTTBenchmark not yet implemented without live audio fixtures');
}
