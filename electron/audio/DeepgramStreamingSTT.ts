...
export class DeepgramStreamingSTT extends EventEmitter {
	...
	public setRecognitionLanguage(key: string): void {
		...
		if (config) {
			...
			if (this.isActive) {
				console.log(
					"[DeepgramStreaming] Language changed while active. Restarting...",
				);
				this.stop();
				this.start();
			}
	...
	public write(chunk: Buffer): void {
		...
		const pcm16 = resampleToMonoPcm16(
			chunk,
		...
		if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
			...
			if (!this.isConnecting && this.shouldReconnect && !this.reconnectTimer) {
				console.log(
					"[DeepgramStreaming] WS not ready. Lazy connecting on new audio...",
				);
				this.connect();
			}
			return;
	...
	private connect(): void {
		...
		console.log(
			`[DeepgramStreaming] Connecting (input=${this.inputSampleRate}, target=${this.targetSampleRate}, ch=1)...`,
		);
		...
		socket.on("message", (data: WebSocket.Data) => {
			...
			try {
				...
				const span = startSpan(
					traceId,
					`stt.deepgram.transcript.${isFinal ? "final" : "interim"}`,
				);
				...
				if (span) {
					...
					setSpanAttribute(
						traceId,
						span.spanId,
						"stt.input_sample_rate",
						this.inputSampleRate,
					);
					setSpanAttribute(
						traceId,
						span.spanId,
						"stt.target_sample_rate",
						this.targetSampleRate,
					);
					setSpanAttribute(
						traceId,
				...
				traceLogger.logSttEvent(
					traceId,
					span?.spanId,
					isFinal ? "transcript.final" : "transcript.received",
					{
	...
	private scheduleReconnect(): void {
		...
		if (this.reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
			...
			const error = new Error(
				`Deepgram max reconnect attempts (${MAX_RECONNECT_ATTEMPTS}) reached`,
			);
			console.error(
				"[DeepgramStreaming] Max reconnect attempts reached. Giving up.",
			);
		...
		console.log(
			`[DeepgramStreaming] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})...`,
		);
	...
	private startConnectionGuard(): void {
		...
		this.connectionGuardTimer = setInterval(() => {
			...
			console.warn(
				"[DeepgramStreaming] Connection guard detected inactive socket. Reconnecting...",
			);
			this.connect();
	...
	private startLivenessWatchdog(socket: WebSocket): void {
		...
		this.livenessTimer = setInterval(() => {
			...
			console.warn(
				"[DeepgramStreaming] No inbound activity while audio is flowing. Recycling socket...",
			);
	...
	private armConnectTimeout(socket: WebSocket): void {
		...
		this.connectTimeoutTimer = setTimeout(() => {
			...
			console.warn(
				"[DeepgramStreaming] Connection attempt timed out. Recycling socket...",
			);
	...
	private handleSocketClose(
		...
		this.clearConnectTimeout();
		console.log(
			`[DeepgramStreaming] Closed (code=${code}, reason=${reason.toString()})`,
		);
