...
export class ElevenLabsStreamingSTT extends EventEmitter {
	...
	public setRecognitionLanguage(key: string): void {
		...
		if (config) {
			...
			if (this.languageCode !== newCode) {
				console.log(
					`[ElevenLabsStreaming] Language changed: ${this.languageCode} -> ${newCode}`,
				);
	...
	private connect(): void {
		...
		console.log(
			`[ElevenLabsStreaming] Connecting... key=${this.apiKey?.slice(0, 8)}...`,
		);
				...
				switch (msgType) {
					...
					default:
						...
						if (msg.error) {
							...
							this.emit(
								"error",
								new Error(msg.error?.message || "ElevenLabs server error"),
							);
						} else {
							console.log(
