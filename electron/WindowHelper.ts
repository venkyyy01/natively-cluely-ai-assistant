...
export class WindowHelper {
	...
	public createWindow(): void {
		...
		const launcherSettings: Electron.BrowserWindowConstructorOptions = {
			...
			show: false,
			skipTaskbar: false, // Primary window should always be visible to user
		...
		if (this.shouldUseStealthRuntime()) {
			...
			this.launcherWindow = this.createDirectWindow(
				launcherSettings,
				`${startUrl}?window=launcher`,
				"Launcher",
			);
		...
		this.launcherContentWindow?.webContents.on("crashed", (_event, killed) => {
			console.error(
				`[WindowHelper] Launcher content window crashed (killed=${killed})`,
			);
		});
		...
		this.launcherContentWindow?.webContents.on(
			...
			(_event, details) => {
				console.error(
					`[WindowHelper] Launcher render process gone: reason=${details.reason} exitCode=${details.exitCode}`,
				);
			},
		);
		...
		const overlaySettings: Electron.BrowserWindowConstructorOptions = {
			...
			resizable: true,
			movable: true,
			skipTaskbar: false, // Overlay should be visible to user even in stealth mode
		...
		if (this.shouldUseStealthRuntime()) {
			...
			this.overlayRuntime = null;
			this.overlayWindow = this.createDirectWindow(
				overlaySettings,
				`${startUrl}?window=overlay`,
				"Overlay",
			);
