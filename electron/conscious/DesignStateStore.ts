...
function isQuestionLike(value: string): boolean {
  return /\?$/.test(value) || /^(how|what|why|when|where|which|can|could|would|walk me through|tell me)/i.test(value);
}
...
function inferFacets(text: string, phase?: InterviewPhase): DesignStateFacet[] {
  ...
  if (/(budget|timeline|deadline|month|week|quarter|engineer|team|region|tenant|user|qps|rps|throughput|latency|availability|consistency|compliance|privacy|cost|sla|slo|limit|constraint|requirement)/i.test(lower)) {
    facets.add('requirements');
  }

  if (/(assume|assuming|let'?s assume|suppose|we can rely on|we can tolerate)/i.test(lower)) {
    facets.add('assumptions');
  }

  if (/(architecture|component|service|pipeline|workflow|gateway|load balancer|cache|queue|database|redis|kafka|partitioner|coordinator|write path|read path)/i.test(lower)) {
    facets.add('architecture');
  }

  if (/(api|endpoint|request|response|grpc|rest|contract|schema|interface|webhook|rpc)/i.test(lower)) {
    facets.add('api_contracts');
  }

  if (/(data model|schema|table|index|entity|entities|ledger|partition key|primary key|secondary index|row|column|document shape)/i.test(lower)) {
    facets.add('data_model');
  }

  if (/(tradeoff|trade-off|overhead|complexity|cost|operationally|consistency|availability|pros|cons)/i.test(lower)) {
    facets.add('tradeoffs');
  }

  if (/(scale|scaling|shard|partition|hot key|hot tenant|fanout|capacity|millions?|billions?|burst|backpressure|replica)/i.test(lower)) {
    facets.add('scaling');
  }

  if (/(failure|failover|retry|timeout|outage|degrade|degradation|recovery|clock skew|duplicate|idempot|poison|disaster|split brain)/i.test(lower)) {
    facets.add('failure_modes');
  }

  if (/(metric|metrics|latency|p50|p95|p99|error rate|availability|throughput|saturation|queue depth|cpu|memory|utilization)/i.test(lower)) {
    facets.add('metrics');
  }
...
function facetAwareOverlap(facet: DesignStateFacet, queryTokens: string[], text: string, entryTokens: string[]): number {
  const lexical = lexicalOverlap(queryTokens, entryTokens);
  const lower = text.toLowerCase();
  let bonus = 0;

  if (facet === 'data_model' && (queryTokens.includes('schema') || queryTokens.includes('model') || queryTokens.includes('data'))) {
    if (/(table|index|schema|append-only|ledger|entity|entities|partition key|secondary index)/i.test(lower)) {
      bonus += 0.28;
    }
  }

  if (facet === 'api_contracts' && (queryTokens.includes('api') || queryTokens.includes('contract') || queryTokens.includes('interface'))) {
    if (/(api|endpoint|request|response|idempotent|webhook|grpc|rest|contract)/i.test(lower)) {
      bonus += 0.26;
    }
  }

  if (facet === 'failure_modes' && (queryTokens.includes('failure') || queryTokens.includes('failover') || queryTokens.includes('reliability'))) {
    if (/(retry|timeout|failover|idempotent|duplicate|outage|degrade|recovery|clock skew)/i.test(lower)) {
      bonus += 0.28;
    }
  }

  if (facet === 'scaling' && (queryTokens.includes('scale') || queryTokens.includes('throughput') || queryTokens.includes('hotspot'))) {
    if (/(shard|hot|throughput|capacity|partition|replica|fanout|backpressure)/i.test(lower)) {
      bonus += 0.24;
    }
  }

  if (facet === 'tradeoffs' && (queryTokens.includes('tradeoff') || queryTokens.includes('tradeoffs'))) {
    bonus += 0.18;
  }
...
export class DesignStateStore {
  ...
  notePinnedItem(text: string, label: string | undefined, timestamp: number, phase?: InterviewPhase): void {
  ...
  noteStructuredResponse(input: {
    ...
    for (const item of response.implementationPlan) {
      const facets = inferFacets(item, input.phase).filter((facet) => facet !== 'open_questions');
      const resolvedFacets: DesignStateFacet[] = facets.length > 0 ? facets : ['architecture'];
  ...
  getRetrievalEntries(query: string, maxPerFacet: number = 2): DesignStateRetrievalEntry[] {
    ...
    for (const facet of FACET_ORDER) {
      const facetEntries = this.entries
        ...
        .map((entry) => {
          ...
          const overlap = facetAwareOverlap(entry.facet, queryTokens, entry.text, entry.keywords);
          return {
            entry,
  ...
  restorePersistenceSnapshot(snapshot: PersistedDesignStateState | null | undefined): void {
    if (!snapshot) {
      this.reset();
      return;
    }
  ...
  private addEntry(input: {
    ...
    const existingIndex = this.entries.findIndex((entry) => entry.facet === input.facet && entry.normalized === normalized);
