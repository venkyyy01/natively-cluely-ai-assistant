...
export class AdaptiveContextWindow {
	...
	private computeScore(
		...
		const semanticScore =
			this.computeSemanticScore(entry, queryEmbedding, config) ??
			this.computeLexicalOverlap(query, entry.text);
		const phaseScore = this.computePhaseAlignment(
			entry.phase,
			this.currentPhase,
		);
	...
	private computeSemanticScore(
		...
		const recordedDimension =
			entry.embeddingDimension ?? entry.embedding.length;
		if (
			recordedDimension !== queryEmbedding.length ||
		...
		if (
			...
			config.embeddingModel !== entry.embeddingModel
		) {
			return null;
		}
