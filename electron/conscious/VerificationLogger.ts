...
export class VerificationLogger {
	...
	private initializeDatabase(): void {
      ...
      `);
			...
			console.warn(
				"[VerificationLogger] Database initialization failed, falling back to in-memory logging:",
				error,
			);
	...
	log(entry: Omit<VerificationLogEntry, "id" | "profileId">): void {
      ...
      `);
			...
			console.warn(
				"[VerificationLogger] Failed to log entry, falling back to in-memory:",
				error,
			);
			this.inMemoryLogs.push(fullEntry);
		}
	...
	getLogs(limit: number = 100): VerificationLogEntry[] {
      ...
      `);
			...
			console.warn(
				"[VerificationLogger] Failed to retrieve logs, returning in-memory fallback:",
				error,
			);
			return this.inMemoryLogs.slice(-limit);
		}
	...
	getFailureLogs(limit: number = 100): VerificationLogEntry[] {
      ...
      `);
			...
			console.warn(
				"[VerificationLogger] Failed to retrieve failure logs:",
				error,
			);
			return this.inMemoryLogs
				.filter((l) => l.verdict === "fail")
