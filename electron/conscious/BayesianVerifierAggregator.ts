...
export class BayesianVerifierAggregator {
  ...
  aggregate(results: VerifierResult[]): BayesianAggregationResult {
    ...
    const validResults = results.filter(r => r.weight > 0 && r.confidence >= 0);
    ...
    for (const result of normalizedResults) {
      const likelihood = result.passed ? result.confidence : (1 - result.confidence);
      posterior *= Math.pow(likelihood, result.weight);
    }
   ...
   */
  static deterministicResult(passed: boolean, confidence: number = 0.5): VerifierResult {
    return {
      name: 'deterministic',
      confidence,
      passed,
