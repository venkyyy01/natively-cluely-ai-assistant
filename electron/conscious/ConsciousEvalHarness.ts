...
import type { ConsciousVerifier } from "./ConsciousVerifier";
import {
...
function buildBaselineResponse(): ConsciousModeStructuredResponse {
	return {
		...
		pushbackResponses: [
			"The design keeps the write path simple while isolating noisy tenants.",
		],
		likelyFollowUps: [],
		codeTransition: "",
	};
...
function buildBehavioralResponse(
	overrides: Partial<ConsciousModeStructuredResponse["behavioralAnswer"]> = {},
	...
	return {
		...
		behavioralAnswer: {
			...
			headline:
				"I aligned a skeptical partner team around a safer rollout plan.",
			situation:
				"We were about to launch a billing change and the partner team wanted to compress testing to hit the date.",
			task: "I needed to protect the launch quality without turning the discussion into a turf fight.",
			action:
				"I pulled the recent incident data, showed exactly where regressions were most likely, proposed a two-phase rollout with ownership checkpoints, and walked both managers through the rollback plan until we agreed on the sequence.",
			result:
				"We shipped one week later, avoided a repeat incident, and cut rollout rollback volume by 38 percent on the next release.",
			whyThisAnswerWorks: [
...
export function getDefaultConsciousEvalScenarios(): ConsciousEvalScenario[] {
	return [
		...
		},
		{
			...
			family: "system_design",
			description:
				"Tradeoff follow-up should reject a shallow duplicate answer",
			priorQuestion: "How would you partition a multi-tenant analytics system?",
			followUpQuestion: "What are the tradeoffs?",
			response: {
		...
		},
		{
			...
			family: "system_design",
			description:
				"Metric probe should accept an answer with scale and measurement content",
			priorQuestion: "How would you partition a multi-tenant analytics system?",
			followUpQuestion: "What metrics would you watch first?",
			response: {
				...buildBaselineResponse(),
				scaleConsiderations: [
					"I would watch p95 latency, hot-tenant skew, and queue lag first",
				],
			},
			expected: "accept",
		},
		{
			...
			family: "system_design",
			description:
				"Metric probe should reject an answer with no measurement detail",
			priorQuestion: "How would you partition a multi-tenant analytics system?",
			followUpQuestion: "What metrics would you watch first?",
			response: {
				...buildBaselineResponse(),
		...
		},
		{
			...
			family: "behavioral",
			description:
				"Behavioral follow-up should accept a grounded STAR story with strong action depth",
		...
		},
		{
			...
			family: "behavioral",
			description:
				"Behavioral follow-up should reject weak STAR answers with shallow action detail",
			priorQuestion: "Tell me about a time you handled disagreement on a team.",
			followUpQuestion:
				"Tell me about a time you handled disagreement on a team.",
			response: buildBehavioralResponse({
		...
		},
		{
			...
			family: "provenance",
			description:
				"Unsupported technology claims should fail provenance even when the verifier passes",
			priorQuestion: "How would you evolve the ingestion path?",
			followUpQuestion: "Go deeper on the storage layer.",
			response: {
				...buildBaselineResponse(),
				openingReasoning:
					"I would move the write path to Cassandra to absorb the tenant spikes.",
				implementationPlan: [
					"Use Cassandra for the write path",
					"Keep Kafka for async fan-out",
				],
			},
			expected: "accept",
			expectedProvenance: "reject",
			semanticContextBlock:
				"<conscious_semantic_memory>Technologies: Redis, Kafka</conscious_semantic_memory>",
		},
		{
			id: "provenance-metric-reject",
			family: "provenance",
			description:
				"Unsupported metric claims should fail provenance even when the verifier passes",
			priorQuestion: "How would you evolve the ingestion path?",
			followUpQuestion: "Go deeper on the rollout checks.",
			response: {
				...buildBaselineResponse(),
				scaleConsiderations: [
					"I would target 10ms p99 latency immediately before broad rollout",
				],
			},
			expected: "accept",
			expectedProvenance: "reject",
			semanticContextBlock:
				"<conscious_semantic_memory>Current production baseline is 70ms p99 latency.</conscious_semantic_memory>",
		},
	];
}
...
function selectReplayContext(
	question: string,
	contextItems: ConsciousReplayContextItem[],
): ConsciousReplayContextItem[] {
	...
	return [
		{
			...
			description:
				"Reconstructs route, selected context, verifier verdict, and fallback reason for a follow-up.",
		...
		},
		{
			...
			family: "topic_shift",
			description:
				"Explicit topic shifts should reset the active thread while keeping the verifier trace visible.",
		...
		},
		{
			...
			description:
				"Deterministic continuation phrases should keep a technical thread alive for implementation follow-ups.",
			...
			activeThread: {
				...
				response: {
					...response,
					openingReasoning:
						"I would start with a durable dedupe key and replay protection.",
					implementationPlan: [
						"Generate a stable idempotency key",
						"Persist processed deliveries",
					],
					edgeCases: [
						"Retries can arrive out of order",
						"The provider can replay stale events",
					],
				},
				followUpCount: 1,
...
function summarizeResults<
	T extends { passed: boolean; scenario: { family: string } },
>(results: T[]): ConsciousEvalSummary {
...
export async function runConsciousEvalHarness(options: {
	...
	results: ConsciousEvalScenarioResult[];
	summary: ConsciousEvalSummary;
}> {
	...
	for (const scenario of scenarios) {
		...
		const route: ConsciousModeQuestionRoute = {
			qualifies: true,
		...
		const verifierPassed =
			scenario.expected === "accept" ? verdict.ok : !verdict.ok;
		const provenancePassed =
			expectedProvenance === "accept"
...
export async function runConsciousReplayHarness(options: {
	...
	results: ConsciousReplayScenarioResult[];
	summary: ConsciousEvalSummary;
}> {
	...
	for (const scenario of scenarios) {
		const route = classifyConsciousModeQuestion(
			scenario.question,
			scenario.activeThread,
		);
		const selectedContext = selectReplayContext(
			scenario.question,
			scenario.contextItems,
		);
		let verifierVerdict: { ok: boolean; reason?: string } = {
			ok: false,
			reason: "route_not_qualified",
		};
		let fallbackReason: ConsciousReplayTrace["fallbackReason"] =
			"route_not_qualified";
		...
		if (route.qualifies) {
			...
			if (scenario.activeThread) {
				store.recordStructuredSuggestion(
					scenario.activeThread.rootQuestion,
					scenario.activeThread.response,
					"start",
				);
			}
			const reaction = classifier.classify({
			...
			fallbackReason = verifierVerdict.ok
				? undefined
				: "conscious_verification_failed";
		}
		...
		const passed =
			route.qualifies === scenario.expected.route.qualifies &&
...
export function getDefaultConsciousE2EScenarios(): ConsciousE2EScenario[] {
	...
	return [
		...
		},
		{
			...
			description:
				"When circuit is open, conscious route falls back to standard",
...
export async function runConsciousE2EHarness(options: {
	...
	for (const scenario of scenarios) {
		...
		const effectiveRouteTag = !scenario.consciousModeEnabled
			? "skip"
