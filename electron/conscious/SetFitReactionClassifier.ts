...
export class SetFitReactionClassifier {
  ...
  private async ensureModelLoaded(): Promise<void> {
    ...
    if (this.modelLoadError) {
      throw new Error('Embedding model failed to load, SetFit classifier disabled');
    }
  ...
  private async cachePrototypeEmbeddings(): Promise<void> {
    ...
    for (const [kind, examples] of Object.entries(SetFitReactionClassifier.PROTOTYPES)) {
  ...
  async classify(question: string): Promise<ClassificationResult | null> {
    try {
      ...
      for (const [kind, prototypeEmbeddings] of this.prototypeEmbeddings.entries()) {
        const avgSimilarity = this.computeAverageSimilarity(questionVector, prototypeEmbeddings);
  ...
  private computeAverageSimilarity(vector: number[], prototypeEmbeddings: number[][]): number {
