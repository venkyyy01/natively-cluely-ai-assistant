...
export class ConsciousThreadStore {
	private latestConsciousResponse: ConsciousModeStructuredResponse | null =
		null;
	...
	handleObservedInterviewerTranscript(
		...
		const matchingThread = this.threadManager.findMatchingThread(
			normalized,
			phase,
		);
		...
		if (
			!activeThread &&
			matchingThread &&
			matchingThread.confidence.total >= RESUME_THRESHOLD
		) {
	...
	restorePersistenceSnapshot(
		snapshot: PersistedConsciousThreadState | null | undefined,
	): void {
		if (!snapshot) {
			this.latestConsciousResponse = null;
			this.activeReasoningThread = null;
