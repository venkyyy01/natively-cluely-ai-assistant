...
export function sanitizeProfileData(
  ...
  const sanitizeString = (value: string, path: string): string => {
    const withoutControls = value.replace(CONTROL_CHARS, ' ').replace(/\s+/g, ' ').trim();
    ...
    const retainedLines = lines.filter((line) => {
      const isInjection = PROMPT_INJECTION_PATTERNS.some((pattern) => pattern.test(line));
      if (isInjection) {
  ...
  const visit = (value: unknown, path: string): unknown => {
    ...
    for (const [key, item] of entries.slice(0, maxObjectKeys)) {
      ...
      const sanitizedValue = visit(item, path ? `${path}.${sanitizedKey}` : sanitizedKey);
