  ...
  private detectPhaseOnCurrentThread(
    ...
    for (const signal of PHASE_SIGNALS) {
      ...
      if (keywordMatches.length > 0) {
        const keywordScore = Math.min(0.35, 0.15 + (keywordMatches.length / signal.keywords.length) * 0.20);
        score += keywordScore;
      ...
      if (patternMatches.length > 0) {
        const patternScore = Math.min(0.45, 0.25 + (patternMatches.length / signal.patterns.length) * 0.20);
        score += patternScore;
