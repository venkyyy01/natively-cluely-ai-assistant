...
export class ObservedQuestionStore {
  ...
  getLastQuestion(): string | null {
    return this.questions.length > 0 ? this.questions[this.questions.length - 1].text : null;
  }
  ...
  isLikelyGeneralIntent(lastInterviewerTurn: string | null = this.getLastQuestion()): boolean {
