...
import {
	BayesianVerifierAggregator,
	type VerifierResult,
} from "./BayesianVerifierAggregator";
...
function hasBehavioralImpactCue(text: string): boolean {
	return /(\b\d+(?:\.\d+)?(?:ms|s|x|%|k|m|b)?\b|improv|reduc|increas|decreas|saved|faster|slower|stabil|unblock|delivered|shipped|adopt|retention|latency|throughput|quality|incident|customer|user|team|process|runbook|checklist|learned|next time|would do differently)/i.test(
		text,
	);
}
...
function hasCompleteBehavioralStar(
	response: ConsciousModeStructuredResponse,
): boolean {
...
function hasStrongBehavioralDepth(
	response: ConsciousModeStructuredResponse,
): boolean {
	const behavioral = response.behavioralAnswer;
	if (!behavioral) {
	...
	return (
		...
		actionWords >=
			situationWords + BEHAVIORAL_DEPTH_RULES.minActionAdvantageWords &&
...
function isInferredDominantEvidence(
	evidence: Array<"suggested" | "inferred"> | null | undefined,
): boolean {
...
function hasUnsupportedNumericClaim(
	...
	const useTighterRegex = isVerifierOptimizationActive(
	...
	const useWordBoundary = isVerifierOptimizationActive(
...
function hasUnsupportedTechnologyClaim(
	responseText: string,
	groundingText: string,
): boolean {
	const useExpandedAllowlist = isVerifierOptimizationActive(
...
export class ConsciousVerifier {
		...
		try {
			...
			if (!judgeVerdict) {
				...
				return this.options.requireJudge
					...
					: {
							...ruleVerdict,
							judge: "skipped",
							reason: ruleVerdict.reason ?? "judge_unavailable",
						};
			}
			...
			return this.options.requireJudge
				...
				: {
						...ruleVerdict,
						judge: "skipped",
						reason: ruleVerdict.reason ?? "judge_execution_failed",
					};
		}
	}
	...
	private logVerification(
		input: ConsciousVerifierJudgeInput,
		result: ConsciousVerificationResult,
	...
	private async verifyWithBayesianAggregation(
		...
		results.push(
			BayesianVerifierAggregator.deterministicResult(
				ruleVerdict.ok,
		...
		if (input.provenanceResult) {
			results.push(
				BayesianVerifierAggregator.provenanceResult(
		...
		if (!input.skipJudge && this.judge) {
			try {
				...
				if (judgeVerdict) {
					results.push(
						BayesianVerifierAggregator.judgeResult(
							judgeVerdict.ok,
		...
		return {
			...
			judge: results.find((r) => r.name === "judge")?.passed
				? "pass"
				: "skipped",
	...
	private verifyRuleSet(
		...
		if (
			isBehavioralQuestion(input.question) &&
			!hasCompleteBehavioralStar(input.response)
		) {
			return {
				ok: false,
				reason: "missing_behavioral_star_structure",
				deterministic: "fail",
				judge: "skipped",
			};
		}
		...
		if (
			isBehavioralQuestion(input.question) &&
			!hasStrongBehavioralDepth(input.response)
		) {
			return {
				ok: false,
				reason: "weak_behavioral_depth",
				deterministic: "fail",
				judge: "skipped",
			};
		}
		...
		if (
			reaction?.kind === "tradeoff_probe" &&
			input.response.tradeoffs.length === 0 &&
			input.response.pushbackResponses.length === 0
		) {
			return {
				ok: false,
				reason: "missing_tradeoff_content",
				deterministic: "fail",
				judge: "skipped",
			};
		}
		...
		if (
			reaction?.kind === "metric_probe" &&
			input.response.scaleConsiderations.length === 0 &&
			!/metric|latency|throughput|monitor|measure|slo|sla/.test(responseText)
		) {
			return {
				ok: false,
				reason: "missing_metric_content",
				deterministic: "fail",
				judge: "skipped",
			};
		}
		...
		if (
			reaction?.kind === "challenge" &&
			input.response.pushbackResponses.length === 0 &&
			input.response.tradeoffs.length === 0
		) {
			return {
				ok: false,
				reason: "missing_defense_content",
				deterministic: "fail",
				judge: "skipped",
			};
		}
		...
		if (
			reaction?.kind === "deep_dive" &&
			input.response.edgeCases.length === 0 &&
			input.response.scaleConsiderations.length === 0 &&
			input.response.implementationPlan.length === 0
		) {
			return {
				ok: false,
				reason: "missing_depth_content",
				deterministic: "fail",
				judge: "skipped",
			};
		}
		...
		if (
			reaction?.kind === "example_request" &&
			input.response.implementationPlan.length === 0 &&
			!/example|for instance|for example/.test(responseText)
		) {
			return {
				ok: false,
				reason: "missing_example_content",
				deterministic: "fail",
				judge: "skipped",
			};
		}
		...
		if (
			...
			hypothesis?.latestSuggestedAnswer &&
			responseText.trim() ===
				hypothesis.latestSuggestedAnswer.trim().toLowerCase()
		) {
			return {
				ok: false,
				reason: "duplicate_follow_up_response",
				deterministic: "fail",
				judge: "skipped",
			};
		}
		...
		if (isInferredDominantEvidence(evidence)) {
			...
			if (hasUnsupportedNumericClaim(responseText, groundingText)) {
				return {
					ok: false,
					reason: "unsupported_numeric_claim_in_inferred_state",
					deterministic: "fail",
					judge: "skipped",
				};
			}
			if (hasUnsupportedTechnologyClaim(responseText, groundingText)) {
				return {
					ok: false,
					reason: "unsupported_technology_claim_in_inferred_state",
					deterministic: "fail",
					judge: "skipped",
				};
			}
