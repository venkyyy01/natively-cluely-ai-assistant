...
const CONSTRAINT_PATTERNS: Record<ConstraintType, RegExp> = {
  budget: /\$[\d,]+(?:\.\d{2})?(?:\s?[kmb])?|\b\d+(?:\.\d+)?\s?(?:thousand|million|billion|k|m|b)\s?(?:usd|dollars?)?\b/gi,
  deadline: /(?:by|before|until|due)\s+(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?|\b\d{1,2}\/\d{1,2}(?:\/\d{2,4})?\b/gi,
  headcount: /\b\d+\s*(?:people|engineers?|developers?|designers?|employees?|team members?|ftes?|headcount)\b/gi,
  duration: /\b\d+\s*(?:weeks?|months?|quarters?|sprints?|days?)\b/gi,
  percentage: /\b\d+(?:\.\d+)?%|\b\d+(?:\.\d+)?\s*percent\b/gi,
  count: /\b\d+\s*(?:features?|requirements?|milestones?|deliverables?|items?)\b/gi,
};

function normalizeBudget(raw: string): string {
  const value = raw.trim().toLowerCase().replace(/,/g, '');

  const match = value.match(/\$?(\d+(?:\.\d+)?)(?:\s?([kmb])|\s?(thousand|million|billion))?/i);
...
export function extractConstraints(text: string): ExtractedConstraint[] {
  ...
  for (const [type, pattern] of Object.entries(CONSTRAINT_PATTERNS) as [ConstraintType, RegExp][]) {
