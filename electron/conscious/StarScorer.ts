...
export class StarScorer {
  private static readonly ACTION_VERBS = new Set([
    ...
    'engineered', 'deployed', 'launched', 'shipped', 'delivered', 'released',
    'migrated', 'refactored', 'optimized', 'scaled', 'improved', 'enhanced',
    'fixed', 'resolved', 'debugged', 'troubleshooted', 'analyzed', 'investigated',
    'led', 'managed', 'coordinated', 'orchestrated', 'spearheaded', 'drove',
    'wrote', 'coded', 'programmed', 'scripted', 'automated', 'integrated',
    'configured', 'setup', 'installed', 'maintained', 'monitored', 'tested',
  ]);

  private static readonly IMPACT_CUES = /\b(\d+(?:\.\d+)?(?:ms|s|x|%|k|m|b)?|improv|reduc|increas|decreas|saved|faster|slower|stabil|unblock|delivered|shipped|adopt|retention|latency|throughput|quality|incident|customer|user|team|process|runbook|checklist|learned|next time|would do differently)\b/i;
  ...
  private extractFeatures(response: ConsciousModeStructuredResponse): StarFeatures {
    const behavioral = response.behavioralAnswer!;
    ...
    const actionToSituationRatio = situationWordCount > 0 
      ? actionWordCount / situationWordCount 
      : actionWordCount > 0 ? 2 : 0;
    
    const actionToTaskRatio = taskWordCount > 0 
  ...
  private computeOverallScore(features: StarFeatures): number {
    ...
    const normalizedActionWordCount = Math.min(1, features.actionWordCount / 20);
    const normalizedResultWordCount = Math.min(1, features.resultWordCount / 15);
    const normalizedActionToSituationRatio = Math.min(1, features.actionToSituationRatio / 3);
    const normalizedActionToTaskRatio = Math.min(1, features.actionToTaskRatio / 3);
    ...
    const weightedSum = 
      ...
      normalizedActionToSituationRatio * StarScorer.FEATURE_WEIGHTS.actionToSituationRatio +
      normalizedActionToTaskRatio * StarScorer.FEATURE_WEIGHTS.actionToTaskRatio +
