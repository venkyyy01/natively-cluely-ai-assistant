...
export class ConfidenceScorer {
	...
	calculateResumeConfidence(
		...
		const hasExplicitMarker = EXPLICIT_RESUME_MARKERS.some((p) =>
			p.test(transcript),
		);
		...
		const recentInterruption =
			thread.interruptedBy &&
		...
		const embeddingScore =
			thread.embedding && queryEmbedding
		...
		const useCalibration = isVerifierOptimizationActive(
			"useConfidenceCalibration",
		);
		const calibratedTotal =
			useCalibration && this.calibrator
	...
	private calculateBM25(
		...
		for (const term of queryTerms) {
			...
			const tf = docKeywordsLower.filter(
