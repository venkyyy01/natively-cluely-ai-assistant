...
export class ConsciousAccelerationOrchestrator {
	...
	private lastPauseDecision: {
		action: PauseAction;
		confidence: PauseConfidence;
		at: number;
	} | null = null;
	...
	private prefetchedIntents = new Map<
		string,
		{ intent: IntentResult; fetchedAt: number }
	>();
	...
	constructor(options: ConsciousAccelerationOptions = {}) {
		...
		this.pauseThresholdTuner = new PauseThresholdTuner(
			this.pauseDetector.getConfig(),
		);
		this.pauseDetector.setActionHandler((action: PauseAction) => {
			...
			if (this.classifierLane) {
				void this.classifierLane
					...
					.catch((error: unknown) => {
						console.error(
							"[ConsciousAccelerationOrchestrator] Semantic classifier lane rejected pause action:",
							error,
						);
					});
				return;
	...
	private async handlePauseAction(action: PauseAction): Promise<void> {
		...
		if (
			(action === "hard_speculate" || action === "commit") &&
			isOptimizationActive("usePrefetching")
		) {
	...
	setIntentClassifier(
		classifier:
			| ((query: string, transcriptRevision: number) => Promise<IntentResult>)
			| null,
	): void {
	...
	updateTranscriptSegments(
		segments: Array<{ text: string; timestamp: number; speaker: string }>,
		transcriptRevision?: number,
	): void {
		if (!this.enabled) {
			return;
	...
	onSilenceStart(transcript?: string): void {
		...
		if (this.classifierLane) {
			void this.classifierLane
				...
				.catch((error: unknown) => {
					console.error(
						"[ConsciousAccelerationOrchestrator] Semantic classifier lane rejected silence evaluation:",
						error,
					);
				});
			return;
	...
	onUserSpeaking(transcript?: string): void {
		...
		if (
			this.lastPauseDecision &&
			(this.lastPauseDecision.action === "hard_speculate" ||
				this.lastPauseDecision.action === "commit")
		) {
			const resumedAfterMs = Date.now() - this.lastPauseDecision.at;
			if (resumedAfterMs <= 1500) {
				this.pauseThresholdTuner.recordFalsePositiveResume(
					this.lastPauseDecision.action,
					this.lastPauseDecision.confidence,
				);
				this.pauseDetector.updateConfig(this.pauseThresholdTuner.getConfig());
	...
	async getSpeculativeAnswer(
		query: string,
		transcriptRevision: number,
		waitMs: number = 0,
	): Promise<string | null> {
		const preview = await this.getSpeculativeAnswerPreview(
			query,
			transcriptRevision,
			waitMs,
		);
	...
	getPrefetchedIntent(
		...
		if (
			...
			ConsciousAccelerationOrchestrator.PREFETCHED_INTENT_TTL_MS
		) {
	...
	private abortInflightIntentPrefetch(): void {
		if (this.intentPrefetchAbortController) {
			this.intentPrefetchAbortController.abort(
				new Error("intent_prefetch_cancelled"),
			);
			this.intentPrefetchAbortController = null;
		}
	}

	private isSpeculativeEntryStale(
		...
		if (
			typeof expectedTranscriptRevision === "number" &&
			entry.transcriptRevision !== expectedTranscriptRevision
		) {
			return true;
		}
	...
	private async deriveSpeculativeCandidates(): Promise<
		Array<{ query: string; transcriptRevision: number; embedding: number[] }>
	> {
		...
		return Promise.all(
			candidateQueries.map(async (candidate) => ({
	...
	private async selectSpeculativeEntry(
		query: string,
		transcriptRevision: number,
	): Promise<SpeculativeAnswerEntry | null> {
		const entries = Array.from(this.speculativeAnswerEntries.values()).filter(
			(entry) => entry.transcriptRevision === transcriptRevision,
		);
		if (entries.length === 0) {
			return null;
		}
		...
		const exact = entries.find(
			(entry) =>
				this.normalizeQuery(entry.query) === this.normalizeQuery(query),
		);
		if (exact) {
			return exact;
		}
		...
		if (useFuzzy) {
			...
			if (bestMatch) {
				console.log(
					`[ConsciousAccelerationOrchestrator] Fuzzy match found: "${query}" -> "${bestMatch.query}" (similarity: ${bestSimilarity.toFixed(3)})`,
				);
				return bestMatch;
			}
		}
	...
	private async maybeStartSpeculativeAnswer(): Promise<void> {
		...
		const prefetchedIntent = this.getPrefetchedIntent(
			latestQuery,
			this.latestTranscriptRevision,
		);
		...
		for (const candidate of candidates) {
			const key = this.buildSpeculativeKey(
				candidate.query,
			...
			entry.completionPromise = (async (): Promise<string | null> => {
				try {
					...
					for await (const chunk of stream) {
						...
						if (
							generation !== this.speculativeGeneration ||
							candidate.transcriptRevision !== this.latestTranscriptRevision
						) {
					...
					if (!abortController.signal.aborted) {
						console.warn(
							"[ConsciousAccelerationOrchestrator] Speculative answer generation failed:",
							error,
						);
					}
					entry.completed = true;
	...
	private evictStaleSpeculativeEntries(): void {
		if (
			this.speculativeAnswerEntries.size <=
			ConsciousAccelerationOrchestrator.MAX_SPECULATIVE_ENTRIES
		) {
			return;
		}

		const entries = Array.from(this.speculativeAnswerEntries.values()).sort(
	...
	private async maybePrefetchIntent(): Promise<void> {
		...
		const promise = (async (): Promise<void> => {
			try {
				...
				if (!abortController.signal.aborted) {
					console.warn(
						"[ConsciousAccelerationOrchestrator] Intent preclassification failed:",
						error,
					);
				}
			} finally {
	...
	async getSpeculativeAnswerPreview(
		query: string,
		transcriptRevision: number,
		waitMs: number = 0,
	): Promise<SpeculativeAnswerPreview | null> {
		if (!this.enabled) {
			return null;
		}
		...
		if (!entry.completed && waitMs > 0) {
			...
			await Promise.race([
				...
				new Promise<symbol>((resolve) =>
					setTimeout(() => resolve(timeoutSentinel), waitMs),
				),
			]);
		}
	...
	async finalizeSpeculativeAnswer(
		...
		if (
			typeof expectedCommitToken === "number" &&
			entry.commitToken !== expectedCommitToken
		) {
			return null;
		}
		...
		if (resolved && this.lastPauseDecision) {
			this.pauseThresholdTuner.recordSuccessfulReuse(
				this.lastPauseDecision.confidence,
			);
			this.pauseDetector.updateConfig(this.pauseThresholdTuner.getConfig());
			this.lastPauseDecision = null;
	...
	private invalidateSpeculation(recordOutcome: boolean): void {
		if (
			recordOutcome &&
			this.enabled &&
			this.speculativeAnswerEntries.size > 0
		) {
