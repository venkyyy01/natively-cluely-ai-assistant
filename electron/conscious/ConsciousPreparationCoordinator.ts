...
import {
	...
	detectConsciousQuestionMode,
} from "./ConsciousAnswerPlanner";
import type { ConsciousContextComposer } from "./ConsciousContextComposer";
import type {
...
export interface ConsciousPreparationResult {
	...
	temporalContext: ReturnType<
		ConsciousContextComposer["compose"]
	>["temporalContext"];
...
export class ConsciousPreparationCoordinator {
	...
	constructor(
		...
		this.tokenBudgetManager =
			tokenBudgetManager ?? new TokenBudgetManager("openai");
	}
	...
	private buildEvidenceContextBlock(input: {
		...
		const preferenceBlock = this.session.getConsciousResponsePreferenceContext(
			input.answerPlan.questionMode,
		);
		...
		const sorted = [...blocksWithTokens].sort(
	...
	private shouldRebuildPreparedTranscript(
		currentPlan: ConsciousAnswerPlan,
		nextPlan: ConsciousAnswerPlan,
	): boolean {
		return (
			currentPlan.questionMode !== nextPlan.questionMode ||
	...
	async prepareRoute(input: {
		...
		coordinator?:
			| import("../llm/providers/IntentClassificationCoordinator").IntentClassificationCoordinator
			| null;
		...
		const shouldUseScreenshotConsciousRoute =
	...
	async prepareReasoningContext(input: {
		...
		if (input.resolvedQuestion && this.session.isConsciousModeEnabled()) {
			try {
				if (input.useConsciousAcceleration) {
					const assembledContext = await input.getAssembledContext(
						input.resolvedQuestion,
						input.tokenBudget,
					);
					contextItems = assembledContext.contextItems;
				} else {
					contextItems = await input.getConsciousRelevantContext(
						input.resolvedQuestion,
						input.tokenBudget,
					);
				}
				...
				console.warn(
					"[ConsciousPreparation] Context retrieval failed, using fallback context items:",
					error,
				);
			}
		}
		...
		if (sanitizedProfile.warnings.length > 0) {
			console.warn(
				"[ConsciousPreparation] Profile data sanitized before prompt use:",
				{
		...
		const planningQuestion =
			input.resolvedQuestion || input.lastInterviewerTurn || "";
		const stateBlock =
			this.retrievalOrchestrator.buildStateBlock(planningQuestion);
		...
		const longMemoryBlock =
			this.session.getConsciousLongMemoryContext(planningQuestion);
		const reaction = this.session.getLatestQuestionReaction();
		const hypothesis = this.session.getLatestAnswerHypothesis();
		let preferenceSummary = this.session.getConsciousResponsePreferenceSummary(
			detectConsciousQuestionMode(planningQuestion),
		);
		...
		if (
			answerPlan.questionMode !== detectConsciousQuestionMode(planningQuestion)
		) {
			preferenceSummary = this.session.getConsciousResponsePreferenceSummary(
				answerPlan.questionMode,
			);
		...
		const { intentResult, totalContextAssemblyMs, timedOut } =
			await this.consciousIntentService.resolve({
				...
				isLikelyGeneralIntent: this.session.isLikelyGeneralIntent(
		...
		let resolvedPreferenceSummary =
			this.session.getConsciousResponsePreferenceSummary(
				answerPlan.questionMode,
			);
		...
		if (resolvedAnswerPlan.questionMode !== answerPlan.questionMode) {
			resolvedPreferenceSummary =
				this.session.getConsciousResponsePreferenceSummary(
					resolvedAnswerPlan.questionMode,
				);
