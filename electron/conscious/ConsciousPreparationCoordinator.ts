...
export class ConsciousPreparationCoordinator {
	...
	async prepareReasoningContext(input: {
		...
		const preferenceSummary =
			this.session.getConsciousResponsePreferenceSummary(questionMode);
		...
		const estimatedEvidenceTokens = evidenceBlocks
			.join(" ")
			.split(/\s+/).length;
		...
		if (estimatedEvidenceTokens > SOFT_TOKEN_BUDGET) {
			const trimmed = evidenceBlocks
				.map((block) =>
					block
						.split(/\s+/)
						.slice(0, Math.floor(SOFT_TOKEN_BUDGET / evidenceBlocks.length))
						.join(" "),
				)
				.filter(Boolean);
			console.warn(
				`[ConsciousPreparation] Evidence context exceeded soft token budget (${estimatedEvidenceTokens} > ${SOFT_TOKEN_BUDGET}). Trimming blocks proportionally.`,
			);
