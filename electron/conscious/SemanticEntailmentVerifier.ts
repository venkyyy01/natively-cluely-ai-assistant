...
export class SemanticEntailmentVerifier {
  ...
  private async ensureModelLoaded(): Promise<void> {
    ...
    if (this.modelLoadError) {
      throw new Error('NLI model failed to load, semantic entailment verification disabled');
    }
   ...
   */
  async checkEntailment(claim: string, groundingText: string): Promise<EntailmentResult> {
    try {
      ...
      const truncatedClaim = this.truncateToTokens(claim, SemanticEntailmentVerifier.MAX_TOKENS / 2);
      const truncatedGrounding = this.truncateToTokens(groundingText, SemanticEntailmentVerifier.MAX_TOKENS / 2);
      ...
      const labelMap: Record<string, 'entailment' | 'contradiction' | 'neutral'> = {
      ...
      console.warn('[SemanticEntailmentVerifier] Entailment check failed:', error);
   ...
   */
  async verifyTermSemantically(term: string, groundingText: string): Promise<boolean> {
    ...
    if (result.label === 'entailment' && result.score >= SemanticEntailmentVerifier.ENTAILMENT_THRESHOLD) {
      return true;
    }
   ...
   */
  async verifyTermsSemantically(terms: string[], groundingText: string): Promise<Map<string, boolean>> {
    const results = new Map<string, boolean>();

    for (const term of terms) {
      const isSupported = await this.verifyTermSemantically(term, groundingText);
