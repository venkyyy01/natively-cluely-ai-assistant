...
export class ConsciousOrchestrator {
	...
	private isDeterministicContinuationPhrase(loweredQuestion: string): boolean {
		return (
			/^(what are the tradeoffs\??|how would you shard this\??|what happens during failover\??|what metrics would you watch( first)?\??)$/i.test(
				loweredQuestion,
			) ||
	...
	private isTopicallyCompatibleWithThread(
		question: string,
	...
	prepareRoute(input: {
		...
		if (
			...
			!this.isTopicallyCompatibleWithThread(
				input.question,
				currentReasoningThread,
			)
		) {
