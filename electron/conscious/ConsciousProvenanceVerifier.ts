...
const TECH_NOUN_PATTERN =
	/(?:database|db|queue|cache|broker|stream|topic|cluster|index|vector|service|api|sdk|framework|library|runtime|warehouse|search|engine|store|model|provider|platform|orchestrator)s?/i;
...
function extractDynamicTechnologyCandidates(text: string): string[] {
	...
	const techSuffixPattern =
		/\b[A-Z][A-Za-z0-9.+#-]*(?:DB|SQL|JS|API|SDK|QL|ML|AI)\b/g;
	const beforeTechNounPattern =
		/\b([A-Z][A-Za-z0-9.+#-]{2,})\s+(?:database|db|queue|cache|broker|stream|topic|cluster|index|vector|service|api|sdk|framework|library|runtime|warehouse|search|engine|store|model|provider|platform)\b/g;
	const afterTechVerbPattern =
		/\b(?:use|using|used|with|via|on|into|through|choose|choosing|pick|picking|migrate to|integrate with)\s+([A-Z][A-Za-z0-9.+#-]{2,})\b/g;
	const lowercaseAfterTechVerbPattern =
		/\b(?:use|using|used|with|via|on|into|through|choose|choosing|pick|picking|migrate to|integrate with)\s+([a-z][a-z0-9.+#-]{2,})\b/g;
	...
	for (const match of text.matchAll(afterTechVerbPattern)) {
		...
		const after = text.slice(
			...
			(match.index ?? 0) + match[0].length + 48,
		);
		if (
			TECH_NOUN_PATTERN.test(after) ||
			/^[A-Z0-9-]+$/.test(term) ||
			/[A-Z].*[A-Z]/.test(term)
		) {
...
function extractGroundingTechnologyVocabulary(...contexts: string[]): string[] {
	...
	const technologyListPattern =
		/\b(?:technologies|technology|stack|tooling|providers?)\s*:\s*([^\n]+)/gi;
	const beforeTechNounPattern =
		/\b([a-z][a-z0-9.+#-]{2,})\s+(?:database|db|queue|cache|broker|stream|topic|cluster|index|vector|service|api|sdk|framework|library|runtime|warehouse|search|engine|store|model|provider|platform|orchestrator)s?\b/gi;
	const afterTechVerbPattern =
		/\b(?:using|used|with|via|on|through|choose|choosing|pick|picking|integrate with|migrate to)\s+([a-z][a-z0-9.+#-]{2,})\b/gi;
...
function extractNumbers(text: string): string[] {
	return Array.from(
		new Set(
			(text.match(/\b\d+(?:\.\d+)?(?:ms|s|x|%|k|m|b)?\b/gi) || []).map(
...
export class ConsciousProvenanceVerifier {
	...
	private normalizeGroundingContext(input: {
		...
		const semanticContext = (input.semanticContextBlock || "")
			.trim()
			.toLowerCase();
		const evidenceContext = (input.evidenceContextBlock || "")
			.trim()
			.toLowerCase();
	...
	private async findUnsupportedTerms(
		terms: string[],
		...
		for (const term of terms) {
			...
			if (useSemantic) {
				try {
					const isSemanticallySupported =
						await this.semanticEntailmentVerifier.verifyTermSemantically(
							term,
							strictContext,
						);
					...
					console.warn(
						"[ConsciousProvenanceVerifier] Semantic entailment check failed, falling back to unsupported:",
						error,
					);
	 ...
	 */
	private responseHasTechnologyOrMetricClaim(
		response: ConsciousModeStructuredResponse,
	): boolean {
	...
	async verify(input: {
		...
		if (useRAG && input.transcriptIndex) {
			...
			if (ragResults.length > 0) {
				...
				expandedSemanticContext = [expandedSemanticContext, ragContext]
					.filter(Boolean)
					.join(" ");
				console.log(
					`[ConsciousProvenanceVerifier] RAG expanded grounding with ${ragResults.length} segments`,
				);
			}
		}
		...
		if (!hasStrictGroundingContext) {
			...
			if (this.responseHasTechnologyOrMetricClaim(input.response)) {
				console.log(
					"[ConsciousProvenanceVerifier] No grounding context available; passing through technology/metric claims (unverifiable, not rejected)",
				);
			}
			return { ok: true };
		}
