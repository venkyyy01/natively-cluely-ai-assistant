...
const CLARIFICATION_PATTERNS: PatternMatch[] = [
  { pattern: /^\s*(sorry|pardon|excuse me)[,.\s]+(what|come again|can you repeat)/i },
  { pattern: /^\s*(what (do you|did you|does that|did that) mean|what does that mean)/i },
  { pattern: /^\s*(can you (clarify|explain|elaborate|repeat)|could you (clarify|explain|repeat))/i },
  { pattern: /^\s*(i'?m not sure (what|i)|i don'?t (understand|get|follow))\b/i },
  { pattern: /^\s*(wait,?\s*(what|hold on|sorry)|huh\??|come again\??)\b/i },
  { pattern: /^\s*(so (what )?you'?re saying|so that means|so basically you mean)/i },
...
const REFINEMENT_PATTERNS: PatternMatch[] = [
  { pattern: /\b(make it (shorter|brief|concise)|shorten (this|that|it)|cut it down|tldr|too long)\b/i, refinementIntent: 'shorten' },
...
const ACKNOWLEDGEMENT_PATTERNS: PatternMatch[] = [
  { pattern: /^\s*(got it|makes sense|i see|understood|ok(ay)?|alright|fair enough)\b[\s!.,?]*$/i },
  { pattern: /^\s*(yeah|yes|right|exactly|that'?s right|true|correct)\b[\s!.,?]*$/i },
  { pattern: /^\s*(no|nope|not really|not quite|i don'?t think so)\b[\s!.,?]*$/i },
  { pattern: /^\s*(interesting|cool|nice|good (point|to know)|fascinating|wow|huh)\b[\s!.,?]*$/i },
  { pattern: /^\s*(mm-?hmm|uh-?huh|hmm|oh)\b[\s!.,?]*$/i },
];

const OFF_TOPIC_ASIDE_PATTERNS: PatternMatch[] = [
  { pattern: /^\s*(by the way|btw|side note|on a (different|separate) note|quick (question|aside)|random question)/i },
  { pattern: /^\s*(actually,?\s*(can|let me|i)|on second thought|come to think of it)/i },
...
const TECHNICAL_KEYWORDS = [
  'system design', 'architecture', 'algorithm', 'data structure',
  'database', 'cache', 'queue', 'api', 'sdk', 'microservice',
  'scalability', 'latency', 'throughput', 'concurrency', 'distributed',
  'tradeoff', 'tradeoffs', 'optimize', 'optimization', 'performance',
  'redis', 'kafka', 'postgres', 'mysql', 'mongodb', 'kubernetes',
  'docker', 'aws', 'gcp', 'azure', 's3', 'lambda',
  'time complexity', 'space complexity', 'big o',
  'leetcode', 'implement', 'function', 'class', 'method',
];

const BEHAVIORAL_INDICATORS = [
  'tell me about a time', 'describe a situation', 'walk me through',
  'have you ever', 'how did you handle', 'how would you handle',
  'give me an example of', 'what was your role', 'a project where',
];
