...
export class ThreadManager {
	...
	findMatchingThread(
		...
		const effectiveEmbedding =
			transcriptEmbedding || this.buildPseudoEmbedding(transcript);
	...
	restoreSuspendedThreads(
		snapshots: SuspendedThreadSnapshot[] | null | undefined,
	): void {
		...
		for (const snapshot of snapshots) {
			...
			const topic =
				typeof snapshot.topic === "string" && snapshot.topic.trim()
					? snapshot.topic.trim()
					: "Suspended discussion";
			const goal =
				typeof snapshot.goal === "string" && snapshot.goal.trim()
					? snapshot.goal.trim()
					: `Discuss ${topic}`;
			...
			const resumeKeywords = Array.from(
				new Set([
					...
					...(snapshot.resumeKeywords ?? []).filter(
						(keyword) =>
			...
			restored.push({
				...
				tokenCount: 0,
				resumeCount: 0,
				embedding: this.buildPseudoEmbedding(
					`${topic} ${goal} ${resumeKeywords.join(" ")}`,
				),
			});
		}
