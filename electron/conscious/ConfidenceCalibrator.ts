...
export class IsotonicCalibrator {
  ...
  fit(samples: TrainingSample[]): void {
    ...
    for (let i = 0; i < binCount; i++) {
      ...
      const avgRaw = binSamples.reduce((sum, s) => sum + s.rawScore, 0) / binSamples.length;
  ...
  private poolBins(a: CalibrationBin, b: CalibrationBin): CalibrationBin {
    ...
    const weightedCalibrated = (a.calibrated * a.count + b.calibrated * b.count) / totalCount;
  ...
  persist(profileId: string): void {
    try {
      ...
      console.warn('[IsotonicCalibrator] Failed to persist calibration data:', error);
    }
  }
  ...
  static load(profileId: string): IsotonicCalibrator {
    ...
    try {
      const filePath = path.join(os.homedir(), '.nately', 'calibration', `${profileId}.json`);
      ...
      console.warn('[IsotonicCalibrator] Failed to load calibration data:', error);
    }
