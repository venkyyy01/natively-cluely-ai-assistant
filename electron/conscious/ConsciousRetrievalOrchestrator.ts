...
interface ConsciousRetrievalSession {
	...
	getContext?(lastSeconds: number): Array<{
...
function isFallbackLikeAssistantText(value: string): boolean {
	...
	return LIVE_RAG_ASSISTANT_FALLBACK_MARKERS.some((marker) =>
...
export class ConsciousRetrievalOrchestrator {
	...
	buildStateBlock(question: string): string {
		...
		if (hypothesis) {
			lines.push(
				`LIKELY_USER_ANSWER_CONFIDENCE: ${hypothesis.confidence.toFixed(2)}`,
			);
			lines.push(
				`LIKELY_USER_THEMES: ${hypothesis.likelyThemes.join(" | ") || "n/a"}`,
			);
			lines.push(
				`LIKELY_USER_ANSWER_SUMMARY: ${hypothesis.latestSuggestedAnswer || "n/a"}`,
			);
		}
		...
		if (response) {
			...
			if (response.scaleConsiderations.length > 0) {
				lines.push(
					`KNOWN_SCALE_CONSIDERATIONS: ${response.scaleConsiderations.join(" | ")}`,
				);
			}
			if (response.pushbackResponses.length > 0) {
				lines.push(
					`KNOWN_PUSHBACKS: ${response.pushbackResponses.join(" | ")}`,
				);
			}
		}
	...
	buildPack(input: {
		...
		const liveRagItems =
			input.contextItems ??
			(typeof this.session.getContext === "function"
				? this.session.getContext(lastSeconds)
				: undefined) ??
			[];
		const liveRagBlock = this.buildLiveRagBlock({
			...
			maxItems: 6,
			existingContextText: [stateDedupBlock, longMemoryBlock, evidenceBlock]
				.filter(Boolean)
				.join("\n"),
		});
		return {
	...
	buildLiveRagBlock(input: {
		...
		const now = Date.now();
		const normalizedExistingContext = normalizeComparisonText(
			input.existingContextText || "",
		);
		...
		const scored = input.contextItems
			...
			.filter(
				(item) =>
					!normalizedExistingContext.includes(
						normalizeComparisonText(item.sanitizedText),
					),
			)
			.filter(
				(item) =>
					item.role !== "assistant" ||
					!isFallbackLikeAssistantText(item.sanitizedText),
			)
			.map((item) => {
				...
				const speakerWeight =
					item.role === "interviewer" ? 1 : item.role === "user" ? 0.9 : 0.45;
				return {
					item,
