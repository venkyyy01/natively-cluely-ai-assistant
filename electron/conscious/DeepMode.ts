...
import type { ConsciousModeStructuredResponse, ConsciousBehavioralAnswer } from '../ConsciousMode';
...
export function classifyDeepModeQuestion(question: string | null | undefined): boolean {
...
export type ClaimCategory = 'technology' | 'metric' | 'experience' | 'design' | 'behavioral';
...
export function extractClaims(response: ConsciousModeStructuredResponse): Claim[] {
  const claims: Claim[] = [];

  if (response.openingReasoning) {
    claims.push({ field: 'openingReasoning', text: response.openingReasoning, category: 'design' });
  }

  for (const item of response.implementationPlan) {
    claims.push({ field: 'implementationPlan', text: item, category: 'design' });
  }
  ...
  for (const item of response.scaleConsiderations) {
    claims.push({ field: 'scaleConsiderations', text: item, category: 'metric' });
  }
  ...
  if (response.codeTransition) {
    claims.push({ field: 'codeTransition', text: response.codeTransition, category: 'technology' });
  }
...
function extractBehavioralClaims(answer: ConsciousBehavioralAnswer): Claim[] {
  const claims: Claim[] = [];
  if (answer.situation) claims.push({ field: 'behavioralAnswer.situation', text: answer.situation, category: 'behavioral' });
  if (answer.task) claims.push({ field: 'behavioralAnswer.task', text: answer.task, category: 'behavioral' });
  if (answer.action) claims.push({ field: 'behavioralAnswer.action', text: answer.action, category: 'behavioral' });
  if (answer.result) claims.push({ field: 'behavioralAnswer.result', text: answer.result, category: 'metric' });
  return claims;
}
