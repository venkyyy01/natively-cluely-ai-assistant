...
export class ConsciousRefinementOrchestrator {
  ...
  buildPrompt(input: RefinementBuildInput): RefinementPrompt {
    ...
    const previousAnswer = truncate(input.previousAnswer.trim(), MAX_PREVIOUS_ANSWER_CHARS);
    const lastQuestion = truncate((input.lastInterviewerQuestion ?? '').trim(), MAX_QUESTION_CHARS);
    ...
    userMessageParts.push(`PREVIOUS_ANSWER:\n${previousAnswer || '(no previous answer was captured)'}`);
    userMessageParts.push(`USER_REQUEST: ${userRequest}`);
    userMessageParts.push('Output ONLY the refined spoken answer. No quotes, no preamble.');
