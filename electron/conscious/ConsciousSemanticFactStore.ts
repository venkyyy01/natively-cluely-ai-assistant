...
function fallbackCategoryPriority(
	category: ConsciousSemanticFact["category"],
): number {
	switch (category) {
		case "experience":
			return 0;
...
export class ConsciousSemanticFactStore {
	...
	seedFromProfileData(profileData: any): void {
		...
		if (identity?.name || identity?.summary) {
			facts.push({
				...
				tags: tokenize(
					`${identity?.name || ""} ${identity?.role || ""} ${identity?.summary || ""}`,
				),
			});
		}
		...
		if (activeJD) {
			facts.push({
				...
				tags: tokenize(
					`${activeJD.title || ""} ${activeJD.company || ""} ${(activeJD.technologies || []).join(" ")} ${(activeJD.keywords || []).join(" ")}`,
				),
			});
	...
	getTopFacts(input: {
		...
		limit?: number;
	}): ConsciousSemanticFact[] {
		const limit = input.limit || 5;
		const tokens = tokenize(
			`${input.question} ${(input.reaction?.targetFacets || []).join(" ")} ${input.reaction?.kind || ""}`,
		);
		const scored = this.facts
			.map((fact) => ({
				...fact,
				score: overlapScore(tokens, fact.text, fact.tags),
			}))
		...
		const selected: ConsciousSemanticFact[] = scored
			.slice(0, limit)
			.map(({ score, ...fact }) => fact);
		...
		const seenTitles = new Set(
			selected.map((fact) => `${fact.category}:${fact.title}`),
		);
		const fallbackFacts = this.facts
			.filter((fact) =>
				["experience", "project", "identity"].includes(fact.category),
			)
			.sort(
				(a, b) =>
					fallbackCategoryPriority(a.category) -
					fallbackCategoryPriority(b.category),
			);
	...
	buildContextBlock(input: {
		...
		for (const fact of facts) {
			lines.push(
				`[${fact.category.toUpperCase()}] ${fact.title}: ${fact.text}`,
			);
		}
