...
function withTimeout<T>(
	...
	return Promise.race([
		...
		new Promise<null>((resolve) => {
			setTimeout(() => {
				console.log(
					`[LayeredIntentRouter] ${label} timed out after ${timeoutMs}ms`,
				);
				resolve(null);
			}, timeoutMs);
		}),
...
export class LayeredIntentRouter {
	...
	async route(input: LayeredRouterInput): Promise<LayeredRouterDecision> {
		...
		const [setFitResult, slmResult, regexResult, embeddingResult] =
			await Promise.all([
		...
		if (ensembleDecision && isReliableIntent(ensembleDecision)) {
			return {
				...
				ensemble: fastResults.map((r) => ({
					...
					confidence: r.result?.confidence ?? 0,
					used:
						r.result !== null && r.result.intent === ensembleDecision.intent,
				})),
			};
		}
	 ...
	 */
	private resolveEnsemble(
		results: FastClassifierResult[],
	): IntentResult | null {
		const validResults = results.filter(
			(r): r is FastClassifierResult & { result: IntentResult } =>
				r.result !== null,
		);
		...
		const authoritativeModelResults = validResults.filter(
			(r) =>
				(r.provider === "setfit" || r.provider === "slm") &&
				isReliableIntent(r.result),
		);
		const reliableSetFit = authoritativeModelResults.find(
			(r) => r.provider === "setfit",
		);
		const reliableSlm = authoritativeModelResults.find(
			(r) => r.provider === "slm",
		);
		...
		if (reliableSetFit && reliableSlm) {
			if (reliableSetFit.result.intent === reliableSlm.result.intent) {
				return {
					...
					confidence: Math.min(
						Math.max(
							reliableSetFit.result.confidence,
							reliableSlm.result.confidence,
						) + 0.04,
						0.96,
					),
		...
		for (const [intent, classifiers] of byIntent) {
			...
			if (strongOnes.length >= 2) {
				const avgConfidence =
					strongOnes.reduce((sum, r) => sum + r.result.confidence, 0) /
					strongOnes.length;
				return {
		...
		const highConfidence = validResults.filter(
			(r) => r.result.confidence >= 0.82,
		);
		if (highConfidence.length === 1) {
			...
			const contradiction = validResults.find(
				(r) =>
					r.result.intent !== winner.result.intent &&
		...
		const effectiveThreshold = validResults.length <= 2 ? 0.5 : 0.65;
...
export async function routeIntent(
	input: LayeredRouterInput,
): Promise<LayeredRouterDecision> {
