...
export class FollowUpQuestionsLLM {
    ...
    async *generateStream(context: string): AsyncGenerator<string> {
        try {
            ...
            console.error("[FollowUpQuestionsLLM] Stream Failed:", new LLMError(
                "Follow-up questions stream generation failed",
                e,
                { contextLength: context.length }
            ));
    ...
    async generateLegacy(context: string): Promise<string> {
        ...
        if (result.success) {
            ...
            console.error("[FollowUpQuestionsLLM] Generation failed (legacy mode):", result.error);
            return "";
        }
    }
}
