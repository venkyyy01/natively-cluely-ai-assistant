...
let slmClassifierOverrideForTesting: SlmClassifierOverrideForTesting | null =
	null;
...
const INTENT_ANSWER_SHAPES: Record<ConversationIntent, string> = {
	clarification:
		"Give a direct, focused 1-2 sentence clarification. No setup, no context-setting.",
	follow_up:
		"Continue the narrative naturally. 1-2 sentences. No recap of what was already said.",
	deep_dive:
		"Provide a structured but concise explanation. Use concrete specifics, not abstract concepts.",
	behavioral:
		"For explicit behavioral questions, answer in clear STAR with most depth in Action. For hidden behavioral questions, give one short approach statement and then one concrete example that proves it. Focus on personal actions, decision logic, outcomes, and lessons learned when relevant.",
	example_request:
		"Provide ONE concrete, detailed example. Make it realistic and specific.",
	summary_probe:
		"Confirm the summary briefly and add one clarifying point if needed.",
	coding:
		"Provide a FULL, complete, working and production-ready code implementation (including necessary boilerplate like Java imports/classes). Start with a brief approach description, then the fully runnable code block, then a concise explanation of why this approach works.",
	general:
		"Respond naturally based on context. Keep it conversational and direct.",
};
...
function likelyIntentCue(text: string): ConversationIntent | null {
	...
	if (
		/(implement|write code|debug|algorithm|lru|typescript|javascript|handler code|api payload|function|refactor|snippet)/i.test(
			normalized,
		)
	) {
		return "coding";
	...
	if (
		/(so you are saying|so you re saying|let me make sure|to summarize|so to summarize|if i understood correctly|am i right|do i have this right|to confirm)/i.test(
			normalized,
		)
	) {
	...
	if (
		/(clarify|what do you mean|can you explain|unpack|how so|what exactly|when you say|break that down)/i.test(
			normalized,
		)
	) {
	...
	const hasStrongBehavioral =
		/\b(tell me about a time|describe a time|describe a situation where you|when have you|share an experience|walk me through a failure|walk me through .+ (time|situation|conflict|failure|mistake|decision))\b/i.test(
			normalized,
		);
	const hasDeepDive =
		/\b(tradeoff|trade.off|why would you choose|why choose|why not|compare|latency|freshness|consistency|availability|throughput|distributed systems|microservice|load balancer|consensus|raft|sharding|replication|rate limiting|circuit breaker|idempotency|backpressure|system design|design a|design an|how would you (build|design|scale|handle|approach)|architecture|scalability|partition tolerance|concurrency|parallelism|deadlock|race condition|big o|database|indexing|transaction|acid|docker|kubernetes|redis|kafka|postgres|mongodb|caching|queue|pipeline)\b/i.test(
			normalized,
		);
	...
	if (
		/(concrete example|specific example|for example|for instance|specific instance|like what|such as)/i.test(
			normalized,
		)
	) {
	...
	if (
		/(tell me about your experience|describe a situation|how do you manage|how do you prioritize|give me an example|what is your .+ style|how do you influence)/i.test(
			normalized,
		)
	) {
		return "behavioral";
...
function calibrateSlmResultByCue(
	text: string,
	slmResult: IntentResult,
): IntentResult {
	...
	const conflictSeverity = isDistantConflict(cue, slmResult.intent)
		? 0.35
		: 0.2;
	const downgradedConfidence = Math.min(
		slmResult.confidence - conflictSeverity,
		0.48,
	);
	return {
		...slmResult,
...
function isDistantConflict(
	cue: ConversationIntent,
	slmIntent: ConversationIntent,
): boolean {
...
class FineTunedClassifier {
	...
	private async ensureLoaded(): Promise<void> {
		...
		this.loadingPromise = (async () => {
			try {
				...
				console.log(
					"[IntentClassifier] Loading fine-tuned classifier (nli-deberta-v3-small)...",
				);
				...
				console.log(
					"[IntentClassifier] Fine-tuned classifier loaded successfully.",
				);
			} catch (e) {
				console.warn(
					"[IntentClassifier] Failed to load fine-tuned model, regex-only fallback:",
					e,
				);
	...
	async classify(
		...
		if (slmClassifierOverrideForTesting) {
			const overrideResult = await slmClassifierOverrideForTesting(
				text,
				traceId,
				spanId,
			);
		...
		try {
			...
			const allScores: Array<{ label: string; score: number }> = Array.isArray(
				result,
			)
				? result
				: [result];
			...
			if (
				...
				getIntentConfidenceService().getSlmMinAcceptScore()
			) {
				return null;
			}

			console.log(
				`[IntentClassifier] SLM classified as "${calibratedResult.intent}" (${(calibratedResult.confidence * 100).toFixed(1)}%): "${text.substring(0, 60)}..."`,
			);
	...
	private applyCueOverrideGate(
		text: string,
		slmResult: IntentResult,
	): IntentResult {
		const cueScores = computeCueScores(text);
		const sorted = Array.from(cueScores.values()).sort(
		...
		if (
			topCue.totalWeight >= CUE_OVERRIDE_MIN_WEIGHT &&
			slmResult.confidence <= CUE_OVERRIDE_SLM_MAX_CONFIDENCE
		) {
			const overrideConfidence = Math.min(
				0.88,
				0.6 + topCue.totalWeight * 0.04,
			);
			console.log(
...
const WEIGHTED_CUES: WeightedCue[] = [
	...
	{
		pattern:
			/\b(can you explain|what do you mean|clarify|could you elaborate on that specific|unpack that|break that down|what exactly do you mean|what exactly is|when you say|how so)\b/i,
		weight: 3.0,
	...
	{
		pattern:
			/\b(what happened|then what|and after that|what's next|how did that go|what came next)\b/i,
		weight: 2.8,
		category: "follow_up",
	},
	...
	{
		pattern:
			/\b(so to summarize|in summary|so basically|so you're saying|so you are saying|let me make sure|if i understood correctly|am i right|correct me if i.m wrong|do i have this right|to confirm)\b/i,
		weight: 3.0,
		category: "summary_probe",
	},

	// ── Coding (very high weight — unambiguous) ──
	{
		pattern:
			/\b(write code|write a function|build a class|implement a method|program|function for|algorithm|how to code|setup a .+ project|using .+ library|debug this|snippet|boilerplate|optimize|refactor|best practice for .+ code|utility method|component for|logic for)\b/i,
		weight: 3.5,
		category: "coding",
	},
	...
	{
		pattern: /\bdescribe a situation where you\b/i,
		weight: 3.5,
		category: "behavioral",
	},
	...
	{
		pattern: /\bgive me an example of a time\b/i,
		weight: 3.5,
		category: "behavioral",
	},
	{
		pattern: /\bwalk me through a failure\b/i,
		weight: 3.5,
		category: "behavioral",
	},
	{
		pattern:
			/\bwalk me through .+ (time|situation|conflict|failure|mistake|decision|disagreement|stakeholder|team challenge|project you led|owned end to end)\b/i,
		weight: 3.2,
		category: "behavioral",
	},
	{
		pattern: /\bconflict with|disagreed with|disagreement with\b/i,
		weight: 2.5,
		category: "behavioral",
	},
	...
	{
		pattern: /\btell me about your experience\b/i,
		weight: 1.2,
		category: "behavioral",
	},
	...
	{
		pattern: /\bhow do you (make|take) .*?decisions?\b/i,
		weight: 1.0,
		category: "behavioral",
	},
	{ pattern: /\bhow do you influence\b/i, weight: 1.2, category: "behavioral" },
	{
		pattern: /\bhow do you prioritize\b/i,
		weight: 1.0,
		category: "behavioral",
	},
	{
		pattern: /\bwhat is your .+ style\b/i,
		weight: 1.0,
		category: "behavioral",
	},
	{
		pattern: /\bwalk me through your experience\b/i,
		weight: 1.2,
		category: "behavioral",
	},
	{
		pattern: /\bleadership|stakeholder\b/i,
		weight: 0.8,
		category: "behavioral",
	},
	...
	{
		pattern: /\btell me more|dive deeper|explain further|how does that work\b/i,
		weight: 2.8,
		category: "deep_dive",
	},
	{
		pattern:
			/\bwalk me through .+ (design|architecture|approach|implementation|system|code|logic|structure|how .+ work|rate limiter|cache|queue|scale|pipeline|workflow|process|model|algorithm|database|schema|api|microservice|load balancer|raft|consensus)\b/i,
		weight: 3.0,
		category: "deep_dive",
	},
	{
		pattern: /\btradeoff|trade.off|why would you choose|why choose|why not\b/i,
		weight: 2.5,
		category: "deep_dive",
	},
	...
	{
		pattern:
			/\b(consistency|availability|latency|freshness|throughput|scalability|reliability|partition tolerance|cap theorem|eventual consistency|strong consistency)\b/i,
		weight: 2.5,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(distributed systems|microservice|load balancer|consensus|raft|paxos|gossip|sharding|replication|caching strategy|rate limiting|circuit breaker|idempotency|backpressure|data pipeline|etl|message queue|pub sub|event driven|cqrs|event sourcing)\b/i,
		weight: 2.8,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(system design|design a|design an|architect|architecture of|how would you (build|design|scale|handle|approach)|how does .+ (work|handle|scale|fail|recover))\b/i,
		weight: 2.6,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(concurrency|parallelism|thread safety|deadlock|race condition|mutex|semaphore|atomic|lock.free|wait.free)\b/i,
		weight: 2.5,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(big o|time complexity|space complexity|hash table|binary search|tree traversal|graph|sorting|dynamic programming|greedy|backtracking|divide and conquer)\b/i,
		weight: 2.5,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(network|tcp|udp|http|dns|ssl|tls|websocket|grpc|rest|rpc|cdn|proxy|firewall)\b/i,
		weight: 1.8,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(database|sql|nosql|indexing|query optimization|transaction|acid|join|normaliz|orm|migration|schema)\b/i,
		weight: 1.8,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(security|authenticat|authoriz|encrypt|oauth|jwt|token|csrf|xss|injection|vulnerability)\b/i,
		weight: 1.8,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(testing|unit test|integration test|e2e|tdd|bdd|mock|stub|coverage|ci|cd|deploy|pipeline|monitor|observ|logging|metric|alert)\b/i,
		weight: 1.5,
		category: "deep_dive",
	},
	{
		pattern:
			/\b(docker|kubernetes|container|orchestrat|vm|cloud|aws|gcp|azure|serverless|lambda|s3|dynamodb|redis|kafka|rabbitmq|postgres|mongodb)\b/i,
		weight: 1.8,
		category: "deep_dive",
	},
	...
	{
		pattern:
			/\b(concrete example|specific instance|specific example|like what|such as|for instance|one concrete|one specific)\b/i,
		weight: 2.0,
...
function resolveCueScores(
	...
	if (!second || top.totalWeight > second.totalWeight * 2) {
		...
		return {
			intent: top.category,
			confidence,
			answerShape: INTENT_ANSWER_SHAPES[top.category],
		};
	}
		...
		if (scoreA && scoreB) {
			...
			if (ratio >= minRatio) {
				...
				return {
					intent: winner,
					confidence,
					answerShape: INTENT_ANSWER_SHAPES[winner],
				};
			}
		}
	}
		...
		console.log(
			`[IntentClassifier] Cue scoring: intent=${result.intent} conf=${(result.confidence * 100).toFixed(0)}% ` +
				`scores={${Array.from(scores.entries())
					.map(([k, v]) => `${k}=${v.totalWeight.toFixed(1)}`)
					.join(", ")}} ` +
...
export function detectIntentByContext(
	...
	const assistantLines = lines.filter(
		(l) => l.includes("[ASSISTANT") || l.includes("[INTERVIEWEE"),
	);
	...
	const hasCodeInRecentAnswers =
		/```|function |class |const |import |def /.test(recentAssistantText);
		...
		if (lastInterviewerText.length < 50) {
			if (hasCodeInRecentAnswers) {
				return {
					intent: "deep_dive",
					confidence: 0.6,
					answerShape: INTENT_ANSWER_SHAPES.deep_dive,
				};
			}
			return {
				intent: "follow_up",
				confidence: 0.7,
				answerShape: INTENT_ANSWER_SHAPES.follow_up,
			};
		}
	...
	return {
		intent: "general",
		confidence: 0.45,
		answerShape: INTENT_ANSWER_SHAPES.general,
	};
}
...
export async function classifyIntentBySlm(
	...
	const effectiveSpanId =
		spanId ?? (traceId ? `classify-slm-${startTime}` : undefined);
	...
	if (traceId) {
		traceLogger.logIntentClassificationEvent(
			traceId,
			effectiveSpanId,
			"completed",
			{
...
export async function classifyIntent(
	...
	if (lastInterviewerTurn) {
		...
		if (patternResult) {
			...
			console.log(
				`[IntentClassifier] Tier 1 (regex) classified as "${result.intent}" (${(result.confidence * 100).toFixed(0)}%): "${lastInterviewerTurn.substring(0, 60)}..."`,
			);
			return result;
		}
	...
	const contextResult = detectIntentByContext(
		recentTranscript,
	...
	console.log(
		`[IntentClassifier] Tier 3 (context) classified as "${result.intent}" (${(result.confidence * 100).toFixed(0)}%): "${lastInterviewerTurn?.substring(0, 60) ?? "N/A"}..."`,
	);
	return result;
}
