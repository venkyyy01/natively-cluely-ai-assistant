...
class ZeroShotClassifier {
	...
	private async ensureLoaded(): Promise<void> {
		...
		this.loadingPromise = (async () => {
			try {
				...
				console.warn(
					"[IntentClassifier] Failed to load zero-shot model, regex-only fallback:",
					e,
				);
	...
	async classify(text: string): Promise<IntentResult | null> {
		...
		try {
			...
			console.log(
				`[IntentClassifier] SLM classified as "${intent}" (${(topScore * 100).toFixed(1)}%): "${text.substring(0, 60)}..."`,
			);
...
function detectIntentByPattern(
	...
	if (
		/(can you explain|what do you mean|clarify|could you elaborate on that specific)/i.test(
			text,
		)
	) {
		return {
			...
			confidence: 0.9,
			answerShape: INTENT_ANSWER_SHAPES.clarification,
		};
	}

	// Follow-up patterns
	if (
		/(what happened|then what|and after that|what.s next|how did that go)/i.test(
			text,
		)
	) {
		return {
	...
	// Behavioral patterns
	if (
		behavioralWalkthrough.test(text) ||
		/(give me an example|tell me about a time|describe a time|describe a situation|when have you|share an experience|how do you manage|what is your .*style|how do you make .*decision|how do you influence|how do you prioritize)/i.test(
			text,
		)
	) {
		return {
			intent: "behavioral",
			confidence: 0.9,
			answerShape: INTENT_ANSWER_SHAPES.behavioral,
		};
	}

	// Deep dive patterns
	if (
		/(tell me more|dive deeper|explain further|walk me through|how does that work)/i.test(
			text,
		)
	) {
		return {
	...
	if (
		/(for example|concrete example|specific instance|like what|such as)/i.test(
			text,
		)
	) {
		return {
			...
			confidence: 0.85,
			answerShape: INTENT_ANSWER_SHAPES.example_request,
		};
	}

	// Summary probe patterns
	if (
		/(so to summarize|in summary|so basically|so you.re saying|let me make sure)/i.test(
			text,
		)
	) {
		return {
			...
			confidence: 0.85,
			answerShape: INTENT_ANSWER_SHAPES.summary_probe,
		};
	}

	// Coding patterns (Broad detection for programming/implementation)
	if (
		/(write code|program|implement|function for|algorithm|how to code|setup a .* project|using .* library|debug this|snippet|boilerplate|example of .* in .*|optimize|refactor|best practice for .* code|utility method|component for|logic for)/i.test(
			text,
		)
	) {
		return {
			intent: "coding",
			confidence: 0.9,
			answerShape: INTENT_ANSWER_SHAPES.coding,
		};
	}
...
export async function classifyIntent(
	...
	if (lastInterviewerTurn) {
		...
		if (lastInterviewerTurn.trim().length > 5) {
			const slmResult =
				await ZeroShotClassifier.getInstance().classify(lastInterviewerTurn);
			if (slmResult) {
				return slmResult;
			}
		}
