...
ANSWER SHAPE: ${intentResult.answerShape}
	...
	async *generateStream(
		...
		options?: {
			...
			onFallbackResponsePrepared?: (
				details: StreamFailureDetails & { hadVisibleOutput: boolean },
			) => void;
		...
		try {
			...
			const primaryQuestion =
				options?.latestQuestion?.trim() || cleanedTranscript;

			const prompt = options?.fastPath
				? FAST_STANDARD_ANSWER_PROMPT
				: UNIVERSAL_WHAT_TO_ANSWER_PROMPT;
			...
			if (
				...
				throw new TypeError(
					"LLMHelper.streamChat must return an async iterable",
				);
			}
			...
			if (!yieldedAnyChunk) {
				...
				options?.onFallbackResponsePrepared?.({
					...failureDetails,
			...
			console.error(
				"[WhatToAnswerLLM] Stream failed after yielding tokens:",
				error,
			);
	...
	async generateReasoningFirst(
		...
		if (intentResult) {
			...
			switch (intentResult.intent) {
				case "behavioral":
					intentHint =
						'This is a behavioral question. Tell one concrete story, own it with "I".';
					break;
				case "coding":
					intentHint =
						"This is a coding question. Code first, explain briefly after.";
					break;
				case "deep_dive":
					intentHint =
						"They want more detail on the same topic. Go deeper, don't start a new topic.";
					break;
				case "clarification":
					intentHint =
						"They want clarification. Keep it short, answer what they actually asked.";
					break;
				default:
		...
		if (temporalContext?.hasRecentResponses) {
			contextParts.push(
				`PREVIOUS_RESPONSES: ${temporalContext.previousResponses.join(" | ")}`,
			);
		}

		contextParts.push(`CONVERSATION:\n${cleanedTranscript}`);

		const message = ["STRUCTURED_REASONING_RESPONSE", ...contextParts].join(
			"\n\n",
		);
		const stream = this.llmHelper.streamChat(
			message,
