...
export class FollowUpLLM {
	...
	async *generateStream(
		...
		try {
			...
			console.error(
				"[FollowUpLLM] Stream Failed:",
				new LLMError("Follow-up stream generation failed", e, {
					previousAnswerLength: previousAnswer.length,
					refinementRequest,
					contextLength: context?.length || 0,
				}),
	...
	async generateReasoningFirstFollowUp(
		...
		return await wrapAsync(
			async () => {
				...
				if (full.trim().length === 0) {
					throw new Error(
						"LLM returned empty response for reasoning-first follow-up generation",
					);
				}
				...
				if (parsed.mode === "invalid") {
					throw new Error(
						`Failed to parse reasoning-first follow-up response: ${full.substring(0, 200)}...`,
					);
				}
	...
	async generateLegacy(
		...
		if (result.success) {
			...
			console.error(
				"[FollowUpLLM] Generation failed (legacy mode):",
				result.error,
			);
			return "";
		}
	...
	async generateReasoningFirstFollowUpLegacy(
		...
		const result = await this.generateReasoningFirstFollowUp(
			reasoningThread,
			followUpQuestion,
			context,
		);
		if (result.success) {
			...
			console.error(
				"[FollowUpLLM] Reasoning-first follow-up failed (legacy mode):",
				result.error,
			);
