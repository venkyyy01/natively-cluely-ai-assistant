...
function parseArgs(): Partial<TrainConfig> {
  ...
  return {
    ...
    batchSize: get('--batch-size=') ? parseInt(get('--batch-size=')!) : undefined,
    learningRate: get('--learning-rate=') ? parseFloat(get('--learning-rate=')!) : undefined,
    warmupRatio: get('--warmup-ratio=') ? parseFloat(get('--warmup-ratio=')!) : undefined,
    weightDecay: get('--weight-decay=') ? parseFloat(get('--weight-decay=')!) : undefined,
    patience: get('--patience=') ? parseInt(get('--patience=')!) : undefined,
    minDelta: get('--min-delta=') ? parseFloat(get('--min-delta=')!) : undefined,
...
function runPythonTrainer(datasetPath: string, config: TrainConfig): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const args = [
      ...
      '--warmup-ratio', String(config.warmupRatio),
      '--weight-decay', String(config.weightDecay),
      '--patience', String(config.patience),
      '--min-delta', String(config.minDelta),
      '--max-seq-length', String(config.maxSeqLength),
    ...
    console.log(`[Train] Spawning Python trainer: ${config.pythonPath} ${args.join(' ')}`);
...
async function runPreTrainingBenchmark(): Promise<void> {
  ...
  console.log(`  npx tsx ${benchmarkPath} --model=slm --output=pre-training-slm.json`);
  console.log(`  npx tsx ${benchmarkPath} --model=embedding --output=pre-training-embedding.json`);
}
...
async function runPostTrainingBenchmark(modelDir: string): Promise<void> {
  ...
  console.log(`  npx tsx ${benchmarkPath} --model=setfit --model-path=${modelDir} --output=post-training-setfit.json`);
}
