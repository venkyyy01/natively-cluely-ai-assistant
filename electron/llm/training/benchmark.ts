...
async function benchmarkModel(
	...
	console.log(
		`[Benchmark] Evaluating ${modelName} on ${testExamples.length} examples...`,
	);
	...
	for (const label of labels) {
		...
		const truePositives = predictions.filter(
			(p) => p.expected === label && p.predicted === label,
		).length;
	...
	const macroF1 =
		Object.values(perClassMetrics).reduce((sum, m) => sum + m.f1, 0) /
		labels.length;
	const weightedF1 =
		Object.values(perClassMetrics).reduce(
...
async function benchmarkCurrentSLM(
	testExamples: Array<{ text: string; label: string }>,
): Promise<BenchmarkResult> {
...
async function benchmarkCurrentSetFit(
	testExamples: Array<{ text: string; label: string }>,
): Promise<BenchmarkResult> {
...
async function benchmarkSemanticRouter(
	testExamples: Array<{ text: string; label: string }>,
): Promise<BenchmarkResult> {
...
async function benchmarkLayeredRouter(
	testExamples: Array<{ text: string; label: string }>,
): Promise<BenchmarkResult> {
...
async function main() {
	...
	for (const result of results) {
		...
		for (const [label, metrics] of Object.entries(result.perClassMetrics)) {
			console.log(
				`  ${label}: P=${(metrics.precision * 100).toFixed(1)}% R=${(metrics.recall * 100).toFixed(1)}% F1=${(metrics.f1 * 100).toFixed(1)}% (n=${metrics.support})`,
			);
		}
	}
...
export { benchmarkCurrentSetFit, benchmarkCurrentSLM, benchmarkModel };
