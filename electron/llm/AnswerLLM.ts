...
import { type LLMError, type Result, wrapAsync } from "../types/Result";
import {
...
export class AnswerLLM {
		...
		return await wrapAsync(
			async () => {
				...
				if (fullResponse.trim().length === 0) {
					throw new Error(
						"LLM returned empty response for reasoning-first generation",
					);
				}
				...
				if (parsed.mode === "invalid") {
					throw new Error(
						`Failed to parse reasoning-first response: ${fullResponse.substring(0, 200)}...`,
					);
				}
	...
	async generateLegacy(question: string, context?: string): Promise<string> {
		...
		if (result.success) {
			...
			console.error(
				"[AnswerLLM] Generation failed (legacy mode):",
				result.error,
			);
			return "";
		}
	 ...
	 */
	async generateReasoningFirstLegacy(
		question: string,
		context?: string,
