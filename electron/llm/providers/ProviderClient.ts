...
export interface ProviderClient {
  ...
  stream(request: ProviderRequest, signal?: AbortSignal): AsyncIterable<StreamEvent>;
}
...
export function withRetryAndTimeout(
  ...
  return {
    ...
    async *stream(request: ProviderRequest, signal?: AbortSignal): AsyncIterable<StreamEvent> {
      let attempt = 0;
      ...
      while (true) {
        ...
        const timeoutTimer = setTimeout(() => timeoutController.abort(), policy.timeoutMs);

        let sawFatalError = false;
        try {
          for await (const event of client.stream(request, timeoutController.signal)) {
            if (event.kind === 'error') {
...
export function createGeminiClient(_apiKey: string): ProviderClient {
  return {
    ...
    async *stream(_req: ProviderRequest, _signal?: AbortSignal) {
      yield { kind: 'error', code: 'not_implemented', message: 'Gemini client not yet migrated', retryable: false } as ErrorEvent;
    },
  };
}
...
export function createAnthropicClient(_apiKey: string): ProviderClient {
  return {
    name: 'anthropic',
    async *stream(_req: ProviderRequest, _signal?: AbortSignal) {
      yield { kind: 'error', code: 'not_implemented', message: 'Anthropic client not yet migrated', retryable: false } as ErrorEvent;
    },
  };
}
...
export function createOpenAIClient(_apiKey: string): ProviderClient {
  return {
    name: 'openai',
    async *stream(_req: ProviderRequest, _signal?: AbortSignal) {
      yield { kind: 'error', code: 'not_implemented', message: 'OpenAI client not yet migrated', retryable: false } as ErrorEvent;
    },
  };
}
...
export function createGroqClient(_apiKey: string): ProviderClient {
  return {
    name: 'groq',
    async *stream(_req: ProviderRequest, _signal?: AbortSignal) {
      yield { kind: 'error', code: 'not_implemented', message: 'Groq client not yet migrated', retryable: false } as ErrorEvent;
    },
  };
}
...
export function createCerebrasClient(_apiKey: string): ProviderClient {
  return {
    name: 'cerebras',
    async *stream(_req: ProviderRequest, _signal?: AbortSignal) {
      yield { kind: 'error', code: 'not_implemented', message: 'Cerebras client not yet migrated', retryable: false } as ErrorEvent;
    },
  };
}
...
export function createOllamaClient(_baseUrl: string): ProviderClient {
  return {
    name: 'ollama',
    async *stream(_req: ProviderRequest, _signal?: AbortSignal) {
      yield { kind: 'error', code: 'not_implemented', message: 'Ollama client not yet migrated', retryable: false } as ErrorEvent;
    },
  };
}
