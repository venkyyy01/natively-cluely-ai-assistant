...
type FoundationIntentHelperEnvelope =
	| FoundationIntentHelperSuccessEnvelope
	| FoundationIntentHelperErrorEnvelope;
...
function compactPreparedTranscript(
	preparedTranscript: string,
	question: string,
): string {
	...
	const dialogueLines = normalizedTranscript.filter((line) =>
		/^\[(INTERVIEWER|ASSISTANT|ME|USER)\]/i.test(line),
	);
	const compactLines = (
		dialogueLines.length > 0 ? dialogueLines : normalizedTranscript
	).slice(-MAX_COMPACT_TRANSCRIPT_LINES);

	const fallbackQuestion = question.trim()
		? [`[INTERVIEWER]: ${question.trim()}`]
		: [];
	const compactTranscript = (
		compactLines.length > 0 ? compactLines : fallbackQuestion
	).join("\n");
	...
	return compactTranscript
		.slice(compactTranscript.length - MAX_COMPACT_TRANSCRIPT_CHARS)
		.trim();
}
...
export function mapExitFailure(stderr: string): IntentProviderErrorType {
	...
	if (
		...
		normalized.includes("model not ready")
	) {
		return "model_not_ready";
	}
	if (
		normalized.includes("unsupported_locale") ||
		normalized.includes("unsupported locale")
	) {
...
export class FoundationModelsIntentProvider implements IntentInferenceProvider {
	...
	private readonly isOptimizationEnabled: (
		flag: "useFoundationModelsIntent",
	) => boolean;
	...
	constructor(options: FoundationModelsIntentProviderOptions = {}) {
		...
		this.helperRunner =
			...
			((helperPath, request, timeoutMs) =>
				this.runHelperDispatch(helperPath, request, timeoutMs));
		this.localeResolver =
		...
		this.isOptimizationEnabled =
			options.isOptimizationEnabled ??
			((flag: "useFoundationModelsIntent"): boolean =>
				isOptimizationActive(flag));
		this.usePersistentHelper =
			options.usePersistentHelper ?? envPersistentEnabled();
	}
	...
	async classify(input: IntentClassificationInput): Promise<IntentResult> {
		...
		if (!available) {
			throw createIntentProviderError(
				"unavailable",
				"Foundation intent provider unavailable on this host",
			);
		}
		...
		if (!helperPath) {
			throw createIntentProviderError(
				"unavailable",
				"Foundation intent helper binary not found",
			);
		}
		...
		const request: FoundationIntentHelperRequest = {
			...
			preparedTranscript: compactPreparedTranscript(
		...
		try {
			const commandResult = await this.helperRunner(
				helperPath,
				request,
				this.timeoutMs,
			);
			...
			if (traceId) {
				traceLogger.logModelInvocation(traceId, spanId, {
					...
					inputTokens:
						request.preparedTranscript.length / 4 + request.question.length / 4, // Rough estimate
				});
			}
				...
				if (commandResult.exitCode !== 0) {
					...
					throw createIntentProviderError(
						...
						commandResult.stderr.trim() ||
							"Foundation helper exited with no output",
					);
				}
				throw createIntentProviderError(
					"invalid_response",
					"Foundation helper returned empty output",
				);
			}
			...
			try {
				...
				) as FoundationIntentHelperEnvelope;
			} catch (error) {
				const message = error instanceof Error ? error.message : String(error);
				throw createIntentProviderError(
					"invalid_response",
					`Foundation helper returned invalid JSON: ${message}`,
				);
			}
			...
			if (
				...
				throw createIntentProviderError(
					"invalid_response",
					"Foundation helper response envelope missing ok flag",
				);
			}
			...
			if (!envelope.ok) {
				...
				throw createIntentProviderError(
					code,
					errorEnvelope.message ?? "Foundation helper reported an error",
				);
			}
			...
			if (!isConversationIntent(envelope.intent)) {
				throw createIntentProviderError(
					"invalid_response",
					`Foundation helper returned unsupported intent: ${String(envelope.intent)}`,
				);
			}
			...
			if (!Number.isFinite(envelope.confidence)) {
				throw createIntentProviderError(
					"invalid_response",
					"Foundation helper returned non-numeric confidence",
				);
			}
			...
			return {
				...
				answerShape:
					typeof envelope.answerShape === "string" &&
					envelope.answerShape.trim()
	...
	private handlePersistentStdoutLine(line: string): void {
		...
		try {
			parsed = JSON.parse(trimmed) as FoundationIntentHelperEnvelope & {
				requestId?: string | null;
			};
		} catch {
			return;
	...
	private async ensurePersistentChild(
		helperPath: string,
		...
		if (this.persistentSpawnCount >= PERSISTENT_MAX_SPAWNS_PER_WINDOW) {
			throw createIntentProviderError(
				"unavailable",
				"Foundation helper persistent process exceeded restart budget",
			);
		}
	...
	private async runPersistentHelperBinary(
		...
		if (!requestId) {
			throw createIntentProviderError(
				"invalid_response",
				"Foundation persistent mode requires requestId",
			);
		}
		...
		return new Promise<HelperCommandResult>((resolve, reject) => {
			const timer = setTimeout(() => {
				...
				try {
					child.stdin.write(
						`${JSON.stringify({ type: "cancel", requestId })}\n`,
					);
				} catch {
					// ignore
				}
				reject(
					createIntentProviderError(
						"timeout",
						`Foundation helper timed out after ${timeoutMs}ms`,
					),
				);
			}, timeoutMs);
			...
			try {
				if (!child.stdin.writableEnded) {
					...
					reject(
						createIntentProviderError(
							"unavailable",
							"Foundation helper stdin closed",
						),
					);
				}
			} catch (error) {
			...
			child.on("error", (error) => {
				finish(() => {
					...
					reject(
						createIntentProviderError(
							code,
							`Failed to launch Foundation helper: ${message}`,
						),
					);
				});
			});
