...
export class SetFitIntentProvider implements IntentInferenceProvider {
  ...
  async classify(input: IntentClassificationInput): Promise<IntentResult> {
    ...
    try {
      let result: { intent: ConversationIntent; confidence: number; latencyMs: number };
  ...
  private async runPythonInference(text: string): Promise<{ intent: ConversationIntent; confidence: number; latencyMs: number }> {
    return new Promise((resolve, reject) => {
      const id = ++this.requestId;
      const timer = setTimeout(() => {
        this.pendingRequests.delete(id);
        reject(createIntentProviderError('timeout', `SetFit Python inference timed out after ${this.inferenceTimeoutMs}ms`));
      ...
      if (this.pythonProc && this.pythonProc.stdin && !this.pythonProc.stdin.destroyed) {
        this.pythonProc.stdin.write(JSON.stringify({ id, text }) + '\n');
      } else {
        this.pendingRequests.delete(id);
        clearTimeout(timer);
        reject(createIntentProviderError('model_not_ready', 'Python subprocess not available'));
      }
    });
  }
  ...
  private async runXenovaInference(text: string): Promise<{ intent: ConversationIntent; confidence: number; latencyMs: number }> {
    ...
    if (!top || typeof top.label !== 'string' || typeof top.score !== 'number') {
      throw createIntentProviderError('invalid_response', 'SetFit Xenova returned malformed output');
    }
  ...
  private async tryLoadPythonServer(): Promise<void> {
    ...
    console.log(`[SetFitIntentProvider] Starting Python server: ${this.pythonPath} ${scriptPath} --model ${modelPath}`);

    const proc = child_process.spawn(this.pythonPath, [scriptPath, '--model', modelPath], {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (resp.status === 'ready') {
            console.log(`[SetFitIntentProvider] Python server ready in ${resp.load_ms?.toFixed(0) ?? '?'}ms`);
    ...
    proc.on('close', (code) => {
      console.warn(`[SetFitIntentProvider] Python process exited with code ${code}`);
      this.pythonReady = false;
      this.pythonProc = null;
    });
