...
import { createRequestAbortController, LLM_API_TIMEOUT_MS, MAX_OUTPUT_TOKENS, withTimeout } from '../../LLMHelper';
...
export async function* streamWithCerebras(
  ...
  if (!helper.cerebrasClient) throw new Error("Cerebras client not initialized");

  const targetModel = modelOverride || helper.getConfiguredFastModel('cerebras');
  const messages: any[] = [];
  if (systemPrompt) {
...
export async function generateWithCerebras(
  ...
  const payloadHash = helper.hashValue({ model: targetModel, userMessage, systemPrompt: systemPrompt || '' });

  return helper.withResponseCache('cerebras', targetModel, systemPromptHash, payloadHash, async () => {
