...
import { GROQ_MODEL, CLAUDE_MODEL } from "../../LLMHelper";
import {
  HARD_SYSTEM_PROMPT,
  GROQ_SYSTEM_PROMPT,
...
function sanitizeError(error: unknown): string {
   ...
   */
export async function generateWithPro(helper: LLMHelper, contents: any[]): Promise<string> {
   ...
   */
export async function generateWithFlash(helper: LLMHelper, contents: any[]): Promise<string> {
    if (!helper.client) throw new Error("Gemini client not initialized")
...
export async function chatWithGemini(helper: LLMHelper, message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false, alternateGroqMessage?: string): Promise<string> {
    try {
      ...
      let effectiveMessage = hasScreenshotInput ? message : helper.applyDefaultBrevityHint(message)
      const structuredScreenshotRequest = helper.isStructuredOutputRequest(effectiveMessage);
      ...
      const forceTextFallback = helper.shouldForceScreenshotTextFallback(imagePaths);
      ...
      // ============================================================
      if (!hasScreenshotInput && helper.knowledgeOrchestrator?.isKnowledgeMode()) {
        try {
          ...
          const knowledgeResult = await helper.knowledgeOrchestrator.processQuestion(message);
          if (knowledgeResult) {
      ...
      const buildMessage = (provider: string, modelId: string, systemPrompt: string) => {
        const preparedUserContent = helper.prepareUserContentForModel(provider, modelId, effectiveMessage, context);
        if (skipPromptForRequest) {
          return preparedUserContent;
        }
        return helper.joinPrompt(systemPrompt, preparedUserContent, helper.getInputTokenBudget(provider, modelId));
      };

      // For OpenAI/Claude: separate system prompt + user message
      const activeOpenAiModel = helper.getActiveOpenAiModel();
      const openaiUserContent = helper.prepareUserContentForModel('openai', activeOpenAiModel, effectiveMessage, context);
      const claudeUserContent = helper.prepareUserContentForModel('claude', CLAUDE_MODEL, effectiveMessage, context);
      ...
      const finalGeminiPrompt = await helper.withSystemPromptCache('gemini', helper.currentModelId, geminiBasePrompt, () => buildSystemPrompt(geminiBasePrompt));
      const finalGroqPrompt = await helper.withSystemPromptCache('groq', GROQ_MODEL, groqBasePrompt, () => buildSystemPrompt(groqBasePrompt));
      ...
      const openaiSystemPrompt = skipPromptForRequest
        ? undefined
        : await helper.withSystemPromptCache('openai', activeOpenAiModel, openAiBasePrompt, () => buildSystemPrompt(openAiBasePrompt));
      const claudeSystemPrompt = skipPromptForRequest
        ? undefined
        : await helper.withSystemPromptCache('claude', CLAUDE_MODEL, claudeBasePrompt, () => buildSystemPrompt(claudeBasePrompt));
      const canUseFastResponse = !isMultimodal && !helper.activeCurlProvider && !helper.customProvider && !helper.useOllama;
      const fastResponseTarget = canUseFastResponse ? helper.getActiveFastResponseTarget() : null;
      if (fastResponseTarget) {
        ...
        try {
          if (fastResponseTarget.provider === 'cerebras') {
            return await helper.generateWithCerebras(openaiUserContent, openaiSystemPrompt, fastResponseTarget.model);
          }

          return await helper.generateWithGroq(combinedMessages.groq, fastResponseTarget.model);
        } catch (e: any) {
        ...
        try {
          if (isMultimodal && imagePaths?.length) {
            const response = await helper.runWithScreenshotOcrFallback(
              ...
              (fallbackMessage) => helper.chatWithCurl(
                fallbackMessage,
                curlSystemPrompt,
                context || "",
                [],
          ...
          console.warn(`[LLMHelper] cURL provider (${helper.activeCurlProvider.name}) returned no response. Falling back to standard routing.`);
        } catch (error: any) {
          console.warn(`[LLMHelper] cURL provider (${helper.activeCurlProvider.name}) failed after ${CURL_PROVIDER_TIMEOUT_MS}ms timeout window. Falling back to standard routing:`, error.message);
        }
        return await helper.runWithProviderFallbackBypass(() => helper.chatWithGemini(
          message,
          originalImagePaths,
          originalContext,
          skipSystemPrompt,
      ...
      if (helper.customProvider) {
        console.log(`[LLMHelper] Using Custom Provider: ${helper.customProvider.name}`);
        ...
        const response = isMultimodal && imagePaths?.length
          ? await helper.runWithScreenshotOcrFallback(
              ...
              (fallbackMessage) => {
                const fallbackUserContent = helper.prepareUserContentForModel('custom', helper.getCurrentModel(), fallbackMessage, context);
                const fallbackCombinedMessage = helper.joinPrompt(customSystemPrompt, fallbackUserContent, helper.getInputTokenBudget('custom', helper.getCurrentModel()));
      ...
      if (helper.isOpenAiModel(helper.currentModelId) && helper.openaiClient) {
        if (isMultimodal && imagePaths?.length) {
          return await helper.runWithScreenshotOcrFallback(
            ...
            imagePaths,
            effectiveMessage,
            () => helper.generateWithOpenai(openaiUserContent, openaiSystemPrompt, imagePaths),
            (fallbackMessage) => {
              const fallbackUserContent = helper.prepareUserContentForModel('openai', activeOpenAiModel, fallbackMessage, context);
              return helper.generateWithOpenai(fallbackUserContent, openaiSystemPrompt);
            },
          );
        }
      ...
      if (helper.isGroqModel(helper.currentModelId) && helper.groqClient) {
        if (isMultimodal && imagePaths) {
          return await helper.runWithScreenshotOcrFallback(
            ...
            imagePaths,
            effectiveMessage,
            () => helper.generateWithGroqMultimodal(openaiUserContent, imagePaths, openaiSystemPrompt),
            (fallbackMessage) => {
              const fallbackUserContent = helper.prepareUserContentForModel('groq', GROQ_MODEL, fallbackMessage, context);
              return helper.generateWithGroq(helper.joinPrompt(finalGroqPrompt, fallbackUserContent, helper.getInputTokenBudget('groq', GROQ_MODEL)));
            },
          );
        }
      ...
      const textOpenAI = helper.modelVersionManager.getTextTieredModels(TextModelFamily.OPENAI).tier1;
      const textGeminiFlash = helper.modelVersionManager.getTextTieredModels(TextModelFamily.GEMINI_FLASH).tier1;
      const textGeminiPro = helper.modelVersionManager.getTextTieredModels(TextModelFamily.GEMINI_PRO).tier1;
      const textClaude = helper.modelVersionManager.getTextTieredModels(TextModelFamily.CLAUDE).tier1;
      const textGroq = helper.modelVersionManager.getTextTieredModels(TextModelFamily.GROQ).tier1;
      ...
      if (isMultimodal) {
        ...
        if (helper.openaiClient) {
          providers.push({
            ...
            execute: () => helper.runWithScreenshotOcrFallback(
              `OpenAI (${textOpenAI})`,
              imagePaths,
              effectiveMessage,
              () => helper.generateWithOpenai(openaiUserContent, openaiSystemPrompt, imagePaths),
        ...
        if (helper.client) {
          providers.push({
            ...
            execute: () => helper.runWithScreenshotOcrFallback(
              ...
              imagePaths,
              effectiveMessage,
              () => helper.tryGenerateResponse(combinedMessages.gemini, imagePaths, textGeminiFlash),
              (fallbackMessage) => {
                const fallbackUserContent = helper.prepareUserContentForModel('gemini', textGeminiFlash, fallbackMessage, context);
                return helper.tryGenerateResponse(helper.joinPrompt(finalGeminiPrompt, fallbackUserContent, helper.getInputTokenBudget('gemini', textGeminiFlash)), undefined, textGeminiFlash);
              },
            )
          });
        }
        if (helper.claudeClient) {
          providers.push({
            ...
            execute: () => helper.runWithScreenshotOcrFallback(
              ...
              (fallbackMessage) => {
                const fallbackUserContent = helper.prepareUserContentForModel('claude', textClaude, fallbackMessage, context);
        ...
        if (helper.client) {
          providers.push({
            ...
            execute: () => helper.runWithScreenshotOcrFallback(
              ...
              imagePaths,
              effectiveMessage,
              () => helper.tryGenerateResponse(combinedMessages.gemini, imagePaths, textGeminiPro),
              (fallbackMessage) => {
                const fallbackUserContent = helper.prepareUserContentForModel('gemini_pro', textGeminiPro, fallbackMessage, context);
                return helper.tryGenerateResponse(helper.joinPrompt(finalGeminiPrompt, fallbackUserContent, helper.getInputTokenBudget('gemini_pro', textGeminiPro)), undefined, textGeminiPro);
              },
            )
          });
        }
        ...
        if (helper.groqClient) {
          providers.push({ name: `Groq (${textGroq})`, execute: () => helper.generateWithGroq(combinedMessages.groq) });
        }
        if (helper.client) {
          providers.push({
            name: `Gemini Flash (${textGeminiFlash})`,
            execute: () => helper.tryGenerateResponse(combinedMessages.gemini, undefined, textGeminiFlash)
          });
          providers.push({
            name: `Gemini Pro (${textGeminiPro})`,
            execute: () => helper.tryGenerateResponse(combinedMessages.gemini, undefined, textGeminiPro)
          });
        }
        if (helper.openaiClient) {
          providers.push({ name: `OpenAI (${textOpenAI})`, execute: () => helper.generateWithOpenai(openaiUserContent, openaiSystemPrompt) });
        }
        if (helper.claudeClient) {
          providers.push({ name: `Claude (${textClaude})`, execute: () => helper.generateWithClaude(claudeUserContent, claudeSystemPrompt) });
        }
      }
...
export async function * streamChatWithGemini(helper: LLMHelper, message: string, imagePaths?: string[], context?: string, skipSystemPrompt: boolean = false, abortSignal?: AbortSignal): AsyncGenerator<string, void, unknown> {
    console.log(`[LLMHelper] streamChatWithGemini called with message:`, message.substring(0, 50));
    const isMultimodal = !!(imagePaths?.length);
    if (isMultimodal) {
      yield* helper.streamChat(message, imagePaths, context, skipSystemPrompt ? "" : undefined, { abortSignal });
      return;
    }
    ...
    const buildCombinedMessage = (systemPrompt: string) => {
      const finalPrompt = skipSystemPrompt ? systemPrompt : helper.injectLanguageInstruction(systemPrompt);
      if (skipSystemPrompt) {
        return context
          ? `CONTEXT:\n${context}\n\nUSER QUESTION:\n${effectiveMessage}`
          : effectiveMessage;
    ...
    const openaiSystemPrompt = skipSystemPrompt ? undefined : helper.injectLanguageInstruction(OPENAI_SYSTEM_PROMPT);
    const claudeSystemPrompt = skipSystemPrompt ? undefined : helper.injectLanguageInstruction(CLAUDE_SYSTEM_PROMPT);
    ...
    if (isMultimodal) {
      ...
      if (helper.openaiClient) {
        providers.push({ name: `OpenAI (${textOpenAI})`, execute: () => helper.streamWithOpenaiMultimodal(userContent, imagePaths!, openaiSystemPrompt) });
      }
      if (helper.client) {
        providers.push({ name: `Gemini Flash (${textGeminiFlash})`, execute: () => helper.streamWithGeminiModel(combinedMessages.gemini, textGeminiFlash, imagePaths, abortSignal) });
      }
      if (helper.claudeClient) {
        providers.push({ name: `Claude (${textClaude})`, execute: () => helper.streamWithClaudeMultimodal(userContent, imagePaths!, claudeSystemPrompt) });
      }
      if (helper.client) {
        providers.push({ name: `Gemini Pro (${textGeminiPro})`, execute: () => helper.streamWithGeminiModel(combinedMessages.gemini, textGeminiPro, imagePaths, abortSignal) });
      }
      if (helper.groqClient) {
        providers.push({ name: `Groq (meta-llama/llama-4-scout-17b-16e-instruct)`, execute: () => helper.streamWithGroqMultimodal(userContent, imagePaths!, openaiSystemPrompt) });
      }
    } else {
      // TEXT-ONLY PROVIDER ORDER: Groq → OpenAI → Claude → Gemini Flash → Gemini Pro
      if (helper.groqClient) {
        providers.push({ name: `Groq (${textGroq})`, execute: () => helper.streamWithGroq(combinedMessages.groq) });
      }
      if (helper.openaiClient) {
        providers.push({ name: `OpenAI (${textOpenAI})`, execute: () => helper.streamWithOpenai(userContent, openaiSystemPrompt) });
      }
      if (helper.claudeClient) {
        providers.push({ name: `Claude (${textClaude})`, execute: () => helper.streamWithClaude(userContent, claudeSystemPrompt) });
      }
      if (helper.client) {
        providers.push({ name: `Gemini Flash (${textGeminiFlash})`, execute: () => helper.streamWithGeminiModel(combinedMessages.gemini, textGeminiFlash, undefined, abortSignal) });
        providers.push({ name: `Gemini Pro (${textGeminiPro})`, execute: () => helper.streamWithGeminiModel(combinedMessages.gemini, textGeminiPro, undefined, abortSignal) });
      }
    }
...
export async function * streamWithGeminiModel(helper: LLMHelper, fullMessage: string, model: string, imagePaths?: string[], abortSignal?: AbortSignal): AsyncGenerator<string, void, unknown> {
    if (!helper.client) throw new Error("Gemini client not initialized");
   ...
   */
export async function * streamWithGeminiParallelRace(helper: LLMHelper, fullMessage: string, imagePaths?: string[], abortSignal?: AbortSignal): AsyncGenerator<string, void, unknown> {
    if (!helper.client) throw new Error("Gemini client not initialized");

    const streams = {
      flash: helper.streamGeminiModelChunks(fullMessage, GEMINI_FLASH_MODEL, imagePaths, abortSignal)[Symbol.asyncIterator](),
      pro: helper.streamGeminiModelChunks(fullMessage, GEMINI_PRO_MODEL, imagePaths, abortSignal)[Symbol.asyncIterator](),
    } as const;
