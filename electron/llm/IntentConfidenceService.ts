...
const STRONG_CONSCIOUS_INTENTS = new Set<string>(['behavioral', 'coding', 'deep_dive']);

function calibrationRow(intent: ConversationIntent): IntentCalibrationEntry {
  const row = INTENT_CONFIDENCE_CALIBRATION[intent as keyof typeof INTENT_CONFIDENCE_CALIBRATION];
  if (row) {
    return row;
  }
...
export class IntentConfidenceService {
  ...
  attachStaleness(
    ...
    const revision =
      typeof input.transcriptRevision === 'number' && Number.isFinite(input.transcriptRevision)
