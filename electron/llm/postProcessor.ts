...
export function validateResponseQuality(response: string): ValidationResult {
	...
	if (blockedPhrases.length > 0) {
		violations.push(
			`Contains AI-speak: ${blockedPhrases.slice(0, 2).join(", ")}`,
		);
	}
	...
	return {
		...
		violations,
		regenerationHint:
			violations.length > 0 ? generateRewriteHint(violations) : undefined,
		metrics: {
...
function detectPromptType(prompt: string): string {
	if (prompt.includes("coding") || prompt.includes("algorithm"))
		return "technical";
	if (prompt.includes("define") || prompt.includes("what is"))
		return "definition";
	return "general";
}
