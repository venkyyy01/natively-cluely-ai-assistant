...
export function hasFinalize(stt: STTProvider): stt is STTProvider & { finalize: () => void } {
  return 'finalize' in stt && typeof stt.finalize === 'function';
}

export function hasSetAudioChannelCount(stt: STTProvider): stt is STTProvider & { setAudioChannelCount: (count: number) => void } {
  return 'setAudioChannelCount' in stt && typeof stt.setAudioChannelCount === 'function';
}

export function hasNotifySpeechEnded(stt: STTProvider): stt is STTProvider & { notifySpeechEnded: () => void } {
  return 'notifySpeechEnded' in stt && typeof stt.notifySpeechEnded === 'function';
}

export function hasDestroy(stt: STTProvider): stt is STTProvider & { destroy: () => void } {
...
export function safeSetAudioChannelCount(stt: STTProvider | null, count: number): void {
