...
async function flushLogQueue(): Promise<void> {
	...
	try {
		...
		if (droppedLogMessages > 0) {
			pending.unshift(
				`[Logging] Dropped ${droppedLogMessages} log messages because the log queue exceeded ${LOG_QUEUE_MAX_SIZE} entries.`,
			);
			droppedLogMessages = 0;
		}
		const content =
			pending.map((msg) => `${new Date().toISOString()} ${msg}`).join("\n") +
			"\n";
...
function safeSetAudioChannelCount(
...
export class AppState {
	...
	constructor() {
		...
		console.log(
			`[AppState] Initialized with isUndetectable=${this.isUndetectable}, disguiseMode=${this.disguiseMode}, consciousModeEnabled=${this.consciousModeEnabled}, accelerationModeEnabled=${accelerationModeEnabled}`,
		);
	...
	private async initializeAccelerationManager(): Promise<void> {
		try {
			const { AccelerationManager, setActiveAccelerationManager } =
				await import("./services/AccelerationManager");
	...
	private createSTTProvider(speaker: "interviewer" | "user"): STTProvider {
		...
		const transcriptHandler = (segment: {
			...
			console.log(
				`[TRANSCRIPT] 📝 ${speaker}: "${segment.text.substring(0, 100)}${segment.text.length > 100 ? "..." : ""}" (final: ${segment.isFinal}, conf: ${segment.confidence?.toFixed(2) || "N/A"}, meeting: ${this.isMeetingActive})`,
			);
			...
			const allImagePaths =
				screenshotQueue.length || extraScreenshotQueue.length
	...
	public async prepareMeetingActivation(metadata?: any): Promise<void> {
		...
		const normalizedMetadata = metadata
			? {
					...
					...(metadata.audio
						? {
								audio: {
									inputDeviceId: normalizeDeviceId(
										metadata.audio.inputDeviceId,
									),
									outputDeviceId: normalizeDeviceId(
		...
		let settled = false;
		this.meetingStartMutex = currentMutex
			.then(async () => {
				if (signal.aborted) {
				...
				this.intelligenceManager
					.getSessionTracker()
					.ensureMeetingContext(this.currentMeetingId);
				...
				return new Promise<void>((resolve, reject) => {
					...
					setTimeout(async () => {
						...
						try {
							...
							if (signal.aborted) {
								console.log(
									"[Main] Start meeting aborted during audio reconfig",
								);
								this.meetingLifecycleState = "idle";
								this.broadcast(
									"meeting-lifecycle-state",
...
async function initializeApp() {
		...
		if (process.platform === "darwin") {
			...
			try {
				const micAccess =
					await systemPreferences.askForMediaAccess("microphone");
				console.log(
					`[Init] 🎤 Microphone access: ${micAccess ? "✅ GRANTED" : "❌ DENIED"}`,
				);

				if (!micAccess) {
					console.error(
						"[Init] 🚨 Microphone access denied - audio transcription will not work!",
					);
					console.error(
						"[Init] 🔧 Please enable microphone access in System Preferences > Security & Privacy > Privacy > Microphone",
					);
				}
			} catch (error) {
				console.error(
					"[Init] 🎤 Failed to check microphone permissions:",
					error,
				);
			}
		}
	...
	app.on("before-quit", async (e) => {
		...
		e.preventDefault();
		console.log(
			"[Main] App quitting, preventing default to ensure pending saves complete...",
		);

		if (appState.getIsMeetingActive()) {
			console.log(
				"[Main] Meeting active during quit, ending meeting before exit...",
			);
			try {
