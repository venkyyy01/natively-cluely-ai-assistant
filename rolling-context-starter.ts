...
export class RollingContextManager {
  ...
  constructor(options: RollingContextOptions = {}, compressor?: SummaryCompressor) {
  ...
  addTranscriptTurn(turn: LiveTurn): void {
    ...
    if (
      ...
      Math.abs(duplicate.timestamp - normalizedTurn.timestamp) < this.options.duplicateWindowMs
    ) {
      return
    }
  ...
  addAssistantMessage(text: string, sourceContext?: string): void {
    ...
    this.fullTranscript.push({ role: 'assistant', text: cleaned, timestamp: now, final: true })
    this.assistantHistory.push({ text: cleaned, timestamp: now, sourceContext })
    if (this.assistantHistory.length > this.options.assistantHistoryLimit) {
      this.assistantHistory = this.assistantHistory.slice(-this.options.assistantHistoryLimit)
    }
  ...
  buildPromptContext(windowMs = this.options.recentWindowMs): PromptContextResult {
  ...
  private evictRecentWindow(): void {
    ...
    this.recentItems = this.recentItems.filter(item => item.timestamp >= cutoff)
  ...
  private async compactIfNeeded(): Promise<void> {
    ...
    if (this.fullTranscript.length <= this.options.transcriptCompactionThreshold) return

    this.compacting = true
    try {
      const batch = this.fullTranscript.slice(0, this.options.transcriptCompactionBatchSize)
      ...
      if (this.compressedEpochs.length > this.options.maxCompressedEpochs) {
        this.compressedEpochs = this.compressedEpochs.slice(-this.options.maxCompressedEpochs)
      }

      this.fullTranscript = this.fullTranscript.slice(this.options.transcriptCompactionBatchSize)
    } finally {
      this.compacting = false
    }
  }
...
export function cleanTranscriptForPrompt(turns: RollingContextItem[], maxTurns = 12): RollingContextItem[] {
  const cleaned = turns
  ...
  const externalTurns = cleaned.filter(turn => turn.role === 'external').slice(-6)
...
function stripFillerWords(text: string): string {
  const filler = new Set([
    'uh', 'um', 'ah', 'hmm', 'like', 'basically', 'actually', 'so', 'well',
    'okay', 'ok', 'yeah', 'yes', 'right', 'sure', 'gotcha', 'alright'
  ])
