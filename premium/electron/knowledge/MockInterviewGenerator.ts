...
export async function generateMockQuestions(
	...
	console.log(
		"[MockInterviewGenerator] Generating mock interview questions...",
	);
	...
	try {
		...
		const validated = parsed.slice(0, 10).map((q) => ({
			...
			category: ([
				...
				? q.category
				: "technical") as MockQuestion["category"],
			difficulty: (["easy", "medium", "hard"].includes(q.difficulty)
				? q.difficulty
				: "medium") as MockQuestion["difficulty"],
			rationale: q.rationale || "",
			suggested_answer_key: q.suggested_answer_key || "",
		}));

		console.log(
			`[MockInterviewGenerator] ✅ Generated ${validated.length} mock questions`,
		);
		return validated;
	} catch (error: any) {
		console.error(
			"[MockInterviewGenerator] Failed to generate questions:",
			error.message,
		);
		return [];
	}
