...
export async function generateStarStories(
	...
	console.log(
		`[StarStoryGenerator] Expanding ${bulletContexts.length} bullets into STAR stories (batches of ${BATCH_SIZE})...`,
	);
	...
	for (let i = 0; i < bulletContexts.length; i += BATCH_SIZE) {
		...
		try {
			...
			console.log(
				`[StarStoryGenerator] Expanded batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(bulletContexts.length / BATCH_SIZE)} (${stories.length} stories)`,
			);
		} catch (error: any) {
			console.warn(
				`[StarStoryGenerator] Batch ${Math.floor(i / BATCH_SIZE) + 1} failed: ${error.message}. Skipping.`,
			);
		}
	...
	console.log(
		`[StarStoryGenerator] ✅ Generated ${allStories.length} STAR stories total`,
	);
	return allStories;
}
...
export function starStoriesToNodes(
	...
	return stories.map((story) => {
		...
		return {
			...
			tags: extractTags(
				`${story.parent_role} ${story.parent_company} ${story.original_bullet} star behavioral`,
			),
		};
	});
}
...
export async function generateStarStoryNodes(
	...
	console.log(
		`[StarStoryGenerator] Embedding ${rawNodes.length} STAR story nodes (batched)...`,
	);
	...
	for (let i = 0; i < rawNodes.length; i += EMBED_BATCH) {
		...
		for (let j = 0; j < batch.length; j++) {
			...
			if (result.status === "rejected") {
				console.warn(
					`[StarStoryGenerator] Failed to embed STAR story ${i + j}: ${(result.reason as Error)?.message}`,
				);
			}
		}
