 ...
 */
export function createDocumentNodes(
	structuredData: any,
	type: DocType,
	...
	console.log(
		`[DocumentChunker] Created ${nodes.length} atomic nodes for ${type}`,
	);
	return nodes;
}
...
export async function chunkAndEmbedDocument(
	...
	console.log(
		`[DocumentChunker] Generating embeddings for ${rawNodes.length} nodes (batched)...`,
	);
	...
	for (let i = 0; i < rawNodes.length; i += EMBED_BATCH_SIZE) {
		...
		for (let j = 0; j < batch.length; j++) {
			...
			if (result.status === "rejected") {
				console.warn(
					`[DocumentChunker] Failed to embed node ${i + j}: ${(result.reason as Error)?.message}. Skipping embedding.`,
				);
			}
		}

		console.log(
			`[DocumentChunker] Embedded batch ${Math.floor(i / EMBED_BATCH_SIZE) + 1}/${Math.ceil(rawNodes.length / EMBED_BATCH_SIZE)} (${Math.min(i + EMBED_BATCH_SIZE, rawNodes.length)}/${rawNodes.length})`,
		);
