...
import {
	...
	findRelevantValueAlignments,
	formatValueAlignmentBlock,
...
export class KnowledgeOrchestrator {
	...
	setKnowledgeMode(enabled: boolean): void {
		...
		console.log(
			`[KnowledgeOrchestrator] Knowledge mode ${enabled ? "ENABLED" : "DISABLED"}`,
		);
	}
	...
	getCultureMappings(): CultureMappingResult | null {
		...
		if (cached) return cached;
		if (this.activeJD?.id) {
			return this.db.getCultureMappings(
				this.activeJD.id,
			) as CultureMappingResult | null;
		}
		return null;
	}
	...
	async ingestDocument(
		filePath: string,
		type: DocType = DocType.RESUME,
	): Promise<{ success: boolean; error?: string }> {
		if (!this.generateContentFn || !this.embedFn) {
			return {
				success: false,
				error: "LLM and embedding functions not configured.",
			};
		}

		try {
			console.log(
				`[KnowledgeOrchestrator] Starting ingestion for ${type} from: ${filePath}`,
			);
			...
			let structuredData = await extractStructuredData<any>(
				rawText,
				type,
			...
			try {
				// 6. Chunk and Embed
				const nodesWithEmbeddings = await chunkAndEmbedDocument(
					structuredData,
					type,
					this.embedFn,
				);
				...
				if (type === DocType.RESUME) {
					try {
						console.log(
							`[KnowledgeOrchestrator] Generating STAR stories for resume...`,
						);
						const starNodes = await generateStarStoryNodes(
							structuredData as StructuredResume,
							this.generateContentFn,
							this.embedFn,
						);
				...
				console.error(
					`[KnowledgeOrchestrator] Embedding/storage failed, rolling back document ${docId}:`,
					embedError.message,
				);
				this.db.deleteDocumentsByType(type);
				this.refreshCache();
				return {
					success: false,
					error: `Embedding failed: ${embedError.message}. Document rolled back.`,
				};
			}
	...
	async processQuestion(
		...
		if (this.embedFn && this.cachedNodes.length > 0) {
			try {
				const scoredNodes = await getRelevantNodes(
					question,
					this.cachedNodes,
					this.embedFn,
					{
				...
				console.warn(
					"[KnowledgeOrchestrator] Relevance scoring failed:",
					error.message,
				);
			}
		}
		...
		if (needsCompanyResearch(question) && this.activeJD) {
			...
			if (jd.company) {
				try {
					...
					if (!dossier) {
						console.warn(
							"[KnowledgeOrchestrator] No cached dossier found, running live research (consider uploading JD first)",
						);
					...
					console.warn(
						"[KnowledgeOrchestrator] Company research failed:",
						error.message,
					);
				}
			}
		...
		if (this.activeJD) {
			...
			if (gapAnalysis?.gaps && gapAnalysis.gaps.length > 0) {
				...
				if (matchingGaps.length > 0) {
					const pivotLines = matchingGaps.map(
						(gap) =>
					...
					console.log(
						`[KnowledgeOrchestrator] Injecting ${matchingGaps.length} pivot script(s) for detected gap skills`,
					);
				}
			}
		}
		...
		if (
			cultureMappings &&
			cultureMappings.mappings.length > 0 &&
			this.activeJD &&
			result
		) {
			const jd = this.activeJD.structured_data as StructuredJD;
			const alignments = findRelevantValueAlignments(
				question,
				cultureMappings.mappings,
				cultureMappings.core_values,
				2,
			);
			if (alignments.length > 0) {
				...
				if (cultureBlock) {
					result.contextBlock = result.contextBlock
						...
						: cultureBlock;
					console.log(
						`[KnowledgeOrchestrator] Injecting ${alignments.length} culture alignment(s) for ${jd.company}`,
					);
				}
			}
		}
	...
	private refreshCache(): void {
		...
		console.log(
			`[KnowledgeOrchestrator] Cache refreshed: ${this.cachedNodes.length} total nodes across all docs`,
		);
	}
...
Keywords: ${jd.keywords?.join(", ")}`;
		...
		try {
			...
			console.warn(
				"[KnowledgeOrchestrator] Failed to generate compact JD header:",
				error.message,
			);
	...
	getCompactJDHeader(): string | null {
		...
		const levelStr = jd.level
			? jd.level.charAt(0).toUpperCase() + jd.level.slice(1)
			: "Mid-level";
