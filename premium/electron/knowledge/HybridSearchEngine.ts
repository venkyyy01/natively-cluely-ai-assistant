...
function scoreNode(
	...
	if (
		...
		node.source_type === DocType.RESUME
	) {
		...
		for (const skill of jdRequiredSkills) {
			...
			if (
				nodeText.includes(skillLower) ||
				nodeTags.some((t) => t.includes(skillLower))
			) {
				score += 0.15;
...
export async function getRelevantNodes(
	...
	try {
		...
		console.warn(
			"[HybridSearchEngine] Failed to embed question, falling back to keyword-only:",
			error.message,
		);
	}
	...
	if (scored.length > 0) {
		console.log(
			`[HybridSearchEngine] Found ${scored.length} relevant nodes (top score: ${scored[0].score.toFixed(3)})`,
		);
	} else {
		console.log(
			`[HybridSearchEngine] No nodes above relevance threshold (${threshold}) — no injection`,
		);
	}
...
export function formatContextBlock(scoredNodes: ScoredNode[]): string {
	...
	const resumeNodes = scoredNodes.filter(
		(sn) => sn.node.source_type === DocType.RESUME,
	);
	const jdNodes = scoredNodes.filter(
		(sn) => sn.node.source_type === DocType.JD,
	);
	...
	if (resumeNodes.length > 0) {
		const resumeLines = resumeNodes.map((sn, i) => {
			const prefix =
				sn.node.category === "experience"
					? `[${sn.node.title} at ${sn.node.organization}]`
					: sn.node.category === "project"
						? `[Project: ${sn.node.title}]`
						: sn.node.category === "education"
							? `[Education: ${sn.node.title}]`
...
export function formatDossierBlock(dossier: CompanyDossier | null): string {
	...
	if (dossier.salary_estimates && dossier.salary_estimates.length > 0) {
		const salaryLines = dossier.salary_estimates.map(
			(s) =>
