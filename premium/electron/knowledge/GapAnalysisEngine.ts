...
export async function analyzeGaps(
	...
	const jdSkills = [...(jd.requirements || []), ...(jd.technologies || [])];
	...
	for (const skill of jdSkills) {
		...
		const experienceMonths = findSkillExperience(
			skillLower,
			skillExperienceMap,
		);
	...
	const matchPercentage =
		totalSkills > 0 ? Math.round((matched.length / totalSkills) * 100) : 100;

	console.log(
		`[GapAnalysisEngine] Match: ${matched.length}/${totalSkills} (${matchPercentage}%), Gaps: ${rawGaps.length}`,
	);
 ...
 */
function findSkillExperience(
	skillLower: string,
	map: SkillExperienceMap,
): number {
	// Direct match
...
async function generatePivotScripts(
	...
	const topExperience = (resume.experience || [])
		.slice(0, 3)
		.map(
			(e) => `${e.role} at ${e.company}: ${e.bullets.slice(0, 2).join("; ")}`,
		)
		.join("\n");

	const gapList = rawGaps
		.map((g, i) => `${i + 1}. "${g.skill}" (${g.gap_type})`)
		.join("\n");
...
Return ONLY the JSON array.`;
	...
	try {
		...
		console.error(
			"[GapAnalysisEngine] Failed to generate pivot scripts:",
			error.message,
		);
