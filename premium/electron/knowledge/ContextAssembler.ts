...
function isBareGreeting(questionLower: string): boolean {
	...
	return (
		GREETING_PATTERNS.includes(trimmed) ||
		GREETING_PATTERNS.some((g) => trimmed === `${g} there`) ||
		(trimmed.length <= 12 &&
			GREETING_PATTERNS.some((g) => trimmed.startsWith(g)))
	);
}
...
function handleBareGreeting(resumeDoc: KnowledgeDocument | null): string {
	const name = resumeDoc
		? (resumeDoc.structured_data as StructuredResume).identity.name?.split(
				" ",
			)[0]
		: "";
...
export async function assemblePromptContext(
	...
	if (isIntro && resumeDoc && generateContentFn) {
		...
		const introResponse = await generateJitIntro(
			resumeDoc,
			jdDoc,
			generateContentFn,
		);
	...
	const systemPromptInjection = buildKnowledgeSystemPrompt(
		identityHeader,
		jdDoc !== null,
		toneXML,
	);
