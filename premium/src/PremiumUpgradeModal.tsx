	...
	useEffect(() => {
		if (isOpen) {
			window.electronAPI
				...
				.then(setHardwareId)
				.catch(() => setHardwareId("unavailable"));
			setStatus("idle");
			setErrorMessage("");
			setLicenseKey("");
		}
	...
	const handleActivate = async () => {
		...
		try {
			...
			if (result?.success) {
				...
				setStatus("error");
				setErrorMessage(
					result?.error || "Activation failed. Please try again.",
				);
			}
		} catch (e: any) {
			setStatus("error");
							...
							{isPremium ? (
								<div className="flex flex-col items-center text-center py-6">
									...
									<p className="text-[13px] text-white/50 mt-2 max-w-[280px] mx-auto leading-relaxed mb-8">
										Your device is fully authorized for Natively's premium
										features including the Profile Engine, Job Description
										Intelligence, and Company Research.
									</p>
									...
									<p className="text-[10px] text-white/30 text-center px-4 mt-4 leading-relaxed">
										Deactivating will remove the license from this device,
										allowing you to use it on another computer.
									</p>
								</div>
							) : (
								<>
									...
									<div className="flex flex-col items-center text-center gap-3 mb-1">
										...
										<div>
											...
											<p className="text-[12px] text-white/40 mt-1 max-w-[260px] mx-auto leading-relaxed">
									...
									<button
										onClick={() =>
											window.electronAPI?.openExternal?.(
												"https://evynignatious.gumroad.com/l/natively",
											)
										}
									...
									<div className="space-y-2.5">
										...
										<button
											...
											className={`w-full py-2.5 rounded-[10px] text-[12px] font-medium transition-all duration-200 flex items-center justify-center gap-2 ${
												status === "success"
