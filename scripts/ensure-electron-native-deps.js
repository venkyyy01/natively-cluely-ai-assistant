...
function needsRebuild() {
	...
	for (const binary of binaries) {
		if (!fs.existsSync(binary.path)) {
			if (!binary.optional) {
				console.log(
					`[ensure-electron-native-deps] Missing ${binary.name} binary: ${binary.path}`,
				);
				return true;
			}
			continue;
		...
		if (!matchesCurrentArch(info)) {
			console.log(
				`[ensure-electron-native-deps] ${binary.name} arch mismatch: ${info}`,
			);
			return true;
		}
	}
...
for (const binary of binaries) {
	if (!fs.existsSync(binary.path)) {
		...
		console.error(
			`[ensure-electron-native-deps] ERROR: ${binary.name} binary missing after rebuild: ${binary.path}`,
		);
		process.exit(1);
	}
	const info = getBinaryInfo(binary.path);
	if (!matchesCurrentArch(info)) {
		console.error(
			`[ensure-electron-native-deps] ERROR: ${binary.name} still has wrong architecture after rebuild: ${info}`,
		);
		console.error(
			`[ensure-electron-native-deps] Expected ${arch} but binary is not compatible`,
		);
		process.exit(1);
	}
...
console.log(
	`[ensure-electron-native-deps] All native dependencies verified for ${platform}-${arch}.`,
);
