...
const { computeWER, computeDiarizationAccuracy } = require('../dist-electron/electron/audio/WERBenchmark.js');
...
function selfTest() {
  ...
  for (const c of cases) {
    ...
    if (ok) {
      ...
      console.error(`FAIL: ref="${c.ref}" hyp="${c.hyp}" expected=${c.expectedWER} got=${result.wer}`);
    }
  }
  ...
  const diarizationResult = computeDiarizationAccuracy(diarizationRef, diarizationHyp);
  console.log(`Diarization accuracy (perfect match): ${diarizationResult.accuracy}`);
...
async function main() {
  ...
  console.log('Live benchmark requires audio fixtures. Place WAV/MP3 files under:');
