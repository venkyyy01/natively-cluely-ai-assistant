...
function buildTscPath() {
  return path.join(process.cwd(), 'node_modules', '.bin', process.platform === 'win32' ? 'tsc.cmd' : 'tsc');
}
...
async function runIntentComparison(label, cases) {
  ...
  const coordinated = await intentEval.runIntentEval(cases, buildClassifier('coordinated', providers));
  const legacy = await intentEval.runIntentEval(cases, buildClassifier('legacy', providers));
  const foundation = await intentEval.runIntentEval(cases, buildClassifier('foundation', providers));
...
async function runReleaseVerification(options = {}) {
  ...
  runCommand('npm', ['run', 'prepare:macos:foundation-intent-helper'], 'helper preparation');
  runCommand(buildTscPath(), ['-p', 'electron/tsconfig.json'], 'electron compilation');
  runCommand('node', ['--test', ...TARGET_TEST_FILES], 'targeted intent/conscious tests');
  ...
  const { DEFAULT_INTENT_EVAL_CASES } = require('../dist-electron/electron/evals/intentClassificationEval.js');
  const { runFoundationIntentLatencySpike } = require('./run-foundation-intent-latency-spike.js');
  ...
  const comparisons = [await runIntentComparison('default', DEFAULT_INTENT_EVAL_CASES)];
  const includeGenerated = options.includeGenerated ?? process.env.NATIVELY_FOUNDATION_RELEASE_INCLUDE_GENERATED !== '0';
  if (includeGenerated) {
    const generatedPath = path.join(process.cwd(), 'electron', 'evals', 'intentEvalVariants.generated.json');
    if (fs.existsSync(generatedPath)) {
      const generatedPayload = JSON.parse(fs.readFileSync(generatedPath, 'utf8'));
      const generatedCases = Array.isArray(generatedPayload) ? generatedPayload : generatedPayload.cases;
  ...
  for (const comparison of comparisons) {
    if (comparison.coordinated.accuracy < comparison.legacy.accuracy) {
      throw new Error(`coordinated accuracy regressed below legacy on ${comparison.label}`);
    }
  }
...
if (require.main === module) {
  main().catch((error) => {
    console.error('Foundation intent release verification failed:', error instanceof Error ? error.message : String(error));
    process.exitCode = 1;
  });
}
