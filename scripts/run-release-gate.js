...
function assertMetricsShape(metrics, label) {
	const missing = REQUIRED_BENCHMARK_METRICS.filter(
		(metric) => typeof metrics?.[metric] !== "number",
	);
	if (missing.length > 0) {
		throw new Error(
			`${label} missing required metric keys: ${missing.join(", ")}`,
		);
	}
}
...
function assertMetricsWithinGate(
	currentMetrics,
	baselineMetrics,
	env = process.env,
) {
	...
	for (const metric of REQUIRED_BENCHMARK_METRICS) {
		...
		if (baselineValue > 0) {
			const allowedRegression =
				baselineValue * (1 + regressionPct / 100) + regressionSlackMs;
...
function findLatestPackagedAppBundle(
	cwd = process.cwd(),
	env = process.env,
	options = {},
) {
	...
	const releaseDir =
		env.NATIVELY_RELEASE_GATE_RELEASE_DIR?.trim() || path.join(cwd, "release");
	...
	appBundles.sort(
		(left, right) => readStats(right).mtimeMs - readStats(left).mtimeMs,
	);
	return appBundles[0];
}

function resolvePackagedHelperLaunchTarget(env = process.env, options = {}) {
	const explicitValidationRequested = isTruthyFlag(
		env.NATIVELY_RELEASE_GATE_VALIDATE_PACKAGED_HELPER ?? "",
	);
	const allowAutoDiscovery = !isTruthyFlag(
		env.NATIVELY_RELEASE_GATE_SKIP_PACKAGED_HELPER_VALIDATION ?? "",
	);
	const platform = options.platform ?? process.platform;
	const discoverPackagedAppBundle =
		options.discoverPackagedAppBundle ?? findLatestPackagedAppBundle;
	...
	const appBinary =
		env.NATIVELY_RELEASE_GATE_APP_BINARY?.trim() ||
...
function validatePackagedHelperLaunch(target, options = {}) {
	const env = options.env ?? process.env;
	const scriptPath =
		options.scriptPath ?? path.join(process.cwd(), "build-and-install.sh");
...
function runReleaseGate(options = {}) {
	...
	const helperTargetResolver =
		options.resolvePackagedHelperLaunchTarget ??
