...
const defaultPackageDir = path.join(root, 'applesilicon', 'macos-foundation-intent-helper');
const defaultOutputDir = path.join(root, 'assets', 'bin', 'macos');
const defaultOutputBinary = path.join(defaultOutputDir, 'foundation-intent-helper');
...
function findBuiltBinary(packageDir, configuration) {
  const candidates = [
    ...
    path.join(packageDir, '.build', 'arm64-apple-macosx', configuration, 'foundation-intent-helper'),
    path.join(packageDir, '.build', 'x86_64-apple-macosx', configuration, 'foundation-intent-helper'),
  ];
...
function prepareMacosFoundationIntentHelper(options = {}) {
  ...
  const configuration = options.configuration ?? process.env.MACOS_FOUNDATION_INTENT_HELPER_CONFIGURATION ?? 'release';
  ...
  const requireHelper = options.requireHelper ?? isTruthyFlag(process.env.NATIVELY_REQUIRE_FOUNDATION_INTENT_HELPER ?? '');
  ...
  const skipWithoutHelper = (reason) => {
    ...
    logFn(`Warning: ${reason}. Continuing without foundation intent helper binary.`);
    return {
      skipped: true,
      reason,
    };
  ...
  if (shouldBuild) {
    if (!commandExistsFn('swift')) {
      if (!builtBinary) {
        return skipWithoutHelper('Swift toolchain unavailable; unable to build foundation intent helper');
      }

      logFn('Swift toolchain unavailable; using existing foundation intent helper build output');
    } else {
      try {
  ...
  if (!builtBinary) {
    return skipWithoutHelper(`Unable to locate built foundation intent helper for configuration '${configuration}'`);
  }
