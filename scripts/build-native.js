...
function getMinimumNapiVersion() {
	...
	try {
		...
		const matches = [...cargoToml.matchAll(/napi(\d+)/g)].map((match) =>
			Number(match[1]),
		);
		...
		console.warn(
			`Warning: Could not determine minimum N-API version from ${cargoTomlPath}.`,
			error,
		);
		return null;
	}
...
function writeAbiMetadata() {
	...
	artifacts.forEach((artifact) => {
		...
		console.log(
			`Wrote ABI metadata: ${path.relative(nativeModulePath, abiPath)} -> ${compatibilityMetadata}`,
		);
	});
}
...
if (os.platform() === "darwin") {
	if (buildCurrentOnly) {
		...
		console.log(
			`Building native audio for current macOS architecture: ${target}`,
		);
