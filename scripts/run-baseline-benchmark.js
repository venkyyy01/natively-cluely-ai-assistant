...
function printComparisonTable(currentMetrics, baseline) {
		...
		.map((metric) => ({
			...
			current:
				currentMetrics[metric] === null
					? "n/a"
					: `${currentMetrics[metric]} ms`,
			baseline: Object.hasOwn(baseline, metric)
				? `${baseline[metric]} ms`
				: "n/a",
		}));
	...
	console.log(
		formatRow(
			"-".repeat(columnWidths.metric),
			"-".repeat(columnWidths.current),
			"-".repeat(columnWidths.baseline),
		),
	);
...
function runBaselineBenchmark(options = {}) {
	...
	const benchmarkDir =
		...
		path.join(os.tmpdir(), "natively-baseline-benchmark");
	const benchmarkFile =
		options.benchmarkFile ??
		path.join(benchmarkDir, "performance-metrics.jsonl");
	const baselineFile =
	...
	const compiledBenchmarkTest =
		options.compiledBenchmarkTest ??
		path.resolve(
			repoRoot,
			"dist-electron",
			"electron",
	...
	const summaryFile =
		options.summaryFile ?? process.env.NATIVELY_BENCHMARK_SUMMARY_FILE;

	rmSync(benchmarkDir, { recursive: true, force: true });
	mkdirSync(benchmarkDir, { recursive: true });

	execFileSync(process.execPath, ["--test", compiledBenchmarkTest], {
		cwd: repoRoot,
		env: {
			...process.env,
