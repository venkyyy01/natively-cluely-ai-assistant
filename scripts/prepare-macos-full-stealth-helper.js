...
const defaultOutputBundleDir = path.join(
	root,
	"assets",
	"xpcservices",
...
function prepareMacosFullStealthHelper(options = {}) {
	...
	const configuration =
		...
		process.env.MACOS_FULL_STEALTH_HELPER_CONFIGURATION ??
		"release";
	const outputBundleDir = options.outputBundleDir ?? defaultOutputBundleDir;
	const bundleIdentifier =
		options.bundleIdentifier ??
		process.env.MACOS_FULL_STEALTH_HELPER_BUNDLE_ID ??
		"com.electron.meeting-notes.macos-full-stealth-helper";
	...
	const requireHelper =
		...
		isTruthyFlag(process.env.NATIVELY_REQUIRE_FULL_STEALTH_HELPER ?? "");
	...
	const skipWithoutHelper = (reason) => {
		...
		logFn(
			`Warning: ${reason}. Continuing without the full stealth helper bundle.`,
		);
		return {
			skipped: true,
			reason,
		...
		if (!commandExistsFn("swift")) {
			if (!builtBinary) {
				return skipWithoutHelper(
					"Swift toolchain unavailable; unable to build the macOS full stealth helper",
				);
			}

			logFn(
				"Swift toolchain unavailable; using existing macOS full stealth helper build output",
			);
		} else {
			try {
	...
	if (!builtBinary) {
		return skipWithoutHelper(
			`Unable to locate built full stealth helper for configuration '${configuration}'`,
		);
	}
