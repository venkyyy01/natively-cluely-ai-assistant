...
test("electron helpers report summary-not-found errors", () => {
	const { parseCoverageSummary } = loadCoverageHelpers(
...
test("electron helpers parse summary lines and pass thresholds", () => {
	const { THRESHOLDS, parseCoverageSummary, evaluateCoverage } =
		loadCoverageHelpers("verify-electron-coverage.js", {
			status: 0,
			stdout: "all files | 100.00 | 100.00 | 100.00\n",
			stderr: "",
		});
	...
	assert.deepEqual(
		{ ...summary },
		{ lines: 58.1, branches: 81.25, functions: 44.4 },
	);
	assert.equal(evaluateCoverage(summary), null);
	assert.deepEqual(
		{ ...THRESHOLDS },
		{ lines: 50, branches: 75, functions: 30 },
	);
});

test("electron helpers report threshold failures clearly", () => {
	const { evaluateCoverage } = loadCoverageHelpers(
		"verify-electron-coverage.js",
		{
			status: 0,
			stdout: "all files | 100.00 | 100.00 | 100.00\n",
...
test("renderer helpers parse summary lines and pass thresholds", () => {
	const { THRESHOLDS, parseCoverageSummary, evaluateCoverage } =
		loadCoverageHelpers("verify-renderer-coverage.js", {
			status: 0,
			stdout: "All files | 100 | 100 | 100 | 100\n",
			stderr: "",
		});
	...
	assert.deepEqual(
		{ ...THRESHOLDS },
		{ statements: 90, branches: 100, functions: 75, lines: 90 },
	);
});
