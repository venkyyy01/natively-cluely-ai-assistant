...
test("findLatestPackagedAppBundle returns the newest packaged app under release", () => {
	...
	const appBundle = findLatestPackagedAppBundle(
		cwd,
		{},
		{
...
test("validatePackagedHelperLaunch sources build-and-install.sh in library mode", () => {
	...
	assert.match(
		calls[0].args[1],
		/validate_packaged_helper_launch_modes "\/Applications\/Natively\.app" "\/Applications\/Natively\.app\/Contents\/MacOS\/Natively"/,
	);
