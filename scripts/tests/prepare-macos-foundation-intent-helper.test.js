...
const {
  ...
  findBuiltBinary,
  isTruthyFlag,
} = require(path.join(repoRoot, 'scripts', 'prepare-macos-foundation-intent-helper.js'));

test('prepare-macos-foundation-intent-helper stages helper binary output', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'foundation-intent-helper-stage-'));
...
test('prepare-macos-foundation-intent-helper locates architecture-specific build outputs', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'foundation-intent-helper-find-'));
  const archSpecific = path.join(tempDir, '.build', 'arm64-apple-macosx', 'release', 'foundation-intent-helper');
...
test('package.json mac build config includes foundation intent helper preparation and resource staging', () => {
  assert.match(packageJson.scripts['prepare:macos:foundation-intent-helper'], /prepare-macos-foundation-intent-helper\.js/);
  assert.match(packageJson.scripts['app:build'], /prepare:macos:foundation-intent-helper/);
  assert.match(packageJson.scripts['app:build:arm64'], /prepare:macos:foundation-intent-helper/);
  assert.match(packageJson.scripts['app:build:x64'], /prepare:macos:foundation-intent-helper/);

  const extraResources = packageJson.build.extraResources ?? [];
  assert.deepEqual(extraResources.some((entry) => (
