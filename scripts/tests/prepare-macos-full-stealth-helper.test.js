...
const {
  ...
  findBuiltBinary,
  stageBundle,
} = require(path.join(repoRoot, 'scripts', 'prepare-macos-full-stealth-helper.js'));

test('prepare-macos-full-stealth-helper stages an XPC bundle with executable and Info.plist', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'full-stealth-helper-stage-'));
  ...
  assert.match(fs.readFileSync(staged.infoPlistPath, 'utf8'), /CFBundlePackageType[\s\S]*XPC!/);
  assert.match(fs.readFileSync(staged.infoPlistPath, 'utf8'), /com.test.full-stealth-helper/);
});

test('prepare-macos-full-stealth-helper locates architecture-specific build outputs', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'full-stealth-helper-find-'));
  const archSpecific = path.join(tempDir, '.build', 'arm64-apple-macosx', 'release', 'macos-full-stealth-helper');
...
test('package.json mac build config stages the full stealth helper into Contents\/XPCServices', () => {
  assert.match(packageJson.scripts['prepare:macos:full-stealth-helper'], /prepare-macos-full-stealth-helper\.js/);
  assert.match(packageJson.scripts['app:build'], /prepare:macos:full-stealth-helper/);
  assert.match(packageJson.scripts['app:build:arm64'], /prepare:macos:full-stealth-helper/);
  assert.match(packageJson.scripts['app:build:x64'], /prepare:macos:full-stealth-helper/);

  const extraFiles = packageJson.build.extraFiles ?? [];
  assert.deepEqual(extraFiles.some((entry) => (
