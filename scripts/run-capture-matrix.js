	...
	for (const run of runs) {
		...
		for (const row of result.results) {
			const status = row.passed
				? "PASS"
				: row.actualResult === "skipped"
					? "SKIP"
					: "FAIL";
			console.log(
				`[${status}] ${row.adapter}:${row.row.id} expected=${row.row.expectedResult} actual=${row.actualResult}${row.reason ? ` reason=${row.reason}` : ""}`,
			);
		}
		...
		if (
			!result.passed &&
			result.results.some((row) => row.actualResult !== "skipped")
		) {
			process.exitCode = 1;
		}
	}
...
function createMacosSckRun() {
	return {
		rows: createDefaultMacosScreenCaptureKitRows({
...
function createBrowserGetDisplayMediaRun() {
	return {
		rows: createDefaultBrowserGetDisplayMediaRows({
			platform: process.platform,
		}),
