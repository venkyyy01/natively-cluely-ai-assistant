...
async function downloadModels() {
	...
	try {
		...
		console.log(
			"[download-models] Downloading Xenova/mobilebert-uncased-mnli...",
		);
		await pipeline(
		...
		console.error(
			"[download-models] Tip: Run with SKIP_MODEL_DOWNLOAD=1 to skip this step",
		);
		process.exit(1);
	}
}
...
if (process.env.SKIP_MODEL_DOWNLOAD === "1") {
	console.log(
		"[download-models] Skipping model download (SKIP_MODEL_DOWNLOAD=1)",
	);
	process.exit(0);
}
