...
function parseCoverageSummary(output) {
	...
	if (percentages.length < 3) {
		throw new Error(
			`Coverage gate failed: could not parse metrics from ${allFilesLine.trim()}`,
		);
	}
...
async function main() {
	...
	console.log(
		`[verify-electron-coverage] Running ${testFiles.length} Electron test files with coverage...`,
	);
