...
function findBuiltBinary(configuration) {
  const candidates = [
    path.join(packageDir, '.build', configuration, 'stealth-virtual-display-helper'),
    path.join(packageDir, '.build', 'arm64-apple-macosx', configuration, 'stealth-virtual-display-helper'),
    path.join(packageDir, '.build', 'x86_64-apple-macosx', configuration, 'stealth-virtual-display-helper'),
  ];
...
function main() {
  ...
  const configuration = process.env.MACOS_VIRTUAL_DISPLAY_HELPER_CONFIGURATION || 'release';
  log(`Building macOS virtual display helper (${configuration})`);
  execFileSync('swift', ['build', '-c', configuration, '--package-path', packageDir], {
    cwd: root,
    stdio: 'inherit',
  });
