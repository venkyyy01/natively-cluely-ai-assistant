...
async function runFoundationIntentLatencySpike(options = {}) {
	...
	return {
		...
		runs,
		successCount:
			runs - Object.values(errorsByCode).reduce((sum, count) => sum + count, 0),
		failureCount: Object.values(errorsByCode).reduce(
			(sum, count) => sum + count,
			0,
		),
		errorsByCode,
...
async function main() {
	const report = await runFoundationIntentLatencySpike({
		runs: parseRuns(process.argv.slice(2)),
	});
